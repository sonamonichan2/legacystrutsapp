...
/**
 * This class represents the mapping of a Filter to a collection of Servlets and
 * URLs
 */
public class ModuleFilterMapping implements Serializable {
	 ...
	 * 
	 * @param module
	 *            - the module to use to construct this ModuleFilterMapping
	 */
	...
	/**
	 * @param servletNames
	 *            a List of all Servlet Names mapped to this filter
	 */
	...
	/**
	 * @param urlPatterns
	 *            a List of all Url Patterns mapped to this filter
	 */
	...
	/**
	 * Return <code>true</code> if the passed Filter passes one or more filter
	 * mappings otherwise, return <code>false</code>.
	 * 
	 * @param filterMapping
	 *            - The {@link ModuleFilterMapping} to check for matching
	 *            servlets and url patterns
	 * @param requestPath
	 *            - The URI of the request to check against the
	 *            {@link ModuleFilterMapping}, with the context path already
	 *            removed (since module filter mappings are relative to the
	 *            context path).
	 * @return - true if the given {@link ModuleFilterMapping} matches the
	 *         passed requestPath For example: Passing a ModuleFilterMapping
	 *         containing a urlPattern of "*" would return true for any
	 *         requestPath Passing a ModuleFilterMapping containing a urlPattern
	 *         of "*.jsp" would return true for any requestPath ending in ".jsp"
	 * @should return false if the requestPath is null
	 * @should return true if the ModuleFilterMapping contains any matching
	 *         urlPatterns for this requestPath
	 * @should return true if the ModuleFilterMapping contains any matching
	 *         servletNames for this requestPath
	 * @should return false if no matches are found for this requestPath
	 */
	public static boolean filterMappingPasses(
			ModuleFilterMapping filterMapping, String requestPath) {
	 ...
	 * patternToCheck otherwise, return <code>false</code>.
	 * 
	 ...
	 */
	public static boolean urlPatternMatches(String patternToCheck,
			String requestPath) {
		...
		log.debug("Checking URL <" + requestPath + "> against pattern <"
				+ patternToCheck + ">");

		// Match exact or full wildcard
		if (patternToCheck.equals("*") || patternToCheck.equals("/*")
				|| patternToCheck.equals(requestPath)) {
			return true;
		}
		...
		if (patternToCheck.startsWith("*.")) {
			...
			if (slash >= 0 && period > slash && period != reqLen - 1
					&& reqLen - period == patLen - 1) {
				return (patternToCheck.regionMatches(2, requestPath,
						period + 1, patLen - 2));
			}
		}
	 ...
	 * filterMapping otherwise return <code>false</code>.
	 * 
	 * @param patternToCheck
	 *            String pattern to check
	 * @param servletName
	 ...
	 * @should return false if no pattern matches
	 */
	public static boolean servletNameMatches(String patternToCheck,
			String servletName) {
		...
		log.debug("Checking servlet <" + servletName + "> against pattern <"
				+ patternToCheck + ">");
	...
	/**
	 * Static method to parse through a Module's configuration file and return a
	 * List of ModuleFilterMapping objects for which there are configuration
	 * elements. Expected XML Format:
	 * 
	 * <pre>
	 * 	&lt;filter-mapping&gt;
	 ...
	 * 
	 * @param module
	 *            - The {@link Module} for which you want to retrieve the
	 *            defined {@link ModuleFilterMapping}s
	 * @return - a List of {@link ModuleFilterMapping}s that are defined for the
	 *         passed {@link Module}
	 */
	public static List<ModuleFilterMapping> retrieveFilterMappings(Module module)
		...
		try {
			...
			NodeList mappingNodes = rootNode
					.getElementsByTagName("filter-mapping");
			if (mappingNodes.getLength() > 0) {
				for (int i = 0; i < mappingNodes.getLength(); i++) {
					ModuleFilterMapping mapping = new ModuleFilterMapping(
							module);
					...
					for (int j = 0; j < configNodes.getLength(); j++) {
						...
						} else if ("url-pattern".equals(configNode
								.getNodeName())) {
							mapping.addUrlPattern(configNode.getTextContent());
						} else if ("servlet-name".equals(configNode
								.getNodeName())) {
			...
			throw new ModuleException(
					"Unable to parse filters in module configuration.", e);
		}
		log.debug("Retrieved " + mappings.size() + " filter-mappings for "
				+ module.getModuleId() + ": " + mappings);
		return mappings;
	}
}
