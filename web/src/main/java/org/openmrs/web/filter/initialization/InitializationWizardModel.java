...
/**
 * The {@link InitializationFilter} uses this model object to hold all
 * properties that are edited by the user in the wizard. All attributes on this
 * model object are added to all templates rendered by the
 * {@link InitializationFilter}.
 */
public class InitializationWizardModel {
	...
	/**
	 * Default database name to use unless user specifies another in the wizard
	 * or they are creating a test installation
	 */
	public static final String DEFAULT_DATABASE_NAME = WebConstants.WEBAPP_NAME;

	/**
	 * Records completed tasks and are displayed at the top of the page upon
	 * error
	 */
	public List<String> workLog = new ArrayList<String>();

	/**
	 * Whether the runtime properties file could possible be created. (only read
	 * by the velocity scripts)
	 */

	public boolean canCreate = true;

	/**
	 * Error message from not being able to create the runtime properties file
	 * (only read by the velocity scripts)
	 */

	public String cannotCreateErrorMessage = "";

	/**
	 * Whether the runtime file can be edited (only read by the velocity
	 * scripts)
	 */
	...
	/**
	 * The location of the runtime properties file (only read by the velocity
	 * scripts)
	 */
	...
	/**
	 * True/false marker for the question
	 * "Do you currently have an OpenMRS database installed"
	 */
	public Boolean hasCurrentOpenmrsDatabase = true;

	/**
	 * True/false marker for the question
	 * "Do you currently have a database user other than root that has read/write access"
	 */
	...
	/**
	 * MySQL root account password used for simple installation. Filled in
	 * simplesetup.vm.
	 */
	...
	/**
	 * DB user that can create an openmrs db user Filled in on
	 * databasetablesanduser.vm
	 */
	 ...
	 * Entered on databasetablesanduser page
	 */
	...
	/**
	 * Asked for on the databasetablesanduser.vm page to know if their existing
	 * database has the tables or not
	 */
	...
	/**
	 * Asked for on the otherproperties.vm page to know if the allow_web_admin
	 * runtime property is true/false
	 */
	public Boolean moduleWebAdmin = Boolean.TRUE;

	/**
	 * Asked for on otherproperties.vm page to know if the runtime property for
	 * auto updating their db is true/false
	 */
