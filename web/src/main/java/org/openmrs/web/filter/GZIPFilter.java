 ...
 * http://www.onjava.com/pub/a/onjava/2003/11/19/filters.html</a>. &copy; 2003
 * Jayson Falkner You may freely use the code both commercially and
 * non-commercially.
 */
public class GZIPFilter extends OncePerRequestFilter {
	...
	private boolean isGZIPEnabled() {
		...
		try {
			...
			log.warn("Unable to get the global property: "
					+ OpenmrsConstants.GLOBAL_PROPERTY_GZIP_ENABLED, e);
	...
	/**
	 * Returns true if path matches pattern in
	 * gzip.acceptCompressedRequestsForPaths property
	 */
	private boolean isCompressedRequestForPathAccepted(String path) {
		try {
			if (cachedGZipCompressedRequestForPathAccepted == null) {
				cachedGZipCompressedRequestForPathAccepted = Context
			...
			for (String acceptPath : cachedGZipCompressedRequestForPathAccepted
					.split(",")) {
			...
			log.warn(
					"Unable to process the global property: "
							+ OpenmrsConstants.GLOBAL_PROPERTY_GZIP_ACCEPT_COMPRESSED_REQUESTS_FOR_PATHS,
					e);
			return false;
		}
