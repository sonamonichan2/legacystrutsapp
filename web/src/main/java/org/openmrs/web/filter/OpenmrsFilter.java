...
/**
 * This is the custom OpenMRS filter. It is defined as the filter of choice in
 * the web.xml file. All page/object calls run through the doFilter method so we
 * can wrap every session with the user's userContext (which holds the user's
 * authenticated info). This is needed because the OpenMRS API keeps
 * authentication information on the current Thread. Web applications use a
 * different thread per request, so before each request this filter will make
 * sure that the UserContext (the authentication information) is on the Thread.
 */
public class OpenmrsFilter extends OncePerRequestFilter {
	...
	/**
	 * This method is called for every request for a page/image/javascript
	 * file/etc The main point of this is to make sure the user's current
	 * userContext is on the session and on the current thread
	 * 
	...
	protected void doFilterInternal(HttpServletRequest httpRequest,
		...
		httpRequest.setAttribute(WebConstants.INIT_REQ_UNIQUE_ID,
		...
		// User context is created if it doesn't already exist and added to the
		// session
		// note: this usercontext storage logic is copied to
		// webinf/view/uncaughtexception.jsp to
		// prevent stack traces being shown to non-authenticated users
		UserContext userContext = (UserContext) httpSession
		...
		if (userContext == null) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Just set user context " + userContext
						+ " as attribute on session");
			}
		} else {
