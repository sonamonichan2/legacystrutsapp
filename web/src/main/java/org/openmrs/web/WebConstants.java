...
public class WebConstants {
	...
	/**
	 * Page in the webapp used for initial setup of the database connection if
	 * no valid one exists
	 */
	public static final String SETUP_PAGE_URL = "initialsetup";

	/**
	 * The url of the module repository. This is filled in at startup by the
	 * value in web.xml
	 */
	public static String MODULE_REPOSITORY_URL = "";

	/**
	 * Global property name for the number of times one IP can fail at logging
	 * in before being locked out. A value of 0 for this property means no IP
	 * lockout checks.
	 * 
	...
	/**
	 * User names of the logged-in users are stored in this map (session id
	 * -&gt; user name) in the ServletContext under this key
	 */
	public static final String CURRENT_USERS = "CURRENT_USERS";

	/**
	 * Session attribute name that specifies if there are any privilege checks
	 * the currently authenticated user failed
	 */
	public static final String INSUFFICIENT_PRIVILEGES = "insufficient_privileges";

	/**
	 * Session attribute name for the url of the page the user was trying to
	 * access when they failed a privilege check
	 */
