...
/**
 * This class is only used to get access to the dispatcher servlet that handles
 * static content. <br>
 * <br>
 * After creation, this object is saved to WebModuleUtil for later use. When
 * Spring's root webApplicationContext is refreshed, this dispatcher servlet
...
public class StaticDispatcherServlet
	...
	/**
	 * Called by the ModuleUtil after adding in a new, updating, starting, or
	 * stopping a module. This needs to be called because each spring dispatcher
	 * servlet creates a new application context, which therefore needs to be
	 * refreshed too.
	 * 
	...
	public void refreshApplicationContext() throws ServletException {
		...
		((XmlWebApplicationContext) getWebApplicationContext())
				.setClassLoader(OpenmrsClassLoader.getInstance());
	...
	public void stopAndCloseApplicationContext() {
		try {
			...
			log.error(
					"Exception while stopping and closing static content dispatcher servlet context: ",
					e);
		}
	}
}
