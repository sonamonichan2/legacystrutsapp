...
/**
 * ModuleActivator tests that need refreshing the spring application context.
 * The only reason why i did not put these in the api projects's
 * ModuleActivatorTest is because when the spring application context is
 * refreshed, classes that the module references which are not in the api but
 * web, will lead to ClassNotFoundException s, hence preventing the refresh. If
 * you want to try this out, just put these tests in ModuleActivatorTest NOTE:
 * The way we start, stop, unload, etc, modules is copied from
...
public class WebModuleActivatorTest extends BaseModuleActivatorTest {
	...
	public void createWebInfFolderIfNotExist() throws Exception {
		// when run from the IDE and this folder does not exist, some tests fail
		// with
		// org.openmrs.module.ModuleException: Unable to load module messages
		// from file:
		// /Projects/openmrs/core/web/target/test-classes/WEB-INF/module_messages_fr.properties

		File folder = new File("target" + File.separatorChar + "test-classes"
	...
	public void shouldCallWillRefreshContextAndContextRefreshedOnRefresh()
		...
		ModuleUtil.refreshApplicationContext(
	...
	public void shouldRefreshOtherModulesOnStoppingModule() {
		...
		// willRefreshContext() and contextRefreshed() methods get called for
		// ONLY the started modules' activators EXCLUDING the stopped module
		// willStop() and stopped() methods get called for ONLY the stopped
		// module's activator

		Module module = ModuleFactory.getModuleById(MODULE3_ID);
		ModuleFactory.stopModule(module);
		WebModuleUtil.stopModule(module,
	...
	public void shouldRefreshOtherModulesOnStartingStoppedModule()
		...
		// willRefreshContext() and contextRefreshed() methods get called for
		// all started modules' activators (including the newly started module)
		// willStart() and started() methods get called for ONLY the newly
		// started module's activator

		// start module3 which was previously stopped
		ModuleFactory.startModule(module);
		WebModuleUtil.startModule(module,
				((XmlWebApplicationContext) applicationContext)
	...
	@Test
	public void shouldRefreshContextForAllStartedModulesOnWebStartup()
			throws Throwable {

		// At OpenMRS start up:
		// willRefreshContext(), contextRefreshed(), willStart() and started()
		// methods get called for all started modules' activators
		Listener.performWebStartOfModules(((XmlWebApplicationContext) applicationContext)
				.getServletContext());
	...
	public void shouldRefreshOtherModulesOnInstallingNewModule()
		...
		// willRefreshContext() and contextRefreshed() methods get called for
		// all started modules' activators (including the newly installed
		// module)
		// willStart() and started() methods get called for ONLY the newly
		// installed module's activator

		// install a new module3
		URL url = OpenmrsClassLoader.getInstance().getResource(
				"org/openmrs/module/include/test3-1.0-SNAPSHOT.omod");
	...
	public void shouldUpgradeModule() throws Exception {
		...
		URL url = OpenmrsClassLoader.getInstance().getResource(
				"org/openmrs/module/include/test3-2.0-SNAPSHOT.omod");
		module.setDownloadURL("file:" + url.getFile());

		createWebInfFolderIfNotExist();

		ModuleFactory.stopModule(module, false, true); // stop the module with
														// these parameters so
														// that mandatory
														// modules can be
														// upgraded
		...
		WebModuleUtil.startModule(newModule,
				((XmlWebApplicationContext) applicationContext)
						.getServletContext(), false);
	...
	public void shouldUpgradeModuleWithDependents() throws Exception {
		...
		URL url = OpenmrsClassLoader.getInstance().getResource(
				"org/openmrs/module/include/test1-2.0-SNAPSHOT.omod");
		module.setDownloadURL("file:" + url.getFile());
