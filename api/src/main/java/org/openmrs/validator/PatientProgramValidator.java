...
public class PatientProgramValidator implements Validator {
	...
	private static final Log log = LogFactory
			.getLog(PatientProgramValidator.class);
	 ...
	 * 
	 * @param obj
	 *            The patient program to validate.
	 * @param errors
	 *            Errors
	 ...
	 * @should fail if there is more than one patientState with the same states
	 *         and startDates
	 * @should fail if there is more than one state with a null start date in
	 *         the same workflow
	 * @should pass if the start date of the first patient state in the work
	 *         flow is null
	 * @should fail if any patient state has an end date before its start date
	 * @should fail if the program property is null
	 * @should fail if any patient states overlap each other in the same work
	 *         flow
	 * @should fail if a patientState has an invalid work flow state
	 * @should fail if a patient program has duplicate states in the same work
	 *         flow
	 ...
	 * @should pass for patient states that have the same start dates in the
	 *         same work flow
	...
	public void validate(Object obj, Errors errors) {
		...
		if (obj == null) {
			throw new IllegalArgumentException(
					"The parameter obj should not be null");
		}
		...
		ValidationUtils.rejectIfEmpty(errors, "dateEnrolled",
				"error.patientProgram.enrolledDateEmpty");

		Date today = new Date();
		if (patientProgram.getDateEnrolled() != null
				&& today.before(patientProgram.getDateEnrolled())) {
			errors.rejectValue("dateEnrolled",
					"error.patientProgram.enrolledDateDateCannotBeInFuture");
		}

		if (patientProgram.getDateCompleted() != null
				&& today.before(patientProgram.getDateCompleted())) {
			errors.rejectValue("dateCompleted",
					"error.patientProgram.completionDateCannotBeInFuture");
		}

		// if enrollment or complete date of program is in future or complete
		// date has come before enroll date we should throw error
		if (patientProgram.getDateEnrolled() != null
				&& OpenmrsUtil.compareWithNullAsLatest(
						patientProgram.getDateCompleted(),
						patientProgram.getDateEnrolled()) < 0) {
			errors.rejectValue("dateCompleted",
					"error.patientProgram.enrolledDateShouldBeBeforecompletionDate");
		}
		...
		for (ProgramWorkflow workFlow : workFlows) {
			...
			if (patientStates != null) {
				// Set to store to keep track of unique valid state and start
				// date combinations
				...
				for (PatientState patientState : patientStates) {
					...
					if (missingRequiredFieldCode != null) {
						errors.rejectValue("states",
								"PatientState.error.requiredField",
								new Object[]{mss
										.getMessage(missingRequiredFieldCode)},
								null);
						return;
					}

					// state should belong to one of the workflows in the
					// program
					// note that we are iterating over getAllWorkflows() here
					// because we want to include
					// retired workflows, and the workflows variable does not
					// include retired workflows
					boolean isValidPatientState = false;
					for (ProgramWorkflow wf : patientProgram.getProgram()
							.getAllWorkflows()) {
					...
					if (!patientState.getState().getProgramWorkflow()
							.equals(workFlow)) {
						continue;
					}

					if (OpenmrsUtil.compareWithNullAsLatest(
							patientState.getEndDate(),
							patientState.getStartDate()) < 0) {
						errors.rejectValue("states",
								"PatientState.error.endDateCannotBeBeforeStartDate");
						return;
					} else if (statesAndStartDates.contains(patientState
							.getState().getUuid()
							+ ""
							+ patientState.getStartDate())) {
						// we already have a patient state with the same work
						// flow state and start date
						errors.rejectValue("states",
								"PatientState.error.duplicatePatientStates");
						return;
					}

					// Ensure that the patient is only in one state at a given
					// time
					if (!foundCurrentPatientState
							&& patientState.getEndDate() == null) {
						foundCurrentPatientState = true;
					} else if (foundCurrentPatientState
							&& patientState.getEndDate() == null) {
						errors.rejectValue("states",
								"PatientProgram.error.cannotBeInMultipleStates");
						return;
					}
					...
					if (latestState == null) {
						...
						if (patientState.compareTo(latestState) > 0) {
							// patient should have already left this state since
							// it is older
							if (latestState.getEndDate() == null) {
								...
								// current state was started before a previous
								// state was ended
								errors.rejectValue(
										"states",
							...
							if (patientState.getEndDate() == null) {
								...
								return;
									...
									latestState.getStartDate(),
									patientState.getEndDate()) < 0) {
								// latest state was started before a previous
								// state was ended
								errors.rejectValue("states",
										"PatientProgram.error.foundOverlappingStates");
								return;
					...
					statesAndStartDates.add(patientState.getState().getUuid()
							+ "" + patientState.getStartDate());
				}
			}
		}
