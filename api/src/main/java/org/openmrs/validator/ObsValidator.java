...
/**
 * Validator for the Obs class. This class checks for anything set on the Obs
 * object that will cause errors or is incorrect. Things checked are similar to:
 * <ul>
...
public class ObsValidator implements Validator {
	 ...
	 * @should fail validation if concept datatype is boolean and valueBoolean
	 *         is null
	 * @should fail validation if concept datatype is coded and valueCoded is
	 *         null
	 * @should fail validation if concept datatype is date and valueDatetime is
	 *         null
	 * @should fail validation if concept datatype is numeric and valueNumeric
	 *         is null
	...
	public void validate(Object obj, Errors errors) {
		...
		ValidateUtil.validateFieldLengths(errors, obj.getClass(),
				"accessionNumber", "valueModifier", "valueComplex", "comment",
				"voidReason");
	}

	/**
	 * Checks whether obs has all required values, and also checks to make sure
	 * that no obs group contains any of its ancestors
	 * 
	 * @param obs
	 * @param errors
	 * @param ancestors
	 * @param atRootNode
	 *            whether or not this is the obs that validate() was originally
	 *            called on. If not then we shouldn't reject fields by name.
	 */
	private void validateHelper(Obs obs, Errors errors, List<Obs> ancestors,
		...
		// if this is an obs group (i.e., parent) make sure that it has no
		// values (other than valueGroupId) set
		...
		// if this is NOT an obs group, make sure that it has at least one value
		// set (not counting obsGroupId)
		else if (obs.getValueBoolean() == null && obs.getValueCoded() == null
				&& obs.getValueCodedName() == null
				&& obs.getValueComplex() == null
				&& obs.getValueDatetime() == null && obs.getValueDrug() == null
		...
		// if there is a concept, and this isn't a group, perform validation
		// tests specific to the concept datatype
			...
			if (dt != null) {
				if (dt.isBoolean() && obs.getValueBoolean() == null) {
					if (atRootNode) {
						...
						errors.rejectValue("groupMembers",
					...
					ConceptNumeric cn = Context.getConceptService()
							.getConceptNumeric(c.getConceptId());
					// If the concept numeric is not precise, the value cannot
					// be a float, so raise an error
					if (!cn.isAllowDecimal()
							&& Math.ceil(obs.getValueNumeric()) != obs
									.getValueNumeric()) {
						if (atRootNode) {
							errors.rejectValue("valueNumeric",
									"error.precision");
						} else {
							errors.rejectValue("groupMembers",
									"Obs.error.inGroupMember");
					...
					// If the number is higher than the absolute range, raise an
					// error
					if (cn.getHiAbsolute() != null
							&& cn.getHiAbsolute() < obs.getValueNumeric()) {
						if (atRootNode) {
							errors.rejectValue("valueNumeric",
									"error.outOfRange.high");
						} else {
							errors.rejectValue("groupMembers",
									"Obs.error.inGroupMember");
						}
					}
					// If the number is lower than the absolute range, raise an
					// error as well
					if (cn.getLowAbsolute() != null
							&& cn.getLowAbsolute() > obs.getValueNumeric()) {
						if (atRootNode) {
							errors.rejectValue("valueNumeric",
									"error.outOfRange.low");
						} else {
							errors.rejectValue("groupMembers",
									"Obs.error.inGroupMember");
						}
				...
				// If valueText is longer than the maxlength, raise an error as
				// well.
				if (dt.isText() && obs.getValueText() != null
						&& obs.getValueText().length() > VALUE_TEXT_MAX_LENGTH) {
					if (atRootNode) {
						errors.rejectValue("valueText",
		...
		if (obs.getValueCoded() != null && obs.getValueDrug() != null
				&& obs.getValueDrug().getConcept() != null) {
			Concept trueConcept = Context.getConceptService().getTrueConcept();
			Concept falseConcept = Context.getConceptService()
					.getFalseConcept();
			// Ignore if this is not a true or false response since they are
			// stored as coded too
			if (!obs.getValueCoded().equals(trueConcept)
					&& !obs.getValueCoded().equals(falseConcept)
					&& !obs.getValueDrug().getConcept()
							.equals(obs.getValueCoded())) {
