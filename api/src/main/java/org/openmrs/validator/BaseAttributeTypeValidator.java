...
public abstract class BaseAttributeTypeValidator<T extends AttributeType<?>>
	 ...
	 * @should require DatatypeConfiguration if Datatype equals Regex-Validated
	 *         Text
	...
	public void validate(Object target, Errors errors) {
		...
		if (attributeType == null) {
			...
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "name",
					"error.name");
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "minOccurs",
					"error.null");
			...
			if (minOccurs != null && minOccurs < 0) {
				errors.rejectValue("minOccurs",
						"AttributeType.minOccursShouldNotBeLessThanZero");
			}

			if (maxOccurs != null) {
				if (maxOccurs < 1) {
					errors.rejectValue("maxOccurs",
							"AttributeType.maxOccursShouldNotBeLessThanOne");
				} else if (maxOccurs < minOccurs) {
					errors.rejectValue("maxOccurs",
							"AttributeType.maxOccursShouldNotBeLessThanMinOccurs");
				}
			}
			...
			if (StringUtils.isBlank(attributeType.getDatatypeClassname())) {
				...
				try {
					CustomDatatype<?> datatype = CustomDatatypeUtil
							.getDatatype(attributeType);
					...
					errors.rejectValue("datatypeConfig",
