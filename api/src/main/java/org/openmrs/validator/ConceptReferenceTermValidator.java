...
public class ConceptReferenceTermValidator implements Validator {
	 ...
	 * @should fail if the concept reference term code is a duplicate in its
	 *         concept source
	 * @should fail if the concept source is null
	 * @should pass if all the required fields are set and valid
	 * @should pass if the duplicate name is for a term from another concept
	 *         source
	 * @should pass if the duplicate code is for a term from another concept
	 *         source
	...
	public void validate(Object obj, Errors errors) throws APIException {
		...
		if (conceptReferenceTerm.getConceptSource() == null) {
			errors.rejectValue("conceptSource",
		...
		ConceptReferenceTerm termWithDuplicateCode = Context
				.getConceptService().getConceptReferenceTermByCode(code,
						conceptReferenceTerm.getConceptSource());
		if (termWithDuplicateCode != null
				&& !OpenmrsUtil.nullSafeEquals(termWithDuplicateCode.getUuid(),
						conceptReferenceTerm.getUuid())) {
			errors.rejectValue("code", "ConceptReferenceTerm.duplicate.code",
					"Duplicate concept reference term code in its concept source: "
							+ code);
		}

		// validate the concept reference term maps
		if (CollectionUtils.isNotEmpty(conceptReferenceTerm
				.getConceptReferenceTermMaps())) {
			int index = 0;
			Set<String> mappedTermUuids = null;
			for (ConceptReferenceTermMap map : conceptReferenceTerm
					.getConceptReferenceTermMaps()) {
				if (map == null) {
				...
				if (map.getConceptMapType() == null) {
					errors.rejectValue("conceptReferenceTermMaps[" + index
							+ "].conceptMapType",
					...
					errors.rejectValue("conceptReferenceTermMaps[" + index
							+ "].termB", "ConceptReferenceTerm.map.sameTerm",
		...
		ValidateUtil.validateFieldLengths(errors, obj.getClass(), "name",
				"code", "version", "description", "retireReason");
	}
}
