...
public class PatientIdentifierValidator implements Validator {
	...
	private static Log log = LogFactory
			.getLog(PatientIdentifierValidator.class);
	 ...
	 * 
	 * @param pi
	 *            - the {@link PatientIdentifier} to validate
	 * @throws PatientIdentifierException
	 *             if the {@link PatientIdentifier} is invalid
	 * @should fail validation if PatientIdentifier is null
	 * @should pass validation if PatientIdentifier is voided
	 * @should fail validation if another patient has a matching identifier of
	 *         the same type
	...
	public static void validateIdentifier(PatientIdentifier pi)
		...
		if (!pi.isVoided()) {
			...
			// Check that location is included if it is required (default
			// behavior is to require it)
			LocationBehavior lb = pi.getIdentifierType().getLocationBehavior();
			if (pi.getLocation() == null
					&& (lb == null || lb == LocationBehavior.REQUIRED)) {
				String identifierString = (pi.getIdentifier() != null) ? pi
						.getIdentifier() : "";
				throw new PatientIdentifierException(Context
	 ...
	 * {@link PatientIdentifierType} Checks for things like blank identifiers,
	 * invalid check digits, and invalid format.
	 * 
	 * @param pit
	 *            - the {@link PatientIdentifierType} to validate against
	 * @param identifier
	 *            - the identifier to check against the passed
	...
	public static void validateIdentifier(String identifier,
			PatientIdentifierType pit) throws PatientIdentifierException {
		...
		if (pit.hasValidator()) {
			IdentifierValidator validator = Context.getPatientService()
					.getIdentifierValidator(pit.getValidator());
	...
	/**
	 * Validates that a given identifier string is valid for a given regular
	 * expression format
	 * 
	 * @param identifier
	 *            - the identifier to check against the passed
	 *            {@link PatientIdentifierType}
	 * @param format
	 *            - the regular expression format to validate against
	 * @param formatDescription
	 *            - user-friendly way of describing format (may be null)
	 * @throws PatientIdentifierException
	 *             if the identifier is does not match the format
	 * @should fail validation if identifier is blank
	 ...
	 * @should include format in error message if no formatDescription is
	 *         specified
	 * @should include formatDescription in error message if specified
	 */
	public static void checkIdentifierAgainstFormat(String identifier,
			String format, String formatDescription)
			throws PatientIdentifierException {

		log.debug("Checking identifier: " + identifier + " against format: "
				+ format);
		...
		if (!identifier.matches(format)) {
			...
			throw new InvalidIdentifierFormatException(getMessage(
					"PatientIdentifier.error.invalidFormat", identifier,
					StringUtils.isNotBlank(formatDescription)
							? formatDescription
							: format));
		}
	 ...
	 *            {@link PatientIdentifierType}
	 * @param validator
	 *            the IdentifierValidator to use to check the identifier
	 * @throws PatientIdentifierException
	 *             if the identifier is does not match the format
	 * @should fail validation if identifier is blank
	...
	private static String getMessage(String messageKey, String... arguments) {
		return Context.getMessageSourceService().getMessage(messageKey,
				arguments, Context.getLocale());
	}
}
