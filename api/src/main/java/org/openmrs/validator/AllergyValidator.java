...
public class AllergyValidator implements Validator {
	 ...
	 * @should fail if nonCodedAllergen is null and allergen is set to other non
	 *         coded
	...
	public void validate(Object target, Errors errors) {
		...
		if (allergy.getAllergen() == null) {
			...
			if (allergen.getAllergenType() == null) {
				errors.rejectValue("allergen",
						"allergyapi.allergenType.required");
			}

			if (allergen.getCodedAllergen() == null
					&& StringUtils.isBlank(allergen.getNonCodedAllergen())) {
				errors.rejectValue("allergen",
						"allergyapi.allergen.codedOrNonCodedAllergen.required");
			} else if (!allergen.isCoded()
					&& StringUtils.isBlank(allergen.getNonCodedAllergen())) {
				errors.rejectValue("allergen",
			...
			if (allergy.getAllergyId() == null && allergy.getPatient() != null) {
				Allergies existingAllergies = patientService
						.getAllergies(allergy.getPatient());
				if (existingAllergies.containsAllergen(allergy)) {
					String name;
					if (allergen.isCoded()) {
						String key = "ui.i18n.Concept.name."
								+ allergen.getCodedAllergen().getUuid();
						name = Context.getMessageSourceService()
								.getMessage(key);
						if (key.equals(name)) {
					...
					errors.rejectValue("allergen",
							"allergyapi.message.duplicateAllergen",
