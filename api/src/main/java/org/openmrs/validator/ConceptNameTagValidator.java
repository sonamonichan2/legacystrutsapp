...
public class ConceptNameTagValidator implements Validator {
	 ...
	 * @should pass validation if tag does not exist and is not null, empty or
	 *         whitespace
	...
	public void validate(Object obj, Errors errors) {
		...
		if (cnt == null) {
			...
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "tag",
					"error.name");

			if (cnt.getTag() != null) {
				ConceptNameTag currentTag = Context.getConceptService()
						.getConceptNameTagByName(cnt.getTag());
				if (currentTag != null
						&& !OpenmrsUtil.nullSafeEqualsIgnoreCase(cnt.getUuid(),
								currentTag.getUuid())
						&& OpenmrsUtil.nullSafeEqualsIgnoreCase(
								currentTag.getTag(), cnt.getTag())) {
					errors.rejectValue("tag", "Concept.name.tag.duplicate");
				}
			}
			ValidateUtil.validateFieldLengths(errors, obj.getClass(), "tag",
					"voidReason");
		}
	}
}
