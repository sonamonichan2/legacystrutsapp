...
public class EncounterValidator implements Validator {
	...
	/**
	 * Validates the given Encounter. Currently checks if the patient has been
	 * set and also ensures that the patient for an encounter and the visit it
	 * is associated to if any, are the same.
	 * 
	 * @param obj
	...
	public void validate(Object obj, Errors errors) throws APIException {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "encounterDatetime",
				"Encounter.datetime.required");
		if (encounter.getVisit() != null
				&& !ObjectUtils.equals(encounter.getVisit().getPatient(),
		...
		if (visit != null && encounterDateTime != null) {
			if (visit.getStartDatetime() != null
					&& encounterDateTime.before(visit.getStartDatetime())) {
				errors.rejectValue("encounterDatetime",
			...
			if (visit.getStopDatetime() != null
					&& encounterDateTime.after(visit.getStopDatetime())) {
				errors.rejectValue("encounterDatetime",
						"Encounter.datetimeShouldBeInVisitDatesRange",
						"The encounter datetime should be between the visit start and stop dates.");
			}
