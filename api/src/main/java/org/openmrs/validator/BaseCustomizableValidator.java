...
/**
 * This abstract class provides utilities for validators for Customizable
 * subclasses.
 * 
 * @since 1.9
 */
public abstract class BaseCustomizableValidator implements Validator {
	...
	/**
	 * Validate the attributes of the given Customizable, given the list of
	 * relevant attribute types
	 */
	@SuppressWarnings("rawtypes")
	public <T extends AttributeType, A extends Attribute> void validateAttributes(
			Customizable<A> customizable, Errors errors,
			Collection<T> attributeTypes) {

		// check to make sure that the target has the right number of each type
		// of attribute
			...
			if ((at.getMinOccurs() > 0 || at.getMaxOccurs() != null)
				...
				if (at.getMinOccurs() > 0 && numFound < at.getMinOccurs()) {
					...
					if (at.getMinOccurs() == 1) {
						errors.rejectValue("activeAttributes",
								"error.required", new Object[]{at.getName()},
								null);
					} else {
						errors.rejectValue("activeAttributes",
		...
		// validate all non-voided attributes for their values (we already
		// checked minOccurs and maxOccurs for the types)
