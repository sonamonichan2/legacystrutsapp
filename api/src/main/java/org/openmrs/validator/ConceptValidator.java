 ...
 * href="https://wiki.openmrs.org/x/-gkdAg"
 * >https://wiki.openmrs.org/x/-gkdAg</a>. Any changes made to this source also
 * need to be reflected on that page.
 */
@Handler(supports = {Concept.class}, order = 50)
public class ConceptValidator extends BaseCustomizableValidator
		implements
			Validator {
	 ...
	 * @should pass if the concept has atleast one fully specified name added to
	 *         it
	 * @should fail if there is a duplicate unretired concept name in the locale
	 * @should fail if there is a duplicate unretired preferred name in the same
	 *         locale
	 * @should fail if there is a duplicate unretired fully specified name in
	 *         the same locale
	 ...
	 * @should pass if the concept being validated is retired and has a
	 *         duplicate name
	 ...
	 * @should pass if the duplicate ConceptName is neither preferred nor fully
	 *         Specified
	 ...
	 * @should fail if there is a duplicate unretired concept name in the same
	 *         locale different than the system locale
	 * @should pass for a new concept with a map created with deprecated concept
	 *         map methods
	 * @should pass for an edited concept with a map created with deprecated
	 *         concept map methods
	...
	public void validate(Object obj, Errors errors) throws APIException,
		...
		ValidationUtils.rejectIfEmpty(errors, "datatype",
				"Concept.datatype.empty");
		ValidationUtils.rejectIfEmpty(errors, "conceptClass",
				"Concept.conceptClass.empty");

		boolean hasFullySpecifiedName = false;
		for (Locale conceptNameLocale : conceptToValidate
			...
			Collection<ConceptName> namesInLocale = conceptToValidate
					.getNames(conceptNameLocale);
			for (ConceptName nameInLocale : namesInLocale) {
				...
				if (nameInLocale.isLocalePreferred() != null) {
					if (nameInLocale.isLocalePreferred()
						...
						if (nameInLocale.isIndexTerm()) {
							log.warn("Preferred name in locale '"
					...
					// should have one preferred name per locale
							...
							&& preferredNameForLocaleFound) {
						log.warn("Found multiple preferred names in locale '"
								+ conceptNameLocale.toString() + "'");
				...
				if (nameInLocale.isFullySpecifiedName()) {
					...
					if (!fullySpecifiedNameForLocaleFound) {
						...
						log.warn("Found multiple fully specified names in locale '"
								+ conceptNameLocale.toString() + "'");
						errors.reject("Concept.error.multipleFullySpecifiedNames");
					}
					if (nameInLocale.isVoided()) {
						log.warn("Fully Specified name in locale '"
								+ conceptNameLocale.toString()
								+ "' shouldn't be a voided name");
				...
				if (nameInLocale.isShort()) {
					...
					// should have one short name per locale
						...
						log.warn("Found multiple short names in locale '"
								+ conceptNameLocale.toString() + "'");
				...
				if (Context.getConceptService().isConceptNameDuplicate(
						nameInLocale)) {
				...
				//
				if (errors.hasErrors()) {
					log.debug("Concept name '" + nameInLocale.getName()
							+ "' for locale '" + conceptNameLocale
							+ "' is invalid");
					// if validation fails for any conceptName in current
					// locale, don't proceed
					// This helps not to have multiple messages shown that are
				...
				if (!nameInLocale.isShort()
						&& !validNamesFoundInLocale.add(nameInLocale.getName()
								.toLowerCase())) {
					throw new DuplicateConceptNameException("'"
							+ nameInLocale.getName()
							+ "' is a duplicate name in locale '"
		...
		if (CollectionUtils.isNotEmpty(conceptToValidate.getConceptMappings())) {
			...
			for (ConceptMap map : conceptToValidate.getConceptMappings()) {
				if (map.getConceptReferenceTerm().getConceptReferenceTermId() == null) {
					// if this term is getting created on the fly e.g. from old
					// legacy code, validate it
					try {
						errors.pushNestedPath("conceptMappings[" + index
				 ...
				 * errors.rejectValue("conceptMappings[" + index +
				 * "].conceptMapType", "Concept.map.typeRequired",
				 * "The concept map type is required for a concept map");
				 * return; }
				 */

				// don't proceed to the next maps since the current one already
				// has errors
				if (errors.hasErrors()) {
					return;
				}
				...
				if (map.getConceptReferenceTerm().getId() != null
						&& !mappedTermIds.add(map.getConceptReferenceTerm()
								.getId())) {
					errors.rejectValue("conceptMappings[" + index + "]",
		...
		ValidateUtil.validateFieldLengths(errors, obj.getClass(), "version",
				"retireReason");
		super.validateAttributes(conceptToValidate, errors, Context
				.getConceptService().getAllConceptAttributeTypes());
	}
}
