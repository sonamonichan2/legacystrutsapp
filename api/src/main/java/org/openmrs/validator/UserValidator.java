...
public class UserValidator implements Validator {
		...
		if (user == null) {
			...
			if (user.getPerson() == null) {
				...
				if (person.getPersonName() == null
						|| StringUtils.isEmpty(person.getPersonName()
								.getFullName())) {
			...
			if (emailAsUsername) {
				boolean isValidUserName = isUserNameAsEmailValid(user
						.getUsername());
				if (!isValidUserName) {
			...
			ValidateUtil.validateFieldLengths(errors, obj.getClass(),
					"username", "systemId", "retireReason");
		}
	}

	/**
	 * Convenience method to check the given username against the regular
	 * expression. <br>
	 * <br>
	...
	public boolean isUserNameValid(String username) {
		...
		// $ = end of line
		// complete meaning = 2-50 characters, the first must be a letter,
		// digit, or _, and the rest may also be - or .
