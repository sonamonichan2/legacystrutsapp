...
public class PersonAttribute extends BaseOpenmrsData
	...
	public Object getHydratedObject() {
		...
		try {
			Class c = OpenmrsClassLoader.getInstance().loadClass(
					getAttributeType().getFormat());
			try {
				...
				log.trace("Unable to call no-arg constructor for class: "
						+ c.getName());
				Object o = c.getConstructor(String.class).newInstance(
						getValue());
				return o;
			}
		} catch (Exception e) {
	 ...
	 * @should return negative if this attribute has lower attribute type than
	 *         argument
	 * @should return negative if other attribute has lower value
	 * @should return negative if this attribute has lower attribute id than
	 *         argument
	 * @should not throw exception if attribute type is null Note: this
	...
	public static class DefaultComparator
			implements
				Comparator<PersonAttribute> {

		@Override
		public int compare(PersonAttribute pa1, PersonAttribute pa2) {
			int retValue;
			if ((retValue = OpenmrsUtil.compareWithNullAsGreatest(
					pa1.getAttributeType(), pa2.getAttributeType())) != 0) {
				return retValue;
			}
			...
			if ((retValue = OpenmrsUtil.compareWithNullAsLatest(
					pa1.getDateCreated(), pa2.getDateCreated())) != 0) {
				return retValue;
			}

			if ((retValue = OpenmrsUtil.compareWithNullAsGreatest(
					pa1.getValue(), pa2.getValue())) != 0) {
				return retValue;
			}
