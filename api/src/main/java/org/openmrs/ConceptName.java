...
/**
 * ConceptName is the real world term used to express a Concept within the idiom
...
public class ConceptName extends BaseOpenmrsObject
	...
	private Locale locale; // ABK: upgraded from a plain string to a full locale
							// object
	...
	/**
	 * Adds a tag to the concept name. If the tag is new (has no existing
	 * occurrences) a new ConceptNameTag will be created with a blank
	 * description.
	 * 
	 * @see Concept#setPreferredName(ConceptName)
	 * @see Concept#setFullySpecifiedName(ConceptName)
	 * @see Concept#setShortName(ConceptName)
	 * @param tag
	 *            human-readable text string for the tag
	 */
	...
	/**
	 * Adds a tag to the concept name. If the tag is new (has no existing
	 * occurrences) a new ConceptNameTag will be created with the given
	 * description.
	 * 
	 * @see Concept#setPreferredName(ConceptName)
	 * @see Concept#setFullySpecifiedName(ConceptName)
	...
	/**
	 * Checks whether the name is explicitly marked as preferred in a locale
	 * with a matching language. E.g 'en_US' and 'en_UK' for language en
	 * 
	 * @see #isPreferredForLocale(Locale)
	 * @param language
	 *            ISO 639 2-letter code for a language
	 * @return true if the name is preferred in a locale with a matching
	...
	public Boolean isPreferredInLanguage(String language) {
		if (!StringUtils.isBlank(language) && this.locale != null
				&& isPreferred() && this.locale.getLanguage().equals(language)) {
			return true;
		}
	...
	/**
	 * Checks whether the name is explicitly marked as preferred in a locale
	 * with a matching country code E.g 'fr_RW' and 'en_RW' for country RW
	 * 
	 * @see #isPreferredForLocale(Locale)
	 * @param country
	 *            ISO 3166 2-letter code for a country
	 * @return true if the name is preferred in a locale with a matching country
	 *         code, otherwise false
	 */
	public Boolean isPreferredInCountry(String country) {
		if (!StringUtils.isBlank(country) && this.locale != null
				&& isPreferred() && this.locale.getCountry().equals(country)) {
			return true;
		}
	 ...
	 * Note that this method is different from
	 * {@link #isPreferredForLocale(Locale)} in that it checks if the given name
	 * is marked as preferred irrespective of the locale in which it is
	 * preferred.
	 * 
	 * @see #isPreferredForLocale(Locale)
	 */
	...
	/**
	 * Checks whether the name is explicitly marked as preferred for the given
	 * locale
	 * 
	 * @param locale
	...
	public Boolean isFullySpecifiedName() {
		return OpenmrsUtil.nullSafeEquals(getConceptNameType(),
	...
	/**
	 * Convenience method for determining whether this is an index Term for a
	 * given locale.
	 * 
	 * @param locale
	 *            The locale in which this concept name should belong as an
	 *            index term
	 * @return true if the name is marked as an index term, otherwise false
	 */
	...
	/**
	 * Convenience method for determining whether this is a synonym in a given
	 * locale.
	 * 
	 * @param locale
	 *            The locale in which this synonym should belong
	 * @return true if the concept name is marked as a synonym in the given
