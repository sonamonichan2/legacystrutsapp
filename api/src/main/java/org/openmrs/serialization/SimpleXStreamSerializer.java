...
/**
 * This serializer uses the xstream library to serialize and deserialize
 * objects. <br>
 * All classes are automatically aliased. So a serialization of the
 * {@link Patient} class will not be: <code>
 * &lt;org.openmrs.Patient ...&gt;
 *   &lt;element
 *   ...
 * &lt;/org.openmrs.Patient&gt;
...
public class SimpleXStreamSerializer implements OpenmrsSerializer {
	...
	public SimpleXStreamSerializer(XStream customXstream)
		...
		xstream.registerConverter(new OpenmrsDynamicProxyConverter(),
				XStream.PRIORITY_VERY_HIGH);

		// this is added to read the prior simpleframework-serialized values.
		// TODO find a better way to do this.
		this.xstream
				.useAttributeFor(ImplementationId.class, "implementationId");
	...
	public <T extends Object> T deserialize(String serializedObject,
		...
		try {
			...
			throw new SerializationException("Unable to deserialize class: "
					+ clazz.getName(), e);
		}
	}

	/**
	 * An instance of this converter needs to be registered with a higher
	 * priority than the rest so that it's called early in the converter chain.
	 * This way, we can make sure we never get to xstream's
