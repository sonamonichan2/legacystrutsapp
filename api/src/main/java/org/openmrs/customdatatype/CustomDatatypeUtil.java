...
public class CustomDatatypeUtil {
	...
	public static CustomDatatype<?> getDatatype(CustomValueDescriptor descriptor) {
		return getDatatype(descriptor.getDatatypeClassname(),
	 ...
	 */
	public static CustomDatatype<?> getDatatype(String datatypeClassname,
			String datatypeConfig) {
		try {
			Class dtClass = Context.loadClass(datatypeClassname);
			CustomDatatype<?> ret = (CustomDatatype<?>) Context
					.getDatatypeService().getDatatype(dtClass, datatypeConfig);
			if (ret == null) {
				throw new CustomDatatypeException("Can't find datatype: "
						+ datatypeClassname);
			}
			return ret;
		} catch (Exception ex) {
			throw new CustomDatatypeException("Error loading "
					+ datatypeClassname + " and configuring it with "
					+ datatypeConfig, ex);
		}
	}
	 ...
	 * @return a configured datatype appropriate for descriptor
	 */
	public static CustomDatatype<?> getDatatypeOrDefault(
	...
	public static CustomDatatypeHandler getHandler(
			CustomValueDescriptor descriptor) {
		return getHandler(getDatatypeOrDefault(descriptor),
	...
	/**
	 * @param dt
	 *            the datatype that this handler should be for
	 * @param preferredHandlerClassname
	 * @param handlerConfig
	 * @return a configured datatype handler with the given classname and
	 *         configuration
	 */
	public static CustomDatatypeHandler getHandler(CustomDatatype<?> dt,
			String preferredHandlerClassname, String handlerConfig) {
			...
			try {
				...
				log.warn(
						"Failed to instantiate and configure preferred handler with class "
								+ preferredHandlerClassname + " and config "
								+ handlerConfig, ex);
			}
		}

		// if we couldn't get the preferred handler (or none was specified) we
		// get the default one by datatype
	 ...
	 */
	public static String serializeSimpleConfiguration(
	...
	/**
	 * Deserializes a simple String-based configuration from the serialized form
	 * used by {@link #serializeSimpleConfiguration(Map)} Utility method for
	 * property-style configuration implementations.
	 * 
	 * @param serializedConfig
	 * @return deserialized configuration
	 * @should deserialize a configuration serialized by the corresponding
	 *         serialize method
	 */
	@SuppressWarnings("unchecked")
	public static Map<String, String> deserializeSimpleConfiguration(
	...
	/**
	 * Uses the appropriate datatypes to convert all values in the input map to
	 * their valueReference equivalents. This is a convenience method for
	 * calling XyzService.getXyz(..., attributeValues, ...).
	 * 
	 * @param datatypeValues
	 * @return a map similar to the input parameter, but with typed values
	 *         converted to their reference equivalents
	 */
	public static <T extends AttributeType<?>, U> Map<T, String> getValueReferences(
		...
		if (datatypeValues != null) {
			...
			for (Map.Entry<T, U> e : datatypeValues.entrySet()) {
				...
				try {
					valueReference = customDatatype
							.getReferenceStringForValue(e.getValue());
				} catch (UnsupportedOperationException ex) {
					throw new APIException(
							"CustomDatatype.error.cannot.search",
							new Object[]{customDatatype.getClass()});
				}
	 ...
	 */
	public static boolean isCompatibleHandler(CustomDatatypeHandler handler,
			CustomDatatype<?> datatype) {
		List<Class<? extends CustomDatatypeHandler>> handlerClasses = Context
				.getDatatypeService().getHandlerClasses(
	 ...
	 * Iterates over all attributes and calls save on the
	 * {@link ConceptDatatype} for any dirty ones.
	 * 
	 * @param customizable
	 */
	public static void saveAttributesIfNecessary(Customizable<?> customizable) {
		// TODO decide whether we can move this into a
		// SingleCustomValueSaveHandler instead of leaving it here to be called
		// by each Customizable service's save method
	...
	public static void saveIfDirty(SingleCustomValue<?> value) {
		if (value.isDirty()) {
			CustomDatatype datatype = CustomDatatypeUtil.getDatatype(value
					.getDescriptor());
			if (value.getValue() == null) {
				throw new InvalidCustomValueException(value.getClass()
						+ " with type=" + value.getDescriptor()
						+ " cannot be null");
			}
			...
			String newValueReference = datatype.save(value.getValue(),
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T, D extends CustomValueDescriptor> boolean validate(
			SingleCustomValue<D> value) {
		try {
			CustomDatatype<T> datatype = (CustomDatatype<T>) getDatatype(value
					.getDescriptor());
