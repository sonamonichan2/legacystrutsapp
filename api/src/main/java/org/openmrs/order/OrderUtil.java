...
public class OrderUtil {
	 ...
	 * @should false if orderType2 is neither the same nor a subtype of
	 *         orderType1
	...
	/**
	 * Checks if the schedules of the specified orders overlap, Note this only
	 * makes a check that is purely based on dates ignoring other properties
	 * like patient, voided, careSetting and the orderable
	 * 
	 * @since 1.10
	 * @param order1
	 *            order to match
	 ...
	 * @should return true if order1 stops after the order2 has already been
	 *         activated
	...
	public static boolean checkScheduleOverlap(Order order1, Order order2) {
		if (order2.getEffectiveStopDate() == null
				&& order1.getEffectiveStopDate() == null) {
			return true;
		}

		if (order2.getEffectiveStopDate() == null) {
			return OpenmrsUtil.compare(order1.getEffectiveStopDate(),
					order2.getEffectiveStartDate()) > -1;
		}

		if (order1.getEffectiveStopDate() == null) {
			return (OpenmrsUtil.compare(order1.getEffectiveStartDate(),
					order2.getEffectiveStartDate()) > -1)
					&& (OpenmrsUtil.compare(order1.getEffectiveStartDate(),
							order2.getEffectiveStopDate()) < 1);
		}

		return (OpenmrsUtil.compare(order1.getEffectiveStartDate(),
				order2.getEffectiveStopDate()) < 1)
				&& (OpenmrsUtil.compare(order1.getEffectiveStopDate(),
						order2.getEffectiveStartDate()) > -1);
	}
