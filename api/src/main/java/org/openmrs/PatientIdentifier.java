 ...
 * the PatientIdentifierType. A PatientIdentifier also contains a Location.
 * 
...
public class PatientIdentifier extends BaseOpenmrsData
	 ...
	 */
	public PatientIdentifier(String identifier, PatientIdentifierType type,
			Location location) {
	...
	/**
	 * Compares this PatientIdentifier object to the given otherIdentifier. This
	 * method differs from {@link #equals(Object)} in that this method compares
	 * the inner fields of each identifier for equality. Note: Null/empty fields
	 * on <code>otherIdentifier</code> /will not/ cause a false value to be
	 * returned
	 * 
	 ...
	 *            the patientIdentifierId to set
	 */
			...
			if (pi2 != null) {
				...
				if (retValue == 0) {
					retValue = OpenmrsUtil.compareWithNullAsLatest(
							pi1.getDateCreated(), pi2.getDateCreated());
				}
				if (pi1.getIdentifierType() == null
						&& pi2.getIdentifierType() == null) {
					return 0;
				}
				if (pi1.getIdentifierType() == null
						&& pi2.getIdentifierType() != null) {
				...
				if (retValue == 0) {
					retValue = OpenmrsUtil.compareWithNullAsGreatest(pi1
							.getIdentifierType().getPatientIdentifierTypeId(),
							pi2.getIdentifierType()
									.getPatientIdentifierTypeId());
				}
				if (retValue == 0) {
					retValue = OpenmrsUtil.compareWithNullAsGreatest(
							pi1.getIdentifier(), pi2.getIdentifier());
				}

				// if we've gotten this far, just check all identifier values.
				// If they are
