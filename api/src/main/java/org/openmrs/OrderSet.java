...
/**
 * Represents the grouping of orders into a set, so as to give decision support
 * for the doctors
 * 
 * @since 1.12
 */
public class OrderSet extends BaseOpenmrsMetadata implements Serializable {
	...
	/**
	 * Restrictions put on saving an orderSet. ALL: All the members of the
	 * orderSet need to be selected for saving ONE: Only one of the member of
	 * the orderSet needs to be selected for saving ANY: Any of the members of
	 * the orderSet can be selected for saving
	 */
	 ...
	 * 
	 * @param orderSetMember
	 *            the new orderSetMember to be added
	 * @param position
	 *            the position where it is to be added, if position is null it
		...
		if (position != null) {
			if (position < 0 && position >= (-1 - size)) {
				...
				throw new APIException(
						"Cannot add a member which is out of range of the list");
			}
		} else {
			position = size;
		}
	 ...
	 * 
	 * @param orderSetMember
	 *            the new orderSetMember to be added at the end of the current
	 *            list of order set members
	 */
