...
/**
 * A mapped collection of PresentationMessages, all of which are enforced to be
 * in the same locale.
 */
public class PresentationMessageMap implements Map<String, PresentationMessage> {
	...
	/**
	 * Adds all entries from an input Map which have PresentationMessages from
	 * the same locale.
	 * 
	...
	public void putAll(Map<? extends String, ? extends PresentationMessage> t) {
		// Map<String, PresentationMessage> compatibleMap = new HashMap<String,
		// PresentationMessage>();
		for (Entry<? extends String, ? extends PresentationMessage> entry : t
				.entrySet()) {
