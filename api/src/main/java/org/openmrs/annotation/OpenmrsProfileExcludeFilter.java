...
/**
 * Prevents creating a bean if profile is not matched. It returns true if a bean
...
public class OpenmrsProfileExcludeFilter implements TypeFilter {
	...
	public boolean matchOpenmrsProfileAttributes(
			Map<String, Object> openmrsProfile) {
		Object openmrsPlatformVersion = openmrsProfile
				.get("openmrsPlatformVersion");
		...
		for (String moduleAndVersion : modules) {
			...
			for (Module module : ModuleFactory.getStartedModules()) {
				if (module.getModuleId().equals(moduleId)
						&& ModuleUtil.matchRequiredVersions(
								module.getVersion(), moduleVersion)) {
					moduleMatched = true;
					break;
				}
			}
