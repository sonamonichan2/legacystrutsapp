...
public interface EncounterService extends OpenmrsService {
	 ...
	 * @should only cascade the obsdatetimes to obs with different initial
	 *         obsdatetimes
	 ...
	 * @should not assign encounter to visit if the no assign handler is
	 *         registered
	 * @should assign encounter to visit if the assign to existing handler is
	 *         registered
	 * @should assign encounter to visit if the assign to existing or new
	 ...
	 * @should fail if user is not supposed to edit encounters of type of given
	 *         encounter
	 */
	...
	/**
	 * Get all encounters (not voided) for a patient, sorted by
	 * encounterDatetime ascending.
	 * 
	 * @param patient
	 ...
	 * @param loc
	 *            the location this encounter took place
	 * @param fromDate
	 ...
	 *            the type of encounter must be in this list
	 * @param providers
	 *            the provider of this encounter must be in this list
	 * @param visitTypes
	 *            the visit types of this encounter must be in this list
	 * @param visits
	 ...
	 *             {@link #getEncounters(EncounterSearchCriteria)}
	 */
	@Deprecated
	@Authorized({PrivilegeConstants.GET_ENCOUNTERS})
	...
	/**
	 * Get all encounters that match a variety of (nullable) criteria contained
	 * in the parameter object. Each extra value for a parameter that is
	 * provided acts as an "and" and will reduce the number of results returned
	 * 
	 * @param encounterSearchCriteria
	 *            the object containing search parameters
	 * @return a list of encounters ordered by increasing encounterDatetime
	 * @since 1.12
	 ...
	 * @should throw error when trying to save encounter type when encounter
	 *         types are locked
	 */
	...
	/**
	 * Retire an EncounterType. This essentially marks the given encounter type
	 * as a non-current type that shouldn't be used anymore.
	 * 
	 * @param encounterType
	 *            the encounter type to retire
	 * @param reason
	 ...
	 * @should should throw error when trying to retire encounter type when
	 *         encounter types are locked
	 */
	@Authorized({PrivilegeConstants.MANAGE_ENCOUNTER_TYPES})
	...
	/**
	 * Unretire an EncounterType. This brings back the given encounter type and
	 * says that it can be used again
	 * 
	 * @param encounterType
	 *            the encounter type to unretire
	 * @throws APIException
	 * @should unretire type and unmark attributes
	 * @should should throw error when trying to unretire encounter type when
	 *         encounter types are locked
	 */
	@Authorized({PrivilegeConstants.MANAGE_ENCOUNTER_TYPES})
	 ...
	 * @throws APIException
	 * @should purge type
	 * @should should throw error when trying to delete encounter type when
	 *         encounter types are locked
	 */
	 ...
	 * @should throw error if given null parameter
	 * @should include voided encounters in the returned list if includedVoided
	 *         is true
	 * @since 1.7
	 */
	@Authorized({PrivilegeConstants.GET_ENCOUNTERS})
	 ...
	 * @throws APIException
	 * @since 1.8
	 * @should get all the unique encounters that match the specified parameter
	 *         values
	 * @should not return voided encounters if includeVoided is set to true
	 * @should return empty list for empty query
	 */
	@Authorized({PrivilegeConstants.GET_ENCOUNTERS})
	public List<Encounter> getEncounters(String query, Integer start,
			Integer length, boolean includeVoided) throws APIException;

	/**
	 * Searches for encounters by patient id, provider identifier, location,
	 * encounter type, provider, form or provider name. It returns a specific
	 * number of them from the specified starting position. If start and length
	 * are not specified, then all matches are returned
	 * 
	 * @param query
	 *            provider identifier, location, encounter type, provider, form
	 *            or provider name
	 * @param patientId
	 *            the patient id
	 * @param start
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_ENCOUNTERS})
	public List<Encounter> getEncounters(String query, Integer patientId,
	...
	/**
	 * Return the number of encounters matching a patient name or patient
	 * identifier
	 * 
	 * @param query
	 *            patient name or identifier
	 ...
	 * @throws APIException
	 *             thrown if something goes wrong during the retrieval of the
	 *             handler.
	 */
	public EncounterVisitHandler getActiveEncounterVisitHandler()
			throws APIException;
	...
	public EncounterRole saveEncounterRole(EncounterRole encounterRole)
			throws APIException;
	...
	public EncounterRole getEncounterRole(Integer encounterRoleId)
			throws APIException;

	/**
	 * Completely remove an encounter role from database. For super users only.
	 * If dereferencing encounter roles, use
	 * <code>retireEncounterRole(org.openmrs.Encounter, java.lang.String)</code>
	 * 
	 * @param encounterRole
	 *            encounter role object to be purged
	 * @since 1.9
	 * @should purge Encounter Role
	 */
	@Authorized({PrivilegeConstants.PURGE_ENCOUNTER_ROLES})
	public void purgeEncounterRole(EncounterRole encounterRole)
			throws APIException;
	...
	/**
	 * Retire an EncounterRole. This essentially marks the given encounter role
	 * as a non-current type that shouldn't be used anymore.
	 * 
	 * @param encounterRole
	 *            the encounter role to retire
	 * @param reason
	 *            required non-null purpose for retiring this encounter role
	 * @throws APIException
	 * @since 1.9
	 * @should retire type and set attributes
	 * @should throw error if given null reason parameter
	 */
	@Authorized({PrivilegeConstants.MANAGE_ENCOUNTER_ROLES})
	public EncounterRole retireEncounterRole(EncounterRole encounterRole,
			String reason) throws APIException;

	/**
	 * Unretire an EncounterRole. This brings back the given encounter role and
	 * says that it can be used again
	 * 
	 * @param encounterType
	 ...
	 */
	@Authorized({PrivilegeConstants.MANAGE_ENCOUNTER_ROLES})
	public EncounterRole unretireEncounterRole(EncounterRole encounterType)
			throws APIException;

	/**
	 * Gets the unvoided encounters for the specified patient that are not
	 * assigned to any visit. Note that this method will return a maximum of 100
	 * encounters.
	 * 
	 * @param patient
	...
	/**
	 * Gets encounters for the given patient. It populates results with empty
	 * encounters to include visits that have no assigned encounters.
	 * <p>
	 * The empty encounters have only visit set.
	 * 
	 * @param patient
	 *            the patient to match
	 * @param includeVoided
	 *            if voided encounters or visits should be included
	 * @param query
	 *            filters results (defaults to return all results if
	 *            <code>null</code>)
	 * @param start
	 *            index to start with (defaults to 0 if <code>null</code>)
	 * @param length
	 *            number of results to return (default to return all results if
	 *            <code>null</code>)
	...
	public List<Encounter> getEncountersByVisitsAndPatient(Patient patient,
			boolean includeVoided, String query, Integer start, Integer length)
			throws APIException;
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_VISITS})
	public Integer getEncountersByVisitsAndPatientCount(Patient patient,
			boolean includeVoided, String query) throws APIException;

	/**
	 * Filters out all encounters to which given user does not have access. If
	 * user is not specified then implementations should treat authenticated
	 * user from context as given user by default
	 * 
	 * @param encounters
	 *            the list of encounters to be filtered
	 * @param user
	 *            the user instance to filter "visible" encounters for
	 * @return list, that does not include encounters, which can not be shown to
	 *         given user due to permissions check
	 * @should filter encounters if user is not allowed to see some encounters
	 * @should not filter all encounters when the encounter type's view
	 *         privilege column is null
	 */
	@Authorized({PrivilegeConstants.GET_ENCOUNTERS})
	public List<Encounter> filterEncountersByViewPermissions(
			List<Encounter> encounters, User user);

	/**
	 * Determines whether given user is granted to view all encounter types or
	 * not
	 * 
	 * @param subject
	 *            the user whose permission to view all encounter types will be
	 *            checked
	 ...
	 * @should return true when the encounter type's view privilege column is
	 *         null
	 */
	public boolean canViewAllEncounterTypes(User subject);

	/**
	 * Determines whether given user is granted to edit all encounter types or
	 * not
	 * 
	 * @param subject
	 *            the user whose permission to edit all encounter types will be
	 *            checked
	 ...
	 * @should return true when the encounter type's edit privilege column is
	 *         null
	 */
	public boolean canEditAllEncounterTypes(User subject);

	/**
	 * Checks if passed in user can edit given encounter. If user is not
	 * specified, then authenticated user will be taken by default
	 * 
	 * @param encounter
	 *            the encounter instance to be checked
	 * @param subject
	 *            the user, who requests edit access
	 * @return true if user has privilege denoted by <em>editPrivilege</em>
	 *         given on encounter type
	...
	/**
	 * Checks if passed in user can view given encounter. If user is not
	 * specified, then authenticated user will be taken by default
	 * 
	 * @param encounter
	 *            the encounter instance to be checked
	 * @param subject
	 *            the user, who requests view access
	 * @return true if user has privilege denoted by <em>viewPrivilege</em>
	 *         given on encounter type
	...
	/**
	 * Check if the encounter types are locked, and if so, throw exception
	 * during manipulation of encounter type
	 * 
	 ...
	 * @since 1.12
	 * 
	 * @should transfer an encounter with observations but not orders to given
	 *         patient
	 * @should void given encounter
	 * @should void given encounter visit if given encounter is the only
	 *         encounter
	 */
	@Authorized({PrivilegeConstants.EDIT_ENCOUNTERS})
