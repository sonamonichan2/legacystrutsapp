...
/**
 * This service contains methods relating to Form, FormField, and Field. Methods
 * relating to FieldType are in AdministrationService
 */
public interface FormService extends OpenmrsService {
	 ...
	 * @should throw an error when trying to save an existing form while forms
	 *         are locked
	 * @should throw an error when trying to save a new form while forms are
	 *         locked
	 */
	...
	/**
	 * Get form by exact name match. If there are multiple forms with this name,
	 * then this returns the one with the highest version (sorted
	 * alphabetically)
	 * 
	 * @param name
	...
	/**
	 * Get form by exact name &amp; version match. If version is null, then this
	 * method behaves like {@link #getForm(String)}
	 * 
	 * @param name
	 *            exact name of the form to fetch
	 * @param version
	...
	/**
	 * Gets all forms with name similar to the given name. (The precise fuzzy
	 * matching algorithm is not specified.)
	 * 
	 * @param fuzzyName
	 *            approximate name to match
	 * @param onlyLatestVersion
	 *            whether or not to return only the latest version of each form
	 *            (by name)
	 ...
	 * 
	 * @param partialNameSearch
	 *            partial search of name
	 * @param published
	 *            whether the form is published
	 * @param encounterTypes
	 *            whether the form has any of these encounter types
	 * @param retired
	 *            whether the form is retired
	 * @param containingAnyFormField
	 *            includes forms that contain any of the specified FormFields
	 * @param containingAllFormFields
	 *            includes forms that contain all of the specified FormFields
	 * @param fields
	 *            whether the form has any of these fields. If a field is used
	 *            more than once on a form, that form is returning more than
	 *            once in this list
	 * @return All forms that match the criteria
	 * @should get multiple of the same form by field
	 * @should return duplicate form when given fields included in form multiple
	 *         times
	 * @should only return published forms when given published equals true
	 * @should return both published and unpublished when given published is
	 *         null
	 ...
	 * @should return all forms including retired and unretired when retired is
	 *         null
	 * @should return forms containing all form fields in
	 *         containingAllFormFields
	 * @should return forms that have any matching formFields in
	 *         containingAnyFormField
	 */
	@Authorized(PrivilegeConstants.GET_FORMS)
	public List<Form> getForms(String partialNameSearch, Boolean published,
	 ...
	 * except that it returns an integer that is the size of the list that would
	 * be returned
	 * 
	 * @see #getForms(String, Boolean, Collection, Boolean, Collection,
	 *      Collection, Collection)
	 */
	@Authorized(PrivilegeConstants.GET_FORMS)
	public Integer getFormCount(String partialNameSearch, Boolean published,
			Collection<EncounterType> encounterTypes, Boolean retired,
			Collection<FormField> containingAnyFormField,
			Collection<FormField> containingAllFormFields,
			Collection<Field> fields);
	 ...
	 * @should throw an error when trying to duplicate a form while forms are
	 *         locked
	 */
	@Authorized(PrivilegeConstants.MANAGE_FORMS)
	...
	/**
	 * Retires the Form, leaving it in the database, but removing it from data
	 * entry screens
	 * 
	 * @param form
	 *            the Form to retire
	...
	/**
	 * Completely remove a Form from the database. This is not reversible. It
	 * will fail if this form has already been used to create Encounters
	 * 
	 * @param form
	 * @throws APIException
	 * @should delete given form successfully
	 * @should delete form resources for deleted form
	 * @should throw an error when trying to delete a form while forms are
	 *         locked
	 */
	@Authorized(PrivilegeConstants.MANAGE_FORMS)
	...
	/**
	 * Completely remove a Form from the database. This is not reversible. !!
	 * WARNING: Calling this method with cascade=true can be very destructive !!
	 * 
	 * @param form
	 * @param cascade
	 *            whether or not to cascade delete all dependent objects
	 ...
	 * @return list of all field types
	 * @throws APIException
	 * @should get all field types including retired when includeRetired equals
	 *         true
	 * @should get all field types excluding retired when includeRetired equals
	 *         false
	 */
	@Authorized(PrivilegeConstants.GET_FIELD_TYPES)
	...
	/**
	 * Returns all forms that contain the given concept as a field in their
	 * schema. (includes retired forms)
	 * 
	 * @param concept
	 ...
	 */
	@Authorized(PrivilegeConstants.GET_FORMS)
	public List<Form> getFormsContainingConcept(Concept concept)
			throws APIException;
	 ...
	 * (The exact similarity algorithm is unspecified.) (includes retired
	 * fields)
	 * 
	 ...
	 * @param tableNames
	 *            for any of these table names
	 * @param attributeNames
	 *            for any of these attribute names
	 * @param selectMultiple
	 *            whether to return only select-multi fields
	 * @param containsAllAnswers
	 *            fields with all the following answers
	 * @param containsAnyAnswer
	 *            fields with any of the following answers
	 * @param retired
	 ...
	 * @should get fields with selectMultiple equals true when given
	 *         selectMultiple equals true
	 */
	@Authorized(PrivilegeConstants.GET_FORMS)
	...
	/**
	 * Completely removes a Field from the database. Not reversible. !! WARNING:
	 * calling this with cascade=true can be very destructive !!
	 * 
	 * @param field
	 *            the Field to purge
	 * @param cascade
	 *            whether to cascade delete all FormFields pointing to this
	 *            field
	 * @throws APIException
	 * @should throw APIException if cascade is true
	 */
	...
	/**
	 * Finds the FormField defined for this form/concept combination while
	 * discounting any form field found in the <code>ignoreFormFields</code>
	 * collection This method was added when needing to relate observations to
	 * form fields during a display. The use case would be that you know a
	 * Concept for a obs, which was defined on a form (via a formField). You can
	 * relate the formFields to Concepts easily enough, but if a Form reuses a
	 * Concept in two separate FormFields you don't want to only associate that
	 * first formField with that concept. So, keep a running list of formFields
	 * you've seen and pass them back in here to rule them out.
	 * 
	 * @param form
	 *            Form that this concept was found on
	 * @param concept
	 *            Concept (question) on this form that is being requested
	 * @param ignoreFormFields
	 *            FormFields to ignore (aka already seen formfields)
	 * @param force
	 *            if true and there are zero matches because all formFields were
	 *            ignored (because of ignoreFormFields) than the first result is
	 *            returned
	...
	public FormField getFormField(Form form, Concept concept,
			Collection<FormField> ignoreFormFields, boolean force)
			throws APIException;
	...
	/**
	 * Completely removes the given FormField from the database. This is not
	 * reversible
	 * 
	 * @param formField
	...
	/**
	 * Deletes the given field type from the database. This should not be done.
	 * It is preferred to just retired this field type with
	 * #retireFieldType(FieldType)
	 * 
	 * @param fieldType
	 ...
	 * 
	 * @param form
	 *            the Form that the resource belongs to
	 * @param name
	...
	/**
	 * Checks if the forms are locked, and if they are throws an exception when
	 * saving or deleting a form
	 * 
