...
/**
 * Default implementation of the ProgramWorkflow-related services class. This
 * method should not be invoked by itself. Spring injection is used to inject
 * this implementation into the ServiceContext. Which implementation is injected
 * is determined by the spring application context file:
...
public class ProgramWorkflowServiceImpl extends BaseOpenmrsService
		...
		for (ProgramWorkflow workflow : program.getAllWorkflows()) {
			...
			if (workflow.getProgram() == null) {
				...
				throw new APIException(
						"Program.error.contains.ProgramWorkflow",
						new Object[]{workflow.getProgram()});
			}

			// ProgramWorkflowState
			for (ProgramWorkflowState state : workflow.getStates()) {

				if (state.getConcept() == null || state.getInitial() == null
				...
				if (state.getProgramWorkflow() == null) {
					...
					throw new APIException(
							"ProgramWorkflow.error.contains.state",
							new Object[]{workflow.getProgram()});
				}
			}
		}
	...
	public void purgeProgram(Program program, boolean cascade)
		...
		for (PatientProgram patientProgram : Context
				.getProgramWorkflowService().getPatientPrograms(null, program,
	...
	public Program retireProgram(Program program, String reason)
			throws APIException {
		// program.setRetired(true); - Note the BaseRetireHandler aspect is
		// already setting the retired flag and reason
	...
	public Program unretireProgram(Program program) throws APIException {
		...
		for (ProgramWorkflow workflow : program.getAllWorkflows()) {
			if (lastModifiedDate != null
					&& lastModifiedDate.equals(workflow.getDateChanged())) {
	...
	public PatientProgram savePatientProgram(PatientProgram patientProgram)
		...
		if (patientProgram.getPatient() == null
				|| patientProgram.getProgram() == null) {
		...
		for (PatientState state : patientProgram.getStates()) {
			...
			if (state.getPatientProgram() == null) {
				...
				throw new APIException("PatientProgram.already.assigned",
						new Object[]{state.getPatientProgram()});
			}
			if (patientProgram.getVoided() || state.getVoided()) {
				state.setVoided(true);
				if (state.getVoidReason() == null
						&& patientProgram.getVoidReason() != null) {
	...
	/**
	 * @see org.openmrs.api.ProgramWorkflowService#getPatientPrograms(Patient,
	 *      Program, Date, Date, Date, Date, boolean)
	 */
	@Transactional(readOnly = true)
	public List<PatientProgram> getPatientPrograms(Patient patient,
		...
		return dao.getPatientPrograms(patient, program, minEnrollmentDate,
				maxEnrollmentDate, minCompletionDate, maxCompletionDate,
				includeVoided);
	}

	/**
	 * @see org.openmrs.api.ProgramWorkflowService#getPatientPrograms(Cohort,
	 *      Collection)
	 */
	@Transactional(readOnly = true)
	...
	public void purgePatientProgram(PatientProgram patientProgram)
			throws APIException {
		Context.getProgramWorkflowService().purgePatientProgram(patientProgram,
				false);
	...
	public void purgePatientProgram(PatientProgram patientProgram,
		...
		if (cascade && !patientProgram.getStates().isEmpty()) {
			throw new APIException(
					"PatientProgram.cascade.purging.not.implemented",
					(Object[]) null);
		}
	...
	public PatientProgram voidPatientProgram(PatientProgram patientProgram,
		...
		return Context.getProgramWorkflowService().savePatientProgram(
				patientProgram); // The savePatientProgram method handles all of
									// the voiding defaults and cascades
	}
	...
	public ConceptStateConversion saveConceptStateConversion(
			ConceptStateConversion csc) throws APIException {
		if (csc.getConcept() == null || csc.getProgramWorkflow() == null
	...
	public void purgeConceptStateConversion(
		...
		Context.getProgramWorkflowService().purgeConceptStateConversion(
				conceptStateConversion, false);
	}
	 ...
	 */
	public void triggerStateConversion(Patient patient, Concept trigger,
			Date dateConverted) {
		...
		if (trigger == null) {
			throw new APIException(
					"convert.state.patient.without.valid.trigger",
					(Object[]) null);
		}
		...
		for (PatientProgram patientProgram : getPatientPrograms(patient, null,
				null, null, null, null, false)) {
			// skip past patient programs that already completed
			if (patientProgram.getDateCompleted() == null) {
				Set<ProgramWorkflow> workflows = patientProgram.getProgram()
						.getWorkflows();
				for (ProgramWorkflow workflow : workflows) {
					// (getWorkflows() is only returning over nonretired
					// workflows)
					PatientState patientState = patientProgram
							.getCurrentState(workflow);

					// #1080 cannot exit patient from care
					// Should allow a transition from a null state to a terminal
					// state
					// Or we should require a user to ALWAYS add an initial
					// workflow/state when a patient is added to a program
					ProgramWorkflowState currentState = (patientState != null)
							? patientState.getState()
							: null;
					ProgramWorkflowState transitionState = workflow
							.getState(trigger);

					log.debug("Transitioning from current state ["
							+ currentState + "]");
					log.debug("|---> Transitioning to final state ["
							+ transitionState + "]");

					if (transitionState != null
							&& workflow.isLegalTransition(currentState,
									transitionState)) {
						patientProgram.transitionToState(transitionState,
								dateConverted);
						log.debug("State Conversion Triggered: patientProgram="
								+ patientProgram + " transition from "
								+ currentState + " to " + transitionState
								+ " on " + dateConverted);
					}
				}

				// #1068 - Exiting a patient from care causes "not-null property
				// references
				// a null or transient value:
				// org.openmrs.PatientState.dateCreated". Explicitly
				// calling the savePatientProgram() method will populate the
				// metadata properties.
				//
				// #1067 - We should explicitly save the patient program rather
				// than let
