...
public interface OrderService extends OpenmrsService {
	...
	/**
	 * Setter for the Order data access object. The dao is used for saving and
	 * getting orders to/from the database
	 * 
	 * @param dao
	...
	/**
	 * Save or update the given <code>order</code> in the database. If the
	 * OrderType for the order is not specified, then it will be set to the one
	 * set on the OrderContext if any, if none exists on the orderContext, then
	 * it will be set to the one associated to the ConceptClass of the ordered
	 * concept otherwise the save fails. If the CareSetting field of the order
	 * is not specified then it will default to the one set on the passed in
	 * OrderContext if any otherwise the save fails.
	 * 
	 * @param order
	 *            the Order to save
	 * @param orderContext
	 *            the OrderContext object
	 * @return the Order that was saved
	 * @throws APIException
	 * @should not save order if order doesnt validate
	 * @should discontinue existing active order if new order being saved with
	 *         action to discontinue
	 * @should pass if the existing drug order matches the concept and drug of
	 *         the DC order
	 * @should fail if the existing drug order matches the concept and not drug
	 *         of the DC order
	 ...
	 * @should default to care setting and order type defined in the order
	 *         context if null
	 * @should not allow changing the patient of the previous order when
	 *         revising an order
	 * @should not allow changing the careSetting of the previous order when
	 *         revising an order
	 * @should not allow changing the concept of the previous order when
	 *         revising an order
	 * @should not allow changing the drug of the previous drug order when
	 *         revising an order
	 * @should fail if concept in previous order does not match that of the
	 *         revised order
	 * @should fail if the existing drug order matches the concept and not drug
	 ...
	 * @should fail if an active drug order for the same concept and care
	 *         setting exists
	 * @should pass if an active test order for the same concept and care
	 *         setting exists
	 * @should pass if an active order for the same concept exists in a
	 *         different care setting
	 * @should set Order type of Drug Order to drug order if not set and concept
	 *         not mapped
	 * @should set Order type of Test Order to test order if not set and concept
	 *         not mapped
	 * @should throw AmbiguousOrderException if an active drug order for the
	 *         same drug formulation exists
	 * @should pass if an active order for the same concept exists in a
	 *         different care setting
	 * @should fail for revision order if an active drug order for the same
	 *         concept and care settings exists
	 * @should pass for revision order if an active test order for the same
	 *         concept and care settings exists
	 * @should roll the autoExpireDate to the end of the day if it has no time
	 *         component
	 * @should not change the autoExpireDate if it has a time component
	 * @should throw AmbiguousOrderException if disconnecting multiple active
	 *         orders for the given concept
	 * @should throw AmbiguousOrderException if disconnecting multiple active
	 *         drug orders with the same drug
	 */
	@Authorized({PrivilegeConstants.EDIT_ORDERS, PrivilegeConstants.ADD_ORDERS})
	public Order saveOrder(Order order, OrderContext orderContext)
			throws APIException;

	/**
	 * Save or update the given retrospective <code>order</code> in the
	 * database. If the OrderType for the order is not specified, then it will
	 * be set to the one set on the OrderContext if any, if none exists on the
	 * orderContext, then it will be set to the one associated to the
	 * ConceptClass of the ordered concept otherwise the save fails. If the
	 * CareSetting field of the order is not specified then it will default to
	 * the one set on the passed in OrderContext if any otherwise the save
	 * fails. Retrospective entry of orders can affect downstream systems that
	 * acts on orders created. Orders cannot be stopped if they are already
	 * stopped in retrospective entry.
	 * 
	 * @param order
	 *            the Order to save
	 * @param orderContext
	...
	/**
	 * Completely delete an order from the database. This should not typically
	 * be used unless desperately needed. Most orders should just be voided. See
	 * {@link #voidOrder(Order, String)}
	 * 
	 * @param order
	 ...
	 * Completely delete an order from the database. This should not typically
	 * be used unless desperately needed. Most orders should just be voided. See
	 * {@link #voidOrder(Order, String)} This method is different from
	 * purgeOrder(Order order) above: If param cascade is false will completely
	 * delete an order from the database period If param cascade is true will
	 * completely delete an order from the database and delete any Obs that
	...
	/**
	 * Mark an order as voided. This functionally removes the Order from the
	 * system while keeping a semblance
	 * 
	 * @param voidReason
	 *            String reason
	 * @param order
	 ...
	 * @should unset dateStopped of the previous order if the specified order is
	 *         a discontinuation
	 * @should unset dateStopped of the previous order if the specified order is
	 *         a revision
	 */
	...
	/**
	 * Get discontinuation order for the given order, it is the un voided
	 * discontinuation order with a previousOrder that matches the specified
	 * order.
	 * 
	 * @param order
	...
	/**
	 * Get revision order for the given order, it is the order with the changes
	 * that was created as a replacement for the specified order. In other
	 * words, it is the un voided revise order with a previousOrder that matches
	 * the specified order.
	 * 
	 * @param order
	...
	/**
	 * Gets all Orders that match the specified parameters excluding
	 ...
	 */
	@Authorized(PrivilegeConstants.GET_ORDERS)
	public List<Order> getOrders(Patient patient, CareSetting careSetting,
			OrderType orderType, boolean includeVoided);

	/**
	 * Gets all orders for the specified patient including discontinuation
	 * orders
	 * 
	 * @param patient
	 *            the patient to match on
	...
	/**
	 * Unvoid order record. Reverse a previous call to
	 * {@link #voidOrder(Order, String)}
	 * 
	 * @param order
	 ...
	 * @should stop the previous order if the specified order is a
	 *         discontinuation
	...
	/**
	 * Gets all Order objects that use this Concept for a given patient. Orders
	 * will be returned in the order in which they occurred, i.e. sorted by
	 * startDate starting with the latest
	 * 
	 * @param patient
	 *            the patient.
	 * @param concept
	...
	/**
	 * Gets the order matching the specified order number and its previous
	 * orders in the ordering they occurred, i.e if this order has a previous
	 * order, fetch it and if it also has a previous order then fetch it until
	...
	/**
	 * Gets all active orders for the specified patient matching the specified
	 * CareSetting, OrderType as of the specified date. Below is the criteria
	 * for determining an active order:
	 * 
	 * <pre>
	 * - Not voided
	 ...
	 *            the patient
	 * @param orderType
	 *            The OrderType to match
	 * @param careSetting
	 *            the care setting, returns all ignoring care setting if value
	 *            is null
	 * @param asOfDate
	 ...
	 * @should return all active orders for the specified patient and care
	 *         setting
	 ...
	 */
	@Authorized(PrivilegeConstants.GET_ORDERS)
	public List<Order> getActiveOrders(Patient patient, OrderType orderType,
			CareSetting careSetting, Date asOfDate);
	...
	/**
	 * Gets all non retired CareSettings if includeRetired is set to true
	 * otherwise retired ones are included too
	 * 
	 * @param includeRetired
	 *            specifies whether retired care settings should be returned or
	 *            not
	 * @return A List of CareSettings
	 * @should return only un retired care settings if includeRetired is set to
	 *         false
	 ...
	 * @should return only non retired order frequencies if includeRetired is
	 *         set to false
	...
	/**
	 * Gets all non retired order frequencies associated to concepts that match
	 * the specified search phrase
	 * 
	 * @param searchPhrase
	 *            The string to match on
	 * @param locale
	 *            The locale to match on when searching in associated concept
	 *            names
	 * @param exactLocale
	 *            If false then order frequencies associated to concepts with
	 *            names in a broader locale will be matched e.g in case en_GB is
	 *            passed in then en will be matched
	 * @param includeRetired
	 *            Specifies if retired order frequencies that match should be
	 *            included or not
	 * @return List&lt;OrderFrequency&gt;
	 * @since 1.10
	 * @should get non retired frequencies with names matching the phrase if
	 *         includeRetired is false
	 * @should include retired frequencies if includeRetired is set to true
	 * @should get frequencies with names that match the phrase and locales if
	 *         exact locale is false
	 * @should get frequencies with names that match the phrase and locale if
	 *         exact locale is true
	 * @should return unique frequencies
	 * @should reject a null search phrase
	 */
	@Authorized(PrivilegeConstants.GET_ORDER_FREQUENCIES)
	public List<OrderFrequency> getOrderFrequencies(String searchPhrase,
			Locale locale, boolean exactLocale, boolean includeRetired);

	/**
	 * Discontinues an order. Creates a new order that discontinues the
	 * orderToDiscontinue
	 * 
	 ...
	 * @throws APIException
	 *             if the <code>action</code> of orderToDiscontinue is
	 *             <code>Order.Action.DISCONTINUE</code>
	 * @since 1.10
	 * @should set correct attributes on the discontinue and discontinued orders
	 * @should pass for an active order which is scheduled and not started as of
	 *         discontinue date
	...
	public Order discontinueOrder(Order orderToDiscontinue,
			Concept reasonCoded, Date discontinueDate, Provider orderer,
	 ...
	 *             <code>Order.Action.DISCONTINUE</code>
	 * @since 1.10
	 * @should populate correct attributes on the discontinue and discontinued
	 *         orders
	 * @should pass for an active order which is scheduled and not started as of
	 *         discontinue date
	...
	@Authorized({PrivilegeConstants.ADD_ORDERS, PrivilegeConstants.EDIT_ORDERS})
	public Order discontinueOrder(Order orderToDiscontinue,
			String reasonNonCoded, Date discontinueDate, Provider orderer,
			Encounter encounter) throws Exception;
	 ...
	 */
	@Authorized(PrivilegeConstants.MANAGE_ORDER_FREQUENCIES)
	public OrderFrequency retireOrderFrequency(OrderFrequency orderFrequency,
			String reason);
	...
	/**
	 * Get all order types, if includeRetired is set to true then retired ones
	 * will be included otherwise not
	 * 
	 * @param includeRetired
	 *            boolean flag which indicate search needs to look at retired
	 *            order types or not
	...
	/**
	 * Returns all descendants of a given order type for example Given TEST will
	 * get back LAB TEST and RADIOLOGY TEST; and Given LAB TEST, will might get
	 * back SEROLOGY, MICROBIOLOGY, and CHEMISTRY
	 * 
	 * @param orderType
	 *            the order type which needs to search for its' dependencies
	 * @param includeRetired
	 *            boolean flag for include retired order types or not
	 * @return list of order type which matches the given order type
	 */
	@Authorized(PrivilegeConstants.GET_ORDER_TYPES)
	public List<OrderType> getSubtypes(OrderType orderType,
	...
	/**
	 * Gets the possible drug routes, i.e the set members for the concept that
	 * matches the uuid specified as the value for the global property
	...
	/**
	 * Gets the possible drug dosing units, i.e the set members for the concept
	 * that matches the uuid specified as the value for the global property
	...
	/**
	 * Gets the possible units of dispensing, i.e the set members for the
	 * concept that matches the uuid specified as the value for the global
	 * property {@link OpenmrsConstants#GP_DRUG_DISPENSING_UNITS_CONCEPT_UUID}
	 * 
	...
	/**
	 * Gets the possible units of duration, i.e the set members for the concept
	 * that matches the uuid specified as the value for the global property
	...
	/**
	 * Gets the possible test specimen sources, i.e the set members for the
	 * concept that matches the uuid specified as the value for the global
	 * property {@link OpenmrsConstants#GP_TEST_SPECIMEN_SOURCES_CONCEPT_UUID}
	 * 
...
/**
	...
	/**
	 * Saves the orderGroup. It also saves the list of orders that are present
	 * within the orderGroup.
	 * 
	 * @param orderGroup
	 *            the orderGroup to be saved
	 * @since 1.12
	 * @throws APIException
	 */
	@Authorized({PrivilegeConstants.EDIT_ORDERS, PrivilegeConstants.ADD_ORDERS})
