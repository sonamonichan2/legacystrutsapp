...
public class AuditableInterceptor extends EmptyInterceptor {
	...
	private static final Log log = LogFactory
			.getLog(AuditableInterceptor.class);
	...
	public boolean onSave(Object entity, Serializable id,
			Object[] entityCurrentState, String[] propertyNames, Type[] types) {
		return setCreatorAndDateCreatedIfNull(entity, entityCurrentState,
				propertyNames);
	}

	/**
	 * This class method is only called when flushing an updated dirty object,
	 * not inserting objects
	 * 
	...
	public boolean onFlushDirty(Object entity, Serializable id,
		...
		objectWasChanged = setCreatorAndDateCreatedIfNull(entity, currentState,
				propertyNames);
		...
		if (entity instanceof Auditable && propertyNames != null) {
			...
			objectWasChanged = changeProperties(currentState, propertyNames,
					objectWasChanged, propertyValues, false);
		}
		return objectWasChanged;
	}

	/**
	 * Sets the creator and dateCreated fields to the current user and the
	 * current time if they are null. if is a Person Object, sets the
	 * personCreator and personDateCreated fields to the current user and the
	 * current time if they are null.
	 * 
	 * @param entity
	 * @param currentState
	 * @param propertyNames
	 ...
	 */
	private boolean setCreatorAndDateCreatedIfNull(Object entity,
		...
		if (entity instanceof OpenmrsObject) {
			...
			objectWasChanged = changeProperties(currentState, propertyNames,
					objectWasChanged, propertyValues, true);
		}
		return objectWasChanged;
	}

	private boolean changeProperties(Object[] currentState,
			String[] propertyNames, boolean objectWasChanged,
			Map<String, Object> propertyValues, Boolean setNullOnly) {

		for (String property : propertyValues.keySet()) {
			if (changePropertyValue(currentState, propertyNames, property,
					propertyValues.get(property), setNullOnly)) {
	 ...
	 */
	private boolean changePropertyValue(Object[] currentState,
			String[] propertyNames, String propertyToSet, Object value,
			boolean setNullOnly) {
		...
		if (index >= 0 && (currentState[index] == null || !setNullOnly)
