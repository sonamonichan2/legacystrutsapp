...
/**
 * The PatientSearchCriteria class. It has API to return a criteria from the
 * Patient Name and identifier.
 */
public class PatientSearchCriteria {

	private final static Log log = LogFactory
			.getLog(PatientSearchCriteria.class);
	...
	/**
	 * Prepare a hibernate criteria for searching patients by name and/or
	 * identifier.
	 * 
	 * The visibility of this method remains public in order not to break
	 * OpenMRS modules that use this method.
	 * 
	 * Instead of calling this method consider using
	 * {@link org.openmrs.api.PatientService} or
	 ...
	 * @param searchOnNamesOrIdentifiers
	 *            specifies if the logic should find patients that match the
	 *            name or identifier otherwise find patients that match both the
	 *            name and identifier
	 * @return {@link Criteria}
	 */
	public Criteria prepareCriteria(String name, String identifier,
		...
		PatientSearchMode patientSearchMode = getSearchMode(name, identifier,
				identifierTypes, searchOnNamesOrIdentifiers);
		...
		switch (patientSearchMode) {
			...
			case PATIENT_SEARCH_BY_IDENTIFIER :
				...
				criteria.add(prepareCriterionForIdentifier(identifier,
						identifierTypes, matchIdentifierExactly));
				break;

			case PATIENT_SEARCH_BY_NAME_OR_IDENTIFIER :

				// If only name *or* identifier is provided as a search
				// parameter,
				// the respective value is copied to the empty search parameter.
				//
				// As a consequence, the *single* parameter is used to search
				// for both names and identifiers.
				//
				...
				criteria.add(Restrictions
						.disjunction()
						.add(prepareCriterionForName(name))
						.add(prepareCriterionForIdentifier(identifier,
								identifierTypes, matchIdentifierExactly)));
				break;
	...
	/**
	 * Provides a Hibernate criteria object for searching patients by name,
	 * identifier or searchable attribute.
	 * 
	 * The visibility of this method is "default" as this method should NOT be
	 * called directly by classes other than
	 * org.openmrs.api.db.hibernate.HibernatePatientDAO.
	 * 
	 * Instead of calling this method consider using
	 * {@link org.openmrs.api.PatientService} or
	 * {@link org.openmrs.api.db.PatientDAO}.
	 ...
	 * @return criteria for searching by name OR identifier OR searchable
	 *         attributes
	 */
	Criteria prepareCriteria(String query, boolean includeVoided) {
		...
		criteria.add(Restrictions
				.disjunction()
				.add(prepareCriterionForName(query, includeVoided))
				.add(prepareCriterionForAttribute(query, includeVoided))
				.add(prepareCriterionForIdentifier(query,
	 ...
	 *         attributes
	 */
	Criteria prepareCriteria(String query, Boolean matchExactly,
		...
		if (matchExactly == null) {
			criteria.add(Restrictions
					.conjunction()
					.add(prepareCriterionForName(query, null, includeVoided))
					.add(Restrictions.not(prepareCriterionForName(query, true,
							includeVoided)))
					.add(Restrictions.not(prepareCriterionForName(query, false,
							includeVoided))));
			...
			criteria.add(Restrictions
					.disjunction()
					.add(prepareCriterionForName(query, true, includeVoided))
					.add(prepareCriterionForAttribute(query, includeVoided))
					.add(prepareCriterionForIdentifier(query,
							new ArrayList<PatientIdentifierType>(), false,
							includeVoided)));
	...
	PatientSearchMode getSearchMode(String name, String identifier,
		...
		if (StringUtils.isBlank(name)
				&& !(StringUtils.isBlank(identifier) && CollectionUtils
						.isEmpty(identifierTypes))) {
	 ...
	 *            true/false whether or not to included voided patients
	 */
	private Criterion prepareCriterionForIdentifier(String identifier,
			List<PatientIdentifierType> identifierTypes,
			boolean matchIdentifierExactly, boolean includeVoided) {

		identifier = HibernateUtil.escapeSqlWildcards(identifier,
				sessionFactory);
		...
		if (identifier != null) {
			...
			if (matchIdentifierExactly) {
				SimpleExpression matchIdentifier = Restrictions.eq(
				...
				if (org.springframework.util.StringUtils
						.hasLength(patternSearch)) {
					conjunction.add(splitAndGetSearchPattern(identifier,
							patternSearch));
				}
				// if the regex is empty, default to a simple "like" search or
				// if
				// we're in hsql world, also only do the simple like search
				// (because
				// hsql doesn't know how to deal with 'regexp'
				else if ("".equals(regex)
						|| HibernateUtil.isHSQLDialect(sessionFactory)) {
					conjunction.add(getCriterionForSimpleSearch(identifier,
							adminService));
				}
				// if the regex is present, search on that
				else {
					regex = replaceSearchString(regex, identifier);
					conjunction.add(Restrictions.sqlRestriction(
							"identifier regexp ?", regex, StringType.INSTANCE));
				}
			}
		}
	...
	private Criterion prepareCriterionForIdentifier(String identifier,
		...
		return prepareCriterionForIdentifier(identifier, identifierTypes,
	 ...
	 * @param adminService
	 */
	private Criterion getCriterionForSimpleSearch(String identifier,
		...
		StringBuilder likeString = new StringBuilder(prefix).append(identifier)
				.append(suffix);
	 ...
	 * @param patternSearch
	 */
	private Criterion splitAndGetSearchPattern(String identifier,
			String patternSearch) {
		// split the pattern before replacing in case the user searched on a
		// comma
	...
	private String removePadding(String identifier, String regex) {
		String padding = regex.substring(regex.indexOf("^") + 1,
				regex.indexOf("*"));
		...
		if (nameParts.length > 0) {
			...
			for (int i = 0; i < nameParts.length; i++) {
				...
				if (singleName != null && singleName.length() > 0) {
					Criterion singleNameCriterion = getCriterionForName(
							singleName, matchExactly, includeVoided);
					Criterion criterion = singleNameCriterion;

					if (i > 0) {
						multiName.append(" ");
						multiName.append(singleName);
						Criterion multiNameCriterion = getCriterionForName(
								multiName.toString(), matchExactly,
								includeVoided);
						criterion = Restrictions.or(singleNameCriterion,
								multiNameCriterion);
					}
	...
	/**
	 * Returns a criteria object comparing the given string to each part of the
	 * name. <br>
	 * <br>
	 ...
	 * </pre>
	 * 
	 * Except when the name provided is less than min characters (usually 3)
	 * then we will look for an EXACT match by default
	 * 
	 * @param name
	 * @param matchExactly
	 * @param includeVoided
	 ...
	 */
	private Criterion getCriterionForName(String name, Boolean matchExactly,
	...
	private Criterion getCriterionForShortName(String name,
		...
		Criterion criterion = Restrictions
				...
				.add(Restrictions
						.conjunction()
						.add(Restrictions.isNotNull("name.givenName"))
						.add(Restrictions.eq("name.givenName", name)
								.ignoreCase()))
				.add(Restrictions
						.conjunction()
						.add(Restrictions.isNotNull("name.middleName"))
						.add(Restrictions.eq("name.middleName", name)
								.ignoreCase()))
				.add(Restrictions
						.conjunction()
						.add(Restrictions.isNotNull("name.familyName"))
						.add(Restrictions.eq("name.familyName", name)
								.ignoreCase()))
				.add(Restrictions
						.conjunction()
						.add(Restrictions.isNotNull("name.familyName2"))
						.add(Restrictions.eq("name.familyName2", name)
								.ignoreCase()));

		if (!includeVoided) {
			return Restrictions.conjunction()
					.add(Restrictions.eq("name.voided", false)).add(criterion);
		}
		return criterion;
	}
	...
	private Criterion getCriterionForNoExactName(String name,
			boolean includeVoided) {
		MatchMode matchMode = getMatchMode();
		...
		Criterion criterion = Restrictions
				...
				.add(Restrictions
						...
						.add(Restrictions
								.conjunction()
								.add(Restrictions.isNotNull("name.givenName"))
								.add(Restrictions.like("name.givenName", name,
										matchMode)))
						.add(Restrictions
								.conjunction()
								.add(Restrictions.isNotNull("name.middleName"))
								.add(Restrictions.like("name.middleName", name,
										matchMode)))
						.add(Restrictions
								.conjunction()
								.add(Restrictions.isNotNull("name.familyName"))
								.add(Restrictions.like("name.familyName", name,
										matchMode)))
						.add(Restrictions
								.conjunction()
								.add(Restrictions.isNotNull("name.familyName2"))
								.add(Restrictions.like("name.familyName2",
										name, matchMode))))
				.add(Restrictions.disjunction()
						.add(Restrictions.isNull("name.givenName"))
						.add(Restrictions.ne("name.givenName", name)))
				.add(Restrictions.disjunction()
						.add(Restrictions.isNull("name.middleName"))
						.add(Restrictions.ne("name.middleName", name)))
				.add(Restrictions.disjunction()
						.add(Restrictions.isNull("name.familyName"))
						.add(Restrictions.ne("name.familyName", name)))
				.add(Restrictions.disjunction()
						.add(Restrictions.isNull("name.familyName2"))
	...
	MatchMode getMatchMode() {
		...
		if (matchMode
				.equalsIgnoreCase(OpenmrsConstants.GLOBAL_PROPERTY_PATIENT_SEARCH_MATCH_ANYWHERE)) {
	 ...
	 * 
	 * @param regex
	 *            the admin-defined search string containing the @..@'s to be
	 *            replaced
	 * @param identifierSearched
	...
	private String replaceSearchString(String regex, String identifierSearched) {
		...
		if (identifierSearched.length() > 1) {
			// for 2 or more character searches, we allow regex to use last
			// character as check digit
			returnString = returnString.replaceAll("@SEARCH-1@",
					identifierSearched.substring(0,
							identifierSearched.length() - 1));
			returnString = returnString
					.replaceAll("@CHECKDIGIT@", identifierSearched
	...
	private Criterion prepareCriterionForAttribute(String query,
		...
		for (String queryPart : queryParts) {
			conjunction.add(personSearchCriteria.prepareCriterionForAttribute(
					queryPart, includeVoided, matchMode));
		}
