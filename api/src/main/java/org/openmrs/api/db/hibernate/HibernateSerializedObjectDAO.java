...
public class HibernateSerializedObjectDAO implements SerializedObjectDAO {
	...
	public SerializedObject getSerializedObject(Integer id) throws DAOException {
		if (id != null) {
			return (SerializedObject) sessionFactory.getCurrentSession().get(
					SerializedObject.class, id);
		}
		return null;
	}
	 ...
	 */
	public <T extends OpenmrsObject> T getObject(Class<T> baseClass, Integer id)
			throws DAOException {
	 ...
	 */
	public <T extends OpenmrsObject> T getObjectByUuid(Class<T> baseClass,
	...
	/**
	 * @see SerializedObjectDAO#getAllSerializedObjectsByName(Class, String,
	 *      boolean)
	 */
	...
	public List<SerializedObject> getAllSerializedObjectsByName(Class<?> type,
		...
		Criteria c = sessionFactory.getCurrentSession().createCriteria(
				SerializedObject.class);
		c.add(Restrictions.or(Restrictions.eq("type", type.getName()),
				Restrictions.eq("subtype", type.getName())));
		if (exactMatchOnly) {
	...
	public <T extends OpenmrsMetadata> List<T> getAllObjectsByName(
		...
		List<SerializedObject> objects = getAllSerializedObjectsByName(type,
				name, exactMatchOnly);
	...
	public <T extends OpenmrsObject> List<T> getAllObjects(Class<T> type,
			boolean includeRetired) throws DAOException {
		List<T> ret = new ArrayList<T>();
		List<SerializedObject> objects = getAllSerializedObjects(type,
				includeRetired);
		for (SerializedObject serializedObject : objects) {
			ret.add(convertSerializedObject(type, serializedObject));
		}
	...
	public <T extends OpenmrsObject> T saveObject(T object,
		...
		try {
			...
			throw new DAOException("Unable to save object <" + object
					+ "> because serialization failed.", e);
		}
	...
	public <T extends OpenmrsObject> T convertSerializedObject(Class<T> clazz,
		...
		try {
			...
			obj = (T) serializer.deserialize(
					serializedObject.getSerializedData(), subtype);
		} catch (Exception e) {
			ExceptionUtil.rethrowAPIAuthenticationException(e);
			throw new DAOException("Unable to deserialize object: "
					+ serializedObject, e);
		}
		if (obj == null) {
	 ...
	 * passed SerializedObject, defaulting to the default system serializer if
	 * none is explicitly set on the object
	 */
	private OpenmrsSerializer getSerializer(SerializedObject o) {
		if (o != null && o.getSerializationClass() != null) {
			return Context.getSerializationService().getSerializer(
					o.getSerializationClass());
		}
	...
	public void setSupportedTypes(
			List<Class<? extends OpenmrsObject>> supportedTypes) {
