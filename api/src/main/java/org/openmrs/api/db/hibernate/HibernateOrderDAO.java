...
/**
 * This class should not be used directly. This is just a common implementation
 * of the OrderDAO that is used by the OrderService. This class is injected by
 * spring into the desired OrderService class. This injection is determined by
 * the xml mappings and elements in the spring application context:
 * /metadata/api/spring/applicationContext.xml.<br>
 * <br>
...
public class HibernateOrderDAO implements OrderDAO {
	...
	/**
	 * @see org.openmrs.api.db.OrderDAO#getOrders(org.openmrs.OrderType,
	 *      java.util.List, java.util.List, java.util.List, java.util.List)
	 */
	@SuppressWarnings("unchecked")
	public List<Order> getOrders(OrderType orderType, List<Patient> patients,
			List<Concept> concepts, List<User> orderers,
	...
	/**
	 * @see OrderDAO#getOrders(org.openmrs.Patient, org.openmrs.CareSetting,
	 *      java.util.List, boolean, boolean)
	 */
	@Override
	public List<Order> getOrders(Patient patient, CareSetting careSetting,
			List<OrderType> orderTypes, boolean includeVoided,
			boolean includeDiscontinuationOrders) {
		return createOrderCriteria(patient, careSetting, orderTypes,
				includeVoided, includeDiscontinuationOrders).list();
	}
	...
	public Order getOrderByUuid(String uuid) {
		return (Order) sessionFactory.getCurrentSession()
				.createQuery("from Order o where o.uuid = :uuid")
	...
	public Order getDiscontinuationOrder(Order order) {
		Order discontinuationOrder = (Order) sessionFactory.getCurrentSession()
				.createCriteria(Order.class)
				.add(Restrictions.eq("previousOrder", order))
				.add(Restrictions.eq("action", Order.Action.DISCONTINUE))
	...
	public Order getRevisionOrder(Order order) throws APIException {
		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
				Order.class);
		criteria.add(Restrictions.eq("previousOrder", order))
				.add(Restrictions.eq("action", Order.Action.REVISE))
				.add(Restrictions.eq("voided", false));
		return (Order) criteria.uniqueResult();
	}

	@Override
	public List<Object[]> getOrderFromDatabase(Order order,
			boolean isOrderADrugOrder) throws APIException {
	...
	public OrderGroup getOrderGroupByUuid(String uuid) throws DAOException {
		return (OrderGroup) sessionFactory.getCurrentSession()
				.createQuery("from OrderGroup o where o.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	 ...
	 */
	@Override
	public OrderGroup getOrderGroupById(Integer orderGroupId)
			throws DAOException {
		return (OrderGroup) sessionFactory.getCurrentSession().get(
				OrderGroup.class, orderGroupId);
	}
	...
	public Long getNextOrderNumberSeedSequenceValue() {
		GlobalProperty globalProperty = (GlobalProperty) sessionFactory
				.getCurrentSession().get(GlobalProperty.class,
	...
	/**
	 * @see org.openmrs.api.db.OrderDAO#getActiveOrders(org.openmrs.Patient,
	 *      java.util.List, org.openmrs.CareSetting, java.util.Date)
	 */
	@SuppressWarnings("unchecked")
	public List<Order> getActiveOrders(Patient patient,
			List<OrderType> orderTypes, CareSetting careSetting, Date asOfDate) {
		Criteria crit = createOrderCriteria(patient, careSetting, orderTypes,
				false, false);
		crit.add(Restrictions.le("dateActivated", asOfDate));

		Disjunction dateStoppedAndAutoExpDateDisjunction = Restrictions
				.disjunction();
		Criterion stopAndAutoExpDateAreBothNull = Restrictions.and(
		...
		Criterion autoExpireDateEqualToOrAfterAsOfDate = Restrictions.and(
				Restrictions.isNull("dateStopped"),
				Restrictions.ge("autoExpireDate", asOfDate));
		dateStoppedAndAutoExpDateDisjunction
				.add(autoExpireDateEqualToOrAfterAsOfDate);

		dateStoppedAndAutoExpDateDisjunction.add(Restrictions.ge("dateStopped",
				asOfDate));
	...
	/**
	 * Creates and returns a Criteria Object filtering on the specified
	 * parameters
	 * 
	 * @param patient
	 * @param careSetting
	 ...
	 * @return
	 */
	private Criteria createOrderCriteria(Patient patient,
			CareSetting careSetting, List<OrderType> orderTypes,
	...
	public CareSetting getCareSetting(Integer careSettingId) {
		return (CareSetting) sessionFactory.getCurrentSession().get(
				CareSetting.class, careSettingId);
	}
	...
	public CareSetting getCareSettingByUuid(String uuid) {
		return (CareSetting) sessionFactory.getCurrentSession()
				.createQuery("from CareSetting cs where cs.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public CareSetting getCareSettingByName(String name) {
		return (CareSetting) sessionFactory.getCurrentSession()
				.createCriteria(CareSetting.class)
	...
	public OrderFrequency getOrderFrequency(Integer orderFrequencyId) {
		return (OrderFrequency) sessionFactory.getCurrentSession().get(
				OrderFrequency.class, orderFrequencyId);
	}
	...
	public OrderFrequency getOrderFrequencyByUuid(String uuid) {
		return (OrderFrequency) sessionFactory.getCurrentSession()
				.createQuery("from OrderFrequency o where o.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public List<OrderFrequency> getOrderFrequencies(String searchPhrase,
		...
		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
				OrderFrequency.class, "orderFreq");
		...
		criteria.add(Restrictions.ilike("conceptName.name", searchPhrase,
				MatchMode.ANYWHERE));
		if (locale != null) {
			List<Locale> locales = new ArrayList<Locale>(2);
			locales.add(locale);
			// look in the broader locale too if exactLocale is false e.g en for
			// en_GB
	...
	public boolean isOrderFrequencyInUse(OrderFrequency orderFrequency) {
		...
		Map<String, ClassMetadata> metadata = sessionFactory
				.getAllClassMetadata();
		for (Iterator<ClassMetadata> i = metadata.values().iterator(); i
				.hasNext();) {
			...
			for (String name : names) {
				if (classMetadata.getPropertyType(name).getReturnedClass()
						.equals(OrderFrequency.class)) {
	...
	public OrderType getOrderTypeByUuid(String uuid) {
		return (OrderType) sessionFactory.getCurrentSession()
				.createQuery("from OrderType o where o.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public OrderType getOrderTypeByConceptClass(ConceptClass conceptClass) {
		return (OrderType) sessionFactory
				.getCurrentSession()
				.createQuery(
	 ...
	 *      boolean)
	 */
	public List<OrderType> getOrderSubtypes(OrderType orderType,
