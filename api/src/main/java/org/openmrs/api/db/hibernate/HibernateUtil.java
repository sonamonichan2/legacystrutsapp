...
/**
 * This class holds common methods and utilities that are used across the
...
public class HibernateUtil {
	...
	/**
	 * Check and cache whether the currect dialect is HSQL or not. This is
	 * needed because some queries are different if in the hsql world as opposed
	 * to the mysql/postgres world
	 * 
	 ...
	 */
	public static String escapeSqlWildcards(final String oldString,
	...
	/**
	 * Escapes all sql wildcards in the given string, returns the same string if
	 * it doesn't contain any sql wildcards
	 * 
	 * @param oldString
	 *            the string in which to escape the sql wildcards
	 * @param connection
	 *            The underlying database connection
	 * @return the string with sql wildcards escaped if any found otherwise the
	 *         original string is returned
	 */
	public static String escapeSqlWildcards(String oldString,
		...
		if (!StringUtils.isBlank(oldString)) {
			...
			try {
				...
				escapeCharacter = connection.getMetaData()
						.getSearchStringEscape();
			} catch (SQLException e) {
				log.warn("Error generated", e);
			}
			// insert an escape character before each sql wildcard in the search
			// phrase
	 ...
	 *            the serialized attribute values
	 * @param <AT>
	 *            the attribute type
	 */
	public static <AT extends AttributeType> void addAttributeCriteria(
			Criteria criteria, Map<AT, String> serializedAttributeValues) {
		...
		for (Map.Entry<AT, String> entry : serializedAttributeValues.entrySet()) {
			...
			DetachedCriteria detachedCriteria = DetachedCriteria.forClass(
					Location.class).setProjection(Projections.id());
			detachedCriteria.createAlias("attributes", alias);
			detachedCriteria.add(Restrictions.eq(alias + ".attributeType",
					entry.getKey()));
			detachedCriteria.add(Restrictions.eq(alias + ".valueReference",
					entry.getValue()));
	...
	/**
	 * Gets an object as an instance of its persistent type if it is a hibernate
	 * proxy otherwise returns the same passed in object
	 * 
	...
	public static <T> T getRealObjectFromProxy(T persistentObject) {
		...
		if (persistentObject instanceof HibernateProxy) {
			...
			persistentObject = (T) ((HibernateProxy) persistentObject)
