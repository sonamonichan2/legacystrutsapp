...
/**
 * Abstract class implementing basic data access methods for BaseOpenmrsMetadata
 * persistents
 * 
 * @since 1.10
 * 
...
public class HibernateOpenmrsMetadataDAO<T extends BaseOpenmrsMetadata>
		extends
			HibernateOpenmrsObjectDAO<T> implements OpenmrsMetadataDAO<T> {
	 ...
	 */
	public List<T> getAll(boolean includeRetired, Integer firstResult,
			Integer maxResults) {
		Criteria crit = sessionFactory.getCurrentSession().createCriteria(
				mappedClass);
