...
public class HibernateEncounterDAO implements EncounterDAO {
	...
	public List<Encounter> getEncountersByPatientId(Integer patientId)
		...
		Criteria crit = sessionFactory.getCurrentSession()
				.createCriteria(Encounter.class).createAlias("patient", "p")
	...
	public List<Encounter> getEncounters(EncounterSearchCriteria searchCriteria) {
		...
		if (searchCriteria.getPatient() != null
				&& searchCriteria.getPatient().getPatientId() != null) {
		...
		if (searchCriteria.getFromDate() != null) {
			crit.add(Restrictions.ge("encounterDatetime",
					searchCriteria.getFromDate()));
		}
		if (searchCriteria.getToDate() != null) {
			crit.add(Restrictions.le("encounterDatetime",
		...
		if (searchCriteria.getDateChanged() != null) {
			crit.add(Restrictions.or(Restrictions.and(
					...
					Restrictions.ge("dateCreated",
							searchCriteria.getDateChanged())), Restrictions.ge(
					"dateChanged", searchCriteria.getDateChanged())));
		}
		if (searchCriteria.getEnteredViaForms() != null
				&& searchCriteria.getEnteredViaForms().size() > 0) {
			crit.add(Restrictions.in("form",
					searchCriteria.getEnteredViaForms()));
		}
		if (searchCriteria.getEncounterTypes() != null
				&& searchCriteria.getEncounterTypes().size() > 0) {
			crit.add(Restrictions.in("encounterType",
					searchCriteria.getEncounterTypes()));
		}
		if (searchCriteria.getProviders() != null
				&& searchCriteria.getProviders().size() > 0) {
			crit.createAlias("encounterProviders", "ep");
			crit.add(Restrictions.in("ep.provider",
					searchCriteria.getProviders()));
		}
		if (searchCriteria.getVisitTypes() != null
				&& searchCriteria.getVisitTypes().size() > 0) {
			crit.createAlias("visit", "v");
			crit.add(Restrictions.in("v.visitType",
					searchCriteria.getVisitTypes()));
		}
		if (searchCriteria.getVisits() != null
				&& searchCriteria.getVisits().size() > 0) {
	...
	public List<EncounterType> findEncounterTypes(String name)
		...
		return sessionFactory
				...
				.addOrder(Order.asc("name")).addOrder(Order.asc("retired"))
				.list();
	}
	...
	public Date getSavedEncounterDatetime(Encounter encounter) {
		// Usages of this method currently are internal and don't require a
		// flush
		// Otherwise we end up with premature flushes of Immutable types like
		// Obs
	...
	/**
	 * @see org.openmrs.api.db.EncounterDAO#getEncounters(String, Integer,
	...
	public List<Encounter> getEncounters(String query, Integer patientId,
		...
		Criteria criteria = createEncounterByQueryCriteria(query, patientId,
				includeVoided, true);
	...
	public Map<Integer, List<Encounter>> getAllEncounters(Cohort patients) {
		...
		@SuppressWarnings("unchecked")
		List<Encounter> allEncounters = createEncounterCriteria(patients)
				.list();
	...
	private Criteria createEncounterCriteria(Cohort patients) {
		...
		if (patients != null) {
			criteria.add(Restrictions.in("patient.personId",
	 ...
	 */
	@Override
	public Long getCountOfEncounters(String query, Integer patientId,
	...
	/**
	 * Utility method that returns a criteria for searching for patient
	 * encounters that match the specified search phrase
	 * 
	 * @param query
	 ...
	 * @param orderByNames
	 *            specifies whether the encounters should be ordered by person
	 *            names
	 * @return Criteria
	 */
	private Criteria createEncounterByQueryCriteria(String query,
		...
		if (patientId != null) {
			...
			if (StringUtils.isNotBlank(query)) {
				...
				criteria.createAlias("prov.person", "person",
						Criteria.LEFT_JOIN);
				criteria.createAlias("person.names", "personName",
						Criteria.LEFT_JOIN);
				...
				for (String splitName : splitNames) {
					nameOr.add(Restrictions.ilike("personName.givenName",
							splitName, mode));
					nameOr.add(Restrictions.ilike("personName.middleName",
							splitName, mode));
					nameOr.add(Restrictions.ilike("personName.familyName",
							splitName, mode));
					nameOr.add(Restrictions.ilike("personName.familyName2",
							splitName, mode));
				}
				...
				personNameConjuction.add(Restrictions.eq("personName.voided",
						false));
			...
			// As identifier could be all alpha, no heuristic here will work in
			// determining intent of user for querying by name versus identifier
			// So search by both!
			String name = query;
			String identifier = query;
			criteria = new PatientSearchCriteria(sessionFactory, criteria)
					.prepareCriteria(name, identifier,
							new ArrayList<PatientIdentifierType>(), true,
							orderByNames, true);
		}
	...
	/**
	 * @see org.openmrs.api.db.EncounterDAO#saveEncounterRole(EncounterRole
	 *      encounterRole)
	 */
	@Override
	 ...
	 */
	@Override
	public void deleteEncounterRole(EncounterRole encounterRole)
			throws DAOException {
	...
	public EncounterRole getEncounterRole(Integer encounterRoleId)
			throws DAOException {
		return (EncounterRole) sessionFactory.getCurrentSession().get(
				EncounterRole.class, encounterRoleId);
	}
	...
	public List<EncounterRole> getAllEncounterRoles(boolean includeRetired)
		...
		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
				EncounterRole.class);
		return includeRetired ? criteria.list() : criteria.add(
				Restrictions.eq("retired", includeRetired)).list();
	}
	...
	public EncounterRole getEncounterRoleByName(String name)
			throws DAOException {
		return (EncounterRole) sessionFactory.getCurrentSession()
				.createCriteria(EncounterRole.class)
				.add(Restrictions.eq("name", name)).uniqueResult();

	}

	/**
	 * Convenience method since this DAO fetches several different domain
	 * objects by uuid
	 * 
	 * @param uuid
	 *            uuid to fetch
	 * @param table
	 *            a simple classname (e.g. "Encounter")
	 * @return
	 */
	@SuppressWarnings("unchecked")
	...
	public List<Encounter> getEncountersNotAssignedToAnyVisit(Patient patient)
		...
		return sessionFactory.getCurrentSession()
				...
				.add(Restrictions.isNull("visit"))
				.add(Restrictions.eq("voided", false))
				.addOrder(Order.desc("encounterDatetime")).setMaxResults(100)
				.list();
	}
		...
		if (!emptyVisits.isEmpty()) {
			...
			Collections.sort(encounters, new Comparator<Encounter>() {
				...
				public int compare(Encounter o1, Encounter o2) {
					Date o1Date = (o1.getVisit() != null) ? o1.getVisit()
							.getStartDatetime() : o1.getEncounterDatetime();
					Date o2Date = (o2.getVisit() != null) ? o2.getVisit()
							.getStartDatetime() : o2.getEncounterDatetime();
	...
	private void addEmptyVisitsByPatientCriteria(Criteria criteria,
		...
		if (query != null && !StringUtils.isBlank(query)) {
			...
			Disjunction or = Restrictions.disjunction();
			criteria.add(or);
			or.add(Restrictions.ilike("visitType.name", query,
					MatchMode.ANYWHERE));
			or.add(Restrictions.ilike("location.name", query,
					MatchMode.ANYWHERE));
		}
	...
	private void addEncountersByPatientCriteria(Criteria criteria,
			Patient patient, boolean includeVoided, String query) {
		criteria.add(Restrictions.eq("patient", patient));
		...
		if (query != null && !StringUtils.isBlank(query)) {
			criteria.createAlias("visit.visitType", "visitType",
					Criteria.LEFT_JOIN);
			criteria.createAlias("visit.location", "visitLocation",
					Criteria.LEFT_JOIN);
			criteria.createAlias("location", "location", Criteria.LEFT_JOIN);
			...
			or.add(Restrictions.ilike("visitType.name", query,
					MatchMode.ANYWHERE));
			or.add(Restrictions.ilike("visitLocation.name", query,
					MatchMode.ANYWHERE));
			or.add(Restrictions.ilike("location.name", query,
					MatchMode.ANYWHERE));
			or.add(Restrictions.ilike("encounterType.name", query,
					MatchMode.ANYWHERE));
		}
