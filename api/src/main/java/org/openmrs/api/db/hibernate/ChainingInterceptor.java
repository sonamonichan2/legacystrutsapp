...
/**
 * Used by the {@link HibernateSessionFactoryBean} to keep track of multiple
 * interceptors <br>
 * Each of the methods in {@link Interceptor} are called for each interceptor
 * that is added to this class
 * 
 * @since 1.9
 */
public class ChainingInterceptor implements Interceptor {
	...
	// using a linkedhashset to preserve insert order and maintain a list of
	// unique objects
	public Collection<Interceptor> interceptors = new LinkedHashSet<Interceptor>();

	/**
	 * Adds the given interceptor to the list of interceptors to be applied to
	 * hibernate sessions. Interceptors are called in the added order, with core
	 * interceptors being called first
	 * 
	 * @param interceptor
	 *            the interceptor to add to the queue
	 */
	public void addInterceptor(Interceptor interceptor) {
		// do nothing if adding ourself to the list. This would cause infinite
		// looping
		if (interceptor == this) {
	...
	public boolean onFlushDirty(Object entity, Serializable id,
		...
		for (Interceptor i : interceptors) {
			// must be in this order so that java doesn't skip the method call
			// for optimizations
			objectChanged = i.onFlushDirty(entity, id, currentState,
					previousState, propertyNames, types) || objectChanged;
		}
	...
	public boolean onLoad(Object entity, Serializable id, Object[] state,
		...
		for (Interceptor i : interceptors) {
			// must be in this order so that java doesn't skip the method call
			// for optimizations
			objectChanged = i.onLoad(entity, id, state, propertyNames, types)
					|| objectChanged;
		}
	...
	public boolean onSave(Object entity, Serializable id, Object[] state,
		...
		for (Interceptor i : interceptors) {
			// must be in this order so that java doesn't skip the method call
			// for optimizations
			objectChanged = i.onSave(entity, id, state, propertyNames, types)
					|| objectChanged;
		}
	...
	public int[] findDirty(Object entity, Serializable id,
		...
		for (Interceptor i : interceptors) {
			int[] indices = i.findDirty(entity, id, currentState,
	...
	// passes the sql returned from each previous onPrepareStatement onto the
	// next
