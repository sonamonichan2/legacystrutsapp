 ...
 * <br>
 * This class should not be used directly. All database calls should go through
...
public class HibernatePersonDAO implements PersonDAO {
	...
	protected final static Log log = LogFactory
			.getLog(HibernatePersonDAO.class);
	...
	/**
	 * @see org.openmrs.api.PersonService#getSimilarPeople(java.lang.String,
	 *      java.lang.Integer, java.lang.String, java.lang.String)
	 * @see org.openmrs.api.db.PersonDAO#getSimilarPeople(String name, Integer
	 *      birthyear, String gender)
	 */
	...
	public Set<Person> getSimilarPeople(String name, Integer birthyear,
		...
		if (names.length == 1) {
			q.append("(").append(" soundex(pname.givenName) = soundex(:n1)")
					...
					.append(" or soundex(pname.familyName2) = soundex(:n1) ")
					.append(")");
		} else if (names.length == 2) {
			q.append("(")
					.append(" case")
					.append("  when pname.givenName is null then 1")
					.append("  when pname.givenName = '' then 1")
					.append("  when soundex(pname.givenName) = soundex(:n1) then 4")
					.append("  when soundex(pname.givenName) = soundex(:n2) then 3")
					.append("  else 0 ")
					.append(" end")
					.append(" + ")
					.append(" case")
					.append("  when pname.middleName is null then 1")
					.append("  when pname.middleName = '' then 1")
					.append("  when soundex(pname.middleName) = soundex(:n1) then 3")
					.append("  when soundex(pname.middleName) = soundex(:n2) then 4")
					.append("  else 0 ")
					.append(" end")
					.append(" + ")
					.append(" case")
					.append("  when pname.familyName is null then 1")
					.append("  when pname.familyName = '' then 1")
					.append("  when soundex(pname.familyName) = soundex(:n1) then 3")
					.append("  when soundex(pname.familyName) = soundex(:n2) then 4")
					.append("  else 0 ")
					.append(" end")
					.append(" +")
					.append(" case")
					.append("  when pname.familyName2 is null then 1")
					.append("  when pname.familyName2 = '' then 1")
					.append("  when soundex(pname.familyName2) = soundex(:n1) then 3")
					.append("  when soundex(pname.familyName2) = soundex(:n2) then 4")
					.append("  else 0 ").append(" end").append(") > 6");
		} else if (names.length == 3) {
			q.append("(")
					.append(" case")
					.append("  when pname.givenName is null then 0")
					.append("  when soundex(pname.givenName) = soundex(:n1) then 3")
					.append("  when soundex(pname.givenName) = soundex(:n2) then 2")
					.append("  when soundex(pname.givenName) = soundex(:n3) then 1")
					.append("  else 0 ")
					.append(" end")
					.append(" + ")
					.append(" case")
					...
					.append("  when soundex(pname.middleName) = soundex(:n3) then 1")
					.append("  else 0")
					.append(" end")
					.append(" + ")
					.append(" case")
					...
					.append("  when soundex(pname.familyName) = soundex(:n3) then 3")
					.append("  else 0")
					.append(" end")
					.append(" +")
					.append(" case")
					...
					.append("  when soundex(pname.familyName2) = soundex(:n3) then 3")
					.append("  else 0").append(" end").append(") >= 5");
		} else {

			// This is simply an alternative method of name matching which
			// scales better
			// for large names, although it is hard to imagine getting names
			// with more than
			// six or so tokens. This can be easily updated to attain more
			// desirable
			// results; it is just a working alternative to throwing an
			// exception.

			q.append("(").append(" case")
					.append("  when pname.givenName is null then 0");
			for (int i = 0; i < names.length; i++) {
				q.append("  when soundex(pname.givenName) = soundex(:n")
						.append(i + 1).append(") then 1");
			}
			q.append("  else 0").append(" end").append(")").append("+")
					.append("(").append(" case")
					.append("  when pname.middleName is null then 0");
			for (int i = 0; i < names.length; i++) {
				q.append("  when soundex(pname.middleName) = soundex(:n")
						.append(i + 1).append(") then 1");
			}
			q.append("  else 0").append(" end").append(")").append("+")
					.append("(").append(" case")
					.append("  when pname.familyName is null then 0");
			for (int i = 0; i < names.length; i++) {
				q.append("  when soundex(pname.familyName) = soundex(:n")
						.append(i + 1).append(") then 1");
			}
			q.append("  else 0").append(" end").append(")").append("+")
					.append("(").append(" case")
					.append("  when pname.familyName2 is null then 0");
			for (int i = 0; i < names.length; i++) {
				q.append("  when soundex(pname.familyName2) = soundex(:n")
						.append(i + 1).append(") then 1");
			}
			q.append("  else 0").append(" end").append(") >= ")
					.append((int) (names.length * .75)); // if most of the names
															// have at least a
															// hit somewhere
		}
		...
		q.append(" order by pname.givenName asc,")
				.append(" pname.middleName asc,")
				.append(" pname.familyName asc,")
	...
	/**
	 * @see org.openmrs.api.db.PersonDAO#getPeople(java.lang.String,
	 *      java.lang.Boolean)
	 ...
	 * @should get one person by searching for a mix of attribute and voided
	 *         attribute
	...
	public List<Person> getPeople(String searchString, Boolean dead,
		...
		for (String value : values) {
			if (value != null && value.length() > 0) {
				disjunction.add(
						personSearchCriteria.prepareCriterionForName(value,
								voided)).add(
						personSearchCriteria.prepareCriterionForAttribute(
								value, voided, matchMode));
			}
		}
	...
	public PersonAttributeType getPersonAttributeType(Integer typeId) {
		return (PersonAttributeType) sessionFactory.getCurrentSession().get(
				PersonAttributeType.class, typeId);
	}
	...
	public PersonAttribute getPersonAttribute(Integer id) {
		return (PersonAttribute) sessionFactory.getCurrentSession().get(
				PersonAttribute.class, id);
	}
	...
	/**
	 * @see org.openmrs.api.db.PersonDAO#getPersonAttributeTypes(java.lang.String,
	...
	public Relationship getRelationship(Integer relationshipId)
			throws DAOException {
		Relationship relationship = (Relationship) sessionFactory
				.getCurrentSession().get(Relationship.class, relationshipId);
	...
	/**
	 * @see org.openmrs.api.PersonService#getRelationships(org.openmrs.Person,
	 *      org.openmrs.Person, org.openmrs.RelationshipType)
	 * @see org.openmrs.api.db.PersonDAO#getRelationships(org.openmrs.Person,
	 *      org.openmrs.Person, org.openmrs.RelationshipType)
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * @see org.openmrs.api.PersonService#getRelationships(org.openmrs.Person,
	 *      org.openmrs.Person, org.openmrs.RelationshipType, java.util.Date,
	 *      java.util.Date)
	 * @see org.openmrs.api.db.PersonDAO#getRelationships(org.openmrs.Person,
	 *      org.openmrs.Person, org.openmrs.RelationshipType, java.util.Date,
	 *      java.util.Date)
	...
	public List<Relationship> getRelationships(Person fromPerson,
		...
		if (startEffectiveDate != null) {
			criteria.add(Restrictions
					...
					.add(Restrictions.and(
							Restrictions.le("startDate", startEffectiveDate),
							Restrictions.ge("endDate", startEffectiveDate)))
					.add(Restrictions.and(
							Restrictions.le("startDate", startEffectiveDate),
							Restrictions.isNull("endDate")))
					.add(Restrictions.and(Restrictions.isNull("startDate"),
							Restrictions.ge("endDate", startEffectiveDate)))
					.add(Restrictions.and(Restrictions.isNull("startDate"),
							Restrictions.isNull("endDate"))));
		}
		if (endEffectiveDate != null) {
			criteria.add(Restrictions
					.disjunction()
					.add(Restrictions.and(
							Restrictions.le("startDate", endEffectiveDate),
	...
	public RelationshipType getRelationshipType(Integer relationshipTypeId)
			throws DAOException {
		RelationshipType relationshipType = (RelationshipType) sessionFactory
				.getCurrentSession().get(RelationshipType.class,
						relationshipTypeId);
	 ...
	 *      java.lang.Boolean)
	 * @see org.openmrs.api.db.PersonDAO#getRelationshipTypes(java.lang.String,
	 *      java.lang.Boolean)
	 */
	@SuppressWarnings("unchecked")
	public List<RelationshipType> getRelationshipTypes(
			String relationshipTypeName, Boolean preferred) throws DAOException {

		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
				RelationshipType.class);
		criteria.add(Restrictions.sqlRestriction(
				"CONCAT(a_Is_To_B, CONCAT('/', b_Is_To_A)) like (?)",
	...
	public RelationshipType saveRelationshipType(
			RelationshipType relationshipType) throws DAOException {
	 ...
	 * 
	 * @param sessionFactory
	 *            the session factory from which to pull the current session
	 * @param person
	...
	public PersonAttributeType getPersonAttributeTypeByUuid(String uuid) {
		return (PersonAttributeType) sessionFactory
				.getCurrentSession()
				.createQuery(
						"from PersonAttributeType pat where pat.uuid = :uuid")
	 ...
	 */
	public String getSavedPersonAttributeTypeName(
	...
	public Person getPersonByUuid(String uuid) {
		return (Person) sessionFactory.getCurrentSession()
				.createQuery("from Person p where p.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}

	public PersonAddress getPersonAddressByUuid(String uuid) {
		return (PersonAddress) sessionFactory.getCurrentSession()
				.createQuery("from PersonAddress p where p.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public PersonMergeLog getPersonMergeLog(Integer id) throws DAOException {
		return (PersonMergeLog) sessionFactory.getCurrentSession().get(
				PersonMergeLog.class, id);
	}
	...
	public PersonMergeLog getPersonMergeLogByUuid(String uuid)
		...
		return (PersonMergeLog) sessionFactory.getCurrentSession()
				.createQuery("from PersonMergeLog p where p.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public List<PersonMergeLog> getWinningPersonMergeLogs(Person person)
			throws DAOException {
		return (List<PersonMergeLog>) sessionFactory
				.getCurrentSession()
				.createQuery(
						"from PersonMergeLog p where p.winner.id = :winnerId")
				.setInteger("winnerId", person.getId()).list();
	}
	 ...
	 */
	@Override
	public PersonMergeLog getLosingPersonMergeLogs(Person person)
			throws DAOException {
		return (PersonMergeLog) sessionFactory
				.getCurrentSession()
				.createQuery(
						"from PersonMergeLog p where p.loser.id = :loserId")
				.setInteger("loserId", person.getId()).uniqueResult();
	}
	...
	public PersonName getPersonName(Integer personNameId) {
		return (PersonName) sessionFactory.getCurrentSession().get(
				PersonName.class, personNameId);
	}
	...
	public PersonName getPersonNameByUuid(String uuid) {
		return (PersonName) sessionFactory.getCurrentSession()
				.createQuery("from PersonName p where p.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public Relationship getRelationshipByUuid(String uuid) {
		return (Relationship) sessionFactory.getCurrentSession()
				.createQuery("from Relationship r where r.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public RelationshipType getRelationshipTypeByUuid(String uuid) {
		return (RelationshipType) sessionFactory.getCurrentSession()
				.createQuery("from RelationshipType rt where rt.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
