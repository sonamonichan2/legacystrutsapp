...
/**
 * This class has been created to provide backwards compatibility for modules,
 * which need to support OpenMRS 1.12 and before. It is because
 * org.hibernate.classic.Session has been removed in Hibernate 4 (used in
 * OpenMRS 1.12) and sessionFactory.getCurrentSession() has been changed to
 * return org.hiberante.Session. It wraps SessionFactory so that any calls to
 * getCurrentSession() are directed to the correct Session class.
 * 
...
public class DbSession {
	 ...
	 * 
	 * @return The tenant identifier associated with this session, or
	 *         {@code null}
	 */
	...
	/**
	 * Begin a unit of work and return the associated {@link Transaction}
	 ...
	 * existing underlying transaction.
	 * 
	...
	/**
	 * Get the {@link Transaction} instance associated with this session. The
	 * concrete type of the returned {@link Transaction} object is determined by
	 * the {@code hibernate.transaction_factory} property.
	 * 
	 * @return a Transaction instance
	 */
	...
	/**
	 * Create a {@link Query} instance for the named query string defined in the
	 * metadata.
	 * 
	 * @param queryName
	...
	/**
	 * Creates a call to a stored procedure with specific result set entity
	 * mappings. Each class named is considered a "root return".
	 * 
	 * @param procedureName
	 *            The name of the procedure.
	 * @param resultClasses
	 *            The entity(s) to map the result on to.
	 * @return The representation of the procedure call.
	 */
	public ProcedureCall createStoredProcedureCall(String procedureName,
			Class... resultClasses) {
	 ...
	 *            The explicit result set mapping(s) to use for mapping the
	 *            results
	 * @return The representation of the procedure call.
	 */
	public ProcedureCall createStoredProcedureCall(String procedureName,
	...
	/**
	 * Create {@link Criteria} instance for the given class (entity or
	...
	/**
	 * Create {@link Criteria} instance for the given class (entity or
	 * subclasses/implementors), using a specific alias.
	 * 
	 * @param persistentClass
	 *            The class, which is an entity, or has entity
	 *            subclasses/implementors
	 ...
	 * 
	 * @param entityName
	 *            The entity name @return The criteria instance for manipulation
	 *            and execution
	 */
	...
	/**
	 * Create {@link Criteria} instance for the given entity name, using a
	 * specific alias.
	 * 
	 * @param entityName
	 *            The entity name
	...
	/**
	 * Obtain a {@link Session} builder with the ability to grab certain
	 ...
	 * session's flush mode to {@link FlushMode#MANUAL} at the start of the
	 * session (in order to achieve some extra performance).
	 * 
	 * @param flushMode
	 *            the new flush mode
	 * @see FlushMode
	 ...
	 * read-only by default? To determine the read-only/modifiable setting for a
	 * particular entity or proxy:
	 * 
	 ...
	 * are not persisted. When a proxy is initialized, the loaded entity will
	 * have the same read-only/modifiable setting as the uninitialized proxy
	 * has, regardless of the session's current setting. To change the
	 * read-only/modifiable setting for a particular entity or proxy that is
	 * already in this session:
	 * 
	 * @see Session#setReadOnly(Object,boolean) To override this session's
	 *      read-only/modifiable setting for entities and proxies loaded by a
	 *      Query:
	 ...
	 * association is mapped with {@code cascade="replicate"}
	 * 
	 * @param object
	...
	/**
	 * Create a {@link Query} instance for the given collection and filter
	 * string. Contains an implicit {@code FROM} element named {@code this}
	 * which refers to the defined table for the collection elements, as well as
	 * an implicit {@code WHERE} restriction for this particular collection
	 * instance's key value.
	 * 
	 * @param collection
	...
	/**
	 * Create an {@link IdentifierLoadAccess} instance to retrieve the specified
	 * entity type by primary key.
	 * 
	 * @param entityName
	 *            The entity name of the entity type to be retrieved
	 * @return load delegate for loading the specified entity type by primary
	 *         key
	 * @throws HibernateException
	 *             If the specified entity name cannot be resolved as an entity
	 *             name
	 */
	...
	/**
	 * Create an {@link NaturalIdLoadAccess} instance to retrieve the specified
	 * entity by its natural id.
	 * 
	 * @param entityName
	 *            The entity name of the entity type to be retrieved
	...
	/**
	 * Create an {@link SimpleNaturalIdLoadAccess} instance to retrieve the
	 * specified entity by its natural id.
	 * 
	 * @param entityName
	 *            The entity name of the entity type to be retrieved
	 * @return load delegate for loading the specified entity type by natural id
	 ...
	 *             entity, or if the entity does not define a natural-id or if
	 *             its natural-id is made up of multiple attributes.
	 */
	...
	/**
	 * Create an {@link SimpleNaturalIdLoadAccess} instance to retrieve the
	 * specified entity by its simple (single attribute) natural id.
	 * 
	 * @param entityClass
	 *            The entity type to be retrieved
	 * @return load delegate for loading the specified entity type by natural id
	...
	/**
	 * Is the specified entity or proxy read-only? To get the default
	 * read-only/modifiable setting used for entities and proxies that are
	 * loaded into the session:
	 * 
	 * @see org.hibernate.Session#isDefaultReadOnly()
	 * @param entityOrProxy
	 *            an entity or HibernateProxy
	 * @return {@code true} if the entity or proxy is read-only, {@code false}
	 *         if the entity or proxy is modifiable.
	 */
	 ...
	 * the instance is never dirty checked, and changes are not persisted. If
	 * the entity or proxy already has the specified read-only/modifiable
	 * setting, then this method does nothing. To set the default
	 * read-only/modifiable setting used for entities and proxies that are
	 * loaded into the session:
	 * 
	 * @see org.hibernate.Session#setDefaultReadOnly(boolean) To override this
	 *      session's read-only/modifiable setting for entities and proxies
	 *      loaded by a Query:
	 * @see Query#setReadOnly(boolean)
	 * @param entityOrProxy
	 *            an entity or HibernateProxy
	 * @param readOnly
	 *            {@code true} if the entity or proxy should be made read-only;
	...
	/**
	 * Controller for allowing users to perform JDBC related work using the
	 * Connection managed by this Session. After execution returns the result of
	 * the {@link ReturningWork#execute} call.
	 * 
	 * @param work
	 *            The work to be performed.
	 * @param <T>
	 *            The type of the result returned from the work
	...
	/**
	 * Disconnect the session from its underlying JDBC connection. This is
	 * intended for use in cases where the application has supplied the JDBC
	 * connection to the session and which require long-sessions (aka,
	 * conversations).
	 * <p>
	 * It is considered an error to call this method on a session which was not
	 * opened by supplying the JDBC connection and an exception will be thrown.
	 * <p>
	 * For non-user-supplied scenarios, normal transaction management already
	 ...
	 *             profile names
	 * @see org.hibernate.engine.profile.FetchProfile for discussion of this
	 *      feature
	 */
	...
	/**
	 * Convenience access to the {@link TypeHelper} associated with this
	 * session's {@link SessionFactory}.
	 * <p>
	 * Equivalent to calling {@link #getSessionFactory()}.
	 * {@link SessionFactory#getTypeHelper getTypeHelper()}
	 * 
	 * @return The {@link TypeHelper} associated with this session's
	 *         {@link SessionFactory}
	 */
