...
public class HibernateUserDAO implements UserDAO {
	...
	public User saveUser(User user, String password) {
		...
		if (isNewUser && password != null) {
			...
			updateUserPassword(hashedPassword, salt, Context
					.getAuthenticatedUser().getUserId(), new Date(),
					user.getUserId());
		}
	...
	public boolean hasDuplicateUsername(String username, String systemId,
		...
		try {
			...
			usernameWithCheckDigit = new LuhnIdentifierValidator()
					.getValidIdentifier(username);
		} catch (Exception e) {
		}
	...
	public List<User> getUsersByRole(Role role) throws DAOException {
		List<User> users = sessionFactory.getCurrentSession()
				.createCriteria(User.class, "u").createCriteria("roles", "r")
				.add(Restrictions.like("r.role", role.getRole()))
				.addOrder(Order.asc("u.username")).list();
	...
	public List<Privilege> getAllPrivileges() throws DAOException {
		return sessionFactory.getCurrentSession()
				.createQuery("from Privilege p order by p.privilege").list();
	}
	...
	public Privilege getPrivilege(String p) throws DAOException {
		return (Privilege) sessionFactory.getCurrentSession().get(
				Privilege.class, p);
	}
	...
	public List<Role> getAllRoles() throws DAOException {
		return sessionFactory.getCurrentSession()
				.createQuery("from Role r order by r.role").list();
	}
	...
	/**
	 * @see org.openmrs.api.db.UserDAO#changePassword(org.openmrs.User,
	 *      java.lang.String)
	 */
	public void changePassword(User u, String pw) throws DAOException {
		...
		updateUserPassword(newHashedPassword, salt, authUser.getUserId(),
				new Date(), u.getUserId());

	}

	/**
	 * @see org.openmrs.api.db.UserDAO#changeHashedPassword(User, String,
	 *      String)
	 */
	public void changeHashedPassword(User user, String hashedPassword,
			String salt) throws DAOException {
		User authUser = Context.getAuthenticatedUser();
		updateUserPassword(hashedPassword, salt, authUser.getUserId(),
	 ...
	 * @param userId2
	 */
	private void updateUserPassword(String newHashedPassword, String salt,
			Integer changedBy, Date dateChanged, Integer userIdToChange) {
		User changeForUser = getUser(userIdToChange);
		if (changeForUser == null) {
			throw new DAOException(
					"Couldn't find user to set password for userId="
							+ userIdToChange);
		}
		...
		changeForUser.setUserProperty(
				OpenmrsConstants.USER_PROPERTY_LOGIN_ATTEMPTS, "0");
	...
	public void changePassword(String pw, String pw2) throws DAOException {
		...
		updateUserPassword(newHashedPassword, salt, u.getUserId(), new Date(),
				u.getUserId());
	}
	 ...
	 */
	public void changeQuestionAnswer(String pw, String question, String answer)
			throws DAOException {
		User u = Context.getAuthenticatedUser();
	...
	public void changeQuestionAnswer(User u, String question, String answer)
		...
		String hashedAnswer = Security.encodeString(answer.toLowerCase()
				+ credentials.getSalt());
	...
	public List<User> getUsers(String name, List<Role> roles,
		...
		Query query = createUserSearchQuery(name, roles, includeRetired,
				hqlSelectStart);
	...
	public Privilege getPrivilegeByUuid(String uuid) {
		return (Privilege) sessionFactory.getCurrentSession()
				.createQuery("from Privilege p where p.uuid = :uuid")
	...
	public Role getRoleByUuid(String uuid) {
		return (Role) sessionFactory.getCurrentSession()
				.createQuery("from Role r where r.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public LoginCredential getLoginCredential(User user) {
		return (LoginCredential) sessionFactory.getCurrentSession().get(
				LoginCredential.class, user.getUserId());
	}
	...
	public LoginCredential getLoginCredentialByUuid(String uuid) {
		if (uuid == null) {
			...
			return (LoginCredential) sessionFactory.getCurrentSession()
					.createQuery("from LoginCredential where uuid = :uuid")
					.setString("uuid", uuid.trim()).uniqueResult();
		}
	}
	...
	/**
	 * @see org.openmrs.api.db.UserDAO#getUsersByPerson(org.openmrs.Person,
	 *      boolean)
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Utility methods that creates a hibernate query object from the specified
	 * arguments
	 * 
	 * @param name
	 *            The name of the user to search against
	 * @param roles
	 *            the roles to match against
	 * @param includeRetired
	 *            Specifies if retired users should be included or not
	 * @param hqlSelectStart
	 *            The starting phrase of the select statement that includes the
	 *            joined tables
	 * @return the created hibernate query object
	 */
	private Query createUserSearchQuery(String name, List<Role> roles,
			boolean includeRetired, String hqlSelectStart) {
		...
		// from User as user inner join user.person.names as name inner join
		// user.roles as role
		// where (user.username like :name1 or ...and for systemId givenName
		// familyName familyName2...)
		// and (user.username like :name2 or ...and for systemId givenName
		// familyName familyName2...)
		...
		if (name != null) {
			...
			for (String n : names) {
				if (n != null && n.length() > 0) {
					// compare each fragment of the query against username,
					// systemId, given, middle, family, and family2
					String key = "name" + ++counter;
					String value = n + "%";
					namesMap.put(key, value);
					criteria.add("(user.username like :" + key
							+ " or user.systemId like :" + key
							+ " or name.givenName like :" + key
							+ " or name.middleName like :" + key
							+ " or name.familyName like :" + key
							+ " or name.familyName2 like :" + key + ")");
				}
			}
		}
