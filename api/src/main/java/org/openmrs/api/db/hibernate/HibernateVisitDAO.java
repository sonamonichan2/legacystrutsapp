...
/**
 * Hibernate specific visit related functions This class should not be used
 * directly. All calls should go through the
 * {@link org.openmrs.api.VisitService} methods.
 * 
 * @since 1.9
 */
public class HibernateVisitDAO implements VisitDAO {
	...
	public VisitType getVisitType(Integer visitTypeId) {
		return (VisitType) sessionFactory.getCurrentSession().get(
				VisitType.class, visitTypeId);
	}
	...
	public VisitType getVisitTypeByUuid(String uuid) {
		return (VisitType) sessionFactory.getCurrentSession()
				.createQuery("from VisitType vt where vt.uuid = :uuid")
	...
	public List<VisitType> getVisitTypes(String fuzzySearchPhrase) {
		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
				VisitType.class);
		criteria.add(Restrictions.ilike("name", fuzzySearchPhrase,
				MatchMode.ANYWHERE));
	...
	public Visit getVisitByUuid(String uuid) throws DAOException {
		return (Visit) getCurrentSession()
				.createQuery("from Visit v where v.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	/**
	 * @see org.openmrs.api.db.VisitDAO#getVisits(java.util.Collection,
	...
	public List<Visit> getVisits(Collection<VisitType> visitTypes,
		...
		// active visits have null end date, so it doesn't make sense to search
		// against it if include inactive is set to false
		if (!includeInactive) {
			// the user only asked for currently active visits, so stop time
			// needs to be null or after right now
	...
	public List<VisitAttributeType> getAllVisitAttributeTypes() {
		return getCurrentSession().createCriteria(VisitAttributeType.class)
				.list();
	}
	...
	public VisitAttributeType getVisitAttributeType(Integer id) {
		return (VisitAttributeType) getCurrentSession().get(
				VisitAttributeType.class, id);
	}
	...
	public VisitAttributeType getVisitAttributeTypeByUuid(String uuid) {
		return (VisitAttributeType) getCurrentSession()
				.createCriteria(VisitAttributeType.class)
	...
	public VisitAttribute getVisitAttributeByUuid(String uuid) {
		return (VisitAttribute) getCurrentSession()
				.createCriteria(VisitAttribute.class)
				.add(Restrictions.eq("uuid", uuid)).uniqueResult();
	}
	 ...
	 */
	@Override
	public Visit getNextVisit(Visit previousVisit,
			Collection<VisitType> visitTypes, Date maximumStartDate) {
		Criteria criteria = getCurrentSession().createCriteria(Visit.class);
		criteria.add(Restrictions.eq("voided", false))
				.add(Restrictions.gt("visitId", (previousVisit != null)
						? previousVisit.getVisitId()
						: 0)).addOrder(Order.asc("visitId"))
				.add(Restrictions.isNull("stopDatetime")).setMaxResults(1);
