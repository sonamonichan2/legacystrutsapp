...
/**
 * Superclass for all Interceptors that would like to ensure that changes to
 * immutable entities of specific types don't get persisted to the database,
 * more granularity of the immutable properties is also supported so as to allow
 * editing some properties while not for others
 * 
 * <pre>
...
public abstract class ImmutableEntityInterceptor extends EmptyInterceptor {
	...
	private static final Log log = LogFactory
			.getLog(ImmutableEntityInterceptor.class);
	...
	/**
	 * Subclasses can override this to return fields that are allowed to be
	 * edited, returning null or an empty array implies the entity is immutable
	 * 
	...
	/**
	 * Subclasses can override this to specify whether voided or retired items
	 * are mutable
	 * 
	 * @return true if voided or retired objects are mutable otherwise false
	 *         means they are immutable
	 */
	 ...
	 * @should pass if the edited object is voided or retired and ignore is set
	 *         to true
	 * @should fail if the edited object is voided or retired and ignore is set
	 *         to false
	 */
	@Override
	public boolean onFlushDirty(Object entity, Serializable id,
		...
		if (getSupportedType().isAssignableFrom(entity.getClass())) {
			...
			for (int i = 0; i < propertyNames.length; i++) {
				...
				Object previousValue = (previousState != null)
						? previousState[i]
						: null;
				Object currentValue = (currentState != null)
						? currentState[i]
						: null;
			...
			if (CollectionUtils.isNotEmpty(changedProperties)) {
				if (log.isDebugEnabled()) {
					log.debug("The following fields cannot be changed for "
							+ getSupportedType() + ":" + changedProperties);
				}
				throw new APIException("Editing some fields "
