...
/**
 * Hibernate specific implementation of the {@link ContextDAO}. These methods
 * should not be used directly, instead, the methods on the static
 * {@link Context} file should be used.
 * 
 * @see ContextDAO
 * @see Context
 */
public class HibernateContextDAO implements ContextDAO {
	...
	/**
	 * Session factory to use for this DAO. This is usually injected by spring
	...
	/**
	 * @see org.openmrs.api.db.ContextDAO#authenticate(java.lang.String,
	 *      java.lang.String)
	 */
	...
	public User authenticate(String login, String password)
		...
		if (login != null) {
			...
			try {
				candidateUser = (User) session
						...
						.setString(0, login).setString(1, login)
						.setString(2, loginWithDash).uniqueResult();
		...
		if (candidateUser != null && password != null) {
			...
			String lockoutTimeString = candidateUser.getUserProperty(
					OpenmrsConstants.USER_PROPERTY_LOCKOUT_TIMESTAMP, null);
			Long lockoutTime = null;
			if (lockoutTimeString != null && !lockoutTimeString.equals("0")) {
				try {
					// putting this in a try/catch in case the admin decided to
					// put junk into the property
					lockoutTime = Long.valueOf(lockoutTimeString);
				} catch (NumberFormatException e) {
					log.debug("bad value stored in "
							+ OpenmrsConstants.USER_PROPERTY_LOCKOUT_TIMESTAMP
							+ " user property: " + lockoutTimeString);
				}
			}

			// if they've been locked out, don't continue with the
			// authentication
			if (lockoutTime != null) {
				...
				if (System.currentTimeMillis() - lockoutTime > 300000) {
					...
					candidateUser
							.removeUserProperty(OpenmrsConstants.USER_PROPERTY_LOCKOUT_TIMESTAMP);
			...
			String passwordOnRecord = (String) session
					...
					.setInteger(0, candidateUser.getUserId()).uniqueResult();

			String saltOnRecord = (String) session
					.createSQLQuery("select salt from users where user_id = ?")
			...
			if (passwordOnRecord != null
					&& Security.hashMatches(passwordOnRecord, password
							+ saltOnRecord)) {
				...
				// only clean up if the were some login failures, otherwise all
				// should be clean
				...
				// skip out of the method early (instead of throwing the
				// exception)
				...
				try {
					allowedFailedLoginCount = Integer
							.valueOf(Context
	...
	/**
	 * Call the UserService to save the given user while proxying the privileges
	 * needed to do so.
	 * 
	 * @param user
	 *            the User to save
	 ...
	 * @param user
	 *            the user to check
	 * @return the # of login attempts for this user defaulting to zero if none
	 *         defined
	 */
	private Integer getUsersLoginAttempts(User user) {
		String attemptsString = user.getUserProperty(
				OpenmrsConstants.USER_PROPERTY_LOGIN_ATTEMPTS, "0");
		Integer attempts = 0;
		try {
	...
	/**
	 * Convenience method to print out the hibernate cache usage stats to the
	 * log
	 */
	...
	/**
	 * Takes the default properties defined in
	 * /metadata/api/hibernate/hibernate.default.properties and merges it into
	 * the user-defined runtime properties
	 * 
	...
	public void mergeDefaultRuntimeProperties(Properties runtimeProperties) {
		...
		for (Map.Entry<Object, Object> entry : runtimeProperties.entrySet()) {
			...
			if (!prop.startsWith("hibernate")
					&& !runtimeProperties.containsKey("hibernate." + prop)) {
				cache.put("hibernate." + prop, value);
			}
		}
		runtimeProperties.putAll(cache);

		// load in the default hibernate properties from
	...
	public void updateSearchIndexForType(Class<?> type) {
		// From
		// http://docs.jboss.org/hibernate/search/3.3/reference/en-US/html/manual-index-changes.html#search-batchindex-flushtoindexes
		...
		try {
			...
			ScrollableResults results = session.createCriteria(type)
	...
	public void updateSearchIndex() {
		try {
			...
			gp.setPropertyValue(OpenmrsConstants.SEARCH_INDEX_VERSION
					.toString());
	...
	public Future<?> updateSearchIndexAsync() {
		try {
			...
			throw new RuntimeException(
					"Failed to start asynchronous search index update", e);
		}
	}
