...
public class HibernateProgramWorkflowDAO implements ProgramWorkflowDAO {
	...
	public Program getProgram(Integer programId) throws DAOException {
		return (Program) sessionFactory.getCurrentSession().get(Program.class,
				programId);
	}
	...
	/**
	 * @see org.openmrs.api.db.ProgramWorkflowDAO#getProgramsByName(String,
	 *      boolean)
	 */
	...
	public List<Program> findPrograms(String nameFragment) throws DAOException {
		...
		criteria.add(Restrictions.ilike("name", nameFragment,
				MatchMode.ANYWHERE));
	...
	public PatientProgram getPatientProgram(Integer patientProgramId)
			throws DAOException {
		return (PatientProgram) sessionFactory.getCurrentSession().get(
				PatientProgram.class, patientProgramId);
	}

	/**
	 * @see org.openmrs.api.db.ProgramWorkflowDAO#getPatientPrograms(Patient,
	...
	public List<PatientProgram> getPatientPrograms(Patient patient,
		...
		if (minCompletionDate != null) {
			crit.add(Restrictions.or(Restrictions.isNull("dateCompleted"),
					Restrictions.ge("dateCompleted", minCompletionDate)));
		}
	...
	public List<ConceptStateConversion> getAllConceptStateConversions()
		...
		return sessionFactory.getCurrentSession()
				.createCriteria(ConceptStateConversion.class).list();
	}
	...
	public ConceptStateConversion getConceptStateConversion(
		...
		return (ConceptStateConversion) sessionFactory.getCurrentSession().get(
				ConceptStateConversion.class, conceptStateConversionId);
	}
	...
	public ConceptStateConversion getConceptStateConversion(
		...
		if (workflow != null && trigger != null) {
			Criteria criteria = sessionFactory.getCurrentSession()
					.createCriteria(ConceptStateConversion.class, "csc");
	...
	public ConceptStateConversion getConceptStateConversionByUuid(String uuid) {
		return (ConceptStateConversion) sessionFactory
				.getCurrentSession()
				.createQuery(
						"from ConceptStateConversion csc where csc.uuid = :uuid")
	...
	public PatientProgram getPatientProgramByUuid(String uuid) {
		return (PatientProgram) sessionFactory.getCurrentSession()
				.createQuery("from PatientProgram pp where pp.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public Program getProgramByUuid(String uuid) {
		return (Program) sessionFactory.getCurrentSession()
				.createQuery("from Program p where p.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public ProgramWorkflowState getStateByUuid(String uuid) {
		return (ProgramWorkflowState) sessionFactory
				.getCurrentSession()
				.createQuery(
						"from ProgramWorkflowState pws where pws.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}

	public PatientState getPatientStateByUuid(String uuid) {
		return (PatientState) sessionFactory.getCurrentSession()
				.createQuery("from PatientState pws where pws.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
	...
	public ProgramWorkflow getWorkflowByUuid(String uuid) {
		return (ProgramWorkflow) sessionFactory.getCurrentSession()
				.createQuery("from ProgramWorkflow pw where pw.uuid = :uuid")
				.setString("uuid", uuid).uniqueResult();
	}
