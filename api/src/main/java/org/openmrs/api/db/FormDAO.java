...
public interface FormDAO {
	...
	/**
	 * Gets all forms with the given name, sorted (alphabetically) by descending
	 * version
	 * 
	 * @param name
	 *            String name of the forms to get
	 * @return All forms with the given name, sorted by (alphabetically) by
	 *         descending version
	 * @throws DAOException
	 */
	 ...
	 * be called &mdash; <code>Forms</code> should be <em>retired</em> and not
	 * <em>deleted</em> altogether (since many foreign key constraints depend on
	 * forms, deleting a form would require deleting all traces, and any
	 * historical trail would be lost). This method only clears form roles and
	 * attempts to delete the form record. If the form has been included in any
	 * other parts of the database (through a foreign key), the attempt to
	 * delete the form will violate foreign key constraints and fail.
	 * 
	 * @param form
	 *            Form to delete
	 * @throws DAOException
	 ...
	 * @return List&lt;FieldTypes&gt; object with all FieldTypes, possibly
	 ...
	 * @return List&lt;Form&gt; object of all forms, possibly including retired
	 *         ones
	 * @throws DAOException
	 */
	 ...
	 * <p>
	 * This will fail if any other entities reference this field via a foreign
	 * key
	 * 
	 * @param field
	 *            the Field to delete
	...
	/**
	 * Deletes a FormField from the database. This will fail if any other
	 * entities reference this FormField via a foreign key
	 * 
	 * @param formField
	 ...
	 * 
	 * @param forms
	 *            <code>Collection</code> of <code>Form</code> to which the
	 *            requested Fields must belong
	 * @param fieldTypes
	 *            <code>Collection</code> of <code>FieldType</code> of which the
	 *            requested fields must be
	 * @param concepts
	 *            <code>Collection</code> of <code>Concepts</code> which the
	 *            fields must point to
	 * @param tableNames
	 *            Collection of <code>TableName</code>s which the fields must
	 *            point to
	 * @param attributeNames
	 *            Collection of <code>String</code> attribute names which the
	 *            fields must point to
	 * @param selectMultiple
	 *            <code>Boolean</code> value that matching fields must have for
	 *            selectMultiple
	 * @param containsAllAnswers
	 *            <code>Collection</code> of <code>FieldAnswer</code>s, all of
	 *            which a matching field must contain (not yet implemented)
	 * @param containsAnyAnswer
	 *            <code>Collection</code> of <code>FieldAnswer</code>s, any one
	 *            of which a matching field must contain (not yet implemented)
	 * @param retired
	 *            <code>Boolean</code> retired status that fields must match
	...
	/**
	 * Get all forms that contain the given <code>Concept</code> as one of their
	 * fields. (Includes retired forms.)
	 * 
	 * @param concept
	 *            the <code>Concept</code> to search through form fields for
	...
	/**
	 * Gets all forms that match all the criteria. Expects the list objects to
	 * be non-null
	 * 
	 * @param partialName
	 *            String of partial name of form to search on
	 * @param published
	 *            boolean - is the form published?
	 * @param encounterTypes
	 *            Collection of <code>EncounterType</code>s that the form must
	 *            represent
	 * @param retired
	 *            boolean - is the form retired?
	 * @param containingAnyFormField
	 *            Collection of <code>FormField</code>s, any one of which must
	 *            be contained in the form
	 * @param containingAllFormFields
	 *            Collection of <code>FormField</code>s, all of which must be
