...
public interface LocationService extends OpenmrsService {
	...
	/**
	 * Set the data access object that the service will use to interact with the
	 * database. This is set by spring in the applicationContext-service.xml
	 * file
	 * 
	 * @param dao
	 */
	 ...
	 * @return Location object that has location.locationId =
	 ...
	 * @should return Unknown Location if the global property is something else
	 *         that doesnot exist
	 */
	@Authorized({PrivilegeConstants.GET_LOCATIONS})
	 ...
	 * to the #getAllLocations(boolean) method
	 * 
	...
	/**
	 * Gets the locations matching the specified arguments. A null list will
	 * never be returned. An empty list will be returned if there are no
	 * locations. Search is case insensitive. matching this
	 * <code>nameFragment</code>. If start and length are not specified, then
	 * all matches are returned.
	 * 
	 * @param nameFragment
	 *            is the string used to search for locations
	 * @param parent
	 *            only return children of this parent
	 * @param attributeValues
	 *            the attribute values
	 * @param includeRetired
	 *            specifies if retired locations should also be returned
	 * @param start
	 *            the beginning index
	 * @param length
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_LOCATIONS})
	public List<Location> getLocationsByTag(LocationTag tag)
			throws APIException;
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_LOCATIONS})
	public List<Location> getLocationsHavingAnyTag(List<LocationTag> tags)
			throws APIException;
	 ...
	 */
	@Authorized({PrivilegeConstants.MANAGE_LOCATIONS})
	public Location retireLocation(Location location, String reason)
			throws APIException;
	 ...
	 * <code>locationTagId</code>. A null value is returned if no tag exists
	 * with this ID.
	 * 
	 * @param locationTagId
	 *            integer primary key of the location tag to find
	 * @return LocationTag object that has LocationTag.locationTagId =
	 *         <code>locationTagId</code> passed in.
	...
	/**
	 * Returns a location tag given the location's exact name (tag). A null
	 * value is returned if there is no tag with this name.
	 * 
	 * @param tag
	...
	/**
	 * Returns all location tags, includes retired location tags. This method
	 * delegates to the #getAllLocationTags(boolean) method.
	 * 
	...
	/**
	 * Returns location tags that match the beginning of the given string. A
	 * null list will never be returned. An empty list will be returned if there
	 * are no tags. Search is case insensitive. matching this
	 * <code>search</code>
	 * 
	 * @param search
	...
	/**
	 * Retire the given location tag. This effectively removes the tag from
	 * circulation or use.
	 * 
	 * @param tag
	 *            location tag to be retired
	 * @param reason
	 ...
	 */
	@Authorized({PrivilegeConstants.MANAGE_LOCATION_TAGS})
	public LocationTag retireLocationTag(LocationTag tag, String reason)
			throws APIException;

	/**
	 * Unretire the given location tag. This restores a previously retired tag
	 * back into circulation and use.
	 * 
	 * @param tag
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_LOCATIONS})
	public Integer getCountOfLocations(String nameFragment,
			Boolean includeRetired);

	/**
	 * Returns all root locations (i.e. those who have no parentLocation),
	 * optionally including retired ones.
	 * 
	 * @param includeRetired
	...
	/**
	 * Given an Address object, returns all the possible values for the
	 * specified AddressField. This method is not implemented in core, but is
	 * meant to overridden by implementing modules such as the Address Hierarchy
	 * module.
	 * 
	 * @param incomplete
	 ...
	 * @since 1.7.2
	 */
	public List<String> getPossibleAddressValues(Address incomplete,
			String fieldName) throws APIException;
	 ...
	 * @should return null if no location attribute type exists with the given
	 *         id
	 */
	@Authorized(PrivilegeConstants.GET_LOCATION_ATTRIBUTE_TYPES)
	...
	LocationAttributeType saveLocationAttributeType(
	...
	/**
	 * Restores a location attribute type that was previous retired in the
	 * database
	 * 
	 * @param locationAttributeType
	 ...
	 */
	@Authorized(PrivilegeConstants.MANAGE_LOCATION_ATTRIBUTE_TYPES)
	LocationAttributeType unretireLocationAttributeType(
			LocationAttributeType locationAttributeType);
	 ...
	 * @should return null if no location attribute type exists with the
	 *         specified name
	 */
	@Authorized(PrivilegeConstants.GET_LOCATION_ATTRIBUTE_TYPES)
	LocationAttributeType getLocationAttributeTypeByName(
