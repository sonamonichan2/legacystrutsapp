...
/**
 * Contains methods pertaining to management of Programs, ProgramWorkflows,
 * ProgramWorkflowStates, PatientPrograms, PatientStates, and
 * ConceptStateConversions Use:<br>
 * 
 * <pre>
...
public interface ProgramWorkflowService extends OpenmrsService {
	...
	/**
	 * Setter for the ProgramWorkflow DataAccessObject (DAO). The DAO is used
	 * for saving and retrieving from the database
	 * 
	 * @param dao
	 *            - The DAO for this service
	 */
	...
	/**
	 * Save <code>program</code> to database (create if new or update if
	 * changed)
	 * 
	 * @param program
	...
	/**
	 * Returns a program given that programs primary key <code>programId</code>
	 * A null value is returned if no program exists with this programId.
	 * 
	 * @param programId
	 *            integer primary key of the program to find
	 * @return Program object that has program.programId =
	 *         <code>programId</code> passed in.
	 * @throws APIException
	...
	/**
	 * Returns a program given the program's exact <code>name</code> A null
	 * value is returned if there is no program with this name
	 * 
	 * @param name
	 *            the exact name of the program to match on
	 * @return Program matching the <code>name</code> to Program.name
	 * @throws APIException
	 * @throws ProgramNameDuplicatedException
	 *             when there are more than one program in the dB with the given
	 *             name.
	...
	/**
	 * Returns all programs, includes retired programs. This method delegates to
	 * the #getAllPrograms(boolean) method
	 * 
	 * @return List&lt;Program&gt; of all existing programs, including retired
	 *         programs
	 * @throws APIException
	 */
	@Authorized({PrivilegeConstants.GET_PROGRAMS})
	 ...
	 * @return List&lt;Program&gt; all existing programs, including retired
	 *         based on the input parameter
	 * @throws APIException
	 * @should return all programs including retired when includeRetired equals
	 *         true
	 * @should return all programs excluding retired when includeRetired equals
	 *         false
	 */
	@Authorized({PrivilegeConstants.GET_PROGRAMS})
	public List<Program> getAllPrograms(boolean includeRetired)
			throws APIException;

	/**
	 * Returns programs that match the given string. A null list will never be
	 * returned. An empty list will be returned if there are no programs
	 * matching this <code>nameFragment</code>
	 * 
	 * @param nameFragment
	 *            is the string used to search for programs
	 * @return List&lt;Program&gt; - list of Programs whose name matches the
	 *         input parameter
	 * @throws APIException
	 ...
	 * @should return programs when nameFragment matches beginning of program
	 *         name
	...
	/**
	 * Completely remove a program from the database (not reversible) This
	 * method delegates to #purgeProgram(program, boolean) method
	 * 
	 * @param program
	 ...
	 * 
	 * @param cascade
	 *            <code>true</code> to delete related content
	 * @throws APIException
	 * @should delete program successfully
	 ...
	 */
	@Authorized({PrivilegeConstants.MANAGE_PROGRAMS})
	public void purgeProgram(Program program, boolean cascade)
			throws APIException;
	 ...
	 * @param reason
	 *            String for retiring the program
	 ...
	 */
	@Authorized({PrivilegeConstants.MANAGE_PROGRAMS})
	public Program retireProgram(Program program, String reason)
			throws APIException;
	...
	/**
	 * Get a program by its uuid. There should be only one of these in the
	...
	/**
	 * Get a program state by its uuid. There should be only one of these in the
	 * database. If multiple are found, an error is thrown.
	 * 
	 * @param uuid
	 *            the universally unique identifier
	 ...
	 * <code>patientProgramId</code> A null value is returned if no
	 * PatientProgram exists with this patientProgramId.
	 * 
	...
	/**
	 * Returns PatientPrograms that match the input parameters. If an input
	 * parameter is set to null, the parameter will not be used. Calling this
	 * method will all null parameters will return all PatientPrograms in the
	 * database A null list will never be returned. An empty list will be
	 * returned if there are no programs matching the input criteria
	 * 
	 * @param patient
	 *            if supplied all PatientPrograms returned will be for this
	 *            Patient
	 * @param program
	 *            if supplied all PatientPrograms returned will be for this
	 *            Program
	 * @param minEnrollmentDate
	 *            if supplied will limit PatientPrograms to those with
	 *            enrollments on or after this Date
	 * @param maxEnrollmentDate
	 *            if supplied will limit PatientPrograms to those with
	 *            enrollments on or before this Date
	 * @param minCompletionDate
	 *            if supplied will limit PatientPrograms to those completed on
	 *            or after this Date OR not yet completed
	 * @param maxCompletionDate
	 *            if supplied will limit PatientPrograms to those completed on
	 *            or before this Date
	 * @param includeVoided
	 *            if true, will also include voided PatientPrograms
	 * @return List&lt;PatientProgram&gt; of PatientPrograms that match the
	 *         passed input parameters
	 * @throws APIException
	 * @should return patient programs for given patient
	 * @should return patient programs for given program
	 * @should return patient programs with dateEnrolled on or before
	 *         minEnrollmentDate
	 * @should return patient programs with dateEnrolled on or after
	 *         maxEnrollmentDate
	 * @should return patient programs with dateCompleted on or before
	 *         minCompletionDate
	 * @should return patient programs with dateCompleted on or after
	 *         maxCompletionDate
	 ...
	 * This method delegates to #purgePatientProgram(patientProgram, boolean)
	 * method
	 * 
	 * @param patientProgram
	 ...
	 * @should not cascade delete patient program states when cascade equals
	 *         false
	 */
	@Authorized({PrivilegeConstants.PURGE_PATIENT_PROGRAMS})
	public void purgePatientProgram(PatientProgram patientProgram,
	...
	/**
	 * Get all possible outcome concepts for a program. Will return all concept
	 * answers {@link org.openmrs.Concept#getAnswers()} if they exist, then all
	 * concept set members {@link org.openmrs.Concept#getSetMembers()} if they
	 * exist, then empty List.
	 * 
	 * @param programId
	...
	/**
	 * Save ConceptStateConversion to database (create if new or update if
	 * changed)
	 * 
	 * @param conceptStateConversion
	...
	/**
	 * Returns a conceptStateConversion given that conceptStateConversions
	 * primary key <code>conceptStateConversionId</code> A null value is
	 * returned if no conceptStateConversion exists with this
	 * conceptStateConversionId.
	 * 
	 * @param conceptStateConversionId
	...
	public ConceptStateConversion getConceptStateConversion(
			Integer conceptStateConversionId) throws APIException;
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_PROGRAMS})
	public List<ConceptStateConversion> getAllConceptStateConversions()
			throws APIException;

	/**
	 * Completely remove a conceptStateConversion from the database (not
	 * reversible) This method delegates to
	 * #purgeConceptStateConversion(conceptStateConversion, boolean) method
	 * 
	 * @param conceptStateConversion
	 ...
	 *            <code>true</code> to delete related content
	 * @throws APIException
	 * @should cascade delete given concept state conversion when given cascade
	 *         is true
	 * @should not cascade delete given concept state conversion when given
	 *         cascade is false
	 */
	@Authorized({PrivilegeConstants.MANAGE_PROGRAMS})
	public void purgeConceptStateConversion(
			ConceptStateConversion conceptStateConversion, boolean cascade)
			throws APIException;

	/**
	 * Retrieves the ConceptStateConversion that matches the passed
	 * <code>ProgramWorkflow</code> and <code>Concept</code>
	 * 
	 * @param workflow
	 *            - the ProgramWorkflow to check
	 * @param trigger
	 *            - the Concept to check
	 * @return ConceptStateConversion that matches the passed
	 *         <code>ProgramWorkflow</code> and <code>Concept</code>
	 * @throws APIException
	 * @should return concept state conversion for given workflow and trigger
	 */
	public ConceptStateConversion getConceptStateConversion(
			ProgramWorkflow workflow, Concept trigger) throws APIException;

	/**
	 * Get a state by its uuid. There should be only one of these in the
	 * database. If multiple are found, an error is thrown.
	 * 
	 * @param uuid
	 *            the universally unique identifier
	...
	/**
	 * Get a patient program by its uuid. There should be only one of these in
	 * the database. If multiple are found, an error is thrown.
	 * 
	 * @param uuid
	 *            the universally unique identifier
	 ...
	 * @should throw an error when multiple patient programs with same uuid are
	 *         found
	 */
	 ...
	 * @param cohort
	 * @param programs
	 * @return List&lt;PatientProgram&gt; for all Patients in the given Cohort
	 *         that are in the given programs
	 * @should return patient programs with patients in given cohort and
	 *         programs
	 ...
	 * @should return empty list when there is no match for given cohort and
	 *         programs
	 * @should not return null when there is no match for given cohort and
	 *         program
	 * @should not throw NullPointerException when given cohort and programs are
	 *         null
	...
	/**
	 * Returns a list of ProgramWorkflowStates that are using a particular
	 * concept.
	 * 
	 * @param concept
	 *            - The Concept being used.
	...
	/**
	 * Get a concept state conversion by its uuid. There should be only one of
	 * these in the database. If multiple are found, an error is thrown.
	 * 
	 * @param uuid
	 *            the universally unique identifier
	 ...
	 * @should throw an error when multiple program states with same uuid are
	 *         found
	 */
