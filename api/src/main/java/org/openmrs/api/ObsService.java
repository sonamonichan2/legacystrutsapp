 ...
 * </pre>
 * 
 * There are also a number of convenience methods for extracting obs pertaining
 * to certain Concepts, people, or encounters
 * 
 * @see org.openmrs.Obs
...
public interface ObsService extends OpenmrsService {
	...
	/**
	 * Set the given <code>dao</code> on this obs service. The dao will act as
	 * the conduit through with all obs calls get to the database
	 * 
	 * @param dao
	 *            specific ObsDAO to use for this service
	 */
	 ...
	 * given <code>obs</code> to the database. This acts as both the initial
	 * save and an update kind of save. The returned obs will be the same as the
	 * obs passed in. It is included for chaining. If this is an initial save,
	 * the obsId on the given <code>obs</code> object will be updated to reflect
	 * the auto numbering from the database. The obsId on the returned obs will
	 * also have this number. If there is already an obsId on the given
	 * <code>obs</code> object, the given obs will be voided and a new row in
	 * the database will be created that has a new obs id.
	 * 
	 * @param obs
	 *            the Obs to save to the database
	 * @param changeMessage
	 *            String explaining why <code>obs</code> is being changed. If
	 *            <code>obs</code> is a new obs, changeMessage is nullable, or
	 *            if it is being updated, it would be required
	 ...
	 * (see #voidObs(Obs) for that one) If other things link to this obs, an
	...
	/**
	 * Get all Observations for the given person, sorted by obsDatetime
	 * ascending. Does not return voided observations.
	 * 
	 * @param who
	 *            the user to match on
	 * @return a List&lt;Obs&gt; object containing all non-voided observations
	 ...
	 * 
	 * @param whom
	 *            List&lt;Person&gt; to restrict obs to (optional)
	 * @param encounters
	 *            List&lt;Encounter&gt; to restrict obs to (optional)
	 * @param questions
	 *            List&lt;Concept&gt; to restrict the obs to (optional)
	 * @param answers
	 *            List&lt;Concept&gt; to restrict the valueCoded to (optional)
	 * @param personTypes
	 *            List&lt;PERSON_TYPE&gt; objects to restrict this to. Only used
	 *            if <code>whom</code> is an empty list (optional)
	 * @param locations
	 *            The org.openmrs.Location objects to restrict to (optional)
	 * @param sort
	 ...
	 * @param toDate
	 *            the latest Obs date to get (optional)
	...
	public List<Obs> getObservations(List<Person> whom,
			...
			List<Location> locations, List<String> sort, Integer mostRecentN,
	 ...
	 * 
	 *      This method works exactly the same; it only adds accession number
	 *      search criteria. It effectively surpasses the above method; the old
	 *      one is however kept for backward compatibility reasons.
	 * 
	 * @param whom
	 *            List&lt;Person&gt; to restrict obs to (optional)
	 * @param encounters
	...
	/**
	 * This method fetches the count of observations according to the criteria
	 * in the given arguments. All arguments are optional and nullable. If more
	 * than one argument is non-null, the result is equivalent to an "and"ing of
	 * the arguments. (e.g. if both a <code>location</code> and a
	 * <code>fromDate</code> are given, only Obs that are <u>both</u> at that
	 * Location and after the fromDate are returned). <br>
	 * <br>
	 * Note: If <code>whom</code> has elements, <code>personType</code> is
	 ...
	 */
	@Authorized(PrivilegeConstants.GET_OBS)
	public Integer getObservationCount(List<Person> whom,
			List<Encounter> encounters, List<Concept> questions,
			List<Concept> answers, List<PERSON_TYPE> personTypes,
			List<Location> locations, Integer obsGroupId, Date fromDate,
	 ...
	 *      java.util.List, java.util.List, java.util.List, java.util.List,
	 *      java.util.List, java.lang.Integer, java.util.Date, java.util.Date,
	 *      boolean)
	 * 
	 *      This method works exactly the same; it only adds accession number
	 *      search criteria. It effectively surpasses the above method; the old
	 ...
	 * @should get the count of all obs with question concept in given questions
	 *         parameter
	 * @should get the count of all obs with answer concept in given answers
	 *         parameter
	 ...
	 * @should return the count of obs with location in given locations
	 *         parameter
	...
	public Integer getObservationCount(List<Person> whom,
			...
			List<Concept> answers, List<PERSON_TYPE> personTypes,
			List<Location> locations, Integer obsGroupId, Date fromDate,
			Date toDate, boolean includeVoidedObs, String accessionNumber)
			throws APIException;
	 ...
	 */
	@Authorized(PrivilegeConstants.GET_OBS)
	public List<Obs> getObservationsByPersonAndConcept(Person who,
			Concept question) throws APIException;

	/**
	 * Get a complex observation. If obs.isComplex() is true, then returns an
	 * Obs with its ComplexData. Otherwise returns a simple Obs. TODO: Possibly
	 * remove this method. It is confusing because you may have a complexObs,
	 * but not want the complexData attached at the time. Nevertheless, you may
	 * think that this method is necessary to use, when you could just call
	 * getObs(). This will attach the complexData onto the obs.
	 * 
	 * @param obsId
	 ...
	 * 
	 * @param key
	 *            that has been registered with a handler class
	...
	/**
	 * Get the ComplexObsHandler associated with a complex observation Returns
	 * the ComplexObsHandler. Returns null if the Obs.isComplexObs() is false or
	 * there is an error instantiating the handler class.
	 * 
	 * @param obs
	 *            A complex Obs.
	...
	/**
	 * <u>Add</u> the given map to this service's handlers. This method
	 * registers each ComplexObsHandler to this service. If the given String key
	 * exists, that handler is overwritten with the given handler For most
	 * situations, this map is set via spring, see the
	 * applicationContext-service.xml file to add more handlers.
	 * 
	 * @param handlers
	 *            Map of class to handler object
	 * @throws APIException
	 * @since 1.5
	 * @should override handlers with same key
	 * @should add new handlers with new keys
	 */
	public void setHandlers(Map<String, ComplexObsHandler> handlers)
			throws APIException;
	...
	/**
	 * Registers the given handler with the given key If the given String key
	 * exists, that handler is overwritten with the given handler
	 * 
	 * @param key
	 *            the key name to use for this handler
	 * @param handler
	 *            the class to register with this key
	 * @throws APIException
	 * @since 1.5
	...
	/**
	 * Remove the handler associated with the key from list of available
	 * handlers
	 * 
	 * @param key
	 *            the key of the handler to unregister
	 * @since 1.5
	...
	/**
	 * Gets the number of observations(including voided ones) that are using the
	 * specified conceptNames as valueCodedName answers
	 * 
	 * @param conceptNames
	 *            the conceptNames to be searched against
	 * @param includeVoided
	 *            whether voided observation should be included
	 * @return The number of observations using the specified conceptNames as
	 *         valueCodedNames
	 * @should return the count of all observations using the specified
	 *         conceptNames as answers
	 * @should include voided observations using the specified conceptNames as
	 *         answers
	 * @should return zero if no observation is using any of the concepNames in
	 *         the list
	 * @since Version 1.7
	 */
	@Authorized(PrivilegeConstants.GET_OBS)
	public Integer getObservationCount(List<ConceptName> conceptNames,
