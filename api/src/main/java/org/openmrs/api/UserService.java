...
public interface UserService extends OpenmrsService {
	 ...
	 * @throws APIException
	 *             for not existing user and if old password is weak
	 * @since 1.12
	 ...
	 * @should change password for given user if oldPassword is null and
	 *         changing user have privileges
	 * @should throw exception if oldPassword is null and changing user have not
	 *         privileges
	...
	/**
	 * true/false if username or systemId is already in db in username or
	 * system_id columns
	 * 
	 * @param user
	 *            User to compare
	 ...
	 * method delegates to #purgeLocation(location, boolean) method.
	 * 
	 * @param user
	...
	/**
	 * Completely remove a user from the database (not reversible). This is a
	 * delete from the database. This is included for troubleshooting and
	 * low-level system administration. Ideally, this method should <b>never</b>
	 * be called &mdash; <code>Users</code> should be <em>voided</em> and not
	 * <em>deleted</em> altogether (since many foreign key constraints depend on
	 * users, deleting a user would require deleting all traces, and any
	 * historical trail would be lost). This method only clears user roles and
	 * attempts to delete the user record. If the user has been included in any
	 * other parts of the database (through a foreign key), the attempt to
	 * delete the user will violate foreign key constraints and fail.
	 * 
	 * @param cascade
	 ...
	 *            new password to set
	 * @throws APIException
	 * @should update password of given user when logged in user has edit users
	 *         password privilege
	 * @should not update password of given user when logged in user does not
	 *         have edit users password privilege
	 */
	@Authorized({PrivilegeConstants.EDIT_USER_PASSWORDS})
	...
	/**
	 * Changes the current user's password directly. This is most useful if
	 * migrating users from other systems and you want to retain the existing
	 * passwords. This method will simply save the passed hashed password and
	 * salt directly to the database.
	 * 
	 * @param user
	 *            the user whose password you want to change
	 * @param hashedPassword
	 *            - the <em>already hashed</em> password to store
	 * @param salt
	 ...
	 */
	@Authorized({PrivilegeConstants.EDIT_USER_PASSWORDS})
	public void changeHashedPassword(User user, String hashedPassword,
			String salt) throws APIException;
	...
	@Authorized({PrivilegeConstants.EDIT_USER_PASSWORDS})
	@Logging(ignoredArgumentIndexes = {1, 2})
	public void changeQuestionAnswer(User u, String question, String answer)
			throws APIException;
	...
	/**
	 * Compares <code>answer</code> against the <code>user</code>'s secret
	 * answer.
	 * 
	 * @param u
	 *            user
	 ...
	 * @should return false when given answer does not match the stored secret
	 *         answer
	 */
	@Logging(ignoredArgumentIndexes = {1})
	 ...
	 * {@link PersonByNameComparator}) if any part of the search matches
	 * first/last/system id and the user has one at least one of the given
	 * <code>roles</code> assigned to them
	 * 
	 * @param nameSearch
	 *            string to compare to the beginning of user's
	 *            given/middle/family/family2 names
	 * @param roles
	 ...
	 * @param person
	 * @param includeRetired
	 * @return all user accounts that belong to person, including retired ones
	 *         if specified
	 * @throws APIException
	...
	/**
	 * Adds the <code>key</code>/<code>value</code> pair to the given
	 * <code>user</code>.
	 * <p>
	 ...
	 * @should add property with given key and value when key does not already
	 *         exist
	 ...
	 * <code>user</code>'s properties. <b>Implementations of this method should
	 * handle privileges</b>
	 * 
	 * @param user
	 * @param key
	...
	/**
	 * Get/generate/find the next system id to be doled out. Assume check digit
	 * /not/ applied in this method
	 * 
	 ...
	 * {@link PersonByNameComparator}) if any part of the search matches
	 * first/last/system id and the user has one at least one of the given
	 * <code>roles</code> assigned to them. If start and length are not
	 * specified, then all matches are returned, If name is empty or null, then
	 * all all users will be returned taking into consideration the values of
	 ...
	 * @param length
	 *            number of users to return in the batch
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_USERS})
	public List<User> getUsers(String name, List<Role> roles,
			boolean includeRetired, Integer start, Integer length)
			throws APIException;

	/**
	 * Return the number of users with a matching name or system id and have at
	 * least one of the given roles assigned to them
	 * 
	 * @param name
	 *            patient name
	 * @param roles
	 *            all the Roles the user must contain
	 * @param includeRetired
	 *            Specifies whether voided users should be included
	 * @return the number of users matching the given attributes
	 * @since 1.8
	 */
	@Authorized({PrivilegeConstants.GET_USERS})
	public Integer getCountOfUsers(String name, List<Role> roles,
	 ...
	 * @param user
	 *            the authenticated user or <code>null</code> if not
	 *            authenticated
	 * @param privilege
	 *            the checked privilege
	 * @param hasPrivilege
	 *            <code>true</code> if the authenticated user has the required
	 *            privilege or if it is a proxy privilege
	 * @since 1.8.4, 1.9.1, 1.10
	 */
	public void notifyPrivilegeListeners(User user, String privilege,
	 ...
	 * 
	 * @param key
	 *            the authenticated user's property
	 * @param value
	...
	/**
	 * Replaces all user properties with the given map of properties for the
	 * current user
	 * 
	 * @param properties
	 ...
	 */
	@Authorized
	public void changePasswordUsingSecretAnswer(String secretAnswer, String pw)
			throws APIException;
