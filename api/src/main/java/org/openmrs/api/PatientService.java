...
public interface PatientService extends OpenmrsService {
	 ...
	 * @should not throw a NonUniqueObjectException when called with a hand
	 *         constructed patient regression 1375
	 ...
	 * @should update the date changed and changed by on update of the person
	 *         address
	 * @should set the preferred name address and identifier if none is
	 *         specified
	 * @should not set the preferred name address and identifier if they already
	 *         exist
	 ...
	 * then instantiates a new patient from that person, copying over all the
	 * fields.
	 * 
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_PATIENTS})
	Patient getPatientOrPromotePerson(Integer patientOrPersonId)
			throws APIException;
	...
	/**
	 * Get patients based on given criteria The identifier is matched with the
	 * regex <code>OpenmrsConstants.PATIENT_IDENTIFIER_REGEX</code> All
	 * parameters are optional and nullable. If null, it is not included in the
	 * search. Will not return voided patients
	 * 
	 * @param name
	 *            (optional) this is a slight break from the norm, patients with
	 *            a partial match on this name will be returned
	 * @param identifier
	 *            (optional) only patients with a matching identifier are
	 *            returned
	 * @param identifierTypes
	 *            (optional) the PatientIdentifierTypes to restrict to
	 * @param matchIdentifierExactly
	 *            (required) if true, then the given <code>identifier</code>
	 *            must equal the id in the database. if false, then the
	 *            identifier is 'searched' for by using a regular expression
	 ...
	 * @should fetch all patients that partially match given identifier when
	 *         match identifier exactly equals false
	 * @should fetch all patients that exactly match given identifier when match
	 *         identifier exactly equals true
	...
	/**
	 * Void patient record (functionally delete patient from system). Voids
	 * Person and retires Users.
	 * 
	 * @param patient
	 *            patient to be voided
	 * @param reason
	...
	public Patient voidPatient(Patient patient, String reason)
			throws APIException;
	...
	/**
	 * Delete patient from database. This <b>should not be called</b> except for
	 * testing and administration purposes. Use the void method instead.
	 * 
	 * @param patient
	...
	/**
	 * Get all patientIdentifiers that match all of the given criteria Voided
	 * identifiers are not returned
	 * 
	 * @param identifier
	 *            the full identifier to match on
	 * @param patientIdentifierTypes
	 *            the type of identifiers to get
	 * @param locations
	 *            the locations of the identifiers to match
	 * @param patients
	 *            the patients containing these identifiers
	 * @param isPreferred
	 *            if true, limits to only preferred identifiers if false, only
	 *            non preferred. if null, ignores preferred status
	 ...
	 * @should not fetch patient identifiers that partially matches given
	 *         identifier
	 * @should fetch patient identifiers that match given patient identifier
	 *         types
	 * @should fetch patient identifiers that match given locations
	 * @should fetch patient identifiers that match given patients
	 * @should fetch preferred patient identifiers when given is preferred
	 *         equals true
	 * @should fetch non preferred patient identifiers when given is preferred
	 *         equals false
	 * @should fetch preferred and non preferred patient identifiers when given
	 *         is preferred is null
	 */
	@Authorized({PrivilegeConstants.GET_PATIENT_IDENTIFIERS})
	 ...
	 * @should throw error when trying to save a patient identifier type while
	 *         patient identifier types are locked
	 */
	 ...
	 * @return patientIdentifier types list
	 * @throws APIException
	 * @should fetch patient identifier types including retired when include
	 *         retired is true
	 * @should fetch patient identifier types excluding retired when include
	 *         retired is false
	 * @should order as default comparator
	 */
	@Authorized({PrivilegeConstants.GET_IDENTIFIER_TYPES})
	 ...
	 * 
	 * @param name
	 *            name of the type to match on
	 * @param format
	 *            the string format to match on
	 * @param required
	 *            if true, limits to only identifiers marked as required if
	 *            false, only non required. if null, ignores required bit
	 * @param hasCheckDigit
	 *            if true, limits to only check digit'd identifiers if false,
	 *            only non checkdigit'd. if null, ignores checkDigit
	 * @return patientIdentifier types list
	 * @throws APIException
	 * @should fetch patient identifier types that match given name with given
	 *         format
	 * @should fetch required patient identifier types when given required is
	 *         true
	 * @should fetch non required patient identifier types when given required
	 *         is false
	 * @should fetch any patient identifier types when given required is null
	 * @should fetch patient identifier types with check digit when given has
	 *         check digit is true
	 * @should fetch patient identifier types without check digit when given has
	 *         check digit is false
	 * @should fetch any patient identifier types when given has check digit is
	 *         null
	 * @should order as default comparator
	 */
	@Authorized({PrivilegeConstants.GET_IDENTIFIER_TYPES})
	 ...
	 * @should return null when patient identifier type with given uuid does not
	 *         exist
	 */
	@Authorized({PrivilegeConstants.GET_IDENTIFIER_TYPES})
	 ...
	 * @should not return patient identifier type that partially matches given
	 *         name
	 * @should return null when patient identifier type with given name does not
	 *         exist
	 */
	@Authorized({PrivilegeConstants.GET_IDENTIFIER_TYPES})
	 ...
	 * @should throw error when trying to retire a patient identifier type while
	 *         patient identifier types are locked
	 */
	@Authorized({PrivilegeConstants.MANAGE_IDENTIFIER_TYPES})
	public PatientIdentifierType retirePatientIdentifierType(
			PatientIdentifierType patientIdentifierType, String reason)
			throws APIException;
	 ...
	 * @should throw error when trying to unretire a patient identifier type
	 *         while patient identifier types are locked
	 */
	@Authorized({PrivilegeConstants.MANAGE_IDENTIFIER_TYPES})
	 ...
	 * @should throw error when trying to delete a patient identifier type while
	 *         patient identifier types are locked
	 */
	 ...
	 * @should validate when patient has all required and no duplicate and no
	 *         blank patient identifiers
	 * @should ignore voided patient identifier
	 * @should remove identifier and throw error when patient has blank patient
	 *         identifier
	 ...
	 * @should throw error when patient does not have one or more required
	 *         identifiers
	...
	/**
	 * Generic search on patients based on the given string. Implementations can
	 * use this string to search on name, identifier, etc Voided patients are
	 * not returned in search results
	 * 
	 * @param query
	 *            the string to search on
	 * @return a list of matching Patients
	 * @should force search string to be greater than minsearchcharacters global
	 *         property
	 * @should allow search string to be one according to minsearchcharacters
	 *         global property
	 ...
	 * @should return empty list if given query length less than minimum search
	 *         characters
	...
	/**
	 * Generic search on patients based on the given string and returns a
	 * specific number of them from the specified starting position.
	 * Implementations can use this string to search on name, identifier,
	 * searchable person attributes etc. Voided patients are not returned in
	 * search results. If start is 0 and length is not specified, then all
	 ...
	 * on the given <code>patientToMatch</code> object. Assumes there could be a
	 * PersonAttribute on this Patient with PersonAttributeType.name =
	 * "Other Matching Information". This PersonAttribute has a "value" that is
	 * just key value pairs in the form of key:value;nextkey:nextvalue;
	 * 
	 ...
	 * Each attribute that is passed in must be identical to what is stored for
	 * at least one other patient for both patients to be returned.
	 * 
	 * @param attributes
	 *            attributes on a Person or Patient object. similar to: [gender,
	 *            givenName, middleName, familyName]
	 * @return list of patients that match other patients
	 * @throws APIException
	 * @should fetch patients that exactly match on all given attributes
	 * @should not return patients that exactly match on some but not all given
	 *         attributes
	 */
	@Authorized({PrivilegeConstants.GET_PATIENTS})
	 ...
	 * <code>nonPreferred</code> to <code>preferred</code> IFF the data is
	 * missing or null in <code>preferred</code></li>
	 * <li><code>notPreferred</code> is marked as voided</li>
	 * </ol>
	 * 
	 * @param preferred
	 *            The Patient to merge to
	 * @param notPreferred
	 ...
	 * @should not copy over relationships that are only between the preferred
	 *         and notpreferred patient
	 ...
	 * @should merge non duplicate patient identifiers from non preferred to
	 *         preferred patient
	 * @should merge non duplicate patient names from non preferred to preferred
	 *         patient
	 * @should merge non duplicate addresses from non preferred to preferred
	 *         patient
	 * @should merge non voided patient programs from non preferred to preferred
	 *         patient
	 * @should merge non voided relationships from non preferred to preferred
	 *         patient
	 * @should merge observations associated with encounters from non preferred
	 *         to preferred patient
	 * @should merge non voided person attributes from non preferred to
	 *         preferred patient
	 * @should merge other non voided observations from non preferred to
	 *         preferred patient
	 * @should merge other non voided orders from non preferred to preferred
	 *         patient
	 * @should merge non preferred death date when preferred death date is not
	 *         null or empty
	 * @should merge non preferred death cause when preferred death cause is not
	 *         null or empty
	 ...
	 * @should not void relationships for same type and side with different
	 *         relatives
	...
	/**
	 * Convenience method to join multiple patients' information into one
	 * record.
	 * 
	 * @param preferred
	 * @param notPreferred
	 * @throws APIException
	 * @throws SerializationException
	 * @should merge all non Preferred patients in the the notPreferred list to
	 *         preferred patient
	 */
	 ...
	 * they died, etc. TODO Keep this in the PatientService? Or move to
	 * appropriate service
	 * 
	 * @param patient
	 ...
	 * @param causeOfDeath
	 *            - the concept that corresponds with the reason the patient
	 *            died
	 * @param otherReason
	 *            - if the concept representing the reason is OTHER NON-CODED,
	 *            and a string-based "other" reason is supplied
	 * @throws APIException
	 ...
	 * patient died (including any "other" reason there might be) TODO keep this
	 ...
	 * @should throw error when patient already has more than one cause of death
	 *         observations
	 * @should modify existing cause of death observation
	 * @should set death attributes as long as patient is not already dead
	 * @should be tested more thoroughly
	 */
	@Authorized(value = {PrivilegeConstants.GET_PATIENTS,
			PrivilegeConstants.EDIT_OBS}, requireAll = true)
	...
	public IdentifierValidator getIdentifierValidator(
			Class<IdentifierValidator> clazz);
	...
	/**
	 * Checks whether the given patient identifier is already assigned to a
	 * patient other than patientIdentifier.patient
	 * 
	 ...
	 * @should return true when patientIdentifier contains a patient and another
	 *         patient has this id
	 * @should return false when patientIdentifier contains a patient and no
	 *         other patient has this id
	 * @should return true when patientIdentifier does not contain a patient and
	 *         a patient has this id
	 * @should return false when patientIdentifier does not contain a patient
	 *         and no patient has this id
	 * @should ignore voided patientIdentifiers
	 * @should ignore voided patients
	 * @should return true if in use for a location and id type uniqueness is
	 *         set to location
	 * @should return false if in use for another location and id uniqueness is
	 *         set to location
	 ...
	 */
	@Authorized(PrivilegeConstants.GET_PATIENTS)
	public boolean isIdentifierInUseByAnotherPatient(
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_PATIENT_IDENTIFIERS})
	public PatientIdentifier getPatientIdentifier(Integer patientIdentifierId)
			throws APIException;

	/**
	 * Void patient identifier (functionally delete patient identifier from
	 * system)
	 * 
	 * @param patientIdentifier
	 *            patientIdentifier to be voided
	 * @param reason
	...
	public PatientIdentifier voidPatientIdentifier(
			PatientIdentifier patientIdentifier, String reason)
			throws APIException;
	 ...
	 * @should throw an APIException if the patientIdentifier string is a white
	 *         space
	 * @should throw an APIException if the patientIdentifier string is an empty
	 *         string
	 */
	@Authorized({PrivilegeConstants.ADD_PATIENT_IDENTIFIERS,
			PrivilegeConstants.EDIT_PATIENT_IDENTIFIERS})
	public PatientIdentifier savePatientIdentifier(
			PatientIdentifier patientIdentifier) throws APIException;
	 ...
	 * @should void removed allergies and maintain status as see list if some
	 *         allergies are removed
	 * @should void all allergies and set status to unknown if all allergies are
	 *         removed
	 * @should set status to no known allergies for patient without allergies
	 * @should void all allergies and set status to no known allergies if all
	 *         allergies are removed and status set as such
	 ...
	 * 
	 * @param allergyListId
	 *            identifies allergy by internal Ingerger Id
	 * @return the allergy
	 * @throws APIException
	 */
	...
	/**
	 * Creates an AllergyListItem to the Patient's Allergy Active List. Sets the
	 * start date to now, if it is null.
	 * 
	 * @param allergy
	 *            the Allergy
	 * @throws APIException
	 ...
	 */
	@Authorized({PrivilegeConstants.ADD_ALLERGIES,
			PrivilegeConstants.EDIT_ALLERGIES})
	public void saveAllergy(Allergy allergy) throws APIException;

	/**
	 * Resolving the allergy, effectively removes the Allergy from the Patient's
	 * Active List by setting the stop date to now, if null.
	 * 
	 * @param allergy
	 *            the Allergy
	 * @param reason
	...
	public void removeAllergy(Allergy allergy, String reason)
			throws APIException;
	...
	/**
	 * Return the number of unvoided patients with names or patient identifiers
	 * or searchable person attributes starting with or equal to the specified
	 * text
	 * 
	 * @param query
	 *            the string to search on
	 ...
	 * @since 1.8
	 * @should return the right count when a patient has multiple matching
	 *         person names
	 * @should return the right count of patients with a matching identifier
	 *         with no digits
	 */
	@Authorized({PrivilegeConstants.GET_PATIENTS})
	...
	/**
	 * Get a limited size of patients from a given start index based on given
	 * criteria The identifier is matched with the regex
	 * <code>OpenmrsConstants.PATIENT_IDENTIFIER_REGEX</code> All parameters are
	 * optional and nullable. If null, it is not included in the search. Will
	 * not return voided patients
	 * 
	 * @param name
	 *            (optional) this is a slight break from the norm, patients with
	 *            a partial match on this name will be returned
	...
	/**
	 * Check if patient identifier types are locked, and if they are, throws an
	 * exception during manipulation of a patient identifier type
	 * 
