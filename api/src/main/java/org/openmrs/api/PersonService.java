...
public interface PersonService extends OpenmrsService {
	...
	/**
	 * These enumerations are used when determining which person attr types to
	 * display. If listing off a lot of patients/users, one set of types are
	 * shown. When only displaying one patient/user, another type is shown.
	 */
	...
	/**
	 * Find a similar person given the attributes. This does a very loose lookup
	 * with the <code>nameSearch</code> parameter. This does a very loose lookup
	 * on <code>birthyear</code> as well. Any person with a null/missing
	 * birthdate is included and anyone with a birthyear plus/minus one year
	 * from the given <code>birthyear</code> is also included
	 * 
	 * @param nameSearch
	 *            string to search the person's name for
	 * @param birthyear
	...
	public Set<Person> getSimilarPeople(String nameSearch, Integer birthyear,
	 ...
	 * 
	 * @param searchPhrase
	 *            person name to match on
	 * @param dead
	 *            if true will return only dead patients, if false will return
	 *            only alive patients, if null will return both
	...
	@Authorized({PrivilegeConstants.GET_PERSONS})
	public List<Person> getPeople(String searchPhrase, Boolean dead,
			Boolean voided) throws APIException;

	/**
	 * Save the given person attribute type in the database. <br>
	 * If the given type's Id is not empty, then also need to change any global
	 * property which is in
	 * {@link OpenmrsConstants#GLOBAL_PROPERTIES_OF_PERSON_ATTRIBUTES} and
	 * reference this given type, prior to saving this given type. <br>
	 * 
	 * @param type
	 ...
	 * @should throw an error when trying to save person attribute type while
	 ...
	 * @param type
	 * @param retiredReason
	 * @should throw an error when trying to retire person attribute type while
	 *         person attribute types are locked
	 */
	@Authorized({PrivilegeConstants.MANAGE_PERSON_ATTRIBUTE_TYPES})
	...
	public RelationshipType retireRelationshipType(RelationshipType type,
			String retiredReason) throws APIException;
	 ...
	 */
	@Authorized({PrivilegeConstants.MANAGE_RELATIONSHIP_TYPES})
	public RelationshipType unretireRelationshipType(
	 ...
	 * @should throw an error when trying to delete person attribute type while
	 *         person attribute types are locked
	 */
	 ...
	 * 
	 * @param type
	 *            type to be restored from the database
	 * @throws APIException
	 * @should restore person attribute type from database
	 * @should throw an error when trying to unretire person attribute type
	 *         while person attribute types are locked
	 */

	@Authorized({PrivilegeConstants.MANAGE_PERSON_ATTRIBUTE_TYPES})
	public void unretirePersonAttributeType(PersonAttributeType type)
			throws APIException;

	/**
	 * Effectively removes this person from the system. Voids Patient and
	 * retires Users as well.
	 * 
	 * @param person
	 *            person to be voided
	 * @param reason
	...
	/**
	 * Get all PersonAttributeTypes in the database with the option of including
	 * the retired types
	 * 
	 * @param includeRetired
	 *            boolean - include retired attribute types as well?
	 * @return List&lt;PersonAttributeType&gt; object of all
	 *         PersonAttributeTypes, possibly including retired ones
	 * @should return all person attribute types including retired when include
	 *         retired is true
	 * @should return all person attribute types excluding retired when include
	 *         retired is false
	 */
	@Authorized({PrivilegeConstants.GET_PERSON_ATTRIBUTE_TYPES})
	public List<PersonAttributeType> getAllPersonAttributeTypes(
	...
	/**
	 * Find person attribute types matching the given parameters. Retired types
	 * are included in the results
	 * 
	 * @param exactName
	 *            (optional) The name of type
	 * @param format
	 *            (optional) The format for this type
	 * @param foreignKey
	 *            (optional) The foreign key
	 * @param searchable
	 *            (optional) if true only returns searchable types, if false
	 *            returns only nonsearchable and if null returns all
	 ...
	 * @should return empty list when no person attribute types match given
	 *         parameters
	 */
	@Authorized({PrivilegeConstants.GET_PERSON_ATTRIBUTE_TYPES})
	 ...
	 * @should return person attribute when PersonAttribute with given id does
	 *         exist
	 */
	@Authorized({PrivilegeConstants.GET_PERSON_ATTRIBUTE_TYPES})
	 ...
	 * 
	 * @param typeName
	 * @return the PersonAttributeType that has the given name or null if none
	 *         found
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_PERSON_ATTRIBUTE_TYPES})
	public PersonAttributeType getPersonAttributeTypeByName(String typeName)
			throws APIException;
	 ...
	 * @return non-voided Relationship list
	 * @throws APIException
	 * @should return all relationship including voided when include voided
	 *         equals true
	 * @should return all relationship excluding voided when include voided
	 *         equals false
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIPS})
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIPS})
	public List<Relationship> getRelationshipsByPerson(Person p)
			throws APIException;

	/**
	 * Get list of relationships that include Person in person_id or
	 * relative_id. Does not include voided relationships. Accepts an
	 * effectiveDate parameter which, if supplied, will limit the returned
	 * relationships to those that were active on the given date. Such active
	 * relationships include those that have a startDate that is null or less
	 * than or equal to the effectiveDate, and that have an endDate that is null
	 * or greater than or equal to the effectiveDate.
	 * 
	 * @param p
	 *            person object listed on either side of the relationship
	 * @param effectiveDate
	 ...
	 * 
	 * @param fromPerson
	 *            (optional) Person to in the person_id column
	 * @param toPerson
	 *            (optional) Person in the relative_id column
	 * @param relType
	 ...
	 * @should return empty list when no relationship matching given parameters
	 *         exist
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIPS})
	public List<Relationship> getRelationships(Person fromPerson,
			Person toPerson, RelationshipType relType) throws APIException;

	/**
	 * Get relationships stored in the database that are active on the passed
	 * date
	 * 
	 * @param fromPerson
	 *            (optional) Person to in the person_id column
	 ...
	 *            (optional) The RelationshipType to match
	 * @param effectiveDate
	 *            (optional) The date during which the relationship was
	 *            effective
	 * @return relationships matching the given parameters
	 * @throws APIException
	 * @should fetch relationships matching the given from person
	...
	@Authorized({PrivilegeConstants.GET_RELATIONSHIPS})
	public List<Relationship> getRelationships(Person fromPerson,
			Person toPerson, RelationshipType relType, Date effectiveDate)
			throws APIException;

	/**
	 * Get relationships stored in the database that were active during the
	 ...
	 * @should return empty list when no relationship matching given parameters
	 *         exist
	 * @should fetch relationships that were active during the specified date
	 *         range
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIPS})
	public List<Relationship> getRelationships(Person fromPerson,
			Person toPerson, RelationshipType relType, Date startEffectiveDate,
			Date endEffectiveDate) throws APIException;
	 ...
	 * @return relationshipType with given internal identifier or null if none
	 *         found
	 * @throws APIException
	 * @should return relationship type with the given relationship type id
	 * @should return null when no relationship type matches given relationship
	 *         type id
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIP_TYPES})
	 ...
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIP_TYPES})
	public RelationshipType getRelationshipTypeByName(
			String relationshipTypeName) throws APIException;
	 ...
	 *            name to match on
	 * @param preferred
	 *            if true, returns on preferred types, if false returns only the
	 *            nonpreferred types. if null returns both
	 ...
	 * @should return empty list when no preferred relationship type match the
	 *         given name
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIP_TYPES})
	public List<RelationshipType> getRelationshipTypes(
			String relationshipTypeName, Boolean preferred) throws APIException;

	/**
	 * Get relationshipTypes by searching through the names and loosely matching
	 ...
	 * @throws APIException
	 * @should return empty list when no relationship type match the search
	 *         string
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIP_TYPES})
	public List<RelationshipType> getRelationshipTypes(String searchString)
			throws APIException;

	/**
	 * Create or update a relationship between people. Saves the given
	 * <code>relationship</code> to the database
	 * 
	 * @param relationship
	...
	public Relationship voidRelationship(Relationship relationship,
			String voidReason) throws APIException;

	/**
	 * Unvoid Relationship in the database, effectively marking this as a valid
	 * relationship again
	 * 
	 * @param relationship
	 ...
	 */
	@Authorized({PrivilegeConstants.MANAGE_RELATIONSHIP_TYPES})
	public RelationshipType saveRelationshipType(
	...
	/**
	 * Gets the types defined for the given person type (person, user, patient)
	 * and the given type of view (one person vs many person objects)
	 * 
	 * @param personType
	 *            PERSON, USER, PATIENT, or null. Both PERSON and null mean to
	 *            return attr types for both patients and users
	 * @param viewType
	 *            whether this is a listing or viewing or null for both listing
	 *            and viewing
	...
	public List<PersonAttributeType> getPersonAttributeTypes(
			PERSON_TYPE personType, ATTR_VIEW_TYPE viewType)
			throws APIException;

	/**
	 * Voids the given PersonName, effectively deleting the name, from the
	...
	/**
	 * Unvoid PersonName in the database, effectively marking this as a valid
	 * personName again
	 * 
	 * @param personName
	 *            PersonName to unvoid
	 * @return the newly unvoided personName
	 * @throws APIException
	 * @should unvoid voided personName
	 */
	@Authorized({PrivilegeConstants.EDIT_PERSONS})
	public PersonName unvoidPersonName(PersonName personName)
			throws APIException;
	...
	/**
	 * Parses a name into a PersonName (separate Given, Middle, and Family
	 * names)
	 * 
	 * @param name
	...
	/**
	 * Get all relationships for a given type of relationship mapped from the
	 * personA to all of the personB's
	 * 
	 * @param relationshipType
	 *            type of relationship for which to retrieve all relationships
	 * @return all relationships for the given type of relationship
	 * @throws APIException
	 * @should return empty map when no relationship has the matching
	 *         relationship type
	 */
	@Authorized({PrivilegeConstants.GET_RELATIONSHIPS})
	public Map<Person, List<Person>> getRelationshipMap(
			RelationshipType relationshipType) throws APIException;

	/**
	 * Builds the serialized data from
	 * {@link org.openmrs.person.PersonMergeLog#getPersonMergeLogData}, sets the
	 * mergedData String, and the creator and date if null. It then saves the
	 * <code>PersonMergeLog</code> object to the model.
	 * 
	 * @param personMergeLog
	 *            the <code>PersonMergeLog</code> object to save.
	 ...
	 */
	public PersonMergeLog getPersonMergeLogByUuid(String uuid,
			boolean deserialize) throws SerializationException, APIException;
	...
	/**
	 * Gets <code>PersonMergeLog</code> objects by winning person p. Useful for
	 * to getting all persons merged into p.
	 * 
	 * @param person
	 *            the winning person
	 ...
	 */
	public List<PersonMergeLog> getWinningPersonMergeLogs(Person person,
			boolean deserialize) throws SerializationException;

	/**
	 * Gets the <code>PersonMergeLog</code> where person p is the loser. Useful
	 * for getting the person that p was merged into.
	 * 
	 * @param person
	 *            the losing person
	 ...
	 */
	public PersonMergeLog getLosingPersonMergeLog(Person person,
			boolean deserialize) throws SerializationException;

	/**
	 * Voids the given PersonAddress, effectively deleting the personAddress,
	 * from the end-user's point of view.
	 * 
	 * @param personAddress
	 *            PersonAddress to void
	 * @param voidReason
	 *            String reason the personAddress is being voided.
	 * @return the newly saved personAddress
	 * @throws APIException
	 * @should void personAddress with the given reason
	 */
	@Authorized({PrivilegeConstants.EDIT_PERSONS})
	public PersonAddress voidPersonAddress(PersonAddress personAddress,
			String voidReason);

	/**
	 * Unvoid PersonAddress in the database, effectively marking this as a valid
	 * PersonAddress again
	 * 
	 * @param personAddress
	 ...
	 */
	@Authorized({PrivilegeConstants.EDIT_PERSONS})
	public PersonAddress unvoidPersonAddress(PersonAddress personAddress)
			throws APIException;
	...
	/**
	 * Check if the person attribute types are locked, and if they are throws an
	 * exception during manipulation of a person attribute type
	 * 
