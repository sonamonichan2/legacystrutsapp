...
/**
 * Represents an OpenMRS <code>Context</code>, which may be used to authenticate
 * to the database and obtain services in order to interact with the system.<br>
 * <br>
 * The Context is split into a {@link UserContext} and {@link ServiceContext}.
 ...
 * instance. <br>
 * <br>
 * Both the {@link UserContext} and the {@link ServiceContext} should not be
 ...
 * <ol>
 * <li>Call {@link Context#startup(String, String, String, Properties)} to let
 * the Context contact the database</li>
 * <li>Call {@link Context#openSession()} to start a "unit of work".</li>
 * <li>Call {@link Context#authenticate(String, String)} to authenticate the
 * current user on the current thread</li>
 * <li>Call {@link Context#closeSession()} to end your "unit of work" and commit
 * all changes to the database.</li>
 * </ol>
 * <br>
 * Example usage:
 * 
...
public class Context {
	...
	/**
	 * Loads a class with an instance of the OpenmrsClassLoader. Convenience
	 ...
	 * perform authentication/authorization checks.<br>
	 * <br>
	 * This is thread safe since it stores the given user context in
	 * ThreadLocal.
	 * 
	 * @param ctx
	 *            UserContext to set
	...
	/**
	 * Gets the user context from the thread local. This might be accessed by
	...
	public static UserContext getUserContext() {
		...
		if (log.isTraceEnabled()) {
			log.trace("Getting user context " + Arrays.toString(arr)
					+ " from userContextHolder " + userContextHolder);
		}
	...
	/**
	 * Gets the currently defined service context. If one is not defined, one
	...
	/**
	 * Refresh the authenticated user object in the current UserContext. This
	 * should be used when updating information in the database about the
	 * current user and it needs to be reflecting in the (cached)
	...
	/**
	 * Become a different user. (You should only be able to do this as a
	 * superuser.)
	 * 
	 * @param systemId
	 * @throws ContextAuthenticationException
	...
	public static void becomeUser(String systemId)
		...
		// if assuming identity procedure finished successfully, we should
		// change context locale parameter
		Locale locale = null;
		if (user.getUserProperties().containsKey(
				OpenmrsConstants.USER_PROPERTY_DEFAULT_LOCALE)) {
	...
	public static Properties getRuntimeProperties() {
		if (log.isTraceEnabled()) {
			log.trace("getting runtime properties. size: "
					+ runtimeProperties.size());
		}
	...
	/**
	 * Gets the mail session required by the mail message service. This function
	 * forces authentication via the getAdministrationService() method call
	 * 
	...
	private static javax.mail.Session getMailSession() {
		if (mailSession == null) {
			...
			props.setProperty("mail.transport.protocol",
					adminService.getGlobalProperty("mail.transport_protocol"));
			...
			props.setProperty("mail.debug",
					adminService.getGlobalProperty("mail.debug"));
				...
				public PasswordAuthentication getPasswordAuthentication() {
					return new PasswordAuthentication(
							getAdministrationService().getGlobalProperty(
									"mail.user"), getAdministrationService()
									.getGlobalProperty("mail.password"));
				}
			};
	 ...
	 * TODO Ideally, we would be using Spring's method injection to set the
	 * dependencies for the message service.
	 * 
	...
	public static void requirePrivilege(String privilege)
		...
		if (!hasPrivilege(privilege)) {
			...
			if (StringUtils.isNotBlank(privilege)) {
				errorMessage = Context.getMessageSourceService().getMessage(
						"error.privilegesRequired", new Object[]{privilege},
						null);
			} else {
				// Should we even be here if the privilege is blank?
				errorMessage = Context.getMessageSourceService().getMessage(
						"error.privilegesRequiredNoArgs");
			}
	...
	/**
	 * Convenience method. Passes through to
	 * {@link UserContext#addProxyPrivilege(String)}
	 */
	...
	/**
	 * Convenience method. Passes through to
	 * {@link UserContext#removeProxyPrivilege(String)}
	 */
	...
	/**
	 * Convenience method. Passes through to
	 * {@link UserContext#setLocale(Locale)}
	 */
	...
	/**
	 * Used to define a unit of work. All "units of work" should be surrounded
	 * by openSession and closeSession calls.
	 */
	...
	/**
	 * Used to define a unit of work which does not require clearing out the
	 * currently authenticated user. Remember to call
	 * closeSessionWithCurrentUser in a, preferably, finally block after this
	 * work.
	 * 
	 * @since 1.10
	 */
	...
	/**
	 * Used when the a unit of work which started with a call for
	 * openSessionWithCurrentUser has finished. This should be in a, preferably,
	 * finally block.
	 * 
	 * @since 1.10
	 */
	 ...
	 * writing them to the database. If you call this method, and later call
	 * closeSession() or flushSession() your changes are still lost.
	 */
	...
	/**
	 * This method tells whether {@link #openSession()} has been called or not
	 * already. If it hasn't been called, some methods won't work correctly
	 * because a {@link UserContext} isn't available.
	 * 
	...
	/**
	 * Used to re-read the state of the given instance from the underlying
	 * database.
	 * 
	 * @since 2.0
	 * @param obj
	 *            The object to refresh from the database in the session
	 */
	 ...
	 * work. Future updates to this object will not be saved. Future gets of
	 * this object will not fetch this cached copy
	 * 
	 * @param obj
	 *            The object to evict/remove from the session
	 */
	 ...
	 *             if the {@link DatabaseUpdater} has determined that updates
	 ...
	 *             if a module that should be started is not able to
	 * @see InputRequiredException#getRequiredInput()
	 ...
	 * <br>
	 * <b>Note:</b> This method calls {@link Context#openSession()}, so you must
	 * call {@link Context#closeSession()} somewhere on the same thread of this
	 * application so as to not leak memory.
	 * 
	 * @param url
	 *            database url like
	 ...
	 *             cannot continue without input from the user
	 * @throws DatabaseUpdateException
	 *             if the database must be updated. See {@link DatabaseUpdater}
	 * @throws ModuleMustStartException
	 *             if a module that should start is not able to
	...
	public static void startup(String url, String username, String password,
			Properties properties) throws DatabaseUpdateException,
			InputRequiredException, ModuleMustStartException {
		if (properties == null) {
	 ...
	 * @should return the same object when called multiple times for the same
	 *         class
	 */
	...
	/**
	 * Runs through the core data (e.g. privileges, roles, and global
	 * properties) and adds them if necessary.
	 */
	public static void checkCoreDataset() {
		...
		try {
			...
			for (Privilege privilege : Context.getUserService()
					.getAllPrivileges()) {
				currentPrivilegeNames.add(privilege.getPrivilege()
						.toUpperCase());
			}
			Map<String, String> map = OpenmrsUtil.getCorePrivileges();
			for (Map.Entry<String, String> entry : map.entrySet()) {
				String privilegeName = entry.getKey();
				if (!currentPrivilegeNames
						.contains(privilegeName.toUpperCase())) {
		...
		try {
			...
			for (GlobalProperty prop : Context.getAdministrationService()
					.getAllGlobalProperties()) {
				currentPropNames.add(prop.getProperty().toUpperCase());
				if (prop.getDescription() == null) {
					propsMissingDescription.put(prop.getProperty()
							.toUpperCase(), prop);
				}
				if (prop.getDatatypeClassname() == null) {
					propsMissingDatatype.put(prop.getProperty().toUpperCase(),
							prop);
				}
			}

			for (GlobalProperty coreProp : OpenmrsConstants
				...
				if (!currentPropNames.contains(corePropName)) {
					...
					GlobalProperty propToUpdate = propsMissingDescription
							.get(corePropName);
					...
					if (propToUpdate != null
							&& coreProp.getDatatypeClassname() != null) {
						propToUpdate.setDatatypeClassname(coreProp
								.getDatatypeClassname());
						propToUpdate.setDatatypeConfig(coreProp
								.getDatatypeConfig());
						propToUpdate.setPreferredHandlerClassname(coreProp
								.getPreferredHandlerClassname());
						propToUpdate.setHandlerConfig(coreProp
								.getHandlerConfig());
						Context.getAdministrationService().saveGlobalProperty(
								propToUpdate);
					}
		...
		Boolean disableValidation = Boolean.valueOf(as.getGlobalProperty(
				OpenmrsConstants.GP_DISABLE_VALIDATION, "false"));
		ValidateUtil.setDisableValidation(disableValidation);

		PersonName.setFormat(Context.getAdministrationService()
				.getGlobalProperty(
	...
	/**
	 * Runs any needed updates on the current database if the user has the
	 * allow_auto_update runtime property set to true. If not set to true, then
	 * {@link #updateDatabase(Map)} must be called.<br>
	 * <br>
	 * If an {@link InputRequiredException} is thrown, a call to
	 * {@link #updateDatabase(Map)} is required with a mapping from question
	 * prompt to user answer.
	 * 
	 * @param props
	 ...
	 *      question/datatypes
	 */
	private static void checkForDatabaseUpdates(Properties props)
			throws DatabaseUpdateException, InputRequiredException {
		...
		try {
			...
			throw new DatabaseUpdateException(
					"Unable to check if database updates are required", e);
		}
	...
	/**
	 * Updates the openmrs database to the latest. This is only needed if using
	 * the API alone. <br>
	 * <br>
	 * The typical use-case would be: Try to
	 * {@link #startup(String, String, String, Properties)}, if that fails, call
	 * this method to get the database up to speed.
	 * 
	 * @param userInput
	 ...
	 * @since 1.5
	 */
	public static void updateDatabase(Map<String, Object> userInput)
			throws DatabaseUpdateException, InputRequiredException {
	 ...
	 * will be similar in size to mm/dd/yyyy
	 * 
	...
	/**
	 * Gets the simple time format for the current user's locale. The format
	 * will be similar to hh:mm a
	 * 
	 * @return SimpleDateFormat for the user's current locale
	...
	/**
	 * Gets the simple datetime format for the current user's locale. The format
	 * will be similar to mm/dd/yyyy hh:mm a
	 * 
	 * @return SimpleDateFormat for the user's current locale
	 * @see org.openmrs.util.OpenmrsUtil#getDateTimeFormat(Locale)
	 * @should return a pattern with four y characters and two h characters in
	 *         it
	 */
	...
	/**
	 * @return true/false whether the service context is currently being
	 *         refreshed
	 ...
	 */
	public static <T extends Object> List<T> getRegisteredComponents(
			Class<T> type) {
	 ...
	 * @since 1.9.4
	 */
	public static <T> T getRegisteredComponent(String beanName, Class<T> type)
			throws APIException {
	 ...
	 * @since 1.9
	 */
	public static List<OpenmrsService> getModuleOpenmrsServices(
	...
	/**
	 * Updates the search index. It is a blocking operation, which may take even
	 * a few minutes depending on the index size.
	 * <p>
	 * There is no need to call this method in normal usage since the index is
	 * automatically updated whenever DB transactions are committed.
	 * <p>
	 * The method is designated to be used in tests, which rollback
	 * transactions. Note that if the transaction is rolled back, changes to the
	 * index will not be reverted.
	 * 
	 * @since 1.11
	 */
	 ...
	 * <p>
	 * 
	 * @return object representing the result of the started asynchronous
	 *         operation
	 */
