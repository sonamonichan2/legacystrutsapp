...
public class UserContext implements Serializable {
	...
	public Locale getLocale() {
		if (locale == null) {
			// don't cache default locale - allows recognition of changed
	...
	public boolean hasPrivilege(String privilege) {
		...
		if (isAuthenticated()
				&& (getAuthenticatedUser().hasPrivilege(privilege) || getAuthenticatedRole()
						.hasPrivilege(privilege))) {

			// check user's privileges
			Context.getUserService().notifyPrivilegeListeners(
					getAuthenticatedUser(), privilege, true);
			return true;
		...
		Context.getUserService().notifyPrivilegeListeners(
				getAuthenticatedUser(), privilege, false);
		return false;
	}
	...
	/**
	 * Convenience method that sets the default location of the currently
	 * authenticated user using the value of the user's default location
	 * property
	 */
	private void setUserLocation() {
		if (this.user != null) {
			String locationId = this.user
					.getUserProperty(OpenmrsConstants.USER_PROPERTY_DEFAULT_LOCATION);
			if (StringUtils.isNotBlank(locationId)) {
				// only go ahead if it has actually changed OR if wasn't set
				// before
				if (this.locationId == null
						|| this.locationId != Integer.parseInt(locationId)) {
					try {
						this.locationId = Context.getLocationService()
								.getLocation(Integer.valueOf(locationId))
								.getLocationId();
					} catch (NumberFormatException e) {
						// Drop the stored value since we have no match for the
						// set id
						if (this.locationId != null) {
							this.locationId = null;
						}
						log.warn(
								"The value of the default Location property of the user with id:"
										+ this.user.getUserId()
