...
public class ServiceContext implements ApplicationContextAware {
	...
	/**
	 * Services implementing the OpenmrsService interface for each module. The
	 * map is keyed by the full class name including package.
	 * 
	 * @since 1.9
	 */
	@SuppressWarnings("unchecked")
	...
	public static void destroyInstance() {
		if (ServiceContextHolder.instance != null
			...
			if (log.isDebugEnabled()) {
				for (Map.Entry<Class, Object> entry : ServiceContextHolder.instance.services
						.entrySet()) {
					log.debug("Service - " + entry.getKey().getName() + ":"
							+ entry.getValue());
				}
			}

			// Remove advice and advisors that this service added
			for (Class serviceClass : ServiceContextHolder.instance.services
					.keySet()) {
		...
		if (log.isDebugEnabled()) {
			log.debug("Destroying ServiceContext instance: "
	 ...
	 */
	public void setProgramWorkflowService(
	...
	/**
	 * @param orderSetService
	 *            the orderSetService to set
	 */
	...
	/**
	 * Moves advisors and advice added by ServiceContext from the source service
	 * to the target one.
	 * 
	 * @param source
	 ...
	 * 
	 * @param cls
	 *            the class of the cached service
	 ...
	 * 
	 * @param cls
	 *            the class of the caches service to cleanup
	 */
	@SuppressWarnings("unchecked")
		...
		synchronized (refreshingContextLock) {
			try {
				while (refreshingContext) {
					if (log.isDebugEnabled()) {
						log.debug("Waiting to get service: " + cls
								+ " while the context is being refreshed");
					}

					refreshingContextLock.wait();

					if (log.isDebugEnabled()) {
						log.debug("Finished waiting to get service " + cls
	 ...
	 *            Interface to proxy
	 * @param classInstance
	 *            the actual instance of the <code>cls</code> interface
	 */
	@SuppressWarnings("unchecked")
	public void setService(Class cls, Object classInstance) {
		...
		if (cls != null && classInstance != null) {
			try {
				...
				boolean replacingService = cachedService != null
						&& cachedService != classInstance;
				...
				if (noExistingService || replacingService) {
					...
					if (!serviceAdvised) {
						...
						advisedService = (Advised) factory
								.getProxy(OpenmrsClassLoader.getInstance());
					} else {
		...
		try {
			if (!useSystemClassLoader) {
				...
				if (cls != null && log.isDebugEnabled()) {
					try {
						log.debug("cls classloader: "
								+ cls.getClass().getClassLoader() + " uid: "
				...
				try {
					...
					if (log.isDebugEnabled()) {
						log.debug("cls2 classloader: "
								+ cls.getClass().getClassLoader() + " uid: "
								+ cls.getClass().getClassLoader().hashCode());
						// pay attention that here, cls =
						// Class.forName(classString), the system class loader
						// and
						// cls2 is the openmrs class loader, like above.
						log.debug("cls==cls2: "
								+ String.valueOf(cls == OpenmrsClassLoader
										.getInstance().loadClass(classString)));
					}
				} catch (Exception e) { /* pass */
				}
			}
		} catch (ClassNotFoundException e) {
			throw new APIException("service.unable.set",
					new Object[]{classString}, e);
		}

		// add this module service to the normal list of services
		setService(cls, classInstance);

		// Run onStartup for all services implementing the OpenmrsService
		// interface.
		if (OpenmrsService.class.isAssignableFrom(classInstance.getClass())) {
			moduleOpenmrsServices.put(classString,
					(OpenmrsService) classInstance);
	 ...
	 * {@link ApplicationContext} that match the given object type (including
	 * subclasses).
	 * <p>
	 * <b>NOTE: This method introspects top-level beans only.</b> It does
	 * <i>not</i> check nested beans which might match the specified type as
	 * well.
	 * 
	 * @see ApplicationContext#getBeansOfType(Class)
	 * @param type
	 *            the type of Bean to retrieve from the Spring
	 *            {@link ApplicationContext}
	 * @return a List of all registered Beans that are valid instances of the
	 *         passed type
	 * @since 1.5
	 * @should return a list of all registered beans of the passed type
	 * @should return beans registered in a module
	 * @should return an empty list if no beans have been registered of the
	 *         passed type
	 */
	...
	/**
	 * Retrieves a bean that match the given type (including subclasses) and
	 * name.
	 * 
	 * @param beanName
	 *            the name of registered bean to retrieve
	 * @param type
	...
	public <T> T getRegisteredComponent(String beanName, Class<T> type)
		...
		try {
			...
			throw new APIException("service.error.during.getting.component",
					null, beanException);
		}
	}
	 ...
	 * applicationContext of a given type This method recurses through each
	 * parent ApplicationContext
	 * 
	 * @param context
	 *            - The applicationContext to check
	 * @param type
	 *            - The type of component to retrieve
	 * @return all components registered in a Spring applicationContext of a
	 *         given type
	 */
	@SuppressWarnings("unchecked")
	private <T> Map<String, T> getRegisteredComponents(
			ApplicationContext context, Class<T> type) {
		Map<String, T> components = new HashMap<String, T>();
		Map registeredComponents = context.getBeansOfType(type);
		if (log.isTraceEnabled()) {
			log.trace("getRegisteredComponents(" + context + ", " + type
		...
		if (context.getParent() != null) {
			components
					.putAll(getRegisteredComponents(context.getParent(), type));
		}
		return components;
	}
	 ...
	 * 
	 * @param openmrsService
	 *            instance implementing the {@link OpenmrsService} interface.
	 * @param classString
	 *            the full instance class name including the package name.
	 * @since 1.9
	 */
	private void runOpenmrsServiceOnStartup(
		...
		new Thread() {
			...
			public void run() {
				try {
					synchronized (refreshingContextLock) {
						// Need to wait for application context to finish
						// refreshing otherwise we get into trouble.
						while (refreshingContext) {
							if (log.isDebugEnabled()) {
								log.debug("Waiting to get service: "
										+ classString + " while the context"
										+ " is being refreshed");
							}
							...
							if (log.isDebugEnabled()) {
								log.debug("Finished waiting to get service "
										+ classString
										+ " while the context was being refreshed");
							}
						}
	...
	/**
	 * Gets a list of services implementing the {@link OpenmrsService}
	 * interface, for a given module.
	 * 
	 * @param modulePackage
	...
	public List<OpenmrsService> getModuleOpenmrsServices(String modulePackage) {
		...
		for (Entry<String, OpenmrsService> entry : moduleOpenmrsServices
				.entrySet()) {
