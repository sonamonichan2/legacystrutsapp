...
/**
 * This class allows certain tasks to run with elevated privileges. Primary use
 * is scheduling and module startup when there is no user to authenticate as.
 */
public class Daemon {
	 ...
	 * {@link ModuleFactory#startModule(Module)} method uses this to start the
	 * given module in a new thread that is authenticated as the daemon user. <br>
	 ...
	 * @return the module returned from
	 *         {@link ModuleFactory#startModuleInternal(Module)}
	 */
	public static Module startModule(final Module module,
			...
			public void run() {
				...
				try {
					...
					returnedObject = ModuleFactory.startModuleInternal(module,
							isOpenmrsStartup, applicationContext);
				} catch (Exception e) {
					exceptionThrown = e;
				} finally {
		...
		if (startModuleThread.exceptionThrown != null) {
			if (startModuleThread.exceptionThrown instanceof ModuleException) {
				...
				throw new ModuleException("Unable to start module as Daemon",
						startModuleThread.exceptionThrown);
			}
		}
	...
	/**
	 * Executes the given task in a new thread that is authenticated as the
	 * daemon user. <br>
	 * <br>
	 * This can only be called from {@link TimerSchedulerTask} during actual
	 * task execution
	 * 
	 * @param task
	 *            the task to run
	...
	public static void executeScheduledTask(final Task task) throws Exception {
		...
		if (!TimerSchedulerTask.class.isAssignableFrom(callerClass)) {
			throw new APIException("Scheduler.timer.task.only",
					new Object[]{callerClass.getName()});
		}
	...
	/**
	 * Call this method if you are inside a Daemon thread (for example in a
	 * Module activator or a scheduled task) and you want to start up a new
	 * parallel Daemon thread. You may only call this method from a Daemon
	 * thread.
	 * 
	 * @param runnable
	 *            what to run in a new thread
	...
	public static Thread runInNewDaemonThread(final Runnable runnable) {
		...
		if (!isDaemonThread()) {
			throw new APIAuthenticationException(
					"Only daemon threads can spawn new daemon threads");
		}

		// we should consider making DaemonThread public, so the caller can
		// access returnedObject and exceptionThrown
		DaemonThread thread = new DaemonThread() {
			...
			public void run() {
				...
				try {
					...
					// We intentionally do not start a new thread yet, rather
					// wrap the run call in a session.
					runnable.run();
				} finally {
					Context.closeSession();
				}
	...
	/**
	 * @return true if the current thread was started by this class and so is a
	 *         daemon thread that has all privileges
	...
	/**
	 * Calls the {@link OpenmrsService#onStartup()} method, as a daemon, for an
	 ...
	 * @since 1.9
	 */
	public static void runStartupForService(final OpenmrsService service)
			throws ModuleException {
		...
		if (onStartupThread.exceptionThrown != null) {
			if (onStartupThread.exceptionThrown instanceof ModuleException) {
				...
				throw new ModuleException(
						"Unable to run onStartup() method as Daemon",
						onStartupThread.exceptionThrown);
			}
		}
	}

	/**
	 * Executes the given runnable in a new thread that is authenticated as the
	 * daemon user.
	 * 
	 * @param runnable
	 *            an object implementing the {@link Runnable} interface.
	 * @param token
	...
	/**
	 * Executes the given runnable in a new thread that is authenticated as the
	 * daemon user and wait for the thread to finish.
	 * 
	 * @param runnable
	 *            an object implementing the {@link Runnable} interface.
	 * @param token
	 ...
	 * @since 1.9.2
	 */
	public static void runInDaemonThreadAndWait(final Runnable runnable,
			DaemonToken token) {
	 ...
	 * {@link Daemon#executeScheduledTask(Task)} methods so that the returned
	 * object and the exception thrown can be returned to calling class
	 */
	...
	/**
	 * Checks whether user is Daemon. However this is not the preferred method
	 * for checking to see if the current thread is a daemon thread, rather use
	 * Daemon.isDeamonThread(). isDaemonThread is preferred for checking to see
	 * if you are in that thread or if the current thread is daemon.
	 * 
	 * @param user
	 *            user whom we are checking if daemon
