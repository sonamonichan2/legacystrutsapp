...
/**
 * This handler assigns an encounter to an existing visit, where appropriate, or
 * creates a new one.
 * 
 * @see EncounterVisitHandler
 */
public class ExistingOrNewVisitAssignmentHandler
		extends
			ExistingVisitAssignmentHandler implements GlobalPropertyListener {
	...
	public String getDisplayName(Locale locale) {
		return Context.getMessageSourceService().getMessage(
				"visit.assignmentHandler.assignToExistingVisitOrNew", null,
				locale);
	}
	...
	public void beforeCreateEncounter(Encounter encounter) {
		...
		VisitType visitType = encounterVisitMapping.get(encounter
				.getEncounterType());
		if (visitType == null) {
			visitType = loadVisitType(encounter.getEncounterType());

			// replace reference instead of synchronizing
			Map<EncounterType, VisitType> newMap = new HashMap<EncounterType, VisitType>(
		...
		visit.setStopDatetime(OpenmrsUtil.getLastMomentOfDay(encounter
	...
	/**
	 * Get the visit type corresponding to an encounter type by reading valid
	 * mappings from a global property
	 * 
	 * @param encounterType
	 * @return
	 * @throws APIException
	 */
	private static VisitType loadVisitType(EncounterType encounterType)
			throws APIException {
		...
		// Value should be in this format "3:4, 5:2, 1:2, 2:2" for
		...
		if (!StringUtils.isBlank(value)) {
			...
			for (String mapping : mappings) {
				...
				if (index > 0) {
					String encounterTypeIdOrUuid = mapping.substring(0, index)
							.trim();
					if (targetEncounterTypeId.equals(encounterTypeIdOrUuid)
							|| encounterType.getUuid().equals(
									encounterTypeIdOrUuid)) {
						String visitTypeIdOrUuid = mapping.substring(index + 1)
								.trim();
						VisitType visitType = null;
						if (StringUtils.isNumeric(visitTypeIdOrUuid)) {
							visitType = visitService.getVisitType(Integer
									.parseInt(visitTypeIdOrUuid));
						} else {
							visitType = visitService
									.getVisitTypeByUuid(visitTypeIdOrUuid);
						}
						if (visitType != null) {
							return visitType;
						}
			...
			// Reaching here means this encounter type is not in the user's
			// mapping.
			throw new APIException("GlobalProperty.error.loadVisitType",
					new Object[]{encounterType.getName()});
		}
	...
	public boolean supportsPropertyName(String propertyName) {
		return OpenmrsConstants.GP_ENCOUNTER_TYPE_TO_VISIT_TYPE_MAPPING
				.equals(propertyName);
	}
