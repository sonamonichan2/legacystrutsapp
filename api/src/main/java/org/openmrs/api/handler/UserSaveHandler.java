...
/**
 * This class deals with {@link User} objects when they are saved via a save*
...
public class UserSaveHandler implements SaveHandler<User> {
	...
	public void handle(User user, User creator, Date dateCreated, String other) {
		...
		// the framework only automatically recurses on properties that are
		// Collection<OpenmrsObject>
		// so we need to do this manually
		if (user.getPerson() != null) {
			loadLazyHibernateCollections(user);
			RequiredDataAdvice.recursivelyHandle(SaveHandler.class,
					user.getPerson(), creator, dateCreated, other,
