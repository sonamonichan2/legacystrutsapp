...
/**
 * Abstract base implementation of {@link Attribute}. Actual implementations
 * should be able to extend this class, and have very little of their own code.
 * 
 * @param <AT>
 * @param <OwningType>
 * @since 1.9
...
public abstract class BaseAttribute<AT extends AttributeType, OwningType extends Customizable<?>>
	...
	// temporarily holds a typed value, either when getValue() is called the
	// first time (causing valueReference to be converted) or when setValue has
	// been called, but this attribute has not yet been committed to persistent
	// storage
	...
	public Object getValue() throws InvalidCustomValueException {
		if (value == null) {
			value = CustomDatatypeUtil.getDatatype(getAttributeType())
	...
	public int compareTo(Attribute other) {
		...
		if (retValue == 0) {
			retValue = OpenmrsUtil.compareWithNullAsGreatest(getAttributeType()
					.getId(), other.getAttributeType().getId());
		}
		if (retValue == 0) {
			retValue = OpenmrsUtil.compareWithNullAsGreatest(
					getValueReference(), other.getValueReference());
		}
		if (retValue == 0) {
			retValue = OpenmrsUtil.compareWithNullAsGreatest(getId(),
					other.getId());
		}
		return retValue;
	}
