...
public abstract class LayoutSupport<T extends LayoutTemplate> {
			...
			for (T at : this.layoutTemplates) {
				if (at != null
						&& (templateName.equalsIgnoreCase(at.getDisplayName())
								|| templateName.equalsIgnoreCase(at
										.getCodeName()) || templateName
									.equalsIgnoreCase(at.getCountry()))) {
					ret = at;
					log.debug("Found Layout Template named "
							+ at.getDisplayName());
				}
			}
	...
	/**
	 * @param defaultLayoutFormat
	 *            The defaultLayoutFormat to set.
	 */
