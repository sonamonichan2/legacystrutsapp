...
/**
 * A Role is just an aggregater of {@link Privilege}s. {@link User}s contain a
 * number of roles (Users DO NOT contain any privileges directly) Roles can be
 * grouped by inheriting other roles. If a user is given Role A that inherits
 * from Role B, the user has all rights/abilities for both Role A's privileges
 * and for Role B's privileges.
 * 
 * @see Privilege
 */
public class Role extends BaseOpenmrsMetadata implements java.io.Serializable {
	...
	public void addPrivilege(Privilege privilege) {
		...
		if (privilege != null
				&& !containsPrivilege(privileges, privilege.getPrivilege())) {
			privileges.add(privilege);
		}
	}

	private boolean containsPrivilege(Collection<Privilege> privileges,
	 ...
	 * roles privileges. This method does not recurse through the inherited
	 * roles
	 * 
	 * @param privilegeName
	...
	/**
	 * Convenience method to test whether or not this role extends/ inherits
	...
	/**
	 * Returns the full set of roles be looping over inherited roles. Duplicate
	 * roles are dropped.
	 * 
	 * @param total
	...
	public Set<Role> recurseOverParents(final Set<Role> total) {
		...
		Set<Role> allRoles = new HashSet<Role>(); // total roles (parents +
													// children)
	...
	/**
	 * Convenience method to test whether or not this role is a parent of
	 * another role
	 * 
	...
	/**
	 * Returns the full set of child roles be looping over children. Duplicate
	 * roles are dropped.
	 * 
	 * @param total
	 *            Roles already looped over
