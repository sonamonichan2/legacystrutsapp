...
public class HL7ServiceImpl extends BaseOpenmrsService implements HL7Service {
	...
	public List<HL7InQueue> getHL7InQueueBatch(int start, int length,
		...
		return dao.getHL7Batch(HL7InQueue.class, start, length, messageState,
				query);
	}
	...
	/**
	 * @see org.openmrs.hl7.HL7Service#getHL7InArchiveBatch(int, int, int,
	 *      String)
	 */
	@Override
	@Transactional(readOnly = true)
	public List<HL7InArchive> getHL7InArchiveBatch(int start, int length,
			int messageState, String query) throws APIException {
		return dao.getHL7Batch(HL7InArchive.class, start, length, messageState,
				query);
	}
	...
	public Integer countHL7InQueue(int messageState, String query)
			throws APIException {
		return OpenmrsUtil.convertToInteger(dao.countHL7s(HL7InQueue.class,
				messageState, query));
	}
	...
	public Integer countHL7InError(String query) throws APIException {
		return OpenmrsUtil.convertToInteger(dao.countHL7s(HL7InError.class,
				null, query));
	}
	...
	public Integer countHL7InArchive(int messageState, String query)
			throws APIException {
		return OpenmrsUtil.convertToInteger(dao.countHL7s(HL7InArchive.class,
				messageState, query));
	}
	...
	public HL7InArchive getHL7InArchiveByUuid(String uuid) throws APIException {
		if (Hl7InArchivesMigrateThread.isActive()) {
			throw new APIException("Hl7Service.cannot.fetch.archives",
					(Object[]) null);
		}
	...
	public void setHL7Handlers(Map<String, Application> handlers) {
		...
		for (Map.Entry<String, Application> entry : handlers.entrySet()) {
			...
			if (!messageName.contains("_")) {
				throw new APIException("Hl7Service.invalid.messageName",
						(Object[]) null);
			}
	...
	public Person createPersonFromNK1(NK1 nk1) throws HL7Exception {
		...
		if (!goodIdentifiers.isEmpty()) {
			// If we have one identifier, set it as the preferred to make the
			// validator happy.
	...
	public void loadHL7InArchiveData(HL7InArchive archive) throws APIException {
		...
		try {
			archive.setHL7Data(OpenmrsUtil.getFileAsString(new File(new URI(
					archive.getHL7Data()))));
			archive.setLoaded(true);
		} catch (URISyntaxException e) {
			throw new APIException("Hl7Service.malformed.archive.location",
					new Object[]{archive.getHL7Data()}, e);
		} catch (IOException e) {
			throw new APIException("Hl7Service.unable.convert.archive",
					new Object[]{archive.getHL7Data()}, e);
		}
	}
	...
	public void migrateHl7InArchivesToFileSystem(
		...
		// HL7Constants.HL7_STATUS_ARCHIVED indicates the HL7 has been archived
		// to the filesystem
		...
		while (Hl7InArchivesMigrateThread.isActive()
			...
			while (Hl7InArchivesMigrateThread.isActive()
				...
				try {
					...
					progressStatusMap.put(HL7Constants.NUMBER_TRANSFERRED_KEY,
							numberTransferred++);
	...
	private void migrateHL7InArchive(HL7InArchive archive) throws APIException {
		if (archive == null) {
			throw new APIException("Hl7Service.migrate.null.archive",
					(Object[]) null);
		}

		if (!OpenmrsUtil.nullSafeEquals(archive.getMessageState(),
				HL7Constants.HL7_STATUS_PROCESSED)) {
			throw new APIException("Hl7Service.migrate.archive.state",
					(Object[]) null);
		}
	 ...
	 */
	private URI writeHL7InArchiveToFileSystem(HL7InArchive hl7InArchive)
			throws APIException {
		...
		try {
			...
			// write the archive to a separate file while grouping them
			// according to
			// the year, month and date of month when they were stored in the
			// archives table
			...
			File yearDir = new File(destinationDir, Integer.toString(calendar
					.get(Calendar.YEAR)));
			...
			File monthDir = new File(yearDir, df.format(calendar
			...
			File dayDir = new File(monthDir, df.format(calendar
					.get(Calendar.DAY_OF_MONTH)));
			if (!dayDir.isDirectory()) {
				dayDir.mkdirs();
			}

			// use the uuid, source id and source key(if present) to generate
			// the file name
			...
			if (writer.checkError()) {
				log.warn("An Error occured while writing hl7 archive with id '"
						+ hl7InArchive.getHL7InArchiveId()
						+ "' to the file system");
				throw new APIException("Hl7Service.write.no.error",
						(Object[]) null);
			}
