...
public interface HL7Service extends OpenmrsService {
	...
	/**
	 * Get all of the hl7 source objects from the database. This includes
	 * retired ones
	 * 
	...
	/**
	 * Completely remove the source from the database. This should only be used
	 * in rare cases. See {@link #retireHL7Source(HL7Source)}
	 * 
	 * @param hl7Source
	 */
	...
	/**
	 * Return a list of all hl7 in queues based on batch settings and a query
	 * string
	 * 
	 * @param start
	 *            beginning index
	 ...
	 *            size of the batch
	 * @param messageState
	 *            status of the HL7InQueue message
	 * @param query
	 *            search string
	...
	/**
	 * Return a list of all hl7 in errors based on batch settings and a query
	 * string
	 * 
	 * @param start
	 *            beginning index
	...
	/**
	 * Return a list of all hl7 in archives based on batch settings and a query
	 * string
	 * 
	 * @param start
	 *            beginning index
	 ...
	 *            size of the batch
	 * @param messageState
	 *            status of the HL7InArchive message
	 * @param query
	 *            search string
	...
	/**
	 * Get the archive item with the given id, If hl7 archives were moved to the
	 * file system, you can't do a look up by hl7ArchiveId, instead call
	...
	/**
	 * If hl7 migration has been run and the state matches that of processed
	 * items, the method returns a list of all archives in the file system, for
	 * any other state , it returns an empty list, this is because all archives
	 * would have a status of 'processed' after migration and all deleted
	 * archives moved back into the queue with a status of 'deleted' otherwise
	 * it returns archives with a matching state if migration hasn't yet been
	 * run.
	 * 
	...
	/**
	 * Get the queue items given a state (deleted, error, pending, processing,
	 * processed).
	 * 
	 ...
	 * 
	 * @return a List&lt;HL7InError&gt; object with all <code>HL7InError</code>
	 ...
	 *            HL7 component of data type XCN (extended composite ID number
	 *            and name for persons) (see HL7 2.5 manual Ch.2A.86)
	 * @return Internal ID # of the specified person, or null if that person
	 *         can't be found or is ambiguous
	 */
	public Integer resolvePersonId(XCN xcn) throws HL7Exception;

	/**
	 * Resolves location from person location object, and if location id is
	 * specified then returns correspond internal identifier of the specified
	 * location. If only location_name is specified, it tries to return location
	 * internal identifier by given name
	 * 
	 * @param pl
	 ...
	 * @should return internal identifier of location if only location name is
	 *         specified
	 * @should return internal identifier of location if only location id is
	 *         specified
	 ...
	 *         segment, or null if the patient is not found or if the PID
	...
	/**
	 * determines a person (or patient) based on identifiers from a CX array, as
	 * found in a PID or NK1 segment; the first resolving identifier in the list
	 * wins
	 * 
	 * @param identifiers
	 *            CX identifier list from an identifier (either PID or NK1)
	 * @return The internal id number of a Person based on one of the given
	 *         identifiers, or null if the Person is not found
	 * @throws HL7Exception
	 ...
	 * If an error occurs while processing, a new {@link HL7InError} is created
	 * and saved. <br>
	 * If no error occurs, a new {@link HL7InArchive} is created and saved.<br>
	 * The given {@link HL7InQueue} is removed from the hl7 in queue table
	 * regardless of success or failure of the processing.
	 * 
	 * @param inQueue
	 *            the {@link HL7InQueue} to parse and save all encounters/obs to
	 *            the db
	 ...
	 * @should parse oru r01 message using overridden parser provided by a
	 *         module
	 */
	...
	/**
	 * Parses the given {@link Message} and saves the resulting content to the
	 * database
	 * 
	 * @param hl7Message
	 *            the {@link Message} to process and save to the db.
	...
	/**
	 * Method is called by the archives migration thread to transfer hl7 in
	 * archives from the hl7_in_archives database table to the file system
	 * 
	 * @param progressStatusMap
	...
	public void migrateHl7InArchivesToFileSystem(
			Map<String, Integer> progressStatusMap) throws APIException;
	...
	/**
	 * creates a Person from information held in an NK1 segment; if valid
	 * PatientIdentifiers exist, a Patient will be created and returned
	 * 
	 * @param nk1
	 ...
	 * @param archives
	 */
	public void loadHL7InArchiveData(List<HL7InArchive> archives)
			throws APIException;
	 ...
	 * <p>
	 * It calls {@link #getHL7InQueueByUuid(String)},
	 * {@link #getHL7InArchiveByUuid(String)} and
	 * {@link #getHL7InErrorByUuid(String)} consecutively and returns the first
	 * non-null result.
	 * 
	 * @param uuid
