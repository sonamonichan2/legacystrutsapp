...
public class HL7Util {
	...
	public static final String LOCAL_TIMEZONE_OFFSET = new SimpleDateFormat("Z")
			.format(new Date());

	/**
	 * Converts an HL7 timestamp into a java.util.Date object. HL7 timestamps
	 * can be created with varying levels of precision &mdash; e.g., just the
	 * year or just the year and month, etc. Since java.util.Date cannot store a
	 * partial value, we fill in defaults like January, 01 at midnight within
	...
	public static Date parseHL7Timestamp(String s) throws HL7Exception {
		...
		try {
			Date parsedDay = new SimpleDateFormat("yyyyMMdd").parse(s
					.substring(0, 8));
			timeZoneOffset = getTimeZoneOffset(s, parsedDay);
		} catch (ParseException e) {
			throw new HL7Exception("Error parsing date: '" + s.substring(0, 8)
					+ "' for time zone offset'" + s + "'", e);
		}
		s = s.replace(timeZoneOffset, ""); // remove the timezone from the
											// string
		...
		Date date;
		try {
			date = new SimpleDateFormat(TIMESTAMP_FORMAT).parse(dateString
					.toString());
		} catch (ParseException e) {
	...
	/**
	 * Gets the timezone string for this given fullString. If fullString
	 * contains a + or - sign, the strings after those are considered to be the
	 * timezone. <br>
	 * <br>
	 * If the fullString does not contain a timezone, the timezone is determined
	 * from the server's timezone on the "givenDate". (givenDate is needed to
	 * account for daylight savings time.)
	 * 
	 * @param fullString
	 *            the hl7 string being parsed
	 * @param givenDate
	 *            the date that should be used if no timezone exists on the
	 *            fullString
	...
	protected static String getTimeZoneOffset(String fullString, Date givenDate) {
		...
		if (timeZoneFlag) {
			...
			if (timeZoneOffset.length() != 5) {
				log.error("Invalid timestamp because its too short: "
						+ timeZoneOffset);
			}
	...
	/**
	 * Convenience method for parsing HL7 dates (treated just like a timestamp
	 * with only year, month, and day specified)
	 * 
	...
	/**
	 * Converts an HL7 time into a java.util.Date object. Since the
	 * java.util.Date object cannot store just the time, the date will remain at
	 * the epoch (e.g., January 1, 1970). Time more precise than microseconds is
	 * ignored.
	 * 
	 * @param s
	...
	public static Date parseHL7Time(String s) throws HL7Exception {
		...
		Date date;
		try {
			date = new SimpleDateFormat(TIME_FORMAT).parse(timeString
					.toString());
		} catch (ParseException e) {
			throw new HL7Exception("Invalid time format: '" + s + "' ["
					+ timeString + "]", e);
	...
	public static File getHl7ArchivesDirectory() throws APIException {
		...
		if (StringUtils.isBlank(archiveDir)) {
			log.warn("Invalid value for global property '"
					+ OpenmrsConstants.GLOBAL_PROPERTY_HL7_ARCHIVE_DIRECTORY
		...
		// TODO Should take care of the case where the user is using removable
		// media, this might explode
