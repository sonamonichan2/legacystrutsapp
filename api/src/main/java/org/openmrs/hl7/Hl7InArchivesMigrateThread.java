...
/**
 * Separate thread to move the hl7 in archives from the database tables to the
 * filesystem. It is highly recommended to start this thread via DWRHL7Service
 * as opposed to calling the thread's start() method to ensure the thread is
 * started after making all the necessary checks.
 */
public class Hl7InArchivesMigrateThread extends Thread {

	private static final Log log = LogFactory
			.getLog(Hl7InArchivesMigrateThread.class);

	/**
	 * Map holds data about the progress of the transfer process, that is
	 * numberTransferred and numberOfFailedTransfers
	 */
	...
	/**
	 * The different states this thread can be in at a given point during
	 * migration
	 */
	public enum Status {
	...
	public static void setProgressStatusMap(
	...
	public void run() {
		...
		while (isActive() && transferStatus == Status.RUNNING) {
			try {
				...
				if (isActive()) {
					Context.getHL7Service().migrateHl7InArchivesToFileSystem(
							progressStatusMap);
				}
				...
				try {
					...
					log.warn(
							"Hl7 in archive migration thread has been abnormally interrupted",
							e);
				}
	...
	/**
	 * convenience method to set transfer status and active flag to stop
	 * migration
	 */
	...
	/**
	 * Gets the number of failed transfers during migration, this could be that
	 * the system couldn't write them to the file system or couldn't be deleted
	 * from the database.
	 * 
	...
	public static Integer getNumberOfFailedTransfers() {
		...
		return progressStatusMap
				.get(HL7Constants.NUMBER_OF_FAILED_TRANSFERS_KEY);
	}
