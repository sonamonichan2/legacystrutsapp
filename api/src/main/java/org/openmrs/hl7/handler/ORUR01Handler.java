...
public class ORUR01Handler implements Application {
	 ...
	 * @should fail if the provider name type code is not specified and is not a
	 *         personId
	...
	public Message processMessage(Message message) throws ApplicationException {
		...
		if (!(message instanceof ORU_R01)) {
			throw new ApplicationException(Context.getMessageSourceService()
					.getMessage("ORUR01.error.invalidMessage"));
		}
		...
		try {
			...
			log.warn("Error casting " + message.getClass().getName()
					+ " to ORU_R01", e);
			throw new ApplicationException(Context.getMessageSourceService()
					.getMessage("ORUR01.error.invalidMessageType ",
							new Object[]{message.getClass().getName()}, null),
					e);
		} catch (HL7Exception e) {
			log.warn("Error while processing ORU_R01 message", e);
			throw new ApplicationException(Context.getMessageSourceService()
					.getMessage("ORUR01.error.WhileProcessing"), e);
		}
		...
		for (int i = 0; i < numObr; i++) {
			...
			if (!StringUtils.hasText(obr.getUniversalServiceIdentifier()
				...
				throw new HL7Exception(Context.getMessageSourceService()
						.getMessage("ORUR01.errorInvalidOBR ",
			...
			Concept obrConcept = getConcept(
					obr.getUniversalServiceIdentifier(), messageControlId);
			if (obrConcept != null
					&& !ignoredConceptIds.contains(obrConcept.getId())) {
			...
			// loop over the obs and create each object, adding it to the
			// encounter
			...
			for (int j = 0; j < numObs; j++) {
				...
				try {
					...
					if (obs != null) {
						...
						if (obsGrouper != null) {
							// set the obs to the group. This assumes the group
							// is already
							// on the encounter and that when the encounter is
							// saved it will
					...
					// if the sender never specified any text for the proposed
					// concept
					if (!StringUtils.isEmpty(value)) {
						conceptProposals.add(createConceptProposal(encounter,
								questionConcept, value));
					} else {
						errorInHL7Queue = new HL7Exception(Context
								.getMessageSourceService().getMessage(
										"Hl7.proposed.concept.name.empty"),
								proposingException);
					...
					if (errorInHL7Queue != null) {
						throw new HL7Exception(Context
								.getMessageSourceService().getMessage(
										"ORUR01.error.improperlyFormattedOBX",
										new Object[]{PipeParser.encode(obx,
	...
	protected void processNK1(Patient patient, NK1 nk1) throws HL7Exception {
		...
		String relCodingSystem = nk1.getRelationship().getNameOfCodingSystem()
				.getValue();
		if (!relCodingSystem.equals(HL7Constants.HL7_LOCAL_RELATIONSHIP)) {
			throw new HL7Exception(Context.getMessageSourceService()
					.getMessage("ORUR01.error.relationshipCoding",
		...
		if (!Pattern.matches("[0-9]+[AB]", relIdentifier)) {
			throw new HL7Exception(Context.getMessageSourceService()
					.getMessage("ORUR01.error.relationshipType",
		...
		Integer relTypeId = 0;
		try {
			relTypeId = Integer.parseInt(relIdentifier.substring(0,
					relIdentifier.length() - 1));
		...
		RelationshipType relType = Context.getPersonService()
				.getRelationshipType(relTypeId);
		if (relType == null) {
			throw new HL7Exception(Context.getMessageSourceService()
					.getMessage("ORUR01.error.relationshipTypeNotFound",
		...
		// determine if the patient is person A or B; the relIdentifier
		// indicates
		// the relative's side of the relationship, so the patient is the
		// inverse
		boolean patientIsPersonA = relIdentifier.endsWith("B");
		boolean patientCanBeEitherPerson = relType.getbIsToA().equals(
				relType.getaIsToB());
		...
		if (relative != null) {
			if (patientCanBeEitherPerson || patientIsPersonA) {
				rels.addAll(Context.getPersonService().getRelationships(
						patient, relative, relType));
			}
			if (patientCanBeEitherPerson || !patientIsPersonA) {
				rels.addAll(Context.getPersonService().getRelationships(
						relative, patient, relType));
			}
		}
		...
		if (rels.isEmpty()) {
			...
			if (relative == null) {
				...
				if (relative == null) {
					throw new HL7Exception(Context.getMessageSourceService()
							.getMessage("ORUR01.error.relativeNotCreated"));
				}
			}
	...
	/**
	 * finds NK1 segments in an ORU_R01 message. all HAPI-rendered Messages have
	 * at least one NK1 segment but if the original message truly does not
	 * contain an NK1, the setID will be null on the generated NK1
	 * 
	 * @param oru
	...
	public List<NK1> getNK1List(ORU_R01 oru) throws HL7Exception {
		...
		// there will always be at least one NK1, even if the original message
		// does not contain one
		for (int i = 0; i < oru.getPATIENT_RESULT().getPATIENT().getNK1Reps(); i++) {
			// if the setIDNK1 value is null, this NK1 is blank
			if (oru.getPATIENT_RESULT().getPATIENT().getNK1(i).getSetIDNK1()
					.getValue() != null) {
	...
	private Encounter createEncounter(MSH msh, Patient patient, PV1 pv1, ORC orc)
		...
		if (encounterId != null) {
			...
			// Date dateEntered = getDateEntered(orc); // ignore this since we
			// have no place in the data model to store it
		...
		if ("NM".equals(hl7Datatype)) {
			...
			if (value == null || value.length() == 0) {
				...
				if (concept.getDatatype().isBoolean()) {
						...
						throw new HL7Exception(Context
								.getMessageSourceService().getMessage(
										...
										new Object[]{value,
												...
												conceptName.getName(), uid},
										null), e);
					}
				} else if (concept.getDatatype().isCoded()) {
					Concept answer = "1".equals(value) ? Context
					...
					if (conceptAnswers != null && conceptAnswers.size() > 0) {
						for (ConceptAnswer conceptAnswer : conceptAnswers) {
							if (conceptAnswer.getAnswerConcept().getId()
									.equals(answer.getId())) {
					...
					if (!isValidAnswer) {
						throw new HL7Exception(Context
								.getMessageSourceService().getMessage(
										"ORUR01.error.invalidAnswer",
										new Object[]{answer.toString(), uid},
										null));
					}
				} else {
				...
				try {
					...
					throw new HL7Exception(Context.getMessageSourceService()
							.getMessage(
									"ORUR01.error.notnumericConcept",
									new Object[]{value, concept.getConceptId(),
											conceptName.getName(), uid}, null),
							e);
				}
			}
			...
			if (isConceptProposal(valueIdentifier)) {
				...
				try {
					...
					if (HL7Constants.HL7_LOCAL_DRUG.equals(value
							.getNameOfAlternateCodingSystem().getValue())) {
						Drug valueDrug = new Drug();
						valueDrug.setDrugId(Integer.valueOf(value
					...
					throw new HL7Exception(Context.getMessageSourceService()
							.getMessage("ORUR01.error.InvalidConceptId",
			...
			obs.setComplexData(new ComplexData(title, value.getData()
					.getValue()));
		} else {
			...
			throw new HL7Exception(Context.getMessageSourceService()
					.getMessage("ORUR01.error.UpsupportedObsType",
	...
	private ConceptName getConceptName(ST altIdentifier, ID altCodingSystem)
		...
		if (altIdentifier != null
				&& HL7Constants.HL7_LOCAL_CONCEPT_NAME.equals(altCodingSystem
						.getValue())) {
	...
	protected Concept getConcept(String hl7ConceptId, String codingSystem,
		...
		if (codingSystem == null
			...
			try {
				...
				throw new HL7Exception(Context.getMessageSourceService()
						.getMessage("ORUR01.error.hl7ConceptId",
								new Object[]{hl7ConceptId, uid}, null));
			}
		} else {
			// the concept is not local, look it up in our mapping
			return Context.getConceptService().getConceptByMapping(
	...
	private Provider getProvider(PV1 pv1) throws HL7Exception {
		...
		String assignAuth = hl7Provider.getAssigningAuthority()
				.getUniversalID().getValue();
		String type = hl7Provider.getAssigningAuthority().getUniversalIDType()
				.getValue();
		String errorMessage = "";
		if (StringUtils.hasText(id)) {
			String specificErrorMsg = "";
			if (OpenmrsUtil.nullSafeEquals("L", type)) {
				if (HL7Constants.PROVIDER_ASSIGNING_AUTH_PROV_ID
						.equalsIgnoreCase(assignAuth)) {
					try {
						provider = Context.getProviderService().getProvider(
								Integer.valueOf(id));
					} catch (NumberFormatException e) {
						// ignore
					}
					specificErrorMsg = "with provider Id";
				} else if (HL7Constants.PROVIDER_ASSIGNING_AUTH_IDENTIFIER
						.equalsIgnoreCase(assignAuth)) {
					provider = Context.getProviderService()
							.getProviderByIdentifier(id);
					specificErrorMsg = "with provider identifier";
				} else if (HL7Constants.PROVIDER_ASSIGNING_AUTH_PROV_UUID
						.equalsIgnoreCase(assignAuth)) {
					provider = Context.getProviderService().getProviderByUuid(
							id);
				...
				try {
					Person person = Context.getPersonService().getPerson(
							Integer.valueOf(id));
					Collection<Provider> providers = Context
							.getProviderService().getProvidersByPerson(person);
			...
			errorMessage = "Could not resolve provider " + specificErrorMsg
					+ ":" + id;
		} else {
	...
	private Patient getPatient(PID pid) throws HL7Exception {
		...
		if (patientId == null) {
			throw new HL7Exception(Context.getMessageSourceService()
					.getMessage("ORUR01.error.UnresolvedPatient"));
		}
	...
	private Location getLocation(PV1 pv1) throws HL7Exception {
		...
		if (locationId == null) {
			throw new HL7Exception(Context.getMessageSourceService()
					.getMessage("ORUR01.error.UnresolvedLocation"));
		}
	...
	/**
	 * needs to find a Form based on information in MSH-21. example:
	 * 16^AMRS.ELD.FORMID
	 * 
	 * @param msh
	 * @return
	...
	private Form getForm(MSH msh) throws HL7Exception {
		...
		for (EI identifier : msh.getMessageProfileIdentifier()) {
			if (identifier != null && identifier.getNamespaceID() != null) {
				...
				if (OpenmrsUtil.nullSafeEquals(identifierType,
						HL7Constants.HL7_FORM_UUID)) {
					uuid = identifier.getEntityIdentifier().getValue();
				} else if (OpenmrsUtil.nullSafeEquals(identifierType,
						HL7Constants.HL7_FORM_ID)) {
					id = identifier.getEntityIdentifier().getValue();
				} else {
					log.warn("Form identifier type of " + identifierType
							+ " unknown to ORU R01 processor.");
				}
			}
		}
		...
		if (form == null) {
			try {
				...
				throw new HL7Exception(Context.getMessageSourceService()
						.getMessage("ORUR01.error.parseFormId"), e);
			}
		}
	...
	private User getEnterer(ORC orc) throws HL7Exception {
		...
		if (entererId == null) {
			throw new HL7Exception(Context.getMessageSourceService()
					.getMessage("ORUR01.error.UnresolvedEnterer"));
		}
	...
	/**
	 * Creates a ConceptProposal object that will need to be saved to the
	 ...
	 * @return
	 */
	private ConceptProposal createConceptProposal(Encounter encounter,
			Concept concept, String originalText) {
	...
	private void updateHealthCenter(Patient patient, PV1 pv1) {
		...
		if (dischargeToLocation != null && dischargeToLocation.length() > 0) {
			...
			PersonAttributeType healthCenterAttrType = Context
			...
			PersonAttribute currentHealthCenter = patient
					.getAttribute("Health Center");

			if (currentHealthCenter == null
					|| !newLocationId.toString().equals(
							currentHealthCenter.getValue())) {
				PersonAttribute newHealthCenter = new PersonAttribute(
						healthCenterAttrType, newLocationId.toString());

				log.debug("Updating patient's location from "
						+ currentHealthCenter + " to " + newLocationId);
