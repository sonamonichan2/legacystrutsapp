...
public class MailMessageSender implements MessageSender {
	...
	public void send(Message message) throws MessageException {
		try {
			...
			// catch mail-specific exception and re-throw it as app-specific
			// exception
	...
	/**
	 * Converts the message object to a mime message in order to prepare it to
	 * be sent.
	 * 
	 * @param message
	 * @return MimeMessage
	...
	public MimeMessage createMimeMessage(Message message) throws Exception {
		...
		if (!StringUtils.hasText(message.getContentType())) {
			String contentType = Context.getAdministrationService()
					.getGlobalProperty("mail.default_content_type");
			message.setContentType(StringUtils.hasText(contentType)
					? contentType
					: "text/plain");
		}
		...
		if (!message.hasAttachment()) {
			mimeMessage.setContent(message.getContent(),
	...
	private MimeMultipart createMultipart(Message message) throws Exception {
		...
		attachment.setContent(message.getAttachment(),
