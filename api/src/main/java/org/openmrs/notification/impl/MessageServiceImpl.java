...
public class MessageServiceImpl implements MessageService {
	...
	private MessagePreparator messagePreparator; // Prepares message for
													// delivery
	...
	/**
	 * Public constructor Required for use with spring's method injection. Be
	 * careful because this class requires a DAO Context in order to work
	 * properly. Please set the DAO context
	 */
	 ...
	 */
	public Message createMessage(String recipients, String sender,
			String subject, String content) throws MessageException {
		return Context.getMessageService().createMessage(recipients, sender,
				subject, content, null, null, null);
	}
	...
	public Message createMessage(String sender, String subject, String content)
		...
		return Context.getMessageService().createMessage(null, sender, subject,
				content);
	}
	 ...
	 *            the content or body of the message
	 */
	public Message createMessage(String subject, String content)
			throws MessageException {
		return Context.getMessageService().createMessage(null, null, subject,
				content);
	}
	...
	/**
	 * Send a message using the given parameters. This is a convenience method
	 * so that the client does not need to create its own Message object.
	 */
	public void sendMessage(String recipients, String sender, String subject,
			String content) throws MessageException {
	 ...
	 * @param recipientId
	 *            Integer identifier of user (recipient)
	 */
	public void sendMessage(Message message, Integer recipientId)
		...
		message.addRecipient(user
				.getUserProperty(OpenmrsConstants.USER_PROPERTY_NOTIFICATION_ADDRESS));
		// message.setFormat( user(
		// OpenmrsConstants.USER_PROPERTY_NOTIFICATION_FORMAT ) );
		Context.getMessageService().sendMessage(message);
	}
	 ...
	 * 
	 * @param message
	 *            the <code>Message</code> to be sent
	 * @param user
	...
	public void sendMessage(Message message, User user) throws MessageException {
		...
		// message.setFormat( user.getProperty(
		// OpenmrsConstants.USER_PROPERTY_NOTIFICATION_FORMAT ) );
		Context.getMessageService().sendMessage(message);
	}
	 ...
	 */
	public void sendMessage(Message message, Collection<User> users)
			throws MessageException {
	 ...
	 */
	public void sendMessage(Message message, String roleName)
			throws MessageException {
	...
	public void sendMessage(Message message, Role role) throws MessageException {
		...
		Collection<User> users = Context.getUserService().getUsers(null, roles,
				false);
	...
	/**
	 * Prepare a message given the template. The template should be populated
	 * with all necessary data including the variable name-value pairs
	 * 
	 * @param template
	 *            the given <code>Template</code>
	...
	/**
	 * Prepare a message based on a template and data used for variable
	 * subsitution within template.
	 * 
	 * @param templateName
	...
	public Message prepareMessage(String templateName, Map data)
			throws MessageException {
		try {
			Template template = (Template) getTemplatesByName(templateName)
					.get(0);
			template.setData(data);
			return Context.getMessageService().prepareMessage(template);
		} catch (Exception e) {
			throw new MessageException(
					"Could not prepare message with template " + templateName,
					e);
		}
	}
