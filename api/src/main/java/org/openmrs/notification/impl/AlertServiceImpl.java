 ...
 */
@Transactional
public class AlertServiceImpl extends BaseOpenmrsService
		implements
			Serializable,
			AlertService {
	...
	/**
	 * @see org.openmrs.notification.AlertService#notifySuperUsers(String,
	 *      Exception, Object...)
	 */
	public void notifySuperUsers(String messageCode, Exception cause,
			Object... messageArguments) {

		// Generate an internationalized error message with the beginning of the
		// stack trace from cause added onto the end
		String message = Context.getMessageSourceService().getMessage(
				messageCode, messageArguments, Context.getLocale());

		if (cause != null) {
			StringBuilder stackTrace = new StringBuilder();
			// get the first two lines of the stack trace ( no more can fit in
			// the alert text )
			...
			message = message.substring(0,
					Math.min(message.length(), Alert.TEXT_MAX_LENGTH));
		}

		// Send an alert to all administrators
		Alert alert = new Alert(message, Context.getUserService()
				.getUsersByRole(new Role(RoleConstants.SUPERUSER)));

		// Set the alert so that if any administrator 'reads' it it will be
		// marked as read for everyone who received it
		alert.setSatisfiedByAny(true);

		// If there is not user creator for the alert ( because it is being
		// created at start-up )create a user
		// TODO switch this to use the daemon user when ticket TRUNK-120 is
		// complete
		if (alert.getCreator() == null) {
