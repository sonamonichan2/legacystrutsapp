 ...
 * <code>form</code> A form can have many 0 to n fields associated with it in a
 * hierarchical manor. This FormField object governs what/how that takes place
 * 
 * @see org.openmrs.Form
 * @see org.openmrs.Field
 */
public class FormField extends BaseOpenmrsMetadata
		implements
			java.io.Serializable,
			Comparable<FormField> {
	...
	public static class DefaultComparator implements Comparator<FormField> {
		...
		public int compare(FormField ff1, FormField ff2) {
			...
			if (ff1.getField() != null && ff2.getField() != null) {
				int c = ff1.getField().getName()
						.compareTo(ff2.getField().getName());
				if (c != 0) {
					return c;
				}
			}
