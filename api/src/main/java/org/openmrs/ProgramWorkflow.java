...
public class ProgramWorkflow extends BaseOpenmrsMetadata
	...
	public Set<ProgramWorkflowState> getSortedStates() {
		final Comparator<String> naturalComparator = NaturalStrings
		...
		Comparator<ProgramWorkflowState> stateComparator = new Comparator<ProgramWorkflowState>() {
			...
			public int compare(ProgramWorkflowState o1, ProgramWorkflowState o2) {
				return naturalComparator.compare(o1.getConcept().getName()
						.getName(), o2.getConcept().getName().getName());
			}

		};

		Set<ProgramWorkflowState> sorted = new TreeSet<ProgramWorkflowState>(
				stateComparator);
		if (getStates() != null) {
	...
	public List<ProgramWorkflowState> getPossibleNextStates(
		...
		for (ProgramWorkflowState st : getSortedStates()) {
			if (isLegalTransition(
					currentState == null ? null : currentState.getState(), st)) {
				ret.add(st);
			}
		}
		return ret;
