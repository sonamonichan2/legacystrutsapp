...
public class PersonName extends BaseOpenmrsData
	 ...
	 * @param middleName
	 *            String this person's middle name
	 * @param familyName
	 *            String this person's last name
	 */
	 ...
	 * inner fields of each name for equality. Note: Null/empty fields on
	 * <code>otherName</code> /will not/ cause a false value to be returned
	 * 
	 * @param otherName
	...
	public boolean equalsContent(PersonName otherName) {
		return new EqualsBuilder()
				.append(defaultString(otherName.getPrefix()),
						defaultString(prefix))
				.append(defaultString(otherName.getGivenName()),
						defaultString(givenName))
				.append(defaultString(otherName.getMiddleName()),
						defaultString(middleName))
				.append(defaultString(otherName.getFamilyNamePrefix()),
						defaultString(familyNamePrefix))
				.append(defaultString(otherName.getDegree()),
						defaultString(degree))
				.append(defaultString(otherName.getFamilyName()),
						defaultString(familyName))
				.append(defaultString(otherName.getFamilyName2()),
						defaultString(familyName2))
				.append(defaultString(otherName.getFamilyNameSuffix()),
						defaultString(familyNameSuffix)).isEquals();
	}

	/**
	 * bitwise copy of the personName object. NOTICE: THIS WILL NOT COPY THE
	 * PATIENT OBJECT. The PersonName.person object in this object AND the
	...
	public static PersonName newInstance(PersonName pn) {
		...
		PersonName newName = new PersonName(Integer.valueOf(pn
				.getPersonNameId()));
	...
	/**
	 * Convenience method to get all the names of this PersonName and
	 * concatonating them together with spaces in between. If any part of
	 * {@link #getPrefix()}, {@link #getGivenName()}, {@link #getMiddleName()},
	 * etc are null, they are not included in the returned name
	 * 
	...
	public String getFullName() {
		...
		if (OpenmrsConstants.PERSON_NAME_FORMAT_LONG.equals(PersonName
				.getFormat())) {
	 ...
	 * global property because an implementation can define how they want their
	 * names to look (which fields to show/hide)
	 * 
	 ...
	 * @should return negative if other dateCreated is greater Note: this
		...
		public int compare(PersonName pn1, PersonName pn2) {
			...
			if (ret == 0) {
				ret = OpenmrsUtil.compareWithNullAsGreatest(
						pn1.getFamilyName(), pn2.getFamilyName());
			}
			if (ret == 0) {
				ret = OpenmrsUtil.compareWithNullAsGreatest(
			...
			if (ret == 0) {
				ret = OpenmrsUtil.compareWithNullAsGreatest(pn1.getGivenName(),
						pn2.getGivenName());
			}
			if (ret == 0) {
				ret = OpenmrsUtil.compareWithNullAsGreatest(
						pn1.getMiddleName(), pn2.getMiddleName());
			}
			if (ret == 0) {
				ret = OpenmrsUtil.compareWithNullAsGreatest(
			...
			if (ret == 0 && pn1.getDateCreated() != null) {
				ret = OpenmrsUtil.compareWithNullAsLatest(pn1.getDateCreated(),
						pn2.getDateCreated());
			}
