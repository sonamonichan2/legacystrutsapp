...
/**
 * Encapsulates information about the clinical action of a provider requesting
 * something for a patient e.g requesting a test to be performed, prescribing a
 * medication, requesting the patient to enroll on a specific diet etc. There is
 * the notion of effective dates, these are used to determine the span of an
 * order's schedule i.e its effective start and stop dates therefore dates
 * should be interpreted as follows: The effective start of the schedule is the
 * scheduledDate if urgency is set to ON_SCHEDULED_DATE otherwise it is the
 * dateActivated; the effective end date is dateStopped, if it is null then it
 * is the autoExpireDate. For DrugOrders, if the autoExpireDate is not specified
 * then it will be calculated and set by the API based on the duration and
 * frequency, note that frequency is only used in case duration is specified as
 * a recurring interval e.g. 3 times.
 * 
 * @version 1.0
 */
public class Order extends BaseOpenmrsData implements java.io.Serializable {
	...
	/**
	 * Allows the orders if ordered as an orderGroup, to maintain a sequence of
	 * how members are added in the group ex - for two orders of isoniazid and
	 * ampicillin, the sequence of 1 and 2 needed to be maintained
	 */
	private Double sortWeight;

	/**
	 * Allows orders to be linked to a previous order - e.g., an order
	 * discontinue ampicillin linked to the original ampicillin order (the D/C
	 * gets its own order number)
	 */
	...
	/**
	 * The purpose of this method is to allow subclasses of Order to delegate a
	 * portion of their copy() method back to the superclass, in case the base
	 * class implementation changes.
	 * 
	 * @param target
	 *            an Order that will have the state of <code>this</code> copied
	 *            into it
	...
	/**
	 * @param autoExpireDate
	 *            The autoExpireDate to set.
	 */
	...
	/**
	 * @param orderReasonNonCoded
	 *            The orderReasonNonCoded to set.
	 */
	...
	/**
	 * Convenience method to determine if the order is activated as of the
	 * current date
	 * 
	 * @return boolean indicating whether the order was activated before or on
	 *         the current date
	 * @since 2.0
	 ...
	 * 
	 * @param checkDate
	 *            - the date on which to check order. if null, will use current
	 *            date
	 * @return boolean indicating whether the order was activated before or on
	 *         the check date
	 * @since 2.0
	...
	/**
	 * Convenience method to determine if the order was active as of the current
	 * date
	 * 
	 * @since 1.10.1
	...
	/**
	 * Convenience method to determine if the order is active as of the
	 * specified date
	 * 
	 * @param checkDate
	 *            - the date on which to check order. if null, will use current
	 ...
	 * @should return true if an order was activated on the check date
	 * @should return true if an order was activated on the check date but
	...
	public boolean isActive(Date checkDate) {
		...
		return isActivated(checkDate) && !isDiscontinued(checkDate)
				&& !isExpired(checkDate);
	}

	/**
	 * Convenience method to determine if order is started as of the current
	 * date
	 * 
	 * @return boolean indicating whether the order is started as of the current
	 *         date
	 * @since 1.10.1
	...
	/**
	 * Convenience method to determine if the order is started as of the
	 * specified date, returns true only if the order has been activated. In
	 * case of scheduled orders, the scheduledDate becomes the effective start
	 * date that gets used to determined if it is started.
	 * 
	 * @param checkDate
	 *            - the date on which to check order. if null, will use current
	 *            date
	 * @return boolean indicating whether the order is started as of the check
	 *         date
	 * @since 1.10.1
	 * @should return false for a voided order
	 * @should return false if dateActivated is null
	 * @should return false if the order is not yet activated as of the check
	 *         date
	 * @should return false if the order was scheduled to start after the check
	 *         date
	 * @should return true if the order was scheduled to start on the check date
	 * @should return true if the order was scheduled to start before the check
	 *         date
	...
	/**
	 * Convenience method to determine if the order is discontinued as of the
	 * specified date
	 * 
	 * @param checkDate
	 *            - the date on which to check order. if null, will use current
	 *            date
	 * @return boolean indicating whether the order was discontinued on the
	 *         input date
	 * @should return false for a voided order
	 * @should return false if date stopped and auto expire date are both null
	 * @should return false if auto expire date is null and date stopped is
	 *         equal to check date
	 * @should return false if auto expire date is null and date stopped is
	 *         after check date
	 * @should return false if dateActivated is after check date
	 * @should return true if auto expire date is null and date stopped is
	 *         before check date
	 * @should fail if date stopped is after auto expire date
	 * @should return true if check date is after date stopped but before auto
	 *         expire date
	 * @should return true if check date is after both date stopped auto expire
	 *         date
	 * @should return true if the order is scheduled for the future and
	 *         activated on check date but the check date is after date stopped
	 */
	public boolean isDiscontinued(Date checkDate) {
		if (dateStopped != null && autoExpireDate != null
				&& dateStopped.after(autoExpireDate)) {
			throw new APIException(
					"Order.error.invalidDateStoppedAndAutoExpireDate",
					(Object[]) null);
		}
		if (isVoided()) {
			return false;
	...
	/**
	 * Convenience method to determine if the order is expired as of the
	 * specified date
	 * 
	 * @return boolean indicating whether the order is expired at the current
	 *         time
	 * @since 1.10.1
	 */
	 ...
	 *            - the date on which to check order. if null, will use current
	 *            date
	 * @return boolean indicating whether the order was expired on the input
	 *         date
	 * @should return false for a voided order
	 * @should return false if date stopped and auto expire date are both null
	 * @should return false if date stopped is null and auto expire date is
	 *         equal to check date
	 * @should return false if date stopped is null and auto expire date is
	 *         after check date
	 * @should return false if check date is after both date stopped auto expire
	 *         date
	 * @should return false if dateActivated is after check date
	 * @should return false if check date is after date stopped but before auto
	 *         expire date
	 * @should fail if date stopped is after auto expire date
	 * @should return true if date stopped is null and auto expire date is
	 *         before check date
	 * @since 1.10.1
	 */
	...
	/*
	 * orderForm:jsp: <spring:bind path="order.discontinued" /> results in a
	 * call to isDiscontinued() which doesn't give access to the discontinued
	 * property so renamed it to isDiscontinuedRightNow which results in a call
	 * to getDiscontinued.
	 * 
	 * @since 1.5
	 */
	...
	public String toString() {
		...
		return prefix + "Order. orderId: " + orderId + " patient: " + patient
				+ " concept: " + concept + " care setting: " + careSetting;
	}
	 ...
	 * 
	 * @param orderType
	 *            the {@link org.openmrs.OrderType}
	 */
	...
	/**
	 * Creates a discontinuation order for this order, sets the previousOrder
	 * and action fields, note that the discontinuation order needs to be saved
	 * for the discontinuation to take effect
	 * 
	...
	/**
	 * Creates an order for revision from this order, sets the previousOrder and
	 * action field.
	 * 
	 * @return the newly created order
	 * @since 1.10
	...
	/**
	 * The purpose of this method is to allow subclasses of Order to delegate a
	 * portion of their cloneForRevision() method back to the superclass, in
	 * case the base class implementation changes.
	 * 
	 * @param target
	 *            an Order that will have the state of <code>this</code> copied
	 *            into it
	...
	/**
	 * Checks whether this order's orderType matches or is a sub type of the
	 * specified one
	 * 
	 * @since 1.10
	 * @param orderType
	 *            the orderType to match on
	 * @return true if the type of the order matches or is a sub type of the
	 *         other order
	...
	public boolean hasSameOrderableAs(Order otherOrder) {
		...
		return OpenmrsUtil.nullSafeEquals(this.getConcept(),
				otherOrder.getConcept());
	}
	...
	public Date getEffectiveStartDate() {
		return this.urgency == Urgency.ON_SCHEDULED_DATE ? this
	...
	public Date getEffectiveStopDate() {
		return this.getDateStopped() != null ? this.getDateStopped() : this
				.getAutoExpireDate();
	}
	...
	/**
	 * Sets the OrderGroup for that order. If the order is ordered
	 * independently, it does not set an orderGroup for it. If the order is
	 * ordered as an orderGroup, then sets a link to the OrderGroup for that
	 * particular order.
	 * 
	 * @since 1.12
	 * @param orderGroup
	 ...
	 * <tt>sortWeight</tt> is used internally by the API to manage the
	 * sequencing of orders when grouped. This value may be changed by the API
	 * as needed for that purpose. Instead of setting this internal value
