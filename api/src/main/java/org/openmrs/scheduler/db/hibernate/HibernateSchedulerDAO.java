...
public class HibernateSchedulerDAO implements SchedulerDAO {
	...
	private static final Log log = LogFactory
			.getLog(HibernateSchedulerDAO.class);
	...
	public TaskDefinition getTask(Integer taskId) throws DAOException {
		TaskDefinition task = (TaskDefinition) sessionFactory
				.getCurrentSession().get(TaskDefinition.class, taskId);

		if (task == null) {
			log.warn("Task '" + taskId + "' not found");
			throw new ObjectRetrievalFailureException(TaskDefinition.class,
					taskId);
		}
		return task;
	}
	...
	public TaskDefinition getTaskByName(String name) throws DAOException {
		Criteria crit = sessionFactory.getCurrentSession()
				.createCriteria(TaskDefinition.class)
	...
	public List<TaskDefinition> getTasks() throws DAOException {
		return sessionFactory.getCurrentSession()
				.createCriteria(TaskDefinition.class).list();
	}
	 ...
	 * 
	 * @param taskId
	 *            <code>Integer</code> identifier of task to be deleted
	 * @throws DAOException
	 */
	 ...
	 * 
	 * @param taskConfig
	 *            <code>TaskDefinition</code> of task to be deleted
	 * @throws DAOException
	 */
	...
	public Schedule getSchedule(Integer scheduleId) throws DAOException {
		Schedule schedule = (Schedule) sessionFactory.getCurrentSession().get(
				Schedule.class, scheduleId);

		if (schedule == null) {
			log.error("Schedule '" + scheduleId + "' not found");
			throw new ObjectRetrievalFailureException(Schedule.class,
					scheduleId);
		}
		return schedule;
	}
