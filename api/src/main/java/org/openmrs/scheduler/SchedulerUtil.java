...
public class SchedulerUtil {
	 ...
	 * 
	 * @param p
	 *            properties used to start the service
	 */
	...
	/**
	 * Shutdown the scheduler service that is statically associated with the
	 * Context class.
	 */
	public static void shutdown() {
		...
		try {
			Context.addProxyPrivilege(PrivilegeConstants.MANAGE_SCHEDULER);
			// doesn't attempt shutdown if there was an error getting the
			// scheduler service
			if (service != null) {
				service.onShutdown();
			}
	...
	public static void sendSchedulerError(Throwable throwable) {
		try {
			...
			Boolean emailIsEnabled = Boolean
					.valueOf(Context
			...
			if (emailIsEnabled) {
				...
				if (recipients != null) {
					...
					String subject = SchedulerConstants.SCHEDULER_DEFAULT_SUBJECT
							+ " : " + throwable.getClass().getName();
					StringBuilder message = new StringBuilder();
					message.append("\n\nStacktrace\n============================================\n");
					message.append(SchedulerUtil
							.getExceptionAsString(throwable));
					message.append("\n\nSystem Variables\n============================================\n");
					for (Map.Entry<String, String> entry : Context
							.getAdministrationService().getSystemVariables()
							.entrySet()) {
						message.append(entry.getKey()).append(" = ")
								.append(entry.getValue()).append("\n");
					}

					// TODO need to the send the IP information for the server
					// instance that is running this task

					log.debug("Sending scheduler error email to [" + recipients
							+ "] from [" + sender + "] with subject ["
							+ subject + "]:\n" + message);
					Context.getMessageService().sendMessage(recipients, sender,
	...
	/**
	 * Gets the next execution time based on the initial start time (possibly
	 * years ago, depending on when the task was configured in OpenMRS) and the
	 * repeat interval of execution. We need to calculate the
	 * "next execution time" because the scheduled time is most likely in the
	 * past and the JDK timer will run the task X number of times from the start
	 * time until now in order to catch up. The assumption is that this is not
	 * the desired behavior -- we just want to execute the task on its next
	 * execution time. For instance, say we had a scheduled task that ran every
	 * 24 hours at midnight. In the database, the task would likely have a past
	 * start date (e.g. 04/01/2006 12:00am). If we scheduled the task using the
	 * JDK Timer scheduleAtFixedRate(TimerTask task, Date startDate, int
	 * interval) method and passed in the start date above, the JDK Timer would
	 * execute this task once for every day between the start date and today,
	 * which would lead to hundreds of unnecessary (and likely expensive)
	 * executions.
	 * 
	 * @see java.util.Timer
	...
	public static Date getNextExecution(TaskDefinition taskDefinition) {
		...
		try {
			...
			if (firstTime != null) {
				...
				// If the first time is actually in the future, then we use that
				// date/time
				...
				// Calculate time between the first time the process was run and
				// right now (e.g. 3 days, 15 hours)
				...
				// Calculate the time to add to the current time (e.g. 24 hours
				// - 15 hours = 9 hours)
				long additional = ((repeatInterval * 1000) - lastTime);

				nextTime.setTime(new Date(currentTime.getTime() + additional));

				log.debug("The task " + taskDefinition.getName()
						+ " will start at " + nextTime.getTime());
			}
		} catch (Exception e) {
			log.error(
					"Failed to get next execution time for "
							+ taskDefinition.getName(), e);
		}
