...
public class Schedule {
	...
	/**
	 * The date format pattern used to set the start date when start date is
	...
	// public static DEFAULT_DATE_FORMAT = new
	// SimpleDateFormat("MM-dd-yyyy hh:mm:ss");
	public static final int DAILY = MILLISECONDS_PER_SECOND
	 ...
	 * 
	 * @param startTime
	 *            Date for when to start the task (does not need to be in the
	 *            future if the interval is specified).
	 * @param repeatInterval
	 *            interval time in seconds to wait between executing task (&lt;=
	 *            0 indicates that it should only be run once)
	 */
	 ...
	 *            0 indicates that it should only be run once)
	 */
	public Schedule(String name, String description, Date startTime,
			long repeatInterval) {
		this.name = name;
	 ...
	 */
	/*
	 * public void setStartTime(String startTime) { try { this.startTime =
	 * this.dateFormatter.parse(startTime); } catch (Exception e) { //If there's
	 * an error, we'll just set the start time to now. // TODO: This might not
	 * be the desired behavior, so I'll have to come back to it. this.startTime
	 * = new Date(); } }
	 */
