...
/**
 * This class executes the Task.initialize method in a new thread. Extend this
 * class if you want your {@link #initialize(TaskDefinition)} method to run in a
 * new thread (and hence not hold up the "startup" processes)
 */
public class TaskThreadedInitializationWrapper implements Task {

	// Logger
	private Log log = LogFactory
			.getLog(TaskThreadedInitializationWrapper.class);
	...
	/**
	 * @see org.openmrs.scheduler.Task#execute() Executes the task defined in
	 *      the task definition but waits until the initialize method has
	 *      finished
	 */
	public void execute() {
		lock.lock();
	 ...
	 *      Initializes the task and sets the task definition. This method is
	 *      non-blocking by executing in a new thread.
	 */
