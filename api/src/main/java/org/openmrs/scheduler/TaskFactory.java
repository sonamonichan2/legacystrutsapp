...
public class TaskFactory {
	...
	/**
	 * Creates a new instance of Schedulable used to run tasks. By default the
	 * returned task will be the given task wrapped with the
	 * {@link TaskThreadedInitializationWrapper} class so that the
	...
	public Task createInstance(TaskDefinition taskDefinition)
		...
		try {
			...
			Class<?> taskClass = OpenmrsClassLoader.getInstance().loadClass(
					taskDefinition.getTaskClass());

			// Create a new instance of the schedulable class
			Task task = new TaskThreadedInitializationWrapper(
			...
			log.error("OpenmrsClassLoader could not load class: "
					+ taskDefinition.getTaskClass()
			...
			if (log.isDebugEnabled()) {
				// don't need to log errors here necessarily. If its needed, the
				// calling method can log it.
				log.debug(
						"Error creating new task for class "
								+ taskDefinition.getTaskClass(), e);
			}

			throw new SchedulerException("error creating new task for class "
					+ taskDefinition.getTaskClass(), e);
		}
	}
}
