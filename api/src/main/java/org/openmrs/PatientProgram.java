...
public class PatientProgram extends BaseOpenmrsData
	 ...
	 * patientProgramId. The 'states' property will be deep-copied.
	 * 
	 ...
	 * 
	 * @param target
	 *            a PatientProgram that will have the state of <code>this</code>
	 *            copied into it
	...
	/**
	 * Returns true if the associated {@link Patient} is enrolled in the
	 * associated {@link Program} on the passed {@link Date}
	 * 
	 * @param onDate
	 *            - Date to check for PatientProgram enrollment
	 * @return boolean - true if the associated {@link Patient} is enrolled in
	 *         the associated {@link Program} on the passed {@link Date}
	 */
	public boolean getActive(Date onDate) {
		...
		return !getVoided()
				&& (getDateEnrolled() == null || OpenmrsUtil.compare(
						getDateEnrolled(), onDate) <= 0)
				&& (getDateCompleted() == null || OpenmrsUtil.compare(
						getDateCompleted(), onDate) > 0);
	}

	/**
	 * Returns true if the associated {@link Patient} is currently enrolled in
	 * the associated {@link Program}
	 * 
	 * @return boolean - true if the associated {@link Patient} is currently
	 *         enrolled in the associated {@link Program}
	 */
	...
	/**
	 * Returns the {@link PatientState} associated with this PatientProgram that
	 * has an id that matches the passed <code>patientStateId</code>
	 * 
	 * @param patientStateId
	 *            - The identifier to use to lookup a {@link PatientState}
	...
	public PatientState getPatientState(Integer patientStateId) {
		for (PatientState s : getStates()) {
			if (s.getPatientStateId() != null
					&& s.getPatientStateId().equals(patientStateId)) {
				return s;
			}
		}
		return null;
	 ...
	 * {@link ProgramWorkflowState} on the passed {@link Date} by ending the
	 * most recent {@link PatientState} in the {@link PatientProgram} and
	 * creating a new one with the passed {@link ProgramWorkflowState} This will
	 * throw an IllegalArgumentException if the transition is invalid
	 * 
	 * @param programWorkflowState
	 *            - The {@link ProgramWorkflowState} to transition to
	 * @param onDate
	 *            - The {@link Date} of the transition
	 * @throws IllegalArgumentException
	 */
	public void transitionToState(ProgramWorkflowState programWorkflowState,
			Date onDate) {
		PatientState lastState = getCurrentState(programWorkflowState
				.getProgramWorkflow());
		...
		if (lastState != null
				&& !programWorkflowState.getProgramWorkflow()
						.isLegalTransition(lastState.getState(),
								programWorkflowState)) {
			throw new IllegalArgumentException("You can't change from state "
					+ lastState.getState() + " to " + programWorkflowState);
		}
		if (lastState != null) {
	 ...
	 * {@link PatientProgram} If earlier PatientStates exist, it will try to
	 * reset the endDate to null so that the next latest state becomes the
	 * current {@link PatientState}
	 * 
	 * @param workflow
	 *            - The {@link ProgramWorkflow} whose last {@link PatientState}
	 *            within the current {@link PatientProgram} we want to void
	 * @param voidBy
	 *            - The user who is voiding the {@link PatientState}
	 * @param voidDate
	 *            - The date to void the {@link PatientState}
	 * @param voidReason
	 *            - The reason for voiding the {@link PatientState}
	 * @should void state with endDate null if startDates equal
	 */
	public void voidLastState(ProgramWorkflow workflow, User voidBy,
			Date voidDate, String voidReason) {
	 ...
	 * {@link ProgramWorkflow} within this {@link PatientProgram}.
	 * 
	 * @param programWorkflow
	 *            The ProgramWorkflow whose current {@link PatientState} we want
	 *            to retrieve
	...
	public PatientState getCurrentState(ProgramWorkflow programWorkflow) {
		...
		for (PatientState state : getSortedStates()) {
			// states are sorted with the most current state at the last
			// position
			if ((programWorkflow == null || state.getState()
	...
	/**
	 * Returns a Set&lt;PatientState&gt; of all current {@link PatientState}s
	 * for the {@link PatientProgram}
	 * 
	 * @return Set&lt;PatientState&gt; of all current {@link PatientState}s for
	 *         the {@link PatientProgram}
	 */
	...
	/**
	 * Returns a List&lt;PatientState&gt; of all {@link PatientState}s in the
	 * passed {@link ProgramWorkflow} for the {@link PatientProgram}
	 * 
	 * @param programWorkflow
	 *            - The {@link ProgramWorkflow} to check
	 * @param includeVoided
	 *            - If true, return voided {@link PatientState}s in the returned
	 *            {@link List}
	 * @return List&lt;PatientState&gt; of all {@link PatientState}s in the
	 *         passed {@link ProgramWorkflow} for the {@link PatientProgram}
	 */
	public List<PatientState> statesInWorkflow(ProgramWorkflow programWorkflow,
	...
	public String toString() {
		return "PatientProgram(id=" + getPatientProgramId() + ", patient="
				+ getPatient() + ", program=" + getProgram() + ")";
	}
	...
	private List<PatientState> getSortedStates() {
		List<PatientState> sortedStates = new ArrayList<PatientState>(
				getStates());
