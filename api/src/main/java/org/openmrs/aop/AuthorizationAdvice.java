...
public class AuthorizationAdvice implements MethodBeforeAdvice {
	...
	public void before(Method method, Object[] args, Object target)
		...
		if (!privileges.isEmpty()) {
			for (String privilege : privileges) {
				...
				if (log.isDebugEnabled()) {
					log.debug("User has privilege " + privilege + "? "
							+ Context.hasPrivilege(privilege));
				}
				...
				if (Context.hasPrivilege(privilege)) {
					...
					if (requireAll) {
						...
						// to "fail"
						throwUnauthorized(Context.getAuthenticatedUser(),
								method, privilege);
					}
				}
			}
	...
	private void throwUnauthorized(User user, Method method,
		...
		throw new APIAuthenticationException(Context.getMessageSourceService()
	...
	private void throwUnauthorized(User user, Method method, String attr) {
		...
		throw new APIAuthenticationException(Context.getMessageSourceService()
				.getMessage("error.privilegesRequired", new Object[]{attr},
						null));
	}
	...
	private void throwUnauthorized(User user, Method method) {
		...
		throw new APIAuthenticationException(Context.getMessageSourceService()
				.getMessage("error.aunthenticationRequired"));
	}
}
