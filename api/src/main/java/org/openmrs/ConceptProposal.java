 ...
 * system. When defining an observation, a user can "propose" a new concept if
 * one isn't found already. The proposal is a simple text entry that will be
 * reviewed later. When a proposal is (edited and) accepted, the encounter that
 * prompted this proposal is updated with a new observation pointing at the new
 * (or edited) concept.
 */
public class ConceptProposal extends BaseOpenmrsObject
	 ...
	 * Context.getConceptService().saveConceptProposal(/thisObject/) after
	 * calling this method
	 */
