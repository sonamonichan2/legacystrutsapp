...
public class ConceptSetsEditor extends PropertyEditorSupport {
	 ...
	 * 
	 * @param conceptSets
	 *            the current object on the concept
	 */
	...
	public void setAsText(String text) throws IllegalArgumentException {
		...
		if (StringUtils.hasText(text)) {
			...
			for (String id : conceptIds) {
				id = id.trim();
				if (!id.equals("")
						&& !requestConceptIds.contains(Integer.valueOf(id))) { // remove
																				// whitespace,
																				// blank
																				// lines,
																				// and
			...
			List<Integer> originalConceptSetIds = new ArrayList<Integer>(
					originalConceptSets.size());

			// remove all sets that aren't in the request (aka, that have been
			// deleted by the user)
			Collection<ConceptSet> copyOfOriginalConceptSets = new ArrayList<ConceptSet>(
					originalConceptSets);
			for (ConceptSet origConceptSet : copyOfOriginalConceptSets) {
				if (!requestConceptIds.contains(origConceptSet.getConcept()
						.getConceptId())) {
					originalConceptSets.remove(origConceptSet);
				}

				// add to quick list used when adding later
				originalConceptSetIds.add(origConceptSet.getConcept()
						.getConceptId());
			}

			// insert all sets that are new (aka, that have been added by the
			// user).
			...
			for (int x = 0; x < requestConceptIds.size(); x++) {
				...
				if (!originalConceptSetIds.contains(requestConceptId)) {
					// the null weight will be reset in the next step of
					// normalization
					originalConceptSets.add(new ConceptSet(cs
							.getConcept(requestConceptId), new Double(x)));
				} else {
					// find this conceptId in the original set and set its
					// weight
					for (ConceptSet conceptSet : originalConceptSets) {
						if (conceptSet.getConcept().getConceptId()
								.equals(requestConceptId)) {
