...
public class ConceptAnswersEditor extends PropertyEditorSupport {
	...
	/**
	 * Default constructor taking in the original answers. This should be the
	 * actual list on the pojo object to prevent hibernate errors later on.
	 * 
	 * @param originalAnswers
	 *            the list on the pojo
	 */
	...
	/**
	 * loops over the textbox assigned to this property. The textbox is assumed
	 * to be a string of conceptIds^drugIds separated by spaces.
	 * 
	 * @param text
	...
	public void setAsText(String text) throws IllegalArgumentException {
		if (StringUtils.hasText(text)) {
			...
			// set up parameter answer Set for easier add/delete functions and
			// removal of duplicates
			for (String id : conceptIds) {
				id = id.trim();
				if (!id.equals("") && !requestConceptIds.contains(id)) { // remove
																			// whitespace,
																			// blank
																			// lines,
																			// and
			...
			for (ConceptAnswer origConceptAnswer : originalConceptAnswers) {
				...
				for (String conceptId : requestConceptIds) {
					...
					if (id.equals(origConceptAnswer.getAnswerConcept()
							.getConceptId())) {
						if (drugId == null && answerDrug == null) {
							answerDeleted = false;
						} else if ((drugId != null && answerDrug != null)
								&& drugId.equals(origConceptAnswer
										.getAnswerDrug().getDrugId())) {
							answerDeleted = false;
						}
					}
				}
			...
			// as the list comes into 'requestConceptIds' in the order the user
			// wants
			// there are 2 conditions that will require the sort_weights to be
			// reassigned
			// 1) any ConceptAnswer.sortWeight == NULL (meaning it is just
			// added)
			// 2) the list is not in ASCENDING order (example sort order of the
			// list is 1, 2, 10, 9)
			// -startIdx (start index) is where in this list we will start to
			// reassign the sort_weights
			Double lastWeightSeen = null;
			int startIdx = -1;// the idx to start at, if we have a NULL sort
								// weight (new concept answer) or sort weights
								// are not ascending
	...
	/**
	 * Parses the string and returns the Integer concept id Expected string:
	 * "123" or "123^34" ("conceptId^drugId")
	 * 
	 * @param conceptId
	 * @return
	 */
	private Integer getConceptId(String conceptId) {
		if (conceptId.contains("^")) {
			return Integer.valueOf(conceptId.substring(0,
					conceptId.indexOf("^")));
		} else {
	...
	private Integer getDrugId(String conceptId) {
		if (conceptId.contains("^")) {
			return Integer.valueOf(conceptId.substring(
					conceptId.indexOf("^") + 1, conceptId.length()));
		}
