...
/**
 * Allows for serializing/deserializing a Person object to a string so that
 * Spring knows how to pass a Person back and forth through an html form or
 * other medium. <br>
 * <br>
 * In version 1.9, added ability for this to also retrieve Person objects by
 * uuid
 * 
 * @see Person
 */
	...
	public void setAsText(String text) throws IllegalArgumentException {
		...
		if (StringUtils.hasText(text)) {
			try {
				...
				setValue(person);
				if (person == null) {
					log.trace("Unable to get Person by primary key or uuid using input: "
							+ text);
				}
			}
		} else {
