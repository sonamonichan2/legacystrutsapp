...
/**
 * The Logic Service provides a mechanism for both registering and consuming
 * business logic in the form of logic <em>rules</em>. Rules may be run against
 * a single patient or a set of patients. Rules are registered under a unique
 * string <em>token</em>. Later evaluation and/or retrieval of the rule is done
 * through the token. Tokens can be tagged with any number of string
 * word/phrases to simplify organization and lookup of tokens. Data source
 * results can be obtained directly by using a token in the form
 * <em>@foo.bar</em>, where <em>foo</em> is the logic data source name and
 * <em>bar</em> is the key for that data source. For example, the token
 * <em>@person.gender</em> is a direct reference to the <em>gender</em> key of
 * the <em>person</em> logic data source. <h3>Example Usage</h3>
 * 
 * <pre>
...
public interface LogicService {
	 ...
	 * 
	 * @param token
	 *            the lookup key ("token") for this rule
	 * @param rule
	...
	/**
	 * Registers a new rule with the logic service, associating the tags with
	 * the given token
	 * 
	 * @param token
	 *            the unique lookup key ("token") for this rule
	 * @param tags
	 *            words or phrases associated with this token (do not need to be
	 *            unique)
	 * @param rule
	 *            new rule to be registered
	 * @throws LogicException
	 ...
	 * 
	 * @param token
	 *            lookup key ("token") for the rule to be updated
	 * @param rule
	 ...
	 * 
	 * @param token
	 *            lookup key ("token") under which rule to be removed is
	 *            registered
	 * @throws LogicException
	 * @should remove rule
	 */
	 ...
	 * 
	 * @param patientId
	 *            patient for whom the rule is to be calculated
	 * @param expression
	 ...
	 */
	public Result eval(Integer patientId, String expression)
			throws LogicException;

	/**
	 * Evaluates a rule for a given patient, given a token and parameters for
	 * the rule.
	 * 
	 * @param patientId
	 *            patient for whom the rule is to be calculated
	 ...
	 * 
	 * @param patientId
	 *            patient for whom the query is to be run
	 * @param criteria
	 *            question to be answered (along with the token) for the given
	 *            patient
	 * @return result of query
	 * @throws LogicException
	 * @since 1.6.3, 1.7.2, and 1.8
	 */
	public Result eval(Integer patientId, LogicCriteria criteria)
			throws LogicException;
	 ...
	 * @param criteria
	 *            <code>Criteria</code> question to be answered (along with the
	 *            token) for the given patient
	 * @param parameters
	...
	/**
	 * Evaluates multiple logic expressions for a single patient. (The
	 * expressions argument is an array and comes last because using a List
	 * would give this method the same type erasure as the {@link LogicCriteria}
	 * ... version.)
	 * 
	 * @param patientId
	 *            which patient to run the rules on
	 * @param parameters
	 *            global parameters to be passed to all rule evaluations
	 * @param expressions
	 ...
	 * @since 1.6.3, 1.7.2, and 1.8
	 */
	public Map<String, Result> eval(Integer patientId,
			Map<String, Object> parameters, String... expressions)
			throws LogicException;

	/**
	 * Evaluates multiple {@link LogicCriteria} for a single patient. (The
	 * criteria argument is an array and comes last because using a List would
	 * give this method the same type erasure as the {@link String}... version.)
	 * 
	 * @param patientId
	 *            which patient to run the rules on
	 * @param parameters
	 ...
	 * @since 1.6.3, 1.7.2, and 1.8
	 */
	public Map<LogicCriteria, Result> eval(Integer patientId,
			Map<String, Object> parameters, LogicCriteria... criteria)
			throws LogicException;
	 ...
	 * 
	 * @param who
	 *            patients for whom the query is to be run
	 * @param expression
	 ...
	 * @see #parse(String)
	 */
	public Map<Integer, Result> eval(Cohort who, String expression)
			throws LogicException;
	 ...
	 *            patients for whom the query is to be run
	 * @param criteria
	 *            question to be answered (along with the token) for each
	 *            patient
	 * @return result for each patient
	 * @throws LogicException
	 */
	 ...
	 * 
	 * @param who
	 *            patients for whom the query is to run
	 * @param criteria
	 *            question to be answered (along with the token) for each
	 *            patient
	 * @param parameters
	...
	public Map<LogicCriteria, Map<Integer, Result>> eval(Cohort who,
	...
	/**
	 * Fetches the default datatype this token will return when fed to an eval()
	 * call. Results (returned by the logic service) are loosely typed by
	 * design; however, the default datatype can be a useful hint for managing
	 * user interfaces or providing default behavior when working with rules.
	 * 
	 * @param token
	 ...
	 * @return the logic data source with the given name or <code>null</code> if
	 *         there is no data source registered under the given name (must be
	 *         an exact match)
	 */
	 ...
	 * @return LogicCriteria using all possible operand and operator from the
	 *         String input
	 */
