...
/**
 * Abstract handler for some convenience methods Files are stored in the
 * location specified by the global property: "obs.complex_obs_dir"
 * 
 * @since 1.5
 */
public class AbstractHandler {
	...
	/**
	 * Returns a {@link File} for the given obs complex data to be written to.
	 * The output file location is determined off of the
	 * {@link OpenmrsConstants#GLOBAL_PROPERTY_COMPLEX_OBS_DIR} and the file
	 * name is determined off the current obs.getComplexData().getTitle().
	 * 
	 * @param obs
	...
	public File getOutputFileToWrite(Obs obs) throws IOException {
		...
		File dir = OpenmrsUtil.getDirectoryInApplicationDataDirectory(Context
		...
		while (obs.getObsId() == null && outputfile.exists() && i < 100) {
			...
			tmp = String.valueOf(outputfile.getAbsolutePath().replace(
					"." + extension, ""));
			outputfile = null;
			// Append two-digit count number to the filename.
			String filename = (i < 1) ? tmp + "_"
					+ nf.format(Integer.valueOf(++i)) : tmp.replace(
	...
	public String getExtension(String filename) {
		...
		String extension = (filenameParts.length < 2)
				? filenameParts[0]
	...
	public Obs getObs(Obs obs, String view) {
		...
		try {
			complexData = new ComplexData(file.getName(),
	...
	public boolean purgeComplexData(Obs obs) {
		...
		log.warn("Could not delete complex data object for obsId="
				+ obs.getObsId() + " located at " + file.getAbsolutePath());
		return false;
	}
