 ...
 * mime type will be used to save the image. Images are stored in the location
...
public class ImageHandler extends AbstractHandler implements ComplexObsHandler {
	...
	/**
	 * Currently supports all views and puts the Image file data into the
	 * ComplexData object
	 * 
	...
	public Obs getObs(Obs obs, String view) {
		...
		if (ComplexObsHandler.RAW_VIEW.equals(view)) {
			...
			try {
				...
				Iterator<ImageReader> imgReader = ImageIO
						.getImageReaders(imgStream);
				imgStream.close();
				if (imgReader.hasNext()) {
					complexData.setMimeType("image/"
							+ imgReader.next().getFormatName().toLowerCase());
				} else {
					log.warn("MIME type of " + file.getAbsolutePath()
							+ " is not known");
				}
			} catch (FileNotFoundException e) {
				log.error("Image " + file.getAbsolutePath() + " was not found",
						e);
			} catch (IOException e) {
				log.error(
						"Trying to determine MIME type of "
	...
	public Obs saveObs(Obs obs) throws APIException {
		...
		if (data instanceof BufferedImage) {
			...
			try {
				...
				throw new APIException("Obs.error.unable.convert.complex.data",
		...
		try {
			...
			// TODO: Check this extension against the registered extensions for
			// validity
			...
			if (outfile != null && outfile.length() == 0) {
				outfile.delete(); // OpenJDK 7 & 8 may leave a 0-byte file when
									// ImageIO.write(..) fails.
			}
