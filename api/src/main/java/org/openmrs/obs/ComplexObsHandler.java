 ...
 * complex -- where Obs.isComplex() returns true. <br>
 * <br>
 * These handler classes are delegated to by the ObsService and should never be
 * called directly. <br>
 * <br>
 * Use case:
 * 
 * <pre>
 * Obs complexObs = Context.getObsService().getComplexObs(123,
 * 		OpenmrsConstants.RAW_VIEW);
...
public interface ComplexObsHandler {
	 ...
	 * Obs.value_complex, attaches ComplexData onto the Obs and returns the Obs. <br>
	 * The ComplexData is returned in the format specified by the view (which
	 * can be null). <br>
	 * This view is typically a contract between the view and the handler that
	 * has been registered, so they those two know the types of views that can
	 * be handled.
	 * 
	 * @param obs
	 *            an obs without complex data filled in
	 * @param view
	 ...
	 * 
	 * @param view
	 *            view type defined by UI and view/handler
