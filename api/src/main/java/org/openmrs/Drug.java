...
public class Drug extends BaseOpenmrsMetadata implements java.io.Serializable {
	...
	/**
	 * Convenience method that returns a display name for the drug, defaults to
	 * drug.name
	 * 
	...
	/**
	 * @param drugReferenceMaps
	 *            The drugReferenceMaps to set.
	 * @since 1.10
	 */
	...
	/**
	 * Add the given DrugReferenceMap object to this drug's list of drug
	 * reference mappings. If there is already a corresponding DrugReferenceMap
	 * object for this concept, this one will not be added.
	 * 
	...
	public void addDrugReferenceMap(DrugReferenceMap drugReferenceMap) {
		if (drugReferenceMap != null
				&& !getDrugReferenceMaps().contains(drugReferenceMap)) {
			drugReferenceMap.setDrug(this);
			if (drugReferenceMap.getConceptMapType() == null) {
				drugReferenceMap.setConceptMapType(Context.getConceptService()
