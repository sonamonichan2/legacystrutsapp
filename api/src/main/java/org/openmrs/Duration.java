...
public class Duration {
	 ...
	 * @param startDate
	 * @param frequency
	 *            is used to calculate time to be added to startDate when
	 *            duration unit is 'Recurring Interval'
		...
		if (SNOMED_CT_RECURRING_INTERVAL_CODE.equals(code)) {
			if (frequency == null)
				throw new APIException("Duration.error.frequency.null",
						(Object[]) null);
			return addSeconds(startDate,
					(int) (this.duration * SECONDS_PER_DAY / frequency
							.getFrequencyPerDay()));
		} else {
	...
	/**
	 * Returns concept reference term code of the mapping to the SNOMED CT
	 * concept source
	 * 
	 * @param durationUnits
	 * @return a string which is reference term code
	 * @should return null if the concept has no mapping to the SNOMED CT source
	 * @should return the code for the term of the mapping to the SNOMED CT
	 *         source
	 */
	public static String getCode(Concept durationUnits) {
		for (ConceptMap conceptMapping : durationUnits.getConceptMappings()) {
			ConceptReferenceTerm conceptReferenceTerm = conceptMapping
					.getConceptReferenceTerm();
			if (ConceptMapType.SAME_AS_MAP_TYPE_UUID.equals(conceptMapping
					.getConceptMapType().getUuid())
					&& Duration.SNOMED_CT_CONCEPT_SOURCE_HL7_CODE
							.equals(conceptReferenceTerm.getConceptSource()
									.getHl7Code())) {
