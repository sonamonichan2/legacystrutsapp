 ...
 * need return values are defined it can be used to extend any point. A module
 * can contain many extensions for many different points.
 */
...
public abstract class Extension {
	...
	/**
	 * If multiple extensions are added to the same extension point, set the
	 * order of those extensions by overriding this property. Lower order
	 * numbers will generally appear first within the extension point.
	 * 
	 * @return 0
	 */
	public int getOrder() {
	 ...
	 * id is usually pointid|mediaType if mediatype is null, extension id is
	 * just point id
	 * 
	 * @param pointId
	 * @param mediaType
	...
	public static final String toExtensionId(String pointId,
		...
		if (mediaType != null) {
			return new StringBuffer(pointId)
					.append(Extension.extensionIdSeparator).append(mediaType)
					.toString();
		} else {
			return pointId;
		}
