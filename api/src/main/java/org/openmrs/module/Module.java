...
public final class Module {
	 ...
	 * @param version
	 */
	public Module(String name, String moduleId, String packageName,
	...
	public ModuleActivator getModuleActivator() {
		try {
			if (moduleActivator == null) {
				ModuleClassLoader classLoader = ModuleFactory
						.getModuleClassLoader(this);
				if (classLoader == null) {
					throw new ModuleException("The classloader is null",
							getModuleId());
				}
			...
			throw new ModuleException(
					"Unable to load/access moduleActivator: '"
							+ getActivatorName() + "'", name, e);
		} catch (InstantiationException e) {
			throw new ModuleException(
					"Unable to load/instantiate moduleActivator: '"
							+ getActivatorName() + "'", name, e);
	...
	public List<String> getRequiredModules() {
		return requiredModulesMap == null ? null : new ArrayList<String>(
				requiredModulesMap.keySet());
	}
	...
	public String getRequiredModuleVersion(String moduleName) {
		return requiredModulesMap == null ? null : requiredModulesMap
				.get(moduleName);
	}
	 ...
	 *            the requiredModule to add for this module
	 * @param version
	 ...
	 *            the startedBefore modules to set
	 */
	public void setStartBeforeModulesMap(
			Map<String, String> startBeforeModulesMap) {
	...
	public List<String> getStartBeforeModules() {
		return this.startBeforeModulesMap == null
				? null
				: new ArrayList<String>(this.startBeforeModulesMap.keySet());
	}
	...
	public List<String> getAwareOfModules() {
		return awareOfModulesMap == null ? null : new ArrayList<String>(
				awareOfModulesMap.keySet());
	}

	public String getAwareOfModuleVersion(String awareOfModule) {
		return awareOfModulesMap == null ? null : awareOfModulesMap
				.get(awareOfModule);
	}
	...
	public void setExtensionNames(IdentityHashMap<String, String> map) {
		if (log.isDebugEnabled()) {
			for (Map.Entry<String, String> entry : extensionNames.entrySet()) {
				log.debug("Setting extension names: " + entry.getKey() + " : "
						+ entry.getValue());
			}
		}
	...
	private List<Extension> expandExtensionNames() {
		ModuleClassLoader moduleClsLoader = ModuleFactory
				.getModuleClassLoader(this);
		if (moduleClsLoader == null) {
			log.debug(String
					.format("Module class loader is not available, maybe the module %s is stopped/stopping",
							getName()));
		} else if (extensions.size() != extensionNames.size()) {
			for (Map.Entry<String, String> entry : extensionNames.entrySet()) {
				String point = entry.getKey();
				String className = entry.getValue();
				log.debug("expanding extension names: " + point + " : "
						+ className);
				try {
					...
					extensions.add(ext);
					log.debug("Added extension: " + ext.getExtensionId()
							+ " : " + ext.getClass());
				} catch (NoClassDefFoundError e) {
					log.warn(
							getModuleId()
									+ ": Unable to find class definition for extension: "
									+ point, e);
				} catch (ClassNotFoundException e) {
	 ...
	 * @deprecated as of 2.0 because messages are automatically loaded from the
	 *             classpath
	 */
	@Deprecated
	...
	public boolean isCoreModule() {
		return !ModuleUtil.ignoreCoreModules()
	...
	public void setStartupErrorMessage(String e) {
		if (e == null) {
			throw new ModuleException("Startup error message cannot be null",
					this.getModuleId());
		}
	...
	public void setStartupErrorMessage(String exceptionMessage, Throwable t) {
		if (t == null) {
			throw new ModuleException("Startup error value cannot be null",
					this.getModuleId());
		}
	...
	public void setConditionalResources(
			List<ModuleConditionalResource> conditionalResources) {
