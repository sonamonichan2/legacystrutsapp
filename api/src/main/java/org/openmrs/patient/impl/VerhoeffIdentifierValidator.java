...
/**
 * The Verhoeff Check Digit Validator catches all single errors and all adjacent
 * transpositions. See: http://www.cs.utsa.edu/~wagner/laws/verhoeff.html and
 * Wagner, Neal. "Verhoeff's Decimal Error Detection". The Laws of Cryptography
 * with Java Code. p 54. San Antonio, TX: 2003.
...
public class VerhoeffIdentifierValidator
	...
	protected int getCheckDigit(String undecoratedIdentifier) {
		int[] a = getBase(Integer.parseInt(undecoratedIdentifier),
				undecoratedIdentifier.length());
		insertCheck(a);
		return a[0];
	}

	/**
	 * Override to disallow numeric check digits and identifiers that are not
	 * exactly VERHOEFF_ID_LENGTH long.
	 * 
	...
	public boolean isValid(String identifier)
		...
		if (Character.isDigit(identifier.charAt(identifier.length() - 1))) {
			throw new UnallowedIdentifierException(
					"Check digit can not be numeric.");
		}
	 ...
	 * VERHOEFF_UNDECORATED_ID_LENGTH long.
	 * 
