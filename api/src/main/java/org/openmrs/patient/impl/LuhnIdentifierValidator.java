...
/**
 * A IdentifierValidator based on the Regenstrief Institute's version of the
 * Luhn Algorithm.
 */
public class LuhnIdentifierValidator extends BaseHyphenatedIdentifierValidator {
	...
	protected int getCheckDigit(String undecoratedIdentifier) {
		...
		String trimmedUppercaseUndecoratedIdentifier = undecoratedIdentifier
				.trim().toUpperCase();
		...
		for (int i = 0; i < trimmedUppercaseUndecoratedIdentifier.length(); i++) {
			...
			char ch = trimmedUppercaseUndecoratedIdentifier
					.charAt(trimmedUppercaseUndecoratedIdentifier.length() - i
							- 1);
