...
public class ConceptSet extends BaseOpenmrsObject
	...
	public int compareTo(ConceptSet cs) {
		int value = OpenmrsUtil.compareWithNullAsLowest(concept.isRetired(),
				cs.concept.isRetired());
		if (value == 0) {
			value = OpenmrsUtil.compareWithNullAsLowest(this.getSortWeight(),
					cs.getSortWeight());
		}
		return value;
	}
