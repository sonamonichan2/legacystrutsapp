...
public class CreateCodedOrderFrequencyForDrugOrderFrequencyChangeset
		implements
			CustomTaskChange {
	...
	private void insertUniqueFrequencies(JdbcConnection connection,
		...
		try {
			...
			insertOrderFrequencyStatement = connection
					.prepareStatement("insert into order_frequency "
			...
			for (String frequency : uniqueFrequencies) {
				...
				if (orderFrequencyId != null) {
					// a single concept is mapped to more than one text or there
					// is an order frequency already
					continue;
				}

				// Generating UUID for order frequency. Generated UUIDs will be
				// the same if concepts UUIDs are the same.
				String uuid = UpgradeUtil.getConceptUuid(
						connection.getUnderlyingConnection(),
						conceptIdForFrequency);
				uuid += "-6925ebb0-7c69-11e3-baa7-0800200c9a66"; // Adding
																	// random
																	// value for
																	// order
																	// frequency
