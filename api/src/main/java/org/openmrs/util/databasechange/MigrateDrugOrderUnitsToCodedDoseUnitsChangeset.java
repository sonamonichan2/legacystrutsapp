...
public class MigrateDrugOrderUnitsToCodedDoseUnitsChangeset
		implements
			CustomTaskChange {
	...
	public void execute(Database database) throws CustomChangeException {
		...
		try {
			Set<String> uniqueUnits = DatabaseUtil
					.getUniqueNonNullColumnValues("units", "drug_order",
	...
	private void migrateUnitsToCodedValue(JdbcConnection connection,
			Set<String> uniqueUnits) throws CustomChangeException,
		...
		try {
			...
			for (String unit : uniqueUnits) {
				if (StringUtils.isBlank(unit)) {
					...
					Integer conceptIdForUnit = UpgradeUtil
							.getConceptIdForUnits(unit);
					if (conceptIdForUnit == null) {
						throw new CustomChangeException(
								"No concept mapping found for unit: " + unit);
					}
					String dosingUnitsConceptSetUuid = UpgradeUtil
							.getGlobalProperty(
