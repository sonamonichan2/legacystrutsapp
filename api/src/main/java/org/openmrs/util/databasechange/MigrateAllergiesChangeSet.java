...
/**
 * Moves un voided allergies from the old active_list and active_list_allergy
 * tables to the new allergy and allergy_recation tables
 */
public class MigrateAllergiesChangeSet implements CustomTaskChange {
	...
	public void execute(Database database) throws CustomChangeException {
		try {
			...
			sql = "insert into allergy (patient_id, coded_allergen, severity_concept_id, creator, date_created, uuid, comment, allergen_type) "
					+ "values(?,?,?,?,?,?,?,?)";
			PreparedStatement allergyInsertStatement = connection
					.prepareStatement(sql);

			sql = "insert into allergy_reaction (allergy_id, reaction_concept_id, uuid) "
					+ "values (?,?,?)";
			PreparedStatement reactionInsertStatement = connection
					.prepareStatement(sql);
			...
			while (rs.next()) {
				...
				reactionInsertStatement.setInt(2,
						rs.getInt("reaction_concept_id"));
				reactionInsertStatement.setString(3, UUID.randomUUID()
						.toString());
	...
	private void loadSeverityConcepts(Database database) throws Exception {
		mildConcept = getConceptByGlobalProperty(database,
				"allergy.concept.severity.mild");
		moderateConcept = getConceptByGlobalProperty(database,
				"allergy.concept.severity.moderate");
		severeConcept = getConceptByGlobalProperty(database,
				"allergy.concept.severity.severe");
	}

	private Integer getConceptByGlobalProperty(Database database,
			String globalPropertyName) throws Exception {
		...
		if (rs.next()) {
			...
			rs = stmt
					.executeQuery("SELECT concept_id FROM concept WHERE uuid = '"
							+ uuid + "'");
			if (rs.next()) {
