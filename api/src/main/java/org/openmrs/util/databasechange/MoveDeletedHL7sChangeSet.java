...
/**
 * This change set moves "deleted" HL7s from the archive table to the queue
 * table
 */
public class MoveDeletedHL7sChangeSet implements CustomTaskChange {

	protected final static Log log = LogFactory
			.getLog(MoveDeletedHL7sChangeSet.class);
	...
	public void execute(Database database) throws CustomChangeException {
		...
		insertHL7Sql
				.append(" (hl7_source, hl7_source_key, hl7_data, date_created, uuid, message_state)");
		...
		try {
			insertStatement = connection.prepareStatement(insertHL7Sql
					.toString());
			deleteStatement = connection
					.prepareStatement("DELETE FROM hl7_in_archive WHERE hl7_in_archive_id=?");

			// iterate over deleted HL7s
			ResultSet archives = connection.createStatement().executeQuery(
					getDeletedHL7sSql.toString());
			while (archives.next()) {

				// add to the queue
				insertStatement.setString(1, archives.getString(1)); // set
																		// hl7_source
				insertStatement.setString(2, archives.getString(2)); // set
																		// hl7_source_key
				insertStatement.setString(3, archives.getString(3)); // set
																		// hl7_data
				insertStatement.setDate(4, archives.getDate(4)); // set
																	// date_created
			...
			throw new CustomChangeException(
					"Unable to move deleted HL7s from archive table to queue table",
					e);
