...
public class DatabaseUtil {
	...
	/**
	 * Executes the passed SQL query, enforcing select only if that parameter is
	 * set Load the jdbc driver class for the database which is specified by the
	 * connectionUrl and connectionDriver parameters <br>
	 * <br>
	 * This is only needed when loading up a jdbc connection manually for the
	 * first time. This is not needed by most users and development practices
	 * with the openmrs API.
	 * 
	 * @param connectionUrl
	 *            the connection url for the database, such as
	 *            "jdbc:mysql://localhost:3306/..."
	 * @param connectionDriver
	 *            the database driver class name, such as
	 *            "com.mysql.jdbc.Driver"
	 * @throws ClassNotFoundException
	 */
	public static String loadDatabaseDriver(String connectionUrl,
			String connectionDriver) throws ClassNotFoundException {
		if (StringUtils.hasText(connectionDriver)) {
			Class.forName(connectionDriver);
			log.debug("set user defined Database driver class: "
					+ connectionDriver);
		} else {
	...
	/**
	 * Executes the passed SQL query, enforcing select only if that parameter is
	 * set for given Session
	 */
	public static List<List<Object>> executeSQL(Session session, String sql,
			boolean selectOnly) throws DAOException {
		sql = sql.trim();
		boolean dataManipulation = checkQueryForManipulationCommands(sql,
				selectOnly);
		...
		session.doWork(new Work() {
			...
			public void execute(Connection conn) {
				populateResultsFromSQLQuery(conn, query,
						sessionDataManipulation, result);
			}
		});
	 ...
	 */
	public static List<List<Object>> executeSQL(Connection conn, String sql,
			boolean selectOnly) throws DAOException {
		sql = sql.trim();
		boolean dataManipulation = checkQueryForManipulationCommands(sql,
				selectOnly);
	...
	private static boolean checkQueryForManipulationCommands(String sql,
			boolean selectOnly) {
		...
		if (selectOnly && dataManipulation) {
			throw new IllegalArgumentException(
					"Illegal command(s) found in query string");
		}
		return dataManipulation;
	}

	private static void populateResultsFromSQLQuery(Connection conn,
			String sql, boolean dataManipulation, List<List<Object>> results) {
		...
		try {
			...
			throw new DAOException("Error while running sql: " + sql
	 ...
	 * @throws Exception
	 */
	public static <T> Set<T> getUniqueNonNullColumnValues(String columnName,
			String tableName, Class<T> type, Connection connection)
			throws Exception {
		Set<T> uniqueValues = new HashSet<T>();
		final String alias = "unique_values";
		String select = "SELECT DISTINCT " + columnName + " AS " + alias
				+ " FROM " + tableName + " WHERE " + columnName
				+ " IS NOT NULL";
		List<List<Object>> rows = DatabaseUtil.executeSQL(connection, select,
				true);
