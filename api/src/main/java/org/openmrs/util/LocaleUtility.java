...
public class LocaleUtility implements GlobalPropertyListener {
	...
	public static Locale getDefaultLocale() {
		if (defaultLocaleCache == null) {
			if (Context.isSessionOpen()) {
				...
				// if we weren't able to load the locale from the global
				// property,
				// use the default one
				if (defaultLocaleCache == null) {
				...
				// if session is not open, return the default locale without
				// caching
	...
	public static boolean areCompatible(Locale lhs, Locale rhs) {
		if (lhs.equals(rhs)) {
			return true;
		} else if ((("".equals(lhs.getCountry())) || ("".equals(rhs
				.getCountry()))) && lhs.getLanguage().equals(rhs.getLanguage())) {
	 ...
	 * 
	 * @return a collection of all specified and allowed locales with no
	 *         duplicates.
	 ...
	 * @should always have default locale default value included in the returned
	 *         collection
	 * @since 1.7
	 */
	...
	public static void setLocalesAllowedListCache(
			List<Locale> localesAllowedListCache) {
