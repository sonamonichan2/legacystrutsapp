 ...
 * (Locale is currently not used, but will be when we add the ability to
 * localize metadata.)
 * 
 * @since 1.7
 */
public class MetadataComparator implements Comparator<OpenmrsMetadata> {
	...
	public int compare(OpenmrsMetadata left, OpenmrsMetadata right) {
		int temp = OpenmrsUtil.compareWithNullAsLowest(left.isRetired(),
				right.isRetired());
		if (temp == 0) {
			temp = OpenmrsUtil.compareWithNullAsLowest(left.getName(),
					right.getName());
		}
		return temp;
	}
