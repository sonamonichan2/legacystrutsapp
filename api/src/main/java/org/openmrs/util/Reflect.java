...
/**
 * This class has convenience methods to find the fields on a class and
 * superclass as well as methods to check the class type of members in a
 * collection
 */
public class Reflect {
	...
	/**
	 * This method return all the fields (including private) from the given
	...
	public static List<Field> getAllFields(Class<?> fieldClass) {
		List<Field> fields = ClassDataCacher.getInstance()
				.getClassData(fieldClass).getFields();
		return new ArrayList<Field>(fields);
	}

	/**
	 * This method returns true if the given annotation is present on the given
	 * field.
	 * 
	 * @param fieldClass
	 * @param fieldName
	 ...
	 */
	public static boolean isAnnotationPresent(Class<?> fieldClass,
			String fieldName, Class<? extends Annotation> annotation) {
		ClassFields<?> classFields = ClassDataCacher.getInstance()
				.getClassFields(fieldClass);
		try {
	 ...
	 * @param subClass
	 *            Class
	 * @return true if, given subClass is accessible from the parameterized
	 *         class
	 * @should return true if given subClass is accessible from given
	 *         parameterized class
	 * @should return false if given subClass is not accessible from given
	 *         parameterized class
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * @param t
	 * @return true if given type is a subclass, or a generic type bounded by a
	 *         subclass of the parameterized class
	 ...
	 * @should return true if given object is accessible from given
	 *         parameterized class
	 * @should return false if given object is not accessible from given
	 *         parameterized class
	 */
	...
	/**
	 * This method validate the given field is Collection and the elements
	 * should be of parameterized type
	 * 
	 * @param field
	 *            Field
	 * @return boolean
	 * @should return true if given field is Collection and its element type is
	 *         given parameterized class type
	 * @should return false if given field is not a Collection
	 * @should return false if given field is Collection and element type is
	 *         other than given parameterized class type
	 */
	@SuppressWarnings("unchecked")
		...
		if (isCollection(field.getType())) {
			try {
				...
				if (type.getActualTypeArguments()[0] instanceof Class) {
					return (parametrizedClass.isAssignableFrom((Class) type
							.getActualTypeArguments()[0]));
				} else if (type.getActualTypeArguments()[0] instanceof TypeVariable) {
					return isSuperClass((TypeVariable<?>) type
							.getActualTypeArguments()[0]);
				} else {
				}
			} catch (ClassCastException e) {
				// Do nothing. If this exception is thrown, then field is not a
				// Collection of OpenmrsObjects
			}
		}
		return false;
	}
	...
	/**
	 * This method return all the fields (including private) until the given
	 * parameterized class
	 * 
	 * @param subClass
	 *            Class
	...
	public List<Field> getInheritedFields(Class<?> subClass) {
		...
		for (Iterator<Field> iterator = allFields.iterator(); iterator
				.hasNext();) {
	...
	/**
	 * @param field
	 * @return true if, given field is declared in parameterized class or its
	 *         sub classes
	 */
