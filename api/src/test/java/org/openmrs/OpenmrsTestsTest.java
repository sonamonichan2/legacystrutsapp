...
/**
 * Runs tests on the openmrs junit tests TODO: add unit test to make sure all
 * tests have a call to assert* in them. This would help prevent people from
 * making tests that just print results to the screen
 */
...
public class OpenmrsTestsTest {
	...
	public void shouldHaveAtLeastOneTest() throws Exception {
		...
		assertTrue("There should be more than one class but there was only "
		...
		for (Class<TestCase> currentClass : classes) {
			for (Method method : currentClass.getMethods()) {
				...
				if (method.getAnnotation(Test.class) != null) {
					...
					boolean passes = methodName.startsWith("should")
							|| methodName.contains("_should");
					assertTrue(
	...
	/**
	 * Makes sure all "should___" methods in org.openmrs have an "@Test"
	 * annotation on it. This is to help prevent devs from forgetting to put the
	 * annotation and then seeing all tests pass because the new test wasn't
	 * actually ran
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Checks that a user hasn't accidentally created a test class that doesn't
	 * end with "Test". (If it doesn't, it isn't picked up by the test
	 * aggregator Ant target: junit-report) <br>
	 * <br>
	 * This class looks at all classes in the org.openmrs package. If a class
	 * contains an "@Test" annotated method but its class name does not end with
	 * Test, it fails.
	 * 
	 * @throws Exception
	 */
	...
	public void shouldHaveClassNameEndWithTestIfContainsMethodTestAnnotations()
		...
		for (Class<?> currentClass : getClasses("^.*(?<!Test|IT|PT)\\.class$")) {
			...
			if (currentClass.getAnnotation(Ignore.class) == null) {
				...
				Assert.assertFalse(
						currentClass.getName()
								+ " does not end with 'Test' but contains @Test annotated methods",
						foundATestMethod);
			}
		}
	}
	...
	private List<Class> getClasses(String classNameRegex) {
		...
		assertTrue("org.openmrs.test should be a directory",
				directory.isDirectory());

		testClasses = getClassesInDirectory(directory, pattern);

		// check to see if the web layer is also included. Skip it if its not
		// there
		url = classLoader.getResource("org/openmrs/web");
		if (url != null) {
			directory = OpenmrsUtil.url2file(url);
			// make sure we get a directory back
			assertTrue("org.openmrs.web.test should be a directory",
					directory.isDirectory());
	...
	/**
	 * Recurses into the given directory checking that all test methods start
	 * with "testShould"
	 * 
	 * @param directory
	 *            to loop through the files of
	 */
	private List<Class> getClassesInDirectory(File directory, Pattern pattern) {
		...
		for (File currentFile : directory.listFiles()) {
			...
			if (currentFile.isDirectory()) {
				currentDirTestClasses.addAll(getClassesInDirectory(currentFile,
						pattern));
			}

			// if the user only wants classes ending in Test or they want all of
			// them
			if (pattern.matcher(currentFile.getName()).matches()) {
				// strip off the extension
				String className = currentFile.getAbsolutePath().replace(
						".class", "");

				// switch to dot separation
				className = className.replace(File.separator, ".");

				// strip out the beginning (/home/ben/workspace...) up to
				// org.openmrs.
				className = className.substring(className
						.lastIndexOf("org.openmrs."));
