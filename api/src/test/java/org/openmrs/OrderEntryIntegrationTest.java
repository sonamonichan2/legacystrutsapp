...
/**
 * Contains end to end tests for order entry operations i.g placing,
 * discontinuing revising an order
 */
public class OrderEntryIntegrationTest extends BaseContextSensitiveTest {
	...
	public void shouldGetTheActiveDrugOrdersForAPatient() {
		Patient patient = patientService.getPatient(2);
		List<Order> activeDrugOrders = orderService.getActiveOrders(patient,
				orderService.getOrderTypeByName("Drug order"), null, null);
		assertEquals(4, activeDrugOrders.size());
		Order[] expectedDrugOrders = {orderService.getOrder(222),
	...
	public void shouldPlaceADrugOrder() throws Exception {
		...
		int activeDrugOrderCount = orderService.getActiveOrders(patient,
				orderService.getOrderTypeByName("Drug order"), careSetting,
				null).size();
		...
		List<Order> activeOrders = orderService.getActiveOrders(patient,
				orderService.getOrderTypeByName("Drug order"), careSetting,
				null);
	...
	public void shouldPlaceATestOrder() throws Exception {
		...
		Patient patient = patientService.getPatient(7);
		CareSetting careSetting = orderService.getCareSetting(1);
		int activeTestOrderCount = orderService.getActiveOrders(patient,
				orderService.getOrderTypeByName("Test order"), careSetting,
				null).size();
	...
	public void shouldDiscontinueAnActiveOrder() throws Exception {
		...
		int ordersCount = orderService.getActiveOrders(patient, null, null,
				null).size();

		Concept discontinueReason = Context.getConceptService().getConcept(1);
		Provider orderer = providerService.getProvider(1);
		Order discontinuationOrder1 = orderService.discontinueOrder(
				firstOrderToDiscontinue, discontinueReason, null, orderer,
				encounter);
		assertEquals(firstOrderToDiscontinue,
				discontinuationOrder1.getPreviousOrder());

		// Lets discontinue another order with reason being a string instead of
		// concept
		...
		Order discontinuationOrder2 = orderService.discontinueOrder(
				secondOrderToDiscontinue, "Testing", null, orderer, encounter);
		assertEquals(secondOrderToDiscontinue,
				discontinuationOrder2.getPreviousOrder());
		...
		Order discontinuationOrder = thirdOrderToDiscontinue
	...
	/**
	 * This test ensures that the getter for previous order field returns
	 * objects of the actual sub types for subclasses instead proxies that are
	 * instances of Order
	 * 
	 * @throws Exception
	 */
	...
	public void shouldNotReturnAProxyForPreviousOrder() throws Exception {
		...
		Order dcTestOrder = orderService.discontinueOrder(testOrder, "Testing",
				null, testOrder.getOrderer(), testOrder.getEncounter());
		Context.flushSession();
		Context.clearSession();
		dcTestOrder = (TestOrder) orderService.getOrder(
				dcTestOrder.getOrderId()).getPreviousOrder();
	}
	...
	public void shouldAllowEditingADiscontinuationOrder() throws Exception {
		...
		List<Order> originalPatientOrders = orderService
				.getAllOrdersByPatient(originalDCOrder.getPatient());
		final Order previousOrder = originalDCOrder.getPreviousOrder();
		assertNotNull(previousOrder);
		final Date newStartDate = originalDCOrder.getEncounter()
		...
		// We need to flush so that we ensure the interceptor is okay with all
		// this
		Context.flushSession();
		assertTrue(originalDCOrder.isVoided());
		List<Order> newPatientOrders = orderService
				.getAllOrdersByPatient(originalDCOrder.getPatient());
		assertEquals(originalPatientOrders.size() + 1, newPatientOrders.size());
		Collection<Order> newOrders = CollectionUtils.disjunction(
				originalPatientOrders, newPatientOrders);
		assertEquals(1, newOrders.size());
		assertEquals(newOrders.iterator().next().getPreviousOrder(),
				previousOrder);
	}
	...
	public void shouldAllowRetrospectiveDataEntryOfOrders() throws Exception {
		...
		Order dcOrder = orderService.discontinueOrder(order, "Testing",
				stopDate, order.getOrderer(), order.getEncounter());
		Context.flushSession(); // ensures that order is flushed and that the
								// drop milliseconds interceptor is called
		assertEquals(DateUtil.truncateToSeconds(stopDate),
				order.getDateStopped());
		assertEquals(DateUtil.truncateToSeconds(stopDate),
				dcOrder.getAutoExpireDate());
	}
	...
	public void shouldAllowRevisionOfOrdersInRetrospectiveDataEntry()
		...
		assertEquals(DateUtils.addSeconds(order2.getDateActivated(), -1),
				order.getDateStopped());
	}
}
