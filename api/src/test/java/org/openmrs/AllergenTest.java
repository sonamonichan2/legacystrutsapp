...
public class AllergenTest extends BaseContextSensitiveTest {
	...
	private void assertCoded() {
		Assert.assertNotEquals(allergen.getCodedAllergen().getUuid(),
	...
	private void assertNonCoded() {
		...
		Assert.assertEquals(allergen.getNonCodedAllergen(),
				"Non coded allergen");
	...
	public void isSameAllergen_shouldReturnTrueForSameCodedAllergen() {
		...
		Assert.assertTrue(new Allergen(null, c, null)
				.isSameAllergen(new Allergen(null, c, null)));
	}

	@Test
	public void isSameAllergen_shouldReturnFalseForDifferentCodedAllergen() {
		Assert.assertFalse(new Allergen(null, new Concept(1), null)
	...
	public void isSameAllergen_shouldReturnTrueForSameNonCodedAllergen() {
		Concept concept = Context.getConceptService().getConceptByUuid(
				Allergen.getOtherNonCodedConceptUuid());
		Assert.assertTrue(new Allergen(null, concept, "OTHER VALUE")
	...
	public void isSameAllergen_shouldReturnFalseForDifferentNonCodedAllergen() {
		Concept concept = Context.getConceptService().getConceptByUuid(
				Allergen.getOtherNonCodedConceptUuid());
		Assert.assertFalse(new Allergen(null, concept, "OTHER VALUE1")
				.isSameAllergen(new Allergen(null, concept, "OTHER VALUE2")));
	}
}
