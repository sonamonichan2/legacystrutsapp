...
public class LocationTest {
	...
	public void getDescendantLocations_shouldReturnAllDescendantLocationsIfIncludeRetiredIsTrue() {
		...
		rootLocation.setChildLocations(new HashSet<Location>(Arrays.asList(
				locationOne, locationTwo)));

		locationOne.setChildLocations(new HashSet<Location>(Arrays
				.asList(childOflocationOne)));
		locationTwo.setChildLocations(new HashSet<Location>(Arrays
				.asList(childOnfLocationTwo)));

		childOflocationOne.setChildLocations(new HashSet<Location>());
		childOnfLocationTwo.setChildLocations(new HashSet<Location>());

		Set<Location> descendantLocations = rootLocation
				.getDescendantLocations(true);

		Set<Location> expectedLocations = new HashSet<Location>(Arrays.asList(
				locationOne, locationTwo, childOflocationOne,
				childOnfLocationTwo));
	...
	public void getDescendantLocations_shouldReturnNonRetiredDescendantLocationsIfIncludeRetiredIsFalse() {
		...
		// make child-parent relations
		rootLocation.setChildLocations(new HashSet<Location>(Arrays.asList(
				nonRetiredLocation, retiredLocation)));

		nonRetiredLocation.setChildLocations(new HashSet<Location>(Arrays
				.asList(firstChildOfNonRetiredLocation,
						secondChildOfNonRetiredLocation)));
		retiredLocation.setChildLocations(new HashSet<Location>(Arrays
				.asList(firstChildOfRetiredLocation)));

		firstChildOfNonRetiredLocation
				.setChildLocations(new HashSet<Location>());
		secondChildOfNonRetiredLocation
				.setChildLocations(new HashSet<Location>());
		...
		// action
		Set<Location> descendantLocations = rootLocation
				.getDescendantLocations(false);

		Set<Location> expectedLocations = new HashSet<Location>(Arrays.asList(
				nonRetiredLocation, firstChildOfNonRetiredLocation,
				secondChildOfNonRetiredLocation));
