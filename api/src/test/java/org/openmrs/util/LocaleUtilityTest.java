...
public class LocaleUtilityTest extends BaseContextSensitiveTest {
	...
	/**
	 * Two locales, where one has no country specified, but where the language
	 * matches, should be considered compatible.
	 * 
	 * @see LocaleUtility#areCompatible(Locale,Locale)
	 */
	@Test
	...
	/**
	 * Two locales, where one has no country specified, but where the language
	 * does not match, should not be considered compatible.
	 * 
	 * @see LocaleUtility#areCompatible(Locale,Locale)
	 */
	@Test
	...
	public void fromSpecification_shouldGetLocaleFromTwoCharacterLanguageCode()
		...
		Assert.assertEquals(Locale.ENGLISH,
				LocaleUtility.fromSpecification("en"));
	}
	...
	public void fromSpecification_shouldGetLocaleFromLanguageCodeCountryCodeAndVariant()
			throws Exception {
		Locale locale = LocaleUtility
				.fromSpecification("en_US_Traditional_WIN");
	...
	public void getLocalesInOrder_shouldAlwaysHaveDefaultLocaleDefaultValueIncludedInTheReturnedCollection()
		...
		Assert.assertEquals(
				true,
				localesInOrder.contains(LocaleUtility
	...
	public void getLocalesInOrder_shouldHaveDefaultLocaleAsTheFirstElementIfUserHasNoPreferredLocale()
		...
		Set<Locale> localesInOrder = LocaleUtility.getLocalesInOrder();
		Assert.assertEquals(LocaleUtility.getDefaultLocale(), localesInOrder
				.iterator().next());
	}
	...
	public void getLocalesInOrder_shouldReturnASetOfLocalesWithNoDuplicates()
		...
		GlobalProperty gp = new GlobalProperty(
				OpenmrsConstants.GLOBAL_PROPERTY_LOCALE_ALLOWED_LIST,
				"lu_UG, lu, sw_KE, en_US, en, en, en_GB, sw_KE",
				"Test Allowed list of locales");
		Context.getAdministrationService().saveGlobalProperty(gp);
		GlobalProperty defaultLocale = new GlobalProperty(
				OpenmrsConstants.GLOBAL_PROPERTY_DEFAULT_LOCALE, "lu",
				"Test Allowed list of locales");
		Context.getAdministrationService().saveGlobalProperty(defaultLocale);
		Locale lu_UG = new Locale("lu", "UG");
		Context.setLocale(lu_UG);
		// note that unique list of locales should be lu_UG, lu, sw_KE, en_US,
		// en
	...
	/**
	 * This test doesn't really test anything, and it should ALWAYS be the last
	 * method in this class. <br>
	 * <br>
	 * This method just resets the current user's locale so that when things are
	 * run in batches all tests still work.
	 */
	@Test
	...
	public void getDefaultLocale_shouldNotCacheLocaleWhenSessionIsNotOpen()
		...
		Context.getAdministrationService().saveGlobalProperty(
				new GlobalProperty(
						OpenmrsConstants.GLOBAL_PROPERTY_LOCALE_ALLOWED_LIST,
						"en_GB, ja"));
		// set GP default locale to valid locale that is not the OpenmrsConstant
		// default locale
		GlobalProperty gp = Context
				.getAdministrationService()
				.saveGlobalProperty(
		...
		// This might fail if default locale is called before this test is run
		// and so the static defaultLocale is cached
		//
