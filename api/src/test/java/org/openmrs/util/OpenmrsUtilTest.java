...
/**
 * Tests the methods in {@link OpenmrsUtil} TODO: finish adding tests for all
 * methods
 */
public class OpenmrsUtilTest extends BaseContextSensitiveTest {
	...
	public void collectionContains_shouldUseEqualsMethodForComparisonInsteadOfCompareToGivenListCollection()
		...
		// sanity check
		identifiers.add(pi);
		assertFalse("Lists should accept more than one object",
				identifiers.size() == 1);
		...
		assertTrue(
				"Just because the date is null, doesn't make it not in the list anymore",
	...
	public void collectionContains_shouldUseEqualsMethodForComparisonInsteadOfCompareToGivenSortedSetCollection()
		...
		identifiers.add(pi);
		assertTrue(
				"There should still be only 1 identifier in the patient object now",
				identifiers.size() == 1);
	...
	/**
	 * When given a null parameter, the
	 * {@link OpenmrsUtil#url2file(java.net.URL)} method should quietly fail by
	 * returning null
	 * 
	...
	public void validatePassword_shouldFailWithDigitOnlyPasswordIfNotAllowed()
			throws Exception {
		TestUtil.saveGlobalProperty(
				OpenmrsConstants.GP_PASSWORD_REQUIRES_NON_DIGIT, "true");
		OpenmrsUtil.validatePassword("admin", "12345678", "1-8");
	}
	...
	public void validatePassword_shouldPassWithDigitOnlyPasswordIfAllowed()
			throws Exception {
		TestUtil.saveGlobalProperty(
				OpenmrsConstants.GP_PASSWORD_REQUIRES_NON_DIGIT, "false");
		TestUtil.saveGlobalProperty(
	...
	public void validatePassword_shouldFailWithCharOnlyPasswordIfNotAllowed()
			throws Exception {
		TestUtil.saveGlobalProperty(
				OpenmrsConstants.GP_PASSWORD_REQUIRES_DIGIT, "true");
		OpenmrsUtil.validatePassword("admin", "testonly", "1-8");
	}
	...
	public void validatePassword_shouldPassWithCharOnlyPasswordIfAllowed()
			throws Exception {
		TestUtil.saveGlobalProperty(
				OpenmrsConstants.GP_PASSWORD_REQUIRES_DIGIT, "false");
		TestUtil.saveGlobalProperty(
				OpenmrsConstants.GP_PASSWORD_REQUIRES_UPPER_AND_LOWER_CASE,
				"false");
		OpenmrsUtil.validatePassword("admin", "testonly", "1-8");
	...
	public void validatePassword_shouldFailWithShortPasswordIfNotAllowed()
			throws Exception {
		TestUtil.saveGlobalProperty(
				OpenmrsConstants.GP_PASSWORD_MINIMUM_LENGTH, "6");
	...
	public void validatePassword_shouldPassWithShortPasswordIfAllowed()
			throws Exception {
		TestUtil.saveGlobalProperty(
				OpenmrsConstants.GP_PASSWORD_MINIMUM_LENGTH, "0");
	...
	public void getDateFormat_shouldReturnAPatternWithFourYCharactersInIt()
			throws Exception {
		Assert.assertEquals("MM/dd/yyyy", OpenmrsUtil.getDateFormat(Locale.US)
				.toLocalizedPattern());
		Assert.assertEquals("dd/MM/yyyy", OpenmrsUtil.getDateFormat(Locale.UK)
				.toLocalizedPattern());
		Assert.assertEquals("tt.MM.uuuu",
				OpenmrsUtil.getDateFormat(Locale.GERMAN).toLocalizedPattern());
		Assert.assertEquals("dd-MM-yyyy",
				OpenmrsUtil.getDateFormat(new Locale("pt", "pt"))
						.toLocalizedPattern());
	}
	...
	public void containsUpperAndLowerCase_shouldReturnTrueIfStringContainsUpperAndLowerCase()
		...
		Assert.assertTrue(OpenmrsUtil
				.containsUpperAndLowerCase("The number 10"));
	}
	...
	/**
	 * The validate password method should be in a separate jvm here so that the
	 * Context and services are not available to the validatePassword (similar
	 * to how its used in the initialization wizard), but that is not possible
	 * to set up on a test-by-test basis, so we settle by making the user
	...
	public void getDateFormat_shouldNotAllowTheReturnedSimpleDateFormatToBeModified()
		...
		Assert.assertTrue(
				"initial dateFormatCache pattern is modifiable",
				!OpenmrsUtil.getDateFormat(locale).toPattern()
						.equals(sdf.toPattern()));

		// the dateformat cache now contains the format for this locale;
		// checking
		// a second time will guarantee we are looking at cached data and not
		// the
		...
		sdf.applyPattern("yyyymmdd");
		Assert.assertTrue(
				"cached dateFormatCache pattern is modifiable",
				!OpenmrsUtil.getDateFormat(locale).toPattern()
						.equals(sdf.toPattern()));
	}
	...
	public void openmrsDateFormat_shouldParseValidDate() throws Exception {
		SimpleDateFormat sdf = OpenmrsUtil
				.getDateFormat(new Locale("en", "GB"));
	...
	public void shortenedStackTrace_shouldReturnNullIfStackTraceIsNull()
			throws Exception {
		Assert.assertNull("null value was not returned with null parameter",
				OpenmrsUtil.shortenedStackTrace(null));
	}
	...
	public void nullSafeEqualsIgnoreCase_shouldBeCaseInsensitive()
			throws Exception {
		Assert.assertTrue(OpenmrsUtil
				.nullSafeEqualsIgnoreCase("equal", "Equal"));
	}
	...
	public void storeProperties_shouldEscapeSlashes() throws Exception {
		...
		// This method didn't exist in Java 5, which is why we wrote a utility
		// method in the first place, so we should
		// just get rid of our own implementation, and use the underlying java
		// one.
