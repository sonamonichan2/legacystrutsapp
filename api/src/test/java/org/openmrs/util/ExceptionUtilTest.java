...
public class ExceptionUtilTest {
	...
	public void rethrowIfCause_shouldAllowAnIntermediateExceptionToBeRethrown()
		...
		try {
			...
			List<Class<? extends RuntimeException>> chain = Arrays.asList(
			...
			try {
				ExceptionUtil
						.rethrowIfCause(ex, IllegalArgumentException.class);
	...
	/**
	 * Recursively builds up an exception chain with the requested exception
	 * classes in it.
	 * 
	 * @param classesInChain
	 *            the start of the list is the root cause, and the end of the
	 *            list is the outermost wrapped exception
	 */
	private void throwExceptionChain(
			List<Class<? extends RuntimeException>> classesInChain)
			throws Exception {
		if (classesInChain.size() > 1) {
			try {
				throwExceptionChain(classesInChain.subList(0,
						classesInChain.size() - 1));
			} catch (Exception ex) {
				Class<? extends Exception> outer = classesInChain
						.get(classesInChain.size() - 1);
				Constructor<? extends Exception> constructor = outer
