...
public class UserServiceTest extends BaseContextSensitiveTest {
	...
	/**
	 * Methods in this class might authenticate with a different user, so log
	 * that user out after this whole junit class is done.
	 */
	@AfterClass
	 ...
	 * 
	 * @see UserService#saveUser(User,String)
	 */
	@Test
	...
	public void saveUser_shouldCreateNewUserWithBasicElements()
			throws Exception {
		assertTrue(
				"The context needs to be correctly authenticated to by a user",
		...
		assertTrue(
				"The user returned by the create user method should equal the passed in user",
				createdUser.equals(u));

		createdUser = us.getUserByUsername("bwolfe");
		assertTrue("The created user should equal the passed in user",
				createdUser.equals(u));
	}
	...
	public void saveUser_shouldShouldCreateUserWhoIsPatientAlready()
			throws Exception {
		...
		// this avoids a lazy init exception, since we're going to clear the
		// session
		...
		user.addRole(new Role("Some Role", "This is a test role")); // included
																	// in xml
																	// file
		...
		Assert.assertNotNull("User was not created",
				userService.getUser(user.getUserId()));

		Integer shouldCreateUserWhoIsPatientAlreadyTestUserIdCreated = user
				.getUserId();

		Context.flushSession();

		// get the same user we just created and make sure the user portion
		// exists
		User fetchedUser = userService
				.getUser(shouldCreateUserWhoIsPatientAlreadyTestUserIdCreated);
		...
		assertNotNull("Uh oh, the username was not saved",
				fetchedUser.getUsername());
		assertTrue("Uh oh, the username was not saved", fetchedUser
				.getUsername().equals("bwolfe"));
		assertTrue("Uh oh, the role was not assigned",
				fetchedUser.hasRole("Some Role"));
		...
		List<Patient> allPatientsSet = Context.getPatientService()
				.getAllPatients();
	...
	public void saveUser_shouldUpdateUsersUsername() throws Exception {
		...
		assertNotNull(
				"There needs to be a user with username 'admin' in the database",
				u);
		...
		assertEquals(
				"The fetched user should equal the user we tried to update", u,
				u2);
	}
	...
	public void getUsers_shouldMatchSearchToFamilyName2() throws Exception {
		...
		List<User> users = Context.getUserService().getUsers("Johnson", null,
				false);
	...
	public void getPrivilegeByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getUserService().getPrivilegeByUuid(
				"some invalid uuid"));
	}
	...
	public void getRoleByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getUserService().getRoleByUuid(
				"some invalid uuid"));
	}
	...
	public void getUserByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getUserService().getUserByUuid(
				"some invalid uuid"));
	}
	...
	public void changeHashedPassword_shouldChangeTheHashedPasswordForTheGivenUser()
		...
		// This is the assert - checks to see if current user can use the new
		// password
		userService.changePassword("new password", "Another new password1"); // try
																				// to
																				// change
																				// the
																				// password
	...
	public void changePassword_shouldChangePasswordForTheGivenUserAndPassword()
		...
		userService.changePassword("Another new password1",
				"Yet another new password1"); // try to change the password with
												// the new one
	}
	...
	@Test
	@Ignore
	// TODO fix: the question not sticking - null expected:<[the question]> but
	// was:<[]>
	...
	public void changeQuestionAnswer_shouldChangeTheSecretQuestionAndAnswerForGivenUser()
		...
		// need to retrieve the user since the service method does not modify
	...
	public void getAllPrivileges_shouldReturnAllPrivilegesInTheSystem()
			throws Exception {
		executeDataSet(XML_FILENAME);
		List<Privilege> privileges = Context.getUserService()
				.getAllPrivileges();
	...
	public void getAllUsers_shouldNotContainsAnyDuplicateUsers()
		...
		// TODO Need to test with duplicate data in the dataset (not sure if
		// that's possible)

	}

	/**
	 * @verifies {@link UserService#getUserByUuid(String)} test = should fetch
	 *           user with given uuid
	 */
	@Test
	@SkipBaseSetup
	...
	public void getUserByUuid_shouldFetchUserWithGivenUuid() throws Exception {
		...
		User user = userService
				.getUserByUuid("013c49c6-e132-11de-babe-001e378eb67e");
		assertEquals("Did not fetch user with given uuid", user,
				userService.getUser(5505));
	}

	/**
	 * @verifies {@link UserService#getUsersByName(String,String,null)} test =
	 *           should fetch users exactly matching the given givenName and
	 *           familyName
	 */
	@Test
	@SkipBaseSetup
	...
	public void getUsersByName_shouldFetchUsersExactlyMatchingTheGivenGivenNameAndFamilyName()
		...
		// collection [user0_.user_id.names] with element property reference
		// [givenName]
		// [from org.openmrs.User u where u.names.givenName = :givenName and
		// u.names.familyName
	...
	public void getUsersByName_shouldFetchVoidedUsersWhenincludeVoidedIsTrue()
		...
		// u.names.familyName
		// = :familyName]
		List<User> users = userService.getUsersByName("Bruno", "Otterbourg",
				true);
	...
	public void getUsersByName_shouldNotFetchAnyDuplicateUsers()
		...
		UserService userService = Context.getUserService();
		// user with ID 4 has a preferred name "John Doe" and a not preferred
		// name "John Doe."
		// If this method does not fetch any duplicate users, this user should
		// only
	...
	public void getPrivilege_shouldFetchPrivilegeForGivenName()
			throws Exception {
		executeDataSet(XML_FILENAME);
		Privilege privilege = Context.getUserService().getPrivilege(
				"Some Privilege");
		Assert.assertEquals("Some Privilege", privilege.getPrivilege());
	}
	...
	public void getUsers_shouldFetchUsersWithAtLeastOneOfTheGivenRoleObjects()
		...
		Assert.assertEquals(1,
				Context.getUserService().getUsers("Susy Kingman", roles, false)
						.size());
	}
	...
	public void getUsers_shouldFetchUsersWithNameThatContainsGivenNameSearch()
			throws Exception {
		Assert.assertEquals(1,
				Context.getUserService().getUsers("Hippocrates", null, false)
						.size());
	}
	...
	public void getUsers_shouldFetchUsersWithSystemIdThatContainsGivenNameSearch()
			throws Exception {
		Assert.assertEquals(1,
				Context.getUserService().getUsers("2-6", null, true).size());
	}
	...
	public void getUsers_shouldFetchVoidedUsersIfIncludedVoidedIsTrue()
			throws Exception {
		Assert.assertEquals(1,
				Context.getUserService().getUsers("Bruno", null, true).size());
	}
	...
	public void getUsers_shouldFetchAllUsersIfNameSearchIsEmptyOrNull()
			throws Exception {
		Assert.assertEquals(4, Context.getUserService()
				.getUsers("", null, true).size());
		Assert.assertEquals(4,
				Context.getUserService().getUsers(null, null, true).size());
	}
	...
	public void getUsers_shouldNotFetchDuplicateUsers() throws Exception {
		...
		List<User> users = Context.getUserService().getUsers("John Doe", null,
				false);
	...
	public void getUsers_shouldNotFetchVoidedUsersIfIncludedVoidedIsFalse()
		...
		Assert.assertEquals(0,
				Context.getUserService().getUsers("Bruno", null, false).size());
	}
	...
	public void getUsersByRole_shouldFetchUsersAssignedGivenRole()
		...
		Assert.assertEquals(2,
				Context.getUserService().getUsersByRole(new Role("Some Role"))
						.size());
	}
	...
	public void getUsersByRole_shouldNotFetchUserThatDoesNotBelongToGivenRole()
		...
		Assert.assertEquals(
				...
				Context.getUserService()
						.getUsersByRole(new Role("Nonexistent role")).size());
	}
	...
	public void isSecretAnswer_shouldReturnFalseWhenGivenAnswerDoesNotMatchTheStoredSecretAnswer()
		...
		Assert.assertFalse(Context.getUserService().isSecretAnswer(user,
				"not the answer"));
	}
	...
	public void isSecretAnswer_shouldReturnTrueWhenGivenAnswerMatchesStoredSecretAnswer()
		...
		Context.getUserService().changeQuestionAnswer(user, "question",
				"answer");
		Assert.assertTrue(Context.getUserService().isSecretAnswer(user,
				"answer"));
	}
	...
	public void purgePrivilege_shouldDeleteGivenPrivilegeFromTheDatabase()
			throws Exception {
		Context.getUserService()
				.purgePrivilege(new Privilege("Some Privilege"));
		Assert.assertNull(Context.getUserService().getPrivilege(
				"Some Privilege"));
	}
	...
	public void purgePrivilege_shouldThrowErrorWhenPrivilegeIsCorePrivilege()
			throws Exception {
		Context.getUserService().purgePrivilege(
				new Privilege(PrivilegeConstants.ADD_COHORTS));
	}
	...
	public void purgeRole_shouldDeleteGivenRoleFromDatabase() throws Exception {
		...
		Assert.assertNull(Context.getUserService().getRole(
				"Some Role To Delete"));
	}
	...
	public void removeUserProperty_shouldReturnNullIfUserIsNull()
			throws Exception {
		UserService userService = Context.getUserService();
		Assert.assertNull(userService.setUserProperty(null, "some key",
				"some new value"));
	}
	...
	public void savePrivilege_shouldSaveGivenPrivilegeToTheDatabase()
		...
		Privilege savedPrivilege = Context.getUserService().getPrivilege(
				"new privilege name");
	...
	public void setUserProperty_shouldAddPropertyWithGivenKeyAndValueWhenKeyDoesNotAlreadyExist()
		...
		user = userService.getUser(5505);
		Assert.assertEquals("some new value",
				user.getUserProperty("some new key"));
	}
	...
	public void saveRole_shouldThrowErrorIfRoleInheritsFromItself()
		...
		expectedException.expectMessage(Context.getMessageSourceService()
				.getMessage("Role.cannot.inherit.descendant"));
	...
	public void getUsersByPerson_shouldFetchAllAccountsForAPersonWhenIncludeRetiredIsTrue()
		...
		List<User> users = Context.getUserService().getUsersByPerson(person,
				true);
		Assert.assertEquals(3, users.size());
	}
	...
	public void getUsers_shouldNotFailIfRolesAreSearchedButNameIsEmpty()
		...
		Assert.assertEquals(2,
				Context.getUserService().getUsers("", roles, true).size());
	}
	...
	public void getUsers_shouldReturnUsersWhoseRolesInheritRequestedRoles()
		...
		Assert.assertEquals(3,
				Context.getUserService()
						.getUsers(null, roles, true, null, null).size());
	}
	...
	public void saveUserProperty_shouldAddNewPropertyToExistingUserProperties()
		...
		// Authenticate the test user so that Context.getAuthenticatedUser()
		// method returns above user
		...
		User updatedUser = userService.saveUserProperty(USER_PROPERTY_KEY,
				USER_PROPERTY_VALUE);

		assertNotNull(updatedUser.getUserProperty(USER_PROPERTY_KEY));
		assertEquals(USER_PROPERTY_VALUE,
				updatedUser.getUserProperty(USER_PROPERTY_KEY));
		// make sure that properties count is incremented by one
		assertEquals((numberOfUserProperties + 1), updatedUser
	...
	public void saveUserProperties_shouldRemoveAllExistingPropertiesAndAssignNewProperties()
		...
		assertEquals(USER_PROPERTY_VALUE_1,
				updatedUser.getUserProperty(USER_PROPERTY_KEY_1));
		assertEquals(USER_PROPERTY_VALUE_2,
				updatedUser.getUserProperty(USER_PROPERTY_KEY_2));
	}

	/**
	 * @see UserService#changePassword(User,String,String)
	 * @verifies change password for given user if oldPassword is correctly
	 *           passed
	 */
	@Test
	...
	/**
	 * @see UserService#changePassword(User,String,String)
	 * @verifies change password for given user if oldPassword is null and
	 *           changing user have privileges
	 */
	@Test
	...
	public void changePassword_shouldThrowAPIExceptionIfOldPasswordIsNotCorrect()
		...
		expectedException.expectMessage(Context.getMessageSourceService()
				.getMessage("error.privilegesRequired",
						new Object[]{PrivilegeConstants.EDIT_USER_PASSWORDS},
						null));
	...
	/**
	 * @see UserService#changePassword(User,String,String)
	 * @verifies throw exception if oldPassword is null and changing user have
	 *           not privileges
	 */
	@Test
	public void changePassword_shouldThrowExceptionIfOldPasswordIsNullAndChangingUserHaveNotPrivileges()
		...
		// user 6001 has password userServiceTest
		User user6001 = userService.getUser(6001);
		assertFalse(user6001
				.hasPrivilege(PrivilegeConstants.EDIT_USER_PASSWORDS));
		String oldPassword = null;
		String newPassword = "newPasswordString";
		// log in user without change user passwords privileges
		// user6001 has not got required priviliges
	...
	public void changePassword_shouldThrowExceptionIfNewPasswortIsTooShort()
		...
		expectedException.expectMessage(Context.getMessageSourceService()
				.getMessage("error.password.length", new Object[]{"8"}, null));
	...
	public void changePasswordUsingSecretAnswer_shouldUpdatePasswordIfSecretIsCorrect()
		...
		userService.changePasswordUsingSecretAnswer("answer",
				"userServiceTest2");
	...
	public void changePasswordUsingSecretAnswer_shouldNotUpdatePasswordIfSecretIsNotCorrect()
		...
		expectedException.expect(APIException.class);
		expectedException.expectMessage(Context.getMessageSourceService()
				.getMessage("secret.answer.not.correct"));

		userService.changePasswordUsingSecretAnswer("wrong answer",
				"userServiceTest2");
	}
}
