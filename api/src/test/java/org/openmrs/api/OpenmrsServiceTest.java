...
public class OpenmrsServiceTest extends BaseContextSensitiveTest {
	...
	/**
	 * Tests that if two service methods are called (one from inside the other)
	 * the first one will be rolled back if an exception is thrown inside the
	 * second one.
	 * 
	 * <pre>
	...
	public void shouldCheckThatAMethodIsNotRolledBackInCaseOfAnErrorInAnotherInvokedInsideIt()
		...
		int originalPrefEncounterCount = encounterService
				.getEncountersByPatient(prefPatient).size();
		int originalNotPrefEncounterCount = encounterService
				.getEncountersByPatient(notPrefPatient).size();
		Assert.assertTrue(originalNotPrefEncounterCount > 0);

		Cohort notPreferredCohort = new Cohort(notPrefPatient.getPatientId()
				.toString());
		List<PatientProgram> notPrefPrograms = programService
				.getPatientPrograms(notPreferredCohort, programs);
		Assert.assertTrue(notPrefPrograms.size() > 0);

		// Set the program to null so that the patient program is rejected on
		// validation with
		// an APIException, since it is a RuntimeException, all transactions
		...
		// Since the encounters are moved first, that logic should have been
		// rolled back
		Assert.assertEquals(originalPrefEncounterCount, encounterService
				.getEncountersByPatient(prefPatient).size());
	}
}
