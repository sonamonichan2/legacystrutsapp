...
public class LocationServiceTest extends BaseContextSensitiveTest {
	...
	/**
	 * Test to make sure that a simple save to a new location gets persisted to
	 * the database
	 * 
	...
	public void saveLocation_shouldCreateLocationSuccessfully()
		...
		assertNotNull("The saved location should have an id now",
				location.getLocationId());
		assertNotNull("We should get back a location", newSavedLocation);
		assertTrue("The created location needs to equal the pojo location",
				location.equals(newSavedLocation));
	}
	...
	public void saveLocation_shouldUpdateLocationSuccessfully()
		...
		assertTrue("The name should be the same",
				newestLoc.getName().equals(newName));
		assertFalse("The name should be different", origDesc.equals(newDesc));
		assertTrue("The name should be the same", newestLoc.getDescription()
				.equals(newDesc));
	}

	/**
	 * You should be able to add child locations to a location (multi-level),
	 * save the location, and have the child location automatically persisted.
	 * 
	 * @see LocationService#saveLocation(Location)
	 */
	@Test
	...
	public void saveLocation_shouldCascadeSaveToChildLocationFromLocation()
		...
		assertNotNull("newSavedLocation.childLocations must be not null",
				newSavedLocation.getChildLocations());

		// Saved parent location should have exactly 1 child location
		assertEquals(1, newSavedLocation.getChildLocations().size());

		Location newChildA = newSavedLocation.getChildLocations().iterator()
				.next();

		// Child location must be the previously added object
		assertTrue("Child location must be the previously created childA",
				newChildA.equals(childA));

		// Saved level A child location should have child locations
		assertNotNull("newSavedLocation.childLocations must be not null",
				newChildA.getChildLocations());
		...
		assertTrue(
				...
				newChildA.getChildLocations().iterator().next().equals(childB));
	}
	...
	public void saveLocation_shouldRemoveChildLocationFromLocation()
		...
		// Removing a child
		location.removeChildLocation(location.getChildLocations().iterator()
				.next());
		ls.saveLocation(location);
	...
	public void getDefaultLocation_shouldReturnDefaultLocationForTheImplementation()
			throws Exception {
		// set the global property for default location to something other than
		// Unknown Location
		GlobalProperty gp = new GlobalProperty(
				OpenmrsConstants.GLOBAL_PROPERTY_DEFAULT_LOCATION_NAME,
				"Test Parent Location", "Testing default Location");
		Context.getAdministrationService().saveGlobalProperty(gp);
		Assert.assertEquals("Test Parent Location", Context
				.getLocationService().getDefaultLocation().getName());
	}
	...
	public void getLocation_shouldReturnNullWhenNoLocationMatchGivenLocationName()
			throws Exception {
		Assert.assertNull(Context.getLocationService().getLocation(
	...
	public void getLocations_shouldReturnEmptyListWhenNoLocationMatchTheNameFragment()
		...
		Assert.assertEquals(0,
				Context.getLocationService().getLocations("Mansion").size());
	}

	/**
	 * @see LocationService#getLocations(String, org.openmrs.Location,
	 *      java.util.Map, boolean, Integer, Integer)
	 */
	@Test
	...
	public void getLocations_shouldNotFindAnyLocationsIfNoneHaveGivenAttributeValues() {
		...
		Assert.assertEquals(
				...
				Context.getLocationService()
						.getLocations(null, null, attrValues, true, null, null)
						.size());
	}
	...
	public void getLocations_shouldGetLocationsHavingAllMatchingAttributeValues() {
		...
		// Assign phone number 0123456789 and email address admin@facility.com
		// to location #1
		...
		List<Location> locations = Context.getLocationService().getLocations(
				null, null, attrValues, false, null, null);
	...
	public void getLocationsByTag_shouldReturnEmptyListWhenNoLocationsHasTheGivenTag()
		...
		LocationService ls = Context.getLocationService();
		Assert.assertEquals(0,
				ls.getLocationsByTag(ls.getLocationTagByName("Retired")).size());
	}
	...
	public void getLocationsHavingAllTags_shouldReturnAllUnretiredLocationsGivenAnEmptyTagList()
		...
		Assert.assertEquals(5,
				ls.getLocationsHavingAllTags(Collections.EMPTY_LIST).size());
	}

	/**
	 * Get locations that have any of specified set of tags among its child
	 * tags.
	 * 
	...
	public void getLocationsHavingAnyTag_shouldReturnEmptyListWhenNoLocationHasTheGivenTags()
		...
		Assert.assertEquals(
				0,
				ls.getLocationsHavingAnyTag(
						Collections.singletonList(ls
								.getLocationTagByName("Retired"))).size());
	}
	...
	public void getLocationsHavingAnyTag_shouldReturnEmptyListWhenGivenAnEmptyTagList()
		...
		LocationService ls = Context.getLocationService();
		Assert.assertEquals(0,
				ls.getLocationsHavingAnyTag(new ArrayList<LocationTag>())
						.size());
	}
	...
	public void retireLocation_shouldRetireLocationSuccessfully()
		...
		// Both location lists that include retired should be equal in size and
		// not order of elements.
		assertEquals(locationsBeforeRetired.size(),
				locationsAfterRetired.size());

		// Both location lists that do not include retired should not be the
		// same.
	...
	/**
	 * Test to make sure that a simple save to a new location tag gets persisted
	 * to the database
	 * 
	...
	public void saveLocationTag_shouldCreateLocationTagSuccessfully()
		...
		assertNotNull("The saved tag should have an id now",
				tag.getLocationTagId());
		assertNotNull("We should get back a tag", newSavedTag);
		assertTrue("The created tag needs to equal the pojo location",
				tag.equals(newSavedTag));
	}
	...
	public void saveLocationTag_shouldUpdateLocationTagSuccessfully()
		...
		assertFalse("The name should be different", origName.equals(newName));
		assertTrue("The name should NOT be different", newestTag.getName()
				.equals(newName));
		assertFalse("The name should be different", origDesc.equals(newDesc));
		assertTrue("The name should NOT be different", newestTag
	...
	public void saveLocation_shouldAddLocationTagToLocation() throws Exception {
		...
		assertNotNull("newSavedLocation.tags must be not null",
				newSavedLocation.getTags());
		...
		assertTrue("Tag must be the previously added tag", newSavedLocation
				.getTags().iterator().next().equals(tag));
	}
	...
	public void saveLocation_shouldOverwriteTransientTagIfTagWithSameNameExists()
		...
		assertNotNull("Location tag should have an ID now", newSavedLocation
				.getTags().iterator().next().getLocationTagId());
		assertEquals(1, newSavedLocation.getTags().iterator().next()
				.getLocationTagId().intValue());
	}
	...
	public void getLocationTagByName_shouldGetLocationTagByName()
		...
		// Existing tag
		assertEquals(ls.getLocationTag(1),
				ls.getLocationTagByName("General Hospital"));
	...
	public void getLocationTagByName_shouldReturnNullWhenNoLocationTagMatchGivenName()
			throws Exception {
		Assert.assertNull(Context.getLocationService().getLocationTagByName(
				"Hospital of the year 2222"));
	}
	...
	public void getLocationTags_shouldReturnEmptyListWhenNoLocationTagMatchGivenSearchString()
			throws Exception {
		Assert.assertEquals(0,
				Context.getLocationService().getLocationTags("!!!").size());
	}
	...
	public void saveLocation_shouldReturnSavedObject() throws Exception {
		...
		Location savedLocation = Context.getLocationService().saveLocation(
				location);
	...
	public void saveLocation_shouldThrowAPIExceptionIfTransientTagIsNotFound()
			throws Exception {
		LocationTag tagWithoutName = new LocationTag("some random tag name",
				"a nonexistant tag");
		Location location = new Location();
		location.setName("Some name");
		location.setDescription("Some description");
	...
	public void saveLocationTag_shouldReturnSavedObject() throws Exception {
		LocationTag locationTag = new LocationTag("Some tag name",
				"Some description");
		LocationTag savedLocationTag = Context.getLocationService()
				.saveLocationTag(locationTag);
	...
	public void getLocationByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getLocationService().getLocationByUuid(
				"some invalid uuid"));
	}
	...
	public void getLocationTagByUuid_shouldFindObjectGivenValidUuid()
			throws Exception {
		Assert.assertEquals(Integer.valueOf(3), Context.getLocationService()
				.getLocationTagByUuid("0d0eaea2-47ed-11df-bc8b-001e378eb67e")
				.getLocationTagId());
	}
	...
	public void getLocationTagByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getLocationService().getLocationTagByUuid(
				"ffffffff-47ed-11df-bc8b-001e378eb67e"));
	}
	...
	public void getDefaultLocation_shouldReturnUnknownLocationIfTheGlobalPropertyIsSomethingElseThatDoesnotExist()
			throws Exception {
		// set the global property to something that has no match in the
		// location table
		GlobalProperty gp = new GlobalProperty(
				OpenmrsConstants.GLOBAL_PROPERTY_DEFAULT_LOCATION_NAME,
				"None existent Location", "Testing");
		Context.getAdministrationService().saveGlobalProperty(gp);
		Assert.assertEquals("Unknown Location", Context.getLocationService()
	...
	public void getAllLocations_shouldPushRetiredLocationsToTheEndOfTheListWhenIncludeRetiredIsTrue()
		...
		Assert.assertTrue("Retired locations should be at the end of the list",
				locations.get(locations.size() - 1).isRetired());
		Assert.assertTrue("Retired locations should be at the end of the list",
				locations.get(locations.size() - 2).isRetired());
	}
	...
	public void getAllLocationAttributeTypes_shouldReturnAllLocationAttributeTypesIncludingRetiredOnes()
		...
		Assert.assertEquals(2, Context.getLocationService()
				.getAllLocationAttributeTypes().size());
	}
	...
	public void getLocationAttributeType_shouldReturnTheLocationAttributeTypeWithTheGivenId()
			throws Exception {
		executeDataSet(LOC_ATTRIBUTE_DATA_XML);
		Assert.assertEquals("Audit Date", Context.getLocationService()
				.getLocationAttributeType(1).getName());
	}

	/**
	 * @see LocationService#getLocationAttributeType(Integer)
	 * @verifies return null if no location attribute type exists with the given
	 *           id
	 */
	@Test
	public void getLocationAttributeType_shouldReturnNullIfNoLocationAttributeTypeExistsWithTheGivenId()
			throws Exception {
		executeDataSet(LOC_ATTRIBUTE_DATA_XML);
		Assert.assertNull(Context.getLocationService()
				.getLocationAttributeType(999));
	}
	...
	public void getLocationAttributeTypeByUuid_shouldReturnNullIfNoLocationAttributeTypeExistsWithTheGivenUuid()
		...
		executeDataSet(LOC_ATTRIBUTE_DATA_XML);
		Assert.assertNull(Context.getLocationService()
				.getLocationAttributeTypeByUuid("not-a-uuid"));
	}
	...
	public void purgeLocationAttributeType_shouldCompletelyRemoveALocationAttributeType()
		...
		Assert.assertEquals(2, Context.getLocationService()
				.getAllLocationAttributeTypes().size());
		Context.getLocationService().purgeLocationAttributeType(
				Context.getLocationService().getLocationAttributeType(2));
		Assert.assertEquals(1, Context.getLocationService()
				.getAllLocationAttributeTypes().size());
	}
	...
	public void retireLocationAttributeType_shouldRetireALocationAttributeType()
			throws Exception {
		executeDataSet(LOC_ATTRIBUTE_DATA_XML);
		LocationAttributeType vat = Context.getLocationService()
				.getLocationAttributeType(1);
		...
		Context.getLocationService().retireLocationAttributeType(vat,
				"for testing");
	...
	public void saveLocationAttributeType_shouldCreateANewLocationAttributeType()
		...
		Assert.assertEquals(3, Context.getLocationService()
				.getAllLocationAttributeTypes().size());
	}
	...
	public void saveLocationAttributeType_shouldEditAnExistingLocationAttributeType()
		...
		Assert.assertEquals(2, service.getAllLocationAttributeTypes().size());
		Assert.assertEquals("A new name", service.getLocationAttributeType(1)
				.getName());
	}
	...
	public void getLocationAttributeByUuid_shouldGetTheLocationAttributeWithTheGivenUuid()
		...
		Assert.assertEquals(
				"2011-04-25",
				service.getLocationAttributeByUuid(
	...
	public void saveLocation_shouldSetAuditInfoIfAnyItemInTheLocationIsEdited()
		...
		Location editedLocation = Context.getLocationService().saveLocation(
				location);
	...
	public void saveLocationTag_shouldSetAuditInfoIfAnyItemInTheLocationTagIsEdited()
		...
		LocationTag editedTag = Context.getLocationService().saveLocationTag(
				tag);
		Context.flushSession();
	...
	public void getLocationAttributeTypeByName_shouldReturnTheLocationAttributeTypeWithTheSpecifiedName()
			throws Exception {
		executeDataSet(LOC_ATTRIBUTE_DATA_XML);
		LocationAttributeType locationAttributeType = Context
	...
	/**
	 * @see LocationService#getLocationAttributeTypeByName(String)
	 * @verifies return null if no location attribute type exists with the
	 *           specified name +
	 */
	@Test
	public void getLocationAttributeTypeByName_shouldReturnNullIfNoLocationAttributeTypeExistsWithTheSpecifiedName()
		...
		executeDataSet(LOC_ATTRIBUTE_DATA_XML);
		Assert.assertNull(Context.getLocationService()
				.getLocationAttributeTypeByName("not-a-name"));
	}
