...
/**
 * Unit tests for methods that are specific to the {@link ConceptServiceImpl}.
 * General tests that would span implementations should go on the
 * {@link ConceptService}.
 */
public class ConceptServiceImplTest extends BaseContextSensitiveTest {
	...
	public void saveConcept_shouldReturnTheConceptWithNewConceptIDIfCreatingNewConcept()
		...
		ConceptName fullySpecifiedName = new ConceptName(
				"requires one name min", new Locale("fr", "CA"));
	...
	/**
	 * @see ConceptServiceImpl#saveConcept(Concept)
	 * @verifies return the concept with same conceptID if updating existing
	 *           concept
	 */
	...
	public void saveConcept_shouldLeavePreferredNamePreferredIfSet()
		...
		fullySpecifiedName.setConceptNameType(ConceptNameType.FULLY_SPECIFIED); // be
																				// explicit
																				// for
																				// test
																				// case
		ConceptName shortName = new ConceptName("short name", loc);
		shortName.setConceptNameType(ConceptNameType.SHORT); // be explicit for
																// test case
		...
		indexTerm.setConceptNameType(ConceptNameType.INDEX_TERM); // synonyms
																	// are id'd
																	// by a null
																	// type

		// saveConcept never picks an index term for default, so we'll use it
		// for the test
		...
		assertTrue("name was explicitly marked preferred",
				c.getPreferredName(loc).isPreferred());
		assertEquals("name matches", c.getPreferredName(loc).getName(),
				indexTerm.getName());
	}
	...
	public void saveConcept_shouldNotSetDefaultPreferredNameToShortOrIndexTerms()
		...
		// The API will throw a validation error because preferred name is an
		// index term
		// ignore it so we can test the set default preferred name functionality
		try {
			Context.getConceptService().saveConcept(c);
		...
		assertFalse("name was explicitly marked preferred",
				shortName.isPreferred());
		assertFalse("name was explicitly marked preferred",
				indexTerm.isPreferred());
	}

	/**
	 * @see ConceptServiceImpl#saveConcept(Concept)
	 * @verifies set default preferred name to fully specified first If
	 *           Concept.getPreferredName(locale) returns null, saveConcept
	 *           chooses one. The default first choice is the fully specified
	 *           name in the locale. The default second choice is a synonym in
	 *           the locale.
	 */
	@Test
	public void saveConcept_shouldSetDefaultPreferredNameToFullySpecifiedFirst()
		...
		c.setConceptClass(new ConceptClass(1));
		assertFalse(
				"check test assumption - the API didn't automatically set preferred vlag",
				c.getFullySpecifiedName(loc).isPreferred());

		assertNotNull("Concept is legit, save succeeds", Context
				.getConceptService().saveConcept(c));
	...
	/**
	 * @see ConceptServiceImpl#saveConcept(Concept)
	 * @verifies set default preferred name to a synonym second If
	 *           Concept.getPreferredName(locale) returns null, saveConcept
	 *           chooses one. The default first choice is the fully specified
	 *           name in the locale. The default second choice is a synonym in
	 *           the locale.
	...
	public void saveConcept_shouldSetDefaultPreferredNameToASynonymSecond()
		...
		ConceptName fullySpecifiedName = new ConceptName("fully specified",
				otherLocale);
		fullySpecifiedName.setConceptNameType(ConceptNameType.FULLY_SPECIFIED); // be
																				// explicit
																				// for
		...
		assertNull(
				"check test assumption - the API hasn't promoted a name to a fully specified name",
		...
		assertEquals("name matches", c.getPreferredName(loc).getName(),
				synonym.getName());
		assertEquals("fully specified name unchanged",
				c.getPreferredName(otherLocale).getName(),
				fullySpecifiedName.getName());
	...
	public void saveConcept_shouldForceSetFlagIfSetMembersExist()
		...
		conceptSetMember.addName(new ConceptName("Set Member", new Locale("en",
				"US")));
		conceptSetMember.addDescription(new ConceptDescription(
