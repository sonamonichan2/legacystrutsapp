...
public class PatientDAOTest extends BaseContextSensitiveTest {
	...
	private void logPatient(Patient patient) {
		...
		builder.append("class=").append(patient.getClass().getCanonicalName())
				.append(", person=").append(patient.toString())
				.append(", person.names=")
				.append(patient.getNames().toString())
				.append(", person.attributes=")
				.append(patient.getAttributes().toString());
	...
	/**
	 * @see 
	 *      PatientDAO#getPatients(String,String,List<QPatientIdentifierType;>,null
	 *      )
	 */
	@Test
	...
	public void getPatients_shouldEscapeAnAsterixCharacterInIdentifierPhrase()
			throws Exception {
		// Note that all tests for wildcard should be pass in 2s due to the
		...
		PatientIdentifier patientIdentifier = new PatientIdentifier("*567",
				pService.getPatientIdentifierType(5), Context
		...
		PatientIdentifier patientIdentifier6 = new PatientIdentifier("4567",
				pService.getPatientIdentifierType(5), Context
						.getLocationService().getLocation(1));
	...
	public void getPatients_shouldEscapeUnderscoreCharacterInIdentifierPhrase()
			throws Exception {
		deleteAllData();
		baseSetupWithStandardDataAndAuthentication();
		Patient patient2 = pService.getPatient(2);
		PatientIdentifier patientIdentifier = new PatientIdentifier("_567",
				pService.getPatientIdentifierType(5), Context
						.getLocationService().getLocation(1));
		patient2.addIdentifier(patientIdentifier);
		pService.savePatient(patient2);
	...
	public void getAllPatientIdentifierTypes_shouldNotReturnRetired()
			throws Exception {
		List<PatientIdentifierType> patientIdentifierTypes = dao
				.getAllPatientIdentifierTypes(false);
		Assert.assertEquals(
				"patientIdentifierTypes list should have 3 elements", 3,
				patientIdentifierTypes.size());
	}
	...
	public void getAllPatientIdentifierTypes_shouldReturnAll() throws Exception {
		...
		Assert.assertEquals(
				"patientIdentifierTypes list should have 4 elements", 4,
				patientIdentifierTypes.size());
	}
	...
	public void getPatientIdentifiers_shouldLimitByResultsByLocation()
			throws Exception {
		Location location = Context.getLocationService().getLocation(3); // there
																			// is
																			// only
																			// one
																			// identifier
																			...
																			// location
																			// 3
		List<PatientIdentifier> patientIdentifiers = dao.getPatientIdentifiers(
				null, new ArrayList<PatientIdentifierType>(),
				Collections.singletonList(location), new ArrayList<Patient>(),
				null);
	...
	public void getPatientIdentifiers_shouldNotGetVoidedPatientIdentifiers()
		...
		//
		// plus 1 non-voided identifier from
		// HibernatePatientDAOTest-patients.xml
	...
	/**
	 * @see PatientDAO#getPatientIdentifiers(String,List,List,List,Boolean)
	 * @verifies not fetch patient identifiers that partially matches given
	 *           identifier
	 */
	@Test
	public void getPatientIdentifiers_shouldNotFetchPatientIdentifiersThatPartiallyMatchesGivenIdentifier()
		...
		List<PatientIdentifier> patientIdentifiers = dao.getPatientIdentifiers(
				identifier, new ArrayList<PatientIdentifierType>(),
				new ArrayList<Location>(), new ArrayList<Patient>(), null);
	...
	public void getPatientIdentifiers_shouldFetchPatientIdentifiersThatEqualsGivenIdentifier()
		...
		Assert.assertEquals(1, patientIdentifiers.size());
		Assert.assertEquals(identifier, patientIdentifiers.get(0)
				.getIdentifier());
	}

	/**
	 * @see PatientDAO#getPatientIdentifiers(String,List,List,List,Boolean)
	 * @verifies return all matching non voided patient identifiers if is
	 *           preferred is set to false
	 */
	@Test
	public void getPatientIdentifiers_shouldReturnAllMatchingNonVoidedPatientIdentifiersIfIsPreferredIsSetToFalse()
		...
		List<PatientIdentifier> patientIdentifiers = dao.getPatientIdentifiers(
				null, new ArrayList<PatientIdentifierType>(),
				new ArrayList<Location>(), new ArrayList<Patient>(),
				Boolean.FALSE);
	...
	public void getPatientIdentifiers_shouldFetchAllPatientIdentifiersBelongToGivenPatients()
		...
		// There is one identifier[id=12345K] in the test database for patient
		// with id 6
		Patient patientWithId6 = Context.getPatientService().getPatient(6);

		// There is one identifier[id=6TS-4] in the test database for patient
		// with id 7
		Patient patientWithId7 = Context.getPatientService().getPatient(7);

		List<Patient> patientsList = Arrays.asList(patientWithId6,
				patientWithId7);
		...
		assertThat(
				patientIdentifiers,
				containsInAnyOrder(hasIdentifier("12345K"),
						hasIdentifier("6TS-4")));
	}
	...
	private Matcher<PatientIdentifier> hasIdentifier(final String identifier) {
		...
		return new FeatureMatcher<PatientIdentifier, String>(is(identifier),
	...
	public void getAllPatientIdentifierTypes_shouldReturnOrdered()
			throws Exception {
		// given
		PatientIdentifierType patientIdentifierType1 = dao
				.getPatientIdentifierType(1); // non retired, non required

		PatientIdentifierType patientIdentifierType2 = dao
				.getPatientIdentifierType(2); // non retired, required
		patientIdentifierType2.setRequired(true);
		dao.savePatientIdentifierType(patientIdentifierType2);

		PatientIdentifierType patientIdentifierType4 = dao
				.getPatientIdentifierType(4); // retired

		PatientIdentifierType patientIdentifierType5 = dao
				.getPatientIdentifierType(5); // non retired, non required

		// when
		List<PatientIdentifierType> all = dao
				.getAllPatientIdentifierTypes(true);

		// then
		Assert.assertArrayEquals(new Object[]{patientIdentifierType2,
				patientIdentifierType1, patientIdentifierType5,
				patientIdentifierType4}, all.toArray());
	}

	/**
	 * @see PatientDAO#getPatientIdentifierTypes(String, String, Boolean,
	 *      Boolean)
	...
	public void getPatientIdentifierTypes_shouldReturnNonRetiredPatientIdentifierTypesWithGivenName() {
		PatientIdentifierType oldIdNumberNonRetired = dao
				.getPatientIdentifierType(2);

		List<PatientIdentifierType> patientIdentifierTypes = dao
				.getPatientIdentifierTypes("Old Identification Number", null,
						null, null);

		Assert.assertEquals(patientIdentifierTypes.size(), 1);
		Assert.assertEquals(oldIdNumberNonRetired,
				patientIdentifierTypes.get(0));
	}
	 ...
	 * @see PatientDAO#getPatientIdentifierTypes(String, String, Boolean,
	 *      Boolean)
	 * @verifies return non retired patient identifie types that are not
	 *           required
	 */
	@Test
	public void getPatientIdentifierTypes_shouldReturnNonRetiredPatientIdentifierTypesThatAreNotRequired() {
		PatientIdentifierType nonRetiredNonRequired1 = dao
				.getPatientIdentifierType(1);
		PatientIdentifierType nonRetiredNonRequired2 = dao
				.getPatientIdentifierType(2);
		PatientIdentifierType nonRetiredNonRequired3 = dao
		...
		Assert.assertTrue(patientIdentifierTypes
				.contains(nonRetiredNonRequired1));
		Assert.assertTrue(patientIdentifierTypes
				.contains(nonRetiredNonRequired2));
		Assert.assertTrue(patientIdentifierTypes
				.contains(nonRetiredNonRequired3));
	}
	...
	public void getPatientIdentifierTypes_shouldReturnNonRetiredPatientIdentifierTypesThatAreRequired() {
		PatientIdentifierType nonRetiredRequired = dao
	...
	public void getPatientIdentifierTypes_shouldReturnOnlyNonRetiredPatientIdentifierTypes() {
		...
		List<PatientIdentifierType> patientIdentifierTypes = dao
				.getPatientIdentifierTypes(null, null, null, null);
	 ...
	 * @see PatientDAO#getPatientIdentifierTypes(String, String, Boolean,
	 *      Boolean)
	 * @verifies return non retired patient identifier types ordered by required
	 *           first
	 */
	@Test
	public void getPatientIdentifierTypes_shouldReturnNonRetiredPatientIdentifierTypes_OrderedByRequiredFirst() {
		PatientIdentifierType nonRetiredNonRequiredType1 = dao
				.getPatientIdentifierType(1);
		PatientIdentifierType nonRetiredNonRequiredType2 = dao
				.getPatientIdentifierType(5);
		PatientIdentifierType nonRetiredRequiredType = dao
				.getPatientIdentifierType(2);
		...
		Assert.assertArrayEquals(new Object[]{nonRetiredRequiredType,
				nonRetiredNonRequiredType1, nonRetiredNonRequiredType2},
	...
	public void getPatientIdentifierTypes_shouldReturnNonRetiredPatientIdentifierTypes_OrderedByRequiredAndName() {
		...
		Assert.assertArrayEquals(new Object[]{socialSecNumber, oldIdNumber,
				openMRSIdNumber, nationalIdNo},
				patientIdentifierTypes.toArray());
	}
	...
	public void getPatientIdentifierTypes_shouldReturnNonRetiredPatientIdentifierTypes_OrderedByRequiredNameAndTypeId() {
		...
		Assert.assertArrayEquals(new Object[]{openMRSIdNumber, oldIdNumber,
				socialSecNumber, nationalIdNo},
				patientIdentifierTypes.toArray());
	}

	/**
	 * @see PatientDAO#getPatients(String, String, java.util.List, boolean,
	 ...
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	...
	public void getPatients_shouldGetPatientByFamilyName_SignatureNo1()
		...
		Assert.assertFalse(patients.get(0).getGivenName()
				.equalsIgnoreCase(patients.get(1).getGivenName()));
	}
	...
	public void getPatients_shouldGetPatientByWholeName_SignatureNo1()
		...
		List<Patient> patients = dao.getPatients(
				"Bilbo Odilon B. Baggins Senior", 0, 11);
	...
	/**
	 * @verifies not get patient by mix of existing and non-existing name parts
	 *           _ signature no 1
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	 *      boolean, Integer, Integer, boolean)
	 */
	...
	public void getPatients_shouldNotGetPatientByMixOfExistingAndNonexistingNameParts_SignatureNo1()
			throws Exception {
		List<Patient> patients = dao.getPatients("Bilbo Odilon X. Baggins", 0,
				11);
	...
	public void getPatients_shouldGetPatientByShortGivenName_SignatureNo1()
		...
		Assert.assertFalse(patients.get(0).getMiddleName()
	...
	/**
	 * @verifies get patient by whole name made up of short names _ signature no
	 *           1
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	 *      boolean, Integer, Integer, boolean)
	 */
	...
	/**
	 * @verifies not get patient by non-existing single short name _ signature
	 *           no 1
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	 *      boolean, Integer, Integer, boolean)
	 */
	...
	/**
	 * @verifies not get patient by non-existing short name parts _ signature no
	 *           1
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	 *      boolean, Integer, Integer, boolean)
	 */
	...
	/**
	 * @verifies not get patient by mix of existing and non-existing short name
	 *           parts _ signature no 1
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	 *      boolean, Integer, Integer, boolean)
	 */
	@Test
	...
	public void getPatients_shouldGetPatientsWithMatchModeStart_SignatureNo1()
		...
		if (oldPropertyValue != null) {
			...
			globalPropertiesTestHelper
					.purgeGlobalProperty(OpenmrsConstants.GLOBAL_PROPERTY_PATIENT_SEARCH_MATCH_MODE);
		}
	}
	...
	/**
	 * @verifies get patient by searching on names or identifiers and using name
	 *           value as identifier parameter _ signature no 1
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	 *      boolean, Integer, Integer, boolean)
	 */
	@Test
	...
	/**
	 * @verifies get patient by searching on names or identifiers and using
	 *           identifier value as name parameter _ signature no 1
	 * @see HibernatePatientDAO#getPatients(String, String, java.util.List,
	 *      boolean, Integer, Integer, boolean)
	 */
	@Test
	...
	public void getPatients_shouldGetNoPatientByNonexistingAttribute_SignatureNo2()
			throws Exception {
		Assert.assertFalse(personAttributeHelper
				.personAttributeExists("Wizard"));
	...
	public void getPatients_shouldGetNoPatientByNonsearchableAttribute_SignatureNo2()
			throws Exception {
		Assert.assertTrue(personAttributeHelper
				.nonSearchablePersonAttributeExists("Mushroom pie"));
	...
	public void getPatients_shouldGetNoPatientByVoidedAttribute_SignatureNo2()
			throws Exception {
		Assert.assertTrue(personAttributeHelper
				.voidedPersonAttributeExists("Master thief"));
	...
	public void getPatients_shouldGetOnePatientByAttribute_SignatureNo2()
		...
		Assert.assertTrue(personAttributeHelper
				.personAttributeExists("Story teller"));
	...
	/**
	 * @verifies not get patients by searching for non-voided and voided
	 *           attribute _ signature no 2
	 * @see HibernatePatientDAO#getPatients(String, Integer, Integer)
	 */
	@Test
	public void getPatients_shouldNotGetPatientsBySearchingForNonvoidedAndVoidedAttribute_SignatureNo2()
		...
		Assert.assertTrue(personAttributeHelper
				.personAttributeExists("Story teller"));
		Assert.assertFalse(personAttributeHelper
				.voidedPersonAttributeExists("Story teller"));
		Assert.assertTrue(personAttributeHelper
				.voidedPersonAttributeExists("Master thief"));
	...
	public void getPatients_shouldGetMultiplePatientsBySingleAttribute_SignatureNo2()
		...
		globalPropertiesTestHelper
				.setGlobalProperty(
						...
						OpenmrsConstants.GLOBAL_PROPERTY_PERSON_ATTRIBUTE_SEARCH_MATCH_ANYWHERE);
		Assert.assertTrue(personAttributeHelper
				.personAttributeExists("Senior ring bearer"));
	...
	public void getPatients_shouldNotGetPatientsByMultipleAttributes_SignatureNo2()
			throws Exception {
		Assert.assertTrue(personAttributeHelper
				.searchablePersonAttributeExists("Story teller"));
		Assert.assertTrue(personAttributeHelper
				.nonVoidedPersonAttributeExists("Story teller"));

		Assert.assertTrue(personAttributeHelper
				.searchablePersonAttributeExists("Senior ring bearer"));
		Assert.assertTrue(personAttributeHelper
				.nonVoidedPersonAttributeExists("Senior ring bearer"));
	...
	/**
	 * @verifies find the remaining three out of eleven patients _ signature no
	 *           2
	 * @see HibernatePatientDAO#getPatients(String, Integer, Integer)
	 */
	@Test
	...
	public void getPatients_shouldFindPatientsWithExcessiveLength_SignatureNo2()
			throws Exception {
		List<Patient> patients = dao.getPatients("Saruman", 0,
				HibernatePersonDAO.getMaximumSearchResults() + 42);
		Assert.assertEquals(11, patients.size());
	}
	...
	public void getPatients_shouldReturnDistinctPatientList_SignatureNo2()
			throws Exception {
		Assert.assertTrue(personAttributeHelper
				.searchablePersonAttributeExists("Cook"));

		List<Patient> patientsByName = dao.getPatients("Adalgrim Took Cook", 0,
				11);
		Assert.assertEquals(1, patientsByName.size());

		List<Patient> patientsByNameOrAttribute = dao
				.getPatients("Cook", 0, 11);
	...
	/**
	 * @verifies count zero patients when name and identifier and list of
	 *           identifier types are empty _ signature no 1
	 * @see HibernatePatientDAO#getCountOfPatients(String, String,
	...
	/**
	 * @verifies count zero patients when name and identifier and list of
	 *           identifier types are null _ signature no 1
	 * @see HibernatePatientDAO#getCountOfPatients(String, String,
	 *      java.util.List, boolean, boolean)
	 */
	@Test
	...
	public void getDuplicatePatients_shouldGetDuplicatesWithBirthDate()
		...
		List<Patient> patients = dao
				.getDuplicatePatientsByAttributes(attributes);
	...
	/**
	 * @verifies get duplicate patients when gender, birthDate and giveName
	 *           match
	 * @see HibernatePatientDAO#getDuplicatePatientsByAttributes(List)
	 */
	@Test
	...
	/**
	 * @verifies get duplicate patients when gender, birthdate, givenName and
	 *           familyName match
	 * @see HibernatePatientDAO#getDuplicatePatientsByAttributes(List)
	 */
	@Test
	...
	/**
	 * @verifies get duplicate patients when gender, birthdate, givenName,
	 *           familyName and identifier match
	 * @see HibernatePatientDAO#getDuplicatePatientsByAttributes(List)
	 */
	@Test
	...
	/**
	 * @verifies get duplicate patients returns 0 duplicates with
	...
	/**
	 * @verifies get duplicate patients returns duplicates with birthdate when
	 *           one invalid/unsupported attribute is passed along with
	 *           birthdate
	 * @see HibernatePatientDAO#getDuplicatePatientsByAttributes(List)
	 */
	@Test
