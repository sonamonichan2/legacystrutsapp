 ...
 * Currently that file is the {@link HibernateEncounterDAO} This should only
 * have to test methods that don't really have equivalents at the
 * {@link EncounterService} layer.
 */
public class EncounterDAOTest extends BaseContextSensitiveTest {
	...
	public void getSavedEncounterDatetime_shouldGetSavedEncounterDatetimeFromDatabase()
		...
		Date encounterDateFromDatabase = dao
				.getSavedEncounterDatetime(encounter);
		assertEquals(origDate, encounterDateFromDatabase);
	}

	/**
	 * @see EncounterDAO#getEncounters(query, patientId, start, length,
	 *      includeVoided)
	 */
	@Test
	...
	public void getEncounters_shouldWork_WithNameQuery() throws Exception {
		...
		List<Encounter> encounters = dao.getEncounters("John Doe", null, null,
				null, true);
	...
	public void getEncounters_shouldWork_WithIdentifierQuery() throws Exception {
		...
		List<Encounter> encountersByNumericIdentifier = dao.getEncounters(
				"1234", null, null, null, true);
		assertEquals(expectedEncountersForPatientOne,
				encountersByNumericIdentifier);

		List<Encounter> encountersByStringIdentifier = dao.getEncounters(
				"abcd", null, null, null, true);
		assertEquals(expectedEncountersForPatientOne,
				encountersByStringIdentifier);
	}
	...
	public void getEncounters_shouldNotWork_WithPartialIdentifier()
			throws Exception {
		List<Encounter> encountersByPartialIdentifier = dao.getEncounters(
				"123", null, null, null, true);
