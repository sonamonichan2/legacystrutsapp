...
public class PatientSearchCriteriaTest extends BaseContextSensitiveTest {
	...
	public void getQueryParts_shouldProcessSimpleSpaceAsSeparator()
			throws Exception {
		String[] actual = patientSearchCriteria
				.getQueryParts("Anton Bert Charles");
	...
	public void getQueryParts_shouldProcessMixedSeparators() throws Exception {
		String[] actual = patientSearchCriteria
				.getQueryParts("Anton,Bert, Charles Dorian  Ernie");
	...
	public void getMatchMode_shouldReturnStartAsDefaultMatchMode()
		...
		Assert.assertEquals(MatchMode.START,
				patientSearchCriteria.getMatchMode());
	}
	...
	public void getMatchMode_shouldReturnAnywhereAsConfiguredMatchMode()
		...
		Assert.assertEquals(MatchMode.ANYWHERE,
				patientSearchCriteria.getMatchMode());
	 ...
	 * @see PatientSearchCriteria#getSearchMode(String, String, java.util.List,
	 *      boolean)
	 */
	@Test
	public void getSearchMode_shouldIdentifySearchByName() throws Exception {
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_NAME,
				patientSearchCriteria.getSearchMode("name", null, null, false));

		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_NAME,
				patientSearchCriteria.getSearchMode("name", "", null, false));

		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_NAME,
				patientSearchCriteria.getSearchMode("name", "  \n\t", null,
						false));

		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_NAME,
				patientSearchCriteria.getSearchMode("name", null,
	...
	public void getSearchMode_shouldIdentifySearchByIdentifier()
			throws Exception {
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode(null, "identifier", null,
						false));

		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode("", "identifier", null,
						false));

		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode("  \n\t", "identifier",
						null, false));
	}
	...
	public void getSearchMode_shouldIdentifySearchByIdentifierTypeList()
		...
		//
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode(null, null,
						patientIdentifierTypeList, false));
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode("", null,
						patientIdentifierTypeList, false));
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode("  \n\t", null,
						patientIdentifierTypeList, false));
	...
	public void getSearchMode_shouldIdentifySearchByIdentifierAndIdentifierTypeList()
		...
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode(null, "identifier",
						patientIdentifierTypeList, false));
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode("", "identifier",
						patientIdentifierTypeList, false));
		Assert.assertEquals(PatientSearchMode.PATIENT_SEARCH_BY_IDENTIFIER,
				patientSearchCriteria.getSearchMode("  \n\t", "identifier",
						patientIdentifierTypeList, false));
	...
	public void getSearchMode_shouldIdentifySearchByNameOrIdentifier()
		...
		// test cases where "name" is the only parameter that is non-empty and
		// non-blank
		//
		Assert.assertEquals(
				PatientSearchMode.PATIENT_SEARCH_BY_NAME_OR_IDENTIFIER,
				patientSearchCriteria.getSearchMode("name", null, null, true));

		Assert.assertEquals(
				PatientSearchMode.PATIENT_SEARCH_BY_NAME_OR_IDENTIFIER,
				patientSearchCriteria.getSearchMode("name", "", null, true));
		...
		// test cases where "identifier" is the only parameter that is non-empty
		// and non-blank
		//
		Assert.assertEquals(
				PatientSearchMode.PATIENT_SEARCH_BY_NAME_OR_IDENTIFIER,
		...
		// test cases where 'patientIdentifierTypeList' is the only parameter
		// that is non-empty and non-blank
		//
		// testing variations of empty or blank value for name
		//
		Assert.assertEquals(
	...
	public void getSearchMode_shouldIdentifySearchByNameAndIdentifier()
		...
		// test cases where "name" and "identifier" are the only parameters that
		// are non-empty and non-blank
		//
		Assert.assertEquals(
				PatientSearchMode.PATIENT_SEARCH_BY_NAME_AND_IDENTIFIER,
				patientSearchCriteria.getSearchMode("name", "identifier", null,
						false));
		Assert.assertEquals(
				PatientSearchMode.PATIENT_SEARCH_BY_NAME_AND_IDENTIFIER,
				patientSearchCriteria.getSearchMode("name", "identifier",
						new ArrayList<PatientIdentifierType>(), false));

		// test cases where "name" and 'identifierTypeList' are the only
		// parameters that are non-empty and non-blank
		//
		Assert.assertEquals(
				PatientSearchMode.PATIENT_SEARCH_BY_NAME_AND_IDENTIFIER,
				patientSearchCriteria.getSearchMode("name", null,
	...
	public void copySearchParameter_shouldReturnSourceValueWhenTargetIsBlank()
		...
		Assert.assertEquals("source",
				patientSearchCriteria.copySearchParameter("source", null));
		Assert.assertEquals("source",
				patientSearchCriteria.copySearchParameter("source", ""));
		Assert.assertEquals("source",
				patientSearchCriteria.copySearchParameter("source", "   \n\t "));
	}
	...
	public void copySearchParameter_shouldReturnTargetValueWhenTargetIsNonblank()
		...
		Assert.assertEquals("target",
				patientSearchCriteria.copySearchParameter(null, "target"));
		Assert.assertEquals("target",
				patientSearchCriteria.copySearchParameter("", "target"));
		Assert.assertEquals("target",
				patientSearchCriteria.copySearchParameter("   \n\t ", "target"));
		Assert.assertEquals("target",
				patientSearchCriteria.copySearchParameter("source", "target"));
	}
}
