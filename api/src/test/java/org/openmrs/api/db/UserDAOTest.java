...
public class UserDAOTest extends BaseContextSensitiveTest {
	...
	/**
	 * @verifies {@link UserDAO#getUsers(String,List<QRole;>,null)} test =
	...
	public void getUsers_shouldEscapeSqlWildcardsInSearchPhrase()
		...
		String wildcards[] = new String[]{"_"}; // we used to also test %, but
												// UserValidator actually
												// doesn't allow that in
												// usernames. TODO: remove the
												// loop
		// for each of the wildcards in the array, insert a user with a username
		// or names
		// with the wildcards and carry out a search for that user
		for (String wildcard : wildcards) {

			PersonName name = new PersonName(wildcard + "cats", wildcard
					+ "and", wildcard + "dogs");
			name.setDateCreated(new Date());
			u.addName(name);
			...
			int size = dao.getUsers(wildcard + "ca", null, false, null, null)
					.size();
			assertEquals(1, size);

			// if actually the search returned the matching name or system id
			String userName = (dao.getUsers(wildcard + "ca", null, false, null,
					null).get(0).getUsername());
			assertEquals("Test failed since no user containing the character "
					+ wildcard + " was found, ", wildcard + "test" + wildcard,
					userName);
	...
	public void updateUserPassword_shouldNotOverwriteUserSecretQuestionOrAnswer()
		...
		String hashedSecretAnswer = Security.encodeString(SECRET_ANSWER
				+ lc.getSalt());
		assertEquals("question should be set", SECRET_QUESTION,
				lc.getSecretQuestion());
		assertEquals("answer should be set", hashedSecretAnswer,
				lc.getSecretAnswer());
		dao.changePassword(userJoe, "Openmr6zz");
		lc = dao.getLoginCredential(userJoe);
		assertEquals("question should not have changed", SECRET_QUESTION,
				lc.getSecretQuestion());
		assertEquals("answer should not have changed", hashedSecretAnswer,
				lc.getSecretAnswer());
	}
	...
	public void changeHashedPassword_shouldNotOverwriteUserSecretQuestionOrAnswer()
		...
		assertEquals("answer should be set", hashedSecretAnswer,
				lc.getSecretAnswer());
		userJoe.setUserProperty("foo", "bar");
		dao.changeHashedPassword(userJoe, "VakesJkw1",
