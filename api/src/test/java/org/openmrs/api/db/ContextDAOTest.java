...
/**
 * This class tests the {@link ContextDAO} linked to from the Context. Currently
 * that file is the {@link HibernateContextDAO}.<br>
 * <br>
...
public class ContextDAOTest extends BaseContextSensitiveTest {
	...
	public void authenticate_shouldAuthenticateGivenUsernameAndPassword()
		...
		Assert.assertEquals("Should be the admin user", "admin",
				u.getUsername());
	}
	...
	public void authenticate_shouldGiveIdenticalErrorMessagesBetweenUsernameAndPasswordMismatch()
		...
		Assert.assertNotNull(
				"This test depends on there being an admin:test user", user);
		...
		try {
			...
			invalidUsernameErrorMessage = invalidUsernameErrorMessage.replace(
		...
		try {
			...
			invalidPasswordErrorMessage = invalidPasswordErrorMessage.replace(
					"admin", "");
		}

		Assert.assertEquals(invalidUsernameErrorMessage,
	...
	/**
	 * #1580: If you type your password wrong, then log in correctly, the API
	 * will not lock you out after multiple login attempts in the future
	 * 
	 * @see ContextDAO#authenticate(String,String)
	 */
	@Test(expected = ContextAuthenticationException.class)
	...
	/**
	 * @verifies {@link ContextDAO#authenticate(String,String)} test = should
	 *           throw a ContextAuthenticationException if username is an empty
	 *           string
	 */
	@Test(expected = ContextAuthenticationException.class)
	...
	/**
	 * @verifies {@link ContextDAO#authenticate(String,String)} test = should
	 *           throw a ContextAuthenticationException if username is white
	 *           space
	 */
	@Test(expected = ContextAuthenticationException.class)
	...
	public void authenticate_shouldThrowAPIExceptionIfUsernameIsWhiteSpace()
			throws Exception {
		// it would be illegal to save this user (with a whitespace username)
		// but we can get it in the db via xml
