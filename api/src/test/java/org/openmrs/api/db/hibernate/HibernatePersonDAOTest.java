...
public class HibernatePersonDAOTest extends BaseContextSensitiveTest {
	...
	private final static Log log = LogFactory
			.getLog(HibernatePersonDAOTest.class);
	...
	public void getPersonDAO() throws Exception {
		...
		hibernatePersonDAO = (HibernatePersonDAO) applicationContext
				.getBean("personDAO");
	...
	private void logPerson(Person person) {
		...
		builder.append("class=").append(person.getClass().getCanonicalName())
				.append(", person=").append(person.toString())
				.append(", person.names=").append(person.getNames().toString())
				.append(", person.attributes=")
				.append(person.getAttributes().toString());
	...
	public void getPeople_shouldGetEveryOneExceptVoidedByEmptyString()
		...
		// PEOPLE_FROM_THE_SHIRE_XML contains 7 people but more people are
		// defined in the standard test data set
		Assert.assertTrue(people.size() >= 7);

		// assert that all 7 people from PEOPLE_FROM_THE_SHIRE_XML (who are
		// neither dead nor voided) are retrieved
	...
	private void assertPeopleContainPersonID(List<Person> people,
			Integer personID) {
		for (Person person : people) {
		...
		Assert.fail("list of people does not contain person with ID = "
				+ personID);
	}
	...
	public void getPeople_shouldGetNoOneByNonsearchableAttribute()
		...
		Assert.assertTrue(personAttributeHelper
				.nonSearchablePersonAttributeExists("Porridge with honey"));

		List<Person> people = hibernatePersonDAO.getPeople("Porridge honey",
				false);
		logPeople(people);
	...
	public void getPeople_shouldGetNoOneByVoidedAttribute() throws Exception {
		...
		List<Person> people = hibernatePersonDAO.getPeople("Master thief",
				false);
		logPeople(people);
	...
	public void getPeople_shouldGetOnePersonByAttribute() throws Exception {
		...
		List<Person> people = hibernatePersonDAO.getPeople("Story Teller",
				false);
		logPeople(people);
	...
	/**
	 * @verifies get one person by searching for a mix of attribute and voided
	 *           attribute
	 * @see HibernatePersonDAO#getPeople(String, Boolean)
	 */
	@Test
	...
	public void getPeople_shouldGetMultiplePeopleBySingleAttribute()
		...
		Assert.assertFalse(people.get(0).getGivenName()
				.equalsIgnoreCase(people.get(1).getGivenName()));
	}
	...
	public void getPeople_shouldGetMultiplePeopleByMultipleAttributes()
		...
		Assert.assertTrue(personAttributeHelper
				.personAttributeExists("Story teller"));
		List<Person> people = hibernatePersonDAO.getPeople("Story Bearer",
				false);
		logPeople(people);
	...
	public void getPeople_shouldGetNoOneByNonexistingNameAndNonexistingAttribute()
		...
		List<Person> people = hibernatePersonDAO.getPeople("Gandalf Wizard",
				false);
		logPeople(people);
	...
	public void getPeople_shouldGetMultiplePeopleByGivenName() throws Exception {
		...
		Assert.assertTrue(people.get(0).getMiddleName() != people.get(1)
				.getMiddleName());
	}
	...
	public void getPeople_shouldGetMultiplePeopleByMiddleName()
		...
		Assert.assertTrue(people.get(0).getFamilyName() != people.get(1)
				.getFamilyName());
	}
	...
	public void getPeople_shouldGetMultiplePeopleByFamilyName()
		...
		Assert.assertTrue(people.get(0).getGivenName() != people.get(1)
				.getGivenName());
	}
	...
	public void getPeople_shouldGetMultiplePeopleByFamilyName2()
		...
		Assert.assertEquals(2, people.size());
		Assert.assertEquals("papa", people.get(0).getPersonName()
				.getFamilyName2());
		Assert.assertEquals("papa", people.get(1).getPersonName()
				.getFamilyName2());
		Assert.assertTrue(people.get(0).getFamilyName() != people.get(1)
				.getFamilyName());
	}
	...
	public void getPeople_shouldGetOnePersonByMultipleNameParts()
			throws Exception {
		List<Person> people = hibernatePersonDAO.getPeople("echo india mike",
				false);
		logPeople(people);
	...
	public void getPeople_shouldGetMultiplePeopleByMultipleNameParts()
			throws Exception {
		List<Person> people = hibernatePersonDAO.getPeople(
				"bravo delta golf juliet mike ", false);
		logPeople(people);
	...
	public void getPeople_shouldGetNoOneByVoidedName() throws Exception {
		List<Person> people = hibernatePersonDAO.getPeople("voided-delta",
				false);
		logPeople(people);
	...
	public void getPeople_shouldGetVoidedByVoidedNameWhenVoidedIsTrue()
			throws Exception {
		List<Person> people = hibernatePersonDAO.getPeople("voided-bravo",
				false, true);
		logPeople(people);
	...
	public void getPeople_shouldNotGetDeadPerson() throws Exception {
		List<Person> people = hibernatePersonDAO.getPeople("dead-charlie",
				false);
		logPeople(people);
	...
	public void getPeople_shouldGetMultipleDeadPeople() throws Exception {
		...
		Assert.assertEquals(2, people.size());
		Assert.assertEquals("dead-papa", people.get(0).getPersonName()
				.getFamilyName2());
		Assert.assertEquals("dead-papa", people.get(1).getPersonName()
				.getFamilyName2());
		Assert.assertTrue(people.get(0).getFamilyName() != people.get(1)
				.getFamilyName());
	}
	...
	public void getPeople_shouldObeyAttributeMatchMode() throws Exception {
		// exact match mode
		long patientCount = hibernatePersonDAO.getPeople("337-4820", false)
				.size();
	...
	public void savePerson_shouldSavePersonWithBirthDateTime() throws Exception {
		...
		Assert.assertEquals(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
				.parse("2012-05-29 15:23:56"), savedPerson.getBirthDateTime());
	}
