...
public class OrderSetServiceTest extends BaseContextSensitiveTest {
	...
	public void shouldSaveOrderSet() throws Exception {
		...
		Integer initialNumberOfOrderSets = orderSetService.getOrderSets(false)
				.size();
		...
		assertEquals(
				"A new order set was saved to the exisiting list of order sets",
				initialNumberOfOrderSets + 1, orderSets.size());
	...
	public void shouldSaveAndUpdateOrderSet() throws Exception {
		...
		assertNull("OrderSet is new and is not changed",
				orderSetObj.getChangedBy());
		assertNull("OrderSet is new and has no change date",
				orderSetObj.getDateChanged());
		...
		assertNotNull("OrderSet has a order_set_id", orderSetObj.getId()
				.intValue());
		assertEquals("OrderSet has updated description",
				"Test Order Set Description Updated",
				orderSetObj.getDescription());
		assertEquals("OrderSet has updated operator", "ONE", orderSetObj
				.getOperator().toString());

		assertNotNull("OrderSet has been changed", orderSetObj.getChangedBy());
		assertNotNull("OrderSet has been changed on some date",
				orderSetObj.getDateChanged());
	...
	public void shouldRetrieveOrderSetMembersOfAnOrderSet() throws Exception {
		...
		assertEquals("OrderSet should contain orderSetmembers", 2, orderSet
				.getOrderSetMembers().size());

		OrderSet orderSet1 = orderSetService.getOrderSet(2000);

		assertEquals("OrderSet should not contain retired orderSetMembers", 2,
				orderSet1.getOrderSetMembers().size());
	}
	...
	public void shouldNotAutomaticallyPropagateToSetMembersIfExcludingOrderSet()
		...
		for (OrderSet oS : orderSets) {
			numberOfOrderSetMembers = numberOfOrderSetMembers
					+ oS.getOrderSetMembers().size();
		}
	...
	public void shouldAddOrderSetMemberAtLastPositionInsideAnExistingOrderSetIfPositionIsNull()
		...
		OrderSet savedOrderSet = Context.getOrderSetService()
				.getOrderSetByUuid(orderSet.getUuid());

		assertEquals("Size of the orderSetMembers got updated",
				initialSize + 1, savedOrderSet.getOrderSetMembers().size());
		assertEquals("New OrderSetMember got added at last position",
				newOrderSetMember.getUuid(), savedOrderSet.getOrderSetMembers()
						.get(initialSize).getUuid());
	}
	...
	public void shouldAddOrderSetMemberAtSomePositionInsideAnExistingOrderSet()
		...
		assertEquals("Size of the orderSetMembers got updated",
				initialSize + 1, savedOrderSet.getOrderSetMembers().size());
		assertEquals("New OrderSetMember got added at given position",
				newOrderSetMember.getUuid(), savedOrderSet.getOrderSetMembers()
		...
		orderSetMemberToBeAddedAtPosition.setOrderType(orderService
				.getOrderType(100));
		orderSetMemberToBeAddedAtPosition.setConcept(conceptService
				.getConcept(1002));
		...
		orderSet.addOrderSetMember(orderSetMemberToBeAddedAtPosition,
				newPosition);
	}
	...
	public void shouldReturnInTheSameArrangementInWhichTheOrderSetMembersAreSaved()
		...
		List<OrderSetMember> orderSetMembers = new ArrayList<OrderSetMember>(
				Arrays.asList(firstOrderSetMember, thirdOrderSetMember,
						secondOrderSetMember));
		newOrderSet.setOrderSetMembers(orderSetMembers);

		OrderSet savedOrderSet = orderSetService.saveOrderSet(newOrderSet);
		Context.flushSession();

		assertEquals(firstOrderSetMember.getUuid(), savedOrderSet
				.getOrderSetMembers().get(0).getUuid());
		assertEquals(thirdOrderSetMember.getUuid(), savedOrderSet
				.getOrderSetMembers().get(1).getUuid());
		assertEquals(secondOrderSetMember.getUuid(), savedOrderSet
				.getOrderSetMembers().get(2).getUuid());
	}
	...
	public void shouldAddOrderSetMemberFromTheEndOfTheListIfNegativePositionIsGiven()
		...
		assertEquals("New OrderSetMember got added at given position",
				newOrderSetMember.getUuid(), savedOrderSet.getOrderSetMembers()
						.get(position + initialSize + 1).getUuid());
	...
	public void shouldFetchUnRetiredOrderSetMembers() throws Exception {
		...
		OrderSet savedOrderSet = orderSetService.getOrderSetByUuid(orderSet
				.getUuid());
		assertEquals(
				"Count of orderSetMembers are not changed if we get all members",
				initialCountOfMembers, savedOrderSet.getOrderSetMembers()
						.size());

		// Fetching the unRetired members
		int finalSize = savedOrderSet.getUnRetiredOrderSetMembers().size();
		assertEquals(
				"Count of orderSetMembers gets modified if we filter out the retired members",
				initialCountOfMembers - 1, finalSize);
	}
	...
	public void shouldDeleteAnOrderSetMemberInAnOrderSet() throws Exception {
		...
		OrderSet savedOrderSet = orderSetService.getOrderSetByUuid(orderSet
				.getUuid());
		assertEquals(
				"Count of orderSetMembers changes after removing a member from the orderSet",
				initialCountOfMembers - 1, savedOrderSet.getOrderSetMembers()
						.size());
	}
	...
	public void shouldFetchOrderSetMemberByUuid() throws Exception {
		...
		OrderSetMember orderSetMember = orderSetService
				.getOrderSetMemberByUuid(orderSetUuid);
	...
	public void shouldRetireOrderSetAndOrderSetMembersAsWell() throws Exception {
		...
		int initialNumberOfOrderSets = orderSetService.getOrderSets(false)
				.size();
		...
		int numberOfOrderSetsAfterRetire = orderSetService.getOrderSets(false)
				.size();
		...
		List<OrderSetMember> orderSetMembers = retiredOrderSet
				.getOrderSetMembers();
	...
	private OrderSet orderSetBuilder(boolean orderSetRetired,
			boolean orderSetMemberRetired) {
