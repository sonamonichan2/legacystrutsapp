...
public class FormServiceTest extends BaseContextSensitiveTest {
	...
	/**
	 * Creates then updates a form FIXME Break this test case into separate
	 * tests
	 * 
	 * @throws Exception
	 */
	...
	public void getFormField_shouldIgnoreFormFieldsPassedToIgnoreFormFields()
		...
		FormField ff = Context.getFormService().getFormField(new Form(1),
				new Concept(1), null, false);
		...
		FormField ff2 = Context.getFormService().getFormField(new Form(1),
				new Concept(1), ignoreFormFields, false);
		assertNotNull(ff2);
	...
	public void getFormField_shouldNotFailWithNullIgnoreFormFieldsArgument()
		...
		FormField ff = Context.getFormService().getFormField(new Form(1),
				new Concept(3), null, false);
		assertNotNull(ff);
	}

	/**
	 * Make sure that multiple forms are returned if a field is on a form more
	 * than once
	 * 
	...
	public void getForms_shouldReturnDuplicateFormWhenGivenFieldsIncludedInFormMultipleTimes()
		...
		List<Form> forms = formService.getForms(null, null, null, null, null,
				null, fields);
	 ...
	 * @verifies return forms containing all form fields in
	...
	public void getForms_shouldReturnFormsContainingAllFormFieldsInContainingAllFormFields()
		...
		forms = formService.getForms(null, null, null, null, null, formFields,
				null);
		assertEquals(0, forms.size());
	}

	/**
	 * ensure that FormFields in containingAnyFormField parameter are considered
	...
	public void getForms_shouldReturnFormsThatHaveAnyMatchingFormFieldsInContainingAnyFormField()
		...
		List<Form> formsReturned = formService.getForms(null, null, null, null,
				containingAnyFormField, null, null);

		Integer currentNumberOfForms = new Integer(formsReturned.size());

		assertEquals(numberOfExpectedForms, currentNumberOfForms);
		assertTrue(wasFormsSuccessfullyFilteredByMatchingFormFieldsInContainingAnyFormField(
				containingAnyFormField, formsReturned));

	}

	private Collection<FormField> makeFormFieldCollectionSample(
	...
	private boolean doesFormContainAnyFormField(Form form,
			Collection<FormField> containingAnyFormField) {
	...
	public void saveFieldType_shouldCreateNewFieldType() throws Exception {
		...
		Assert.assertNotNull(formService.getFieldType(fieldType
				.getFieldTypeId()));
	}
	...
	public void duplicateForm_shouldClearChangedDetailsAndUpdateCreationDetails()
		...
		// some of these assertions are affected by inserting resources after
		// creating the form
		...
		assertEquals(new Date().getTime(), dupForm.getDateCreated().getTime(),
				oneMinuteDelta);
	}
	...
	public void getFormField_shouldSimplyReturnNullForNonexistentConcepts()
		...
		assertNull(Context.getFormService().getFormField(new Form(1),
				new Concept(293934), null, false));
	}
	...
	public void getFormField_shouldSimplyReturnNullForNonexistentForms()
		...
		assertNull(Context.getFormService().getFormField(new Form(12343),
				new Concept(293934), null, false));
	}
	...
	public void getFieldAnswerByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getFormService().getFieldAnswerByUuid(
				"some invalid uuid"));
	}
	...
	public void getFieldByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getFormService().getFieldByUuid(
				"some invalid uuid"));
	}
	...
	public void getFieldTypeByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getFormService().getFieldTypeByUuid(
				"some invalid uuid"));
	}
	...
	public void getFormByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getFormService().getFormByUuid(
				"some invalid uuid"));
	}
	...
	public void getFormFieldByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getFormService().getFormFieldByUuid(
				"some invalid uuid"));
	}
	...
	public void saveFormField_shouldPropagateSaveToTheFieldPropertyOnTheGivenFormField()
		...
		// the uuid should be set by this method so that the field can be saved
		// successfully
	...
	public void getFormsContainingConcept_shouldGetAllFormsForConcept()
		...
		assertEquals(1,
				Context.getFormService().getFormsContainingConcept(concept)
						.size());
	}
	...
	public void mergeDuplicateFields_shouldMergeDuplicateFieldsInFormFieldsAndThenPurgeTheDuplicateFields()
		...
		Assert.assertNull(Context.getFormService().getFieldByUuid(
				"b1843148-da2f-4349-c9c7-1164b98d91dd"));
	...
	public void saveFormResource_shouldPersistAFormResource() throws Exception {
		...
		FormResource actual = Context.getFormService().getFormResource(
				resourceId);
	...
	public void duplicateForm_shouldCopyResourcesForOldFormToNewForm()
			throws Exception {
		...
		// get the resource
		FormResource actual = Context.getFormService().getFormResource(newForm,
				name);
	...
	public void saveFormResource_shouldOverwriteAnExistingResourceWithSameName()
			throws Exception {
		String name = "Start Date";
		...
		FormResource actual = Context.getFormService().getFormResource(form,
				name);
	...
	public void purgeForm_shouldDeleteFormResourcesForDeletedForm()
			throws Exception {
		// create a new form
		Form form = new Form();
	...
	public void saveFormResource_shouldBeAbleToSaveAnXSLT() throws Exception {
		...
		// make sure the resource is saved
		Collection<FormResource> formResourcesForForm = Context
				.getFormService().getFormResourcesForForm(form);
	...
	/**
	 * convert a resource path to a file into a string containing the file's
	 * contents
	 * 
	 * @param filename
	...
	public void saveFormField_shouldInjectFormFieldsFromSerializableComplexObsHandlers()
		...
		Context.getObsService().registerHandler("NeigborHandler",
				new NeighborHandler());
		...
		Assert.assertEquals(initialFormFieldCount += 3,
				updatedFormFields.size());
		// get the formfields added by the handler and check their parent
		List<FormField> childFormFields = ListUtils.subtract(updatedFormFields,
				originalFormFields);
	 ...
	 * 
	 * @param propertyValue
	 *            value for forms locked GP
	 */
	public void createFormsLockedGPAndSetValue(String propertyValue) {
		GlobalProperty gp = new GlobalProperty(
				OpenmrsConstants.GLOBAL_PROPERTY_FORMS_LOCKED);
