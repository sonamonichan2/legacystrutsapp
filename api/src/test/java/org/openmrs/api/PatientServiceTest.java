...
/**
 * This class tests methods in the PatientService class TODO Add methods to test
 * all methods in PatientService class
 */
public class PatientServiceTest extends BaseContextSensitiveTest {
	...
	public void getAllIdentifierValidators_shouldReturnAllRegisteredIdentifierValidators()
			throws Exception {
		...
		Assert.assertEquals(2, patientService.getAllIdentifierValidators()
				.size());
		assertCollectionContentsEquals(expectedValidators,
	...
	public void getAllIdentifierValidators_shouldTreatEmptyStringsLikeANullEntry()
			throws Exception {
		Assert.assertEquals(null, patientService.getIdentifierValidator(""));
	}

	/**
	 * Convenience method to have a Patient object with all required values
	 * filled in
	 * 
	...
	public void shouldCreatePatient() throws Exception {
		...
		List<PatientIdentifierType> patientIdTypes = patientService
		...
		Patient createdPatient = patientService.getPatient(patient
				.getPatientId());
		assertNotNull(createdPatient);

		assertNotNull(createdPatient.getPatientId());

		Patient createdPatientById = patientService.getPatient(createdPatient
				.getPatientId());
	...
	public void shouldCreatePatientWithValidatedIdentifier() throws Exception {
		...
		PatientIdentifier ident1 = new PatientIdentifier("123-1", pit,
				locationService.getLocation(1));
		PatientIdentifier ident2 = new PatientIdentifier("123", pit,
				locationService.getLocation(1));
		PatientIdentifier ident3 = new PatientIdentifier("123-0", pit,
				locationService.getLocation(1));
		PatientIdentifier ident4 = new PatientIdentifier("123-A", pit,
				locationService.getLocation(1));
		...
		try {
			...
			patientService.savePatient(patient);
			fail("Patient creation should have failed with identifier "
					...
					.contains(
							"failed to validate with reason: "
									+ Context
											.getMessageSourceService()
											.getMessage(
													"PatientIdentifier.error.unallowedIdentifier",
													new Object[]{
															ident2.getIdentifier(),
															new LuhnIdentifierValidator()
																	.getName()},
													null)))) {
				fail("Patient creation should have failed with identifier "
						+ ident2.getIdentifier());
		...
		try {
			...
			fail("Patient creation should have worked with identifiers "
					+ ident3.getIdentifier() + " and " + ident4.getIdentifier());
		}
	}

	/**
	 * Gets the first patient, then sees if it can get that patient by its
	 * identifier as well
	 * 
	 * @throws Exception
	 */
	...
	public void shouldGetPatientsByIdentifier() throws Exception {
		...
		Collection<Patient> johnPatients = patientService.getPatients("John",
				null, null, false);
		assertNotNull("There should be a patient named 'John'", johnPatients);
		assertFalse("There should be a patient named 'John'",
				johnPatients.isEmpty());
		...
		String identifier = firstJohnPatient.getPatientIdentifier()
				.getIdentifier();
		assertNotNull("Uh oh, the patient doesn't have an identifier",
				identifier);
		List<Patient> patients = patientService.getPatients(identifier, null,
				null, false);
		assertTrue(
	...
	public void shouldGetPatientsByNameShouldLimitSize() throws Exception {
		...
		Collection<Patient> patients = patientService.getPatients("John", null,
				null, false);

		assertTrue(
				"The patient list size should be restricted to under the max (1000). its "
						+ patients.size(), patients.size() == 1000);
	...
	/**
	 * Test the PatientService.getPatients(String, String, List) method with
	 * both an identifier and an identifiertype
	 * 
	 * @throws Exception
	 */
	@Test
	...
	public void shouldGetPatientsByIdentifierAndIdentifierType()
		...
		List<Patient> patients = patientService.getPatients("1234", null,
				types, false);
	...
	public void purgePatientIdentifierType_shouldDeleteTypeFromDatabase()
		...
		PatientIdentifierType type = patientService
				.getPatientIdentifierType(patientIdentifierType.getId());

		patientService.purgePatientIdentifierType(type);
		assertNull(patientService
				.getPatientIdentifierType(patientIdentifierType.getId()));
	}
	...
	public void savePatientIdentifierType_shouldUpdateExistingType()
		...
		PatientIdentifierType newerPatientIdentifierType = patientService
				.getPatientIdentifierType(1);
		assertEquals("SOME NEW NAME", newerPatientIdentifierType.getName());
	}

	/**
	 * Make sure the api can handle having a User object that is also a patient
	 * and was previously loaded via hibernate
	 * 
	 * @throws Exception
	 */
	@Test
	public void shouldAllowGettingPatientsThatWereCreatedByUsersWhoArePatients()
			throws Exception {
		...
		assertTrue(
				"When getting a patient, it should be of the class patient, not: "
						+ patient2.getClass(),
				patient2.getClass().equals(Patient.class));

		Patient patient3 = patientService.getPatient(3);
		assertTrue(
				"When getting a patient, it should be of the class patient, not: "
						+ patient3.getClass(),
				patient3.getClass().equals(Patient.class));

		User user2 = Context.getUserService().getUser(2);
		assertTrue("When getting a user, it should be of the class user, not: "
				+ user2.getClass(),
	...
	public void getPatients_shouldForceSearchStringToBeGreaterThanMinsearchcharactersGlobalProperty()
			throws Exception {
	...
	public void getPatients_shouldAllowSearchStringToBeOneAccordingToMinsearchcharactersGlobalProperty()
			throws Exception {
		initializeInMemoryDatabase();
		executeDataSet(FIND_PATIENTS_XML);
		...
		// there is a patient will middle name "F", so this should generate a
		// hit.
	...
	public void getPatient_shouldReturnNullObjectIfPatientIdDoesntExist()
			throws Exception {
	...
	public void mergePatients_shouldNotMergePatientWithItself()
			throws Exception {
		Context.getPatientService().mergePatients(new Patient(2),
				new Patient(2));
	}
	...
	public void mergePatients_shouldChangeUserRecordsOfNonPreferredPersonToPreferredPerson()
			throws Exception {
		executeDataSet(USERS_WHO_ARE_PATIENTS_XML);
		Patient notPreferred = patientService.getPatient(2);
		voidOrders(Collections.singleton(notPreferred));
		Context.getPatientService().mergePatients(patientService.getPatient(6),
				notPreferred);
	 ...
	 */
	@Test
	public void mergePatients_shouldMergeVisitsFromNonPreferredToPreferredPatient()
			throws Exception {
		...
		// patient 2 (not preferred) has 3 unvoided visits (id = 1, 2, 3) and 1
		// voided visit (id = 6)
		...
		// patient 6 (preferred) has 2 unvoided visits (id = 4, 5) and no voided
		// visits
		...
		encounterUuidsThatShouldBeMoved.add(Context.getEncounterService()
				.getEncounter(6).getUuid());
		...
		List<Obs> originalUnvoidedObs = Context.getObsService()
				.getObservationsByPerson(notPreferred);
		...
		List<Visit> mergedVisits = visitService.getVisitsByPatient(merged,
				true, true);

		assertThat(mergedVisits.size(), is(6));
		// in order to keep this test passing when (someday?) we copy visits
		// instead of moving them, use matchers here:
		assertThat(
				mergedVisits,
				containsInAnyOrder(matchingVisit(visit1),
						matchingVisit(visit2), matchingVisit(visit3),
						matchingVisit(visit4), matchingVisit(visit5),
						matchingVisit(visit6)));

		// be sure nothing slipped through without being assigned to the right
		// patient (probably not necessary)
			...
			for (Encounter e : v.getEncounters()) {
				...
				for (Obs o : e.getAllObs(true)) {
					if (!originalUnvoidedObs.contains(o)) {
						assertThat(o.getPerson().getId(), is(v.getPatient()
								.getId()));
					}
				}
			}
		...
		// now check that moving visits and their contained encounters was
		// audited correctly
		PersonMergeLogData mergeLogData = mergeLog.getPersonMergeLogData();
		assertThat(mergeLogData.getMovedVisits().size(), is(4));
		assertThat(
				mergeLogData.getMovedVisits(),
				containsInAnyOrder(visit1.getUuid(), visit2.getUuid(),
						visit3.getUuid(), visit6.getUuid()));

		assertThat(mergeLogData.getMovedEncounters().size(),
				is(encounterUuidsThatShouldBeMoved.size()));
		assertThat(mergeLogData.getMovedEncounters(),
				containsInAnyOrder(encounterUuidsThatShouldBeMoved.toArray()));
	}
			...
			public boolean matches(Visit argument) {
				...
				return OpenmrsUtil.nullSafeEquals(visit.getLocation(),
						...
						&& OpenmrsUtil.nullSafeEquals(visit.getVisitType(),
								expected.getVisitType())
						&& OpenmrsUtil.nullSafeEquals(visit.getIndication(),
								expected.getIndication())
						&& OpenmrsUtil.nullSafeEquals(visit.getStartDatetime(),
								expected.getStartDatetime())
						&& OpenmrsUtil.nullSafeEquals(visit.getStopDatetime(),
								expected.getStopDatetime())
						&& (visit.getEncounters().size() == expected
								.getEncounters().size());
			}
		};
	}
	...
	public void mergePatients_shouldVoidNonPreferredPersonObject()
			throws Exception {
		Patient notPreferred = patientService.getPatient(2);
		voidOrders(Collections.singleton(notPreferred));
		Context.getPatientService().mergePatients(patientService.getPatient(6),
	...
	public void savePatient_shouldCreateNewPatientFromExistingPersonPlusUserObject()
			throws Exception {
		...
		PatientIdentifier patientIdentifier = new PatientIdentifier(
				"some identifier", new PatientIdentifierType(2),
				new Location(1));
		patientIdentifier.setPreferred(true);
	...
	/**
	 * @see PatientService#getPatients(String,String,List<QPatientIdentifierType
	 *      ;>,null)
	 */
	@Test
	...
	public void getPatients_shouldSearchFamilyName2WithName() throws Exception {
		...
		List<Patient> patients = patientService.getPatients("Johnson", null,
				null, false);
	 ...
	 * caused by PatientIdentifierValidator Manually construct a patient with a
	 * correctly-matching patientId and patient identifier with validator.
	 * Calling PatientService.savePatient on that patient leads to a call to
	 * PatientIdentifierValidator.validateIdentifier which used to load the
	 * Patient for that identifier into the hibernate session, leading to a
	 * NonUniqueObjectException when the calling saveOrUpdate on the manually
	 * constructed Patient.
	 * 
	 * @see PatientService#savePatient(Patient)
	 */
	...
	public void savePatient_shouldNotThrowANonUniqueObjectExceptionWhenCalledWithAHandConstructedPatientRegression1375() {
		...
		PatientIdentifier patientIdentifier = new PatientIdentifier("101-6",
				new PatientIdentifierType(1), new Location(1));
		patientIdentifier.setPreferred(true);
		patient.addIdentifier(patientIdentifier);
		patientService.savePatient(patient);
	}
	...
	/**
	 * This test verifies that {@link PersonName}s are fetched correctly from
	 * the hibernate cache. (Or really, not fetched from the cache but instead
	 * are mapped with lazy=false. For some reason Hibernate isn't able to find
	 * objects in the cache if a parent object was the one that loaded them)
	 * 
	 * @throws Exception
	 */
	@Test
	public void shouldFetchNamesForPersonsThatWereFirstFetchedAsPatients()
			throws Exception {
	...
	/**
	 * This test verifies that {@link PersonAddress}es are fetched correctly
	 * from the hibernate cache. (Or really, not fetched from the cache but
	 * instead are mapped with lazy=false. For some reason Hibernate isn't able
	 * to find objects in the cache if a parent object was the one that loaded
	 * them)
	 * 
	 * @throws Exception
	 */
	@Test
	public void shouldFetchAddressesForPersonsThatWereFirstFetchedAsPatients()
			throws Exception {
		Person person = Context.getPersonService().getPerson(2);
		Patient patient = Context.getPatientService().getPatient(2);
	...
	/**
	 * This test verifies that {@link PersonAttribute}s are fetched correctly
	 * from the hibernate cache. (Or really, not fetched from the cache but
	 * instead are mapped with lazy=false. For some reason Hibernate isn't able
	 * to find objects in the cache if a parent object was the one that loaded
	 * them)
	 ...
	 */
	@Test
	public void shouldFetchPersonAttributesForPersonsThatWereFirstFetchedAsPatients()
			throws Exception {
		Person person = Context.getPersonService().getPerson(2);
		Patient patient = Context.getPatientService().getPatient(2);
	...
	public void savePatient_shouldNotThrowANonUniqueObjectExceptionWhenCalledWithAHandConstructedPatient()
			throws Exception {
		Patient patient = new Patient();
		patient.setGender("M");
		patient.setPatientId(2);
	...
	public void isIdentifierInUseByAnotherPatient_shouldIgnoreVoidedPatientIdentifiers()
		...
		PatientIdentifier patientIdentifier = new PatientIdentifier("ABC123",
				pit, null);
		Assert.assertFalse(patientService
				.isIdentifierInUseByAnotherPatient(patientIdentifier));
	}
	...
	public void isIdentifierInUseByAnotherPatient_shouldIgnoreVoidedPatients()
			throws Exception {
		{
			...
			for (PatientIdentifier id : p.getIdentifiers()) {
				if (id.getIdentifier().equals("XYZ")
						&& id.getIdentifierType().getId() == 2) {
					found = true;
					break;
				}
			}
		...
		PatientIdentifierType pit = patientService.getPatientIdentifierType(2);
		PatientIdentifier patientIdentifier = new PatientIdentifier("XYZ", pit,
				null);
		Assert.assertFalse(patientService
				.isIdentifierInUseByAnotherPatient(patientIdentifier));
	}
	...
	public void isIdentifierInUseByAnotherPatient_shouldReturnFalseWhenPatientIdentifierContainsAPatientAndNoOtherPatientHasThisId()
		...
		PatientIdentifierType pit = patientService.getPatientIdentifierType(1);
		PatientIdentifier patientIdentifier = new PatientIdentifier(
				"Nobody could possibly have this identifier", pit, null);
	...
	public void isIdentifierInUseByAnotherPatient_shouldReturnTrueWhenPatientIdentifierContainsAPatientAndAnotherPatientHasThisId()
			throws Exception {
		PatientIdentifierType pit = patientService.getPatientIdentifierType(1);
		PatientIdentifier patientIdentifier = new PatientIdentifier("7TU-8",
				pit, null);
		patientIdentifier.setPatient(patientService.getPatient(2));
		Assert.assertTrue(patientService
				.isIdentifierInUseByAnotherPatient(patientIdentifier));
	}
	...
	public void checkPatientIdentifiers_shouldIgnoreVoidedPatientIdentifier()
		...
		Patient patient = new Patient();
		PatientIdentifier patientIdentifier = new PatientIdentifier();
		patientIdentifier.setIdentifierType(Context.getPatientService()
				.getAllPatientIdentifierTypes(false).get(0));
		patientIdentifier.setLocation(new Location(1));
		patientIdentifier.setVoided(true);
		patientIdentifier.setVoidedBy(Context.getAuthenticatedUser());
		patientIdentifier
				.setVoidReason("Testing whether voided identifiers are ignored");
		patient.addIdentifier(patientIdentifier);
	...
	public void checkPatientIdentifiers_shouldRequireOneNonVoidedPatientIdentifier()
			throws Exception {
	...
	/**
	 * Cannot distinguish between null and empty patient identifiers because you
	 * cannot set the patient identifiers directly. There's only a method to add
	 * and remove patient identifiers.
	 * 
	 * @see PatientService#checkPatientIdentifiers(Patient)
	 */
	@Test(expected = InsufficientIdentifiersException.class)
	...
	public void checkPatientIdentifiers_shouldThrowErrorWhenPatientHasIdenticalIdentifiers()
		...
		PatientIdentifierType patientIdentifierType = Context
				.getPatientService().getAllPatientIdentifierTypes(false).get(0);
	...
	public void getAllIdentifierValidators_shouldReturnAllRegisteredPatientIdentifierValidators()
		...
		Collection<IdentifierValidator> actualValidators = patientService
	...
	public void getAllPatientIdentifierTypes_shouldFetchAllNonRetiredPatientIdentifierTypes()
			throws Exception {
		Collection<PatientIdentifierType> types = Context.getPatientService()
				.getAllPatientIdentifierTypes();
		...
		Assert.assertEquals(
				"Should be exactly three patient identifier types in the dataset",
				3, types.size());
	...
	public void getAllPatientIdentifierTypes_shouldFetchPatientIdentifierTypesIncludingRetiredWhenIncludeRetiredIsTrue()
		...
		Collection<PatientIdentifierType> types = Context.getPatientService()
				.getAllPatientIdentifierTypes(true);
		...
		Assert.assertTrue(
				"There should be at least one retired patient identifier type",
				atLeastOneRetired);
		Assert.assertEquals("Should be exactly four patient identifier types",
				4, types.size());
	}
	...
	public void getIdentifierValidator_shouldReturnPatientIdentifierValidatorGivenClassName()
		...
		Assert.assertEquals("Luhn CheckDigit Validator",
		...
		Assert.assertNotNull(identifierValidator);
		Assert.assertEquals("Verhoeff Check Digit Validator.",
				identifierValidator.getName());
	}
	...
	public void getPatientByExample_shouldFetchPatientMatchingPatientIdOfGivenPatient()
		...
		Patient patient = Context.getPatientService().getPatientByExample(
				examplePatient);
		Assert.assertNotNull(patient);
		Assert.assertTrue(patient.getClass().isAssignableFrom(Patient.class));
	...
	public void getPatientIdentifierType_shouldFetchPatientIdentifierWithGivenPatientIdentifierTypeId()
		...
		Assert.assertTrue(identifierType.getClass().isAssignableFrom(
	...
	public void getPatientIdentifierType_shouldReturnNullWhenPatientIdentifierIdentifierDoesNotExist()
			throws Exception {
		PatientIdentifierType identifierType = Context.getPatientService()
				.getPatientIdentifierType(10000);
	...
	public void getPatientIdentifierTypeByName_shouldNotReturnPatientIdentifierTypeThatPartiallyMatchesGivenName()
			throws Exception {
		PatientIdentifierType identifierType = Context.getPatientService()
				.getPatientIdentifierTypeByName("OpenMRS");
		Assert.assertNull(identifierType);
	}
	...
	public void getPatientIdentifierTypes_shouldFetchPatientIdentifierTypesThatMatchGivenNameWithGivenFormat()
		...
		List<PatientIdentifierType> patientIdentifierTypes = Context
				.getPatientService().getPatientIdentifierTypes(
						"Test OpenMRS Identification Number",
						"java.lang.Integer", null, null);

		Assert.assertEquals(false, patientIdentifierTypes.isEmpty());

		for (PatientIdentifierType patientIdentifierType : patientIdentifierTypes) {
			Assert.assertEquals("Test OpenMRS Identification Number",
	...
	public void getPatientIdentifierTypes_shouldFetchRequiredPatientIdentifierTypesWhenGivenRequiredIsTrue()
		...
		executeDataSet("org/openmrs/api/include/PatientServiceTest-createPatientIdentifierType.xml");
		List<PatientIdentifierType> patientIdentifierTypes = Context
				.getPatientService().getPatientIdentifierTypes(null, null,
						true, null);
	...
	public void savePatientIdentifierType_shouldCreateNewPatientIdentifierType()
		...
		PatientIdentifierType savedIdentifierType = patientService
				.getPatientIdentifierType(identifierType
						.getPatientIdentifierTypeId());
	...
	public void savePatientIdentifierType_shouldUpdateExistingPatientIdentifierType()
		...
		Assert.assertEquals(2, identifierType.getPatientIdentifierTypeId()
				.intValue());
	...
	public void unretirePatientIdentifierType_shouldUnretirePatientIdentifierType()
			throws Exception {
		PatientIdentifierType identifierType = Context.getPatientService()
				.getPatientIdentifierType(4);
		...
		PatientIdentifierType unretiredIdentifierType = Context
				.getPatientService().unretirePatientIdentifierType(
						identifierType);
	...
	public void unvoidPatient_shouldUnvoidGivenPatient() throws Exception {
		...
		Patient voidedPatient = Context.getPatientService().voidPatient(
				patient, "Void for testing");
		...
		Patient unvoidedPatient = Context.getPatientService().unvoidPatient(
				voidedPatient);
	...
	public void voidPatient_shouldVoidAllPatientIdentifiersAssociatedWithGivenPatient()
		...
		Patient voidedPatient = Context.getPatientService().voidPatient(
				patient, "Void for testing");
		for (PatientIdentifier patientIdentifier : voidedPatient
				.getIdentifiers()) {
	...
	public void voidPatient_shouldReturnNullWhenPatientIsNull()
		...
		Patient voidedPatient = patientService.voidPatient(null,
				"No null patient should be voided");
	...
	public void getPatientByUuid_shouldFindObjectGivenValidUuid()
			throws Exception {
	...
	public void getPatientByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getPatientService().getPatientByUuid(
				"some invalid uuid"));
	}
	...
	public void getPatientIdentifierByUuid_shouldFindObjectGivenValidUuid()
			throws Exception {
		String uuid = "ff41928c-3bca-48d9-a4dc-9198f6b2873b";
		PatientIdentifier patientIdentifier = Context.getPatientService()
				.getPatientIdentifierByUuid(uuid);
	...
	public void getPatientIdentifierByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getPatientService()
				.getPatientIdentifierByUuid("some invalid uuid"));
	}
	...
	public void getPatientIdentifierTypeByUuid_shouldFindObjectGivenValidUuid()
			throws Exception {
		String uuid = "1a339fe9-38bc-4ab3-b180-320988c0b968";
		PatientIdentifierType patientIdentifierType = Context
				.getPatientService().getPatientIdentifierTypeByUuid(uuid);
		Assert.assertEquals(1,
				(int) patientIdentifierType.getPatientIdentifierTypeId());
	}
	...
	public void getPatientIdentifierTypeByUuid_shouldReturnNullIfNoObjectFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getPatientService()
				.getPatientIdentifierTypeByUuid("some invalid uuid"));
	}
	...
	public void mergePatients_shouldCopyNonvoidedAddressesToPreferredPatient()
			throws Exception {
	...
	public void mergePatients_shouldOnlyMarkAddressesOfPreferredPatientAsPreferred()
			throws Exception {

		Patient preferred = patientService.getPatient(7);
		Patient notPreferred = patientService.getPatient(8);

		// since the test data has no preferred addresses, we need to mark
		// addresses to preferred to set up the test
		...
		// make sure only the address from the preferred patient is marked as
		// preferred
		for (PersonAddress pa : preferred.getAddresses()) {
			if (pa.getCityVillage().equals("Jabali")) {
	...
	public void mergePatients_shouldCopyNonvoidedIdentifiersToPreferredPatient()
			throws Exception {
		Patient preferred = patientService.getPatient(7);
		Patient notPreferred = patientService.getPatient(8);
		...
		for (PatientIdentifier patientIdentifier : notPreferred
				.getIdentifiers()) {
		...
		Assert.assertTrue(contains(
				new ArrayList<PatientIdentifier>(preferred.getIdentifiers()),
				nonvoidedPI.getIdentifier()));
		Assert.assertNotNull(voidedPI);
		Assert.assertFalse(contains(
				new ArrayList<PatientIdentifier>(preferred.getIdentifiers()),
				voidedPI.getIdentifier()));
	}

	public static boolean contains(List<PatientIdentifier> list,
			String identifier) {
	...
	public void mergePatients_shouldCopyNonvoidedNamesToPreferredPatient()
			throws Exception {
		Patient preferred = patientService.getPatient(7);
		Patient notPreferred = patientService.getPatient(8);
	...
	public void getPatientByUuid_shouldFetchPatientWithGivenUuid()
			throws Exception {
		String uuid = "da7f524f-27ce-4bb2-86d6-6d1d05312bd5";
		Patient patient = Context.getPatientService().getPatientByUuid(uuid);
		Assert.assertEquals(2, (int) patient.getPatientId());
	...
	public void getPatientByUuid_shouldReturnNullIfPatientNotFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getPatientService().getPatientByUuid(
				"some invalid uuid"));
	}
	...
	public void getPatientIdentifierByUuid_shouldFetchPatientIdentifierWithGivenUuid()
			throws Exception {
		String uuid = "ff41928c-3bca-48d9-a4dc-9198f6b2873b";
		PatientIdentifier patientIdentifier = Context.getPatientService()
				.getPatientIdentifierByUuid(uuid);
	...
	public void getPatientIdentifierByUuid_shouldReturnNullIfPatientIdentifierNotFoundWithGivenUuid()
			throws Exception {
		Assert.assertNull(Context.getPatientService()
				.getPatientIdentifierByUuid("some invalid uuid"));
	}
	...
	public void getPatientIdentifiers_shouldReturnOnlyNonVoidedPatientsAndPatientIdentifiers()
			throws Exception {
		...
		Assert.assertFalse(
				"This test expects the patient to be voided BUT the identifier to be NONvoided",
				((PatientIdentifier) (patient.getIdentifiers().toArray()[0]))
						.isVoided());

		// now fetch all identifiers
		List<PatientIdentifier> patientIdentifiers = patientService
				.getPatientIdentifiers(null, null, null, null, null);
		for (PatientIdentifier patientIdentifier : patientIdentifiers) {
			Assert.assertFalse("No voided identifiers should be returned",
					patientIdentifier.isVoided());
			Assert.assertFalse(
					"No identifiers of voided patients should be returned",
					patientIdentifier.getPatient().isVoided());
		}
	}
	...
	public void getPatients_shouldSupportSimpleRegex() throws Exception {
		Context.getAdministrationService()
				.saveGlobalProperty(
						new GlobalProperty(
								...
								"^0*@SEARCH@([A-Z]+-[0-9])?$"));
		PatientIdentifier identifier = new PatientIdentifier("1234-4",
				new PatientIdentifierType(1), new Location(1));
		...
		Context.getPatientService().savePatient(patient);
		assertEquals(1, Context.getPatientService().getPatients("1234-4")
				.size());
	}

	/**
	 * @verifies {@link 
	 *           PatientService#getPatients(String,String,List<QPatientIdentifierType
	 *           ;>,null)} test = should return empty list when no match is
	 *           found
	 */
	@Test
	...
	public void getPatients_shouldReturnEmptyListWhenNoMatchIsFound()
			throws Exception {
		initializeInMemoryDatabase();
		executeDataSet(CREATE_PATIENT_XML);
		authenticate();
		...
		List<Patient> patientList = patientService.getPatients(null, "???",
				null, false);
		assertNotNull(
				"an empty list should be returned instead of a null object",
				patientList);
		assertEquals(0, patientList.size());
	}

	/**
	 * @verifies {@link PatientService#getPatient(Integer)} test = should update
	 *           an existing patient
	 */
	@Test
	...
	public void savePatient_shouldUpdateAnExistingPatient() throws Exception {
		...
		assertTrue("The patient should be listed as male", patient.getGender()
				.equals("M"));
		...
		assertTrue(
				"The updated patient and the orig patient should still be equal",
	...
	/**
	 * @verifies {@link PatientService#savePatient(Patient)} test = should fail
	 *           when patient does not have any patient identifiers
	 */
	@Test
	@Verifies(value = "should fail when patient does not have any patient identifiers", method = "savePatient(Patient)")
	public void savePatient_shouldFailWhenPatientDoesNotHaveAnyPatientIdentifiers()
			throws Exception {
		Patient patient = new Patient();
	...
	/**
	 * @verifies {@link PatientService#getAllPatients()} test = should fetch all
	 *           non voided patients
	 */
	@Test
	@Verifies(value = "should fetch all non voided patients", method = "getAllPatients()")
	public void getAllPatients_shouldFetchAllNonVoidedPatients()
			throws Exception {
	...
	/**
	 * @verifies {@link PatientService#getAllPatients(null)} test = should fetch
	 *           non voided patients when given include voided is false
	 */
	@Test
	@Verifies(value = "should fetch non voided patients when given include voided is false", method = "getAllPatients(boolean)")
	public void getAllPatients_shouldFetchNonVoidedPatientsWhenGivenIncludeVoidedIsFalse()
			throws Exception {
	...
	/**
	 * @verifies {@link PatientService#getAllPatients(null)} test = should fetch
	 *           voided patients when given include voided is true
	 */
	@Test
	@Verifies(value = "should fetch voided patients when given include voided is true", method = "getAllPatients(boolean)")
	public void getAllPatients_shouldFetchVoidedPatientsWhenGivenIncludeVoidedIsTrue()
			throws Exception {
	 ...
	 * @verifies {@link 
	 *           PatientService#getPatients(String,String,List<QPatientIdentifierType
	 *           ;>,null)} test = should fetch all patients that partially match
	 *           given name
	 */
	@SkipBaseSetup
	@Test
	...
	public void getPatients_shouldFetchAllPatientsThatPartiallyMatchGivenName()
			throws Exception {
		initializeInMemoryDatabase();
		executeDataSet(FIND_PATIENTS_XML);
		authenticate();

		List<Patient> patients = patientService.getPatients("Jea", null, null,
				false);
	...
	/**
	 * @verifies {@link PatientService#purgePatient(Patient)} test = should
	...
	public void getPatients_shouldNotReturnVoidedPatients() throws Exception {
		...
		// patient is voided
		List<Patient> patients = patientService.getPatients("I am voided",
				null, null, false);
	...
	public void getPatients_shouldSupportPatternUsingLastDigitAsCheckDigit()
			throws Exception {
		Context.getAdministrationService()
				.saveGlobalProperty(
						new GlobalProperty(
	...
	public void getPatientIdentifier_shouldReturnThePatientsIdentifier()
			throws Exception {

		Assert.assertEquals("101-6", patientService.getPatientIdentifier(2)
				.getIdentifier());
		Assert.assertEquals(1, patientService.getPatientIdentifier(2)
				.getIdentifierType().getPatientIdentifierTypeId().intValue());
	}
	...
	public void voidPatientIdentifier_shouldVoidGivenPatientIdentifierWithGivenReason()
			throws Exception {
		...
		PatientIdentifier patientIdentifierToVoid = patientService
				.getPatientIdentifier(3);

		PatientIdentifier voidedIdentifier = patientService
				.voidPatientIdentifier(patientIdentifierToVoid, "Testing");
		...
		Assert.assertEquals(oldActiveIdentifierSize - 1, patient
	...
	public void savePatientIdentifier_shouldCreateNewPatientIndentifier()
			throws Exception {
		PatientIdentifier patientIdentifier = new PatientIdentifier(
				"677-56-6666", new PatientIdentifierType(4), new Location(1));
		Patient associatedPatient = patientService.getPatient(2);
		patientIdentifier.setPatient(associatedPatient);
		PatientIdentifier createdPatientIdentifier = patientService
	...
	public void savePatientIdentifier_shouldUpdateAnExistingPatientIdentifier()
			throws Exception {
		PatientIdentifier patientIdentifier = patientService
				.getPatientIdentifier(7);
		patientIdentifier.setIdentifier("NEW-ID");
		PatientIdentifier updatedPatientIdentifier = patientService
				.savePatientIdentifier(patientIdentifier);
	...
	public void purgePatientIdentifier_shouldDeletePatientIdentifierFromDatabase()
			throws Exception {
		PatientIdentifier patientIdentifier = patientService
				.getPatientIdentifier(7);
	...
	/**
	 * @verifies {@link PatientService#savePatientIdentifier(PatientIdentifier)}
	 *           test = should throw an APIException when one of the required
	 *           fields is null
	 */
	@Test(expected = APIException.class)
	...
	public void savePatientIdentifier_shouldThrowAnAPIExceptionWhenOneOfTheRequiredFieldsIsNull()
			throws Exception {
		PatientIdentifier patientIdentifier = patientService
				.getPatientIdentifier(7);
	...
	/**
	 * @verifies {@link PatientService#savePatientIdentifier(PatientIdentifier)}
	 *           test = should throw an APIException if the patientIdentifier
	 *           string is a white space
	 */
	@Test(expected = APIException.class)
	...
	public void savePatientIdentifier_shouldThrowAnAPIExceptionIfThePatientIdentifierStringIsAWhiteSpace()
			throws Exception {
		PatientIdentifier patientIdentifier = patientService
				.getPatientIdentifier(7);
	...
	public void savePatientIdentifier_shouldAllowLocationToBeNullWhenLocationBehaviourIsNotUsed() {
		...
		patientIdentifier.getIdentifierType().setLocationBehavior(
	...
	public void savePatientIdentifier_shouldAllowLocationToBeNullWhenLocationBehaviourIsRequired() {
		...
		patientIdentifier.setLocation(null);
		patientIdentifier.getIdentifierType().setLocationBehavior(
				PatientIdentifierType.LocationBehavior.REQUIRED);
		patientService.savePatientIdentifier(patientIdentifier);
	}
	...
	/**
	 * @verifies {@link PatientService#voidPatientIdentifier(PatientIdentifier,String)}
	 *           test = should throw an APIException if the reason is an empty
	 *           string
	 */
	@Test(expected = APIException.class)
	...
	/**
	 * @verifies {@link PatientService#voidPatientIdentifier(PatientIdentifier,String)}
	 *           test = should throw an APIException if the reason is a white
	 *           space character
	 */
	@Test(expected = APIException.class)
	...
	public void mergePatients_shouldNotCreateDuplicateRelationships()
			throws Exception {
		executeDataSet(PATIENT_RELATIONSHIPS_XML);
		...
		// 999->1
		List<Relationship> rels = personService.getRelationships(preferred,
				new Person(1), new RelationshipType(2));
	...
	public void mergePatients_shouldVoidAllRelationshipsForNonPreferredPatient()
			throws Exception {
		executeDataSet(PATIENT_RELATIONSHIPS_XML);
		...
		List<Relationship> rels = personService
				.getRelationshipsByPerson(notPreferred);
		assertTrue("there should not be any relationships for non preferred",
				rels.isEmpty());
	}
	...
	public void mergePatients_shouldAuditCreatedAddresses() throws Exception {
		...
		Assert.assertNotNull(
				"expected new address was not found in the preferred patient after the merge",
				addedAddressUuid);
		Assert.assertTrue(
				"person address creation not audited",
				isValueInList(addedAddressUuid, audit.getPersonMergeLogData()
						.getCreatedAddresses()));
	}
	...
	public void mergePatients_shouldAuditCreatedAttributes() throws Exception {
		...
		Assert.assertNotNull(
				"expected new attribute was not found in the preferred patient after the merge",
				addedAttributeUuid);
		Assert.assertTrue(
				"person attribute creation not audited",
				isValueInList(addedAttributeUuid, audit.getPersonMergeLogData()
	...
	public void mergePatients_shouldAuditCreatedIdentifiers() throws Exception {
		...
		PatientIdentifier patientIdentifier = new PatientIdentifier();
		patientIdentifier.setIdentifier("123-0");
		patientIdentifier.setIdentifierType(patientService
				.getPatientIdentifierType(5));
		patientIdentifier.setLocation(new Location(1));
		...
		Assert.assertNotNull(
				"expected new identifier was not found in the preferred patient after the merge",
				addedIdentifierUuid);
		Assert.assertTrue(
				"person identifier creation not audited",
				isValueInList(addedIdentifierUuid, audit
	...
	public void mergePatients_shouldAuditCreatedNames() throws Exception {
		...
		Assert.assertNotNull(
				"expected new name was not found in the preferred patient after the merge",
				addedNameUuid);
		Assert.assertTrue(
				"person name creation not audited",
				isValueInList(addedNameUuid, audit.getPersonMergeLogData()
						.getCreatedNames()));
	}
	 ...
	 */
	@Test
	public void mergePatients_shouldAuditCreatedPatientPrograms()
			throws Exception {
		...
		PatientProgram program = Context
				...
				.getPatientPrograms(notPreferred, null, null, null, null, null,
						false).get(0);

		// merge the two patients and retrieve the audit object
		PersonMergeLog audit = mergeAndRetrieveAudit(preferred, notPreferred);

		// find the UUID of the program to which the preferred patient was
		// enrolled as a result of the merge
		String enrolledProgramUuid = null;
		List<PatientProgram> programs = Context.getProgramWorkflowService()
				.getPatientPrograms(preferred, null, null, null, null, null,
						false);
		...
		Assert.assertNotNull(
				...
				enrolledProgramUuid);
		Assert.assertTrue(
				"program enrollment not audited",
				isValueInList(enrolledProgramUuid, audit
						.getPersonMergeLogData().getCreatedPrograms()));
	}
	 ...
	 */
	@Test
	public void mergePatients_shouldAuditCreatedRelationships()
			throws Exception {
		// create relationships and retrieve preferred and notPreferredPatient
		// patient
		executeDataSet(PATIENT_RELATIONSHIPS_XML);
		Patient preferred = patientService.getPatient(7);
		Patient notPreferred = patientService.getPatient(2);
		...
		// be related to patient 1
		String createdRelationshipUuid = null;
		List<Relationship> relationships = personService
				.getRelationshipsByPerson(preferred);
		for (Relationship r : relationships) {
			if (r.getPersonB().getId().equals(1)
					|| r.getPersonA().getId().equals(1)) {
		...
		Assert.assertTrue(
				...
				isValueInList(createdRelationshipUuid, audit
	...
	public void mergePatients_shouldAuditVoidedRelationships() throws Exception {
		...
		Assert.assertTrue(
				...
				isValueInList(personService.getRelationship(4).getUuid(), audit
						.getPersonMergeLogData().getVoidedRelationships()));
	}
	...
	public void mergePatients_shouldAuditMovedEncounters() throws Exception {
		...
		Assert.assertTrue(
				...
				isValueInList(Context.getEncounterService().getEncounter(3)
						.getUuid(), audit.getPersonMergeLogData()
						.getMovedEncounters()));
	}
	 ...
	 */
	@Test
	public void mergePatients_shouldAuditMovedIndependentObservations()
			throws Exception {
		// retrieve patients
		Patient preferred = patientService.getPatient(999);
		Patient notPreferred = patientService.getPatient(7);
		...
		// get an observation for notPreferred and make it independent from any
		// encounter
		...
		List<Obs> observations = Context.getObsService()
				.getObservationsByPerson(preferred);
		...
		Assert.assertTrue(
				...
				isValueInList(uuid, audit.getPersonMergeLogData()
						.getMovedIndependentObservations()));
	}
	...
	public void mergePatients_shouldAuditMovedUsers() throws Exception {
		...
		Assert.assertTrue(
				...
				isValueInList(Context.getUserService().getUser(501).getUuid(),
						audit.getPersonMergeLogData().getMovedUsers()));
	}
	...
	public void mergePatients_shouldAuditPriorCauseOfDeath() throws Exception {
		...
		preferred.addName(new PersonName("givenName", "middleName",
				"familyName"));
		patientService.savePatient(preferred);
		...
		voidOrders(Collections.singleton(notPreferred));
		PersonMergeLog audit = mergeAndRetrieveAudit(preferred, notPreferred);
		Assert.assertEquals("prior cause of death was not audited", Context
				.getConceptService().getConcept(3).getUuid(), audit
				.getPersonMergeLogData().getPriorCauseOfDeath());
	}
	...
	public void mergePatients_shouldAuditPriorDateOfBirth() throws Exception {
		...
		// milliseconds are not serialized into the database. they will be
		// ignored in the test
		...
		voidOrders(Collections.singleton(notPreferred));
		PersonMergeLog audit = mergeAndRetrieveAudit(preferred, notPreferred);
		Assert.assertEquals("prior date of birth was not audited", cDate
				.getTime(), audit.getPersonMergeLogData().getPriorDateOfBirth());
	}
	 ...
	 */
	@Test
	public void mergePatients_shouldAuditPriorDateOfBirthEstimated()
			throws Exception {
		// retrieve preferred patient and set a date of birth
		GregorianCalendar cDate = new GregorianCalendar();
		cDate.setTime(new Date());
	...
	public void mergePatients_shouldAuditPriorDateOfDeath() throws Exception {
		...
		voidOrders(Collections.singleton(notPreferred));
		PersonMergeLog audit = mergeAndRetrieveAudit(preferred, notPreferred);
		Assert.assertEquals("prior date of death was not audited", cDate
				.getTime(), audit.getPersonMergeLogData().getPriorDateOfDeath());
	 ...
	 */
	@Test
	public void mergePatients_shouldAuditPriorDateOfDeathEstimated()
			throws Exception {
	...
	public void mergePatients_shouldAuditPriorGender() throws Exception {
		...
		voidOrders(Collections.singleton(notPreferred));
		PersonMergeLog audit = mergeAndRetrieveAudit(preferred, notPreferred);
		Assert.assertEquals("prior gender was not audited", "M", audit
				.getPersonMergeLogData().getPriorGender());
	}
	 ...
	 */
	@Test
	public void mergePatients_shouldNotCopyOverDuplicatePatientIdentifiers()
			throws Exception {
		...
		PatientIdentifierType patientIdentifierType = Context
				.getPatientService().getPatientIdentifierType(5);
		...
		Assert.assertEquals(notPreferred.getIdentifiers().size() - 1, audit
				.getPersonMergeLogData().getCreatedIdentifiers().size());
	}

	private PersonMergeLog mergeAndRetrieveAudit(Patient preferred,
			Patient notPreferred) throws SerializationException {
		patientService.mergePatients(preferred, notPreferred);
	...
	public void mergePatients_shouldNotVoidRelationshipsForSameTypeAndSideWithDifferentRelatives()
			throws Exception {
		executeDataSet(PATIENT_RELATIONSHIPS_XML);
		...
		// and 7->999 (type 1)
		List<Relationship> rels = personService.getRelationships(new Person(7),
				preferred, new RelationshipType(4));
		assertEquals("7->999 (type 4) was removed", 1, rels.size());

		rels = personService.getRelationships(new Person(7), preferred,
				new RelationshipType(1));
		assertEquals("7->999 (type 1) was removed", 1, rels.size());
	}

	/**
	 * @verifies {@link PatientService#savePatient(Patient)} test = should
	 *           update the date changed and changed by on update of the person
	 *           address
	 */
	@Test
	@Verifies(value = "should update the date changed and changed by on update of the person address", method = "savePatient(Patient)")
	public void savePatient_shouldUpdateTheDateChangedAndChangedByOnUpdateOfThePersonAddress()
			throws Exception {
	...
	public void getPatient_shouldReturnNullWhenPersonDoesNotExist()
			throws Exception {
		executeDataSet(USER_WHO_IS_NOT_PATIENT_XML);
	...
	public void voidPatient_shouldRetireUsers() throws Exception {
		...
		Assert.assertFalse(Context.getUserService()
				.getUsersByPerson(patient, false).isEmpty());

		// when
		patientService.voidPatient(patient, "reason");

		// then
		Assert.assertTrue(Context.getUserService()
				.getUsersByPerson(patient, false).isEmpty());
	}
	 ...
	 */
	@Test
	public void getPatients_shouldReturnEmptyListIfNameAndIdentifierIsEmpty()
			throws Exception {
		// given

		// when
		List<Patient> patients = patientService
				.getPatients("", "", null, false);
	...
	public void isIdentifierInUseByAnotherPatient_shouldReturnTrueIfInUseAndIdTypeUniquenessIsNull()
			throws Exception {
		...
		PatientIdentifier pi = new PatientIdentifier(
				duplicateId.getIdentifier(), idType, duplicateId.getLocation());
	...
	public void isIdentifierInUseByAnotherPatient_shouldReturnFalseIfInUseForAnotherLocationAndIdUniquenessIsSetToLocation()
		...
		Assert.assertNotSame(idLocation, duplicateId.getLocation());// sanity
																	// check
		PatientIdentifier pi = new PatientIdentifier(
				duplicateId.getIdentifier(), idType, idLocation);
	 ...
	 */
	@Test
	public void getAllPatientIdentifierTypes_shouldOrderAsDefaultComparator()
			throws Exception {
		List<PatientIdentifierType> list = patientService
				.getAllPatientIdentifierTypes();
		List<PatientIdentifierType> sortedList = new ArrayList<PatientIdentifierType>(
				list);
		Collections.sort(sortedList,
				new PatientIdentifierTypeDefaultComparator());
	 ...
	 */
	@Test
	public void getPatientIdentifierTypes_shouldOrderAsDefaultComparator()
			throws Exception {
		List<PatientIdentifierType> list = patientService
				.getPatientIdentifierTypes(null, null, false, null);
		List<PatientIdentifierType> sortedList = new ArrayList<PatientIdentifierType>(
				list);
		Collections.sort(sortedList,
				new PatientIdentifierTypeDefaultComparator());
	...
	public void mergePatients_shouldMaintainSimilarButDifferentNames()
			throws Exception {
		...
		if ((PersonName.getFormat())
				.equals(OpenmrsConstants.PERSON_NAME_FORMAT_LONG))
			assertThat(names,
					containsFullName("President John Fitzgerald Kennedy Esq."));
		else
	...
	public void mergePatients_shouldMaintainSimilarButDifferentAddresses()
			throws Exception {
		executeDataSet(PATIENT_MERGE_XML);
		Patient preferredPatient = patientService.getPatient(10000);
		Patient nonPreferredPatient = patientService.getPatient(10001);
	...
	/**
	 * @see PatientService#savePatient(Patient)
	 * @verifies set the preferred name address and identifier if none is
	 *           specified
	 */
	@Test
	public void savePatient_shouldSetThePreferredNameAddressAndIdentifierIfNoneIsSpecified()
		...
		Patient patient = new Patient();
		patient.setGender("M");
		PatientIdentifier identifier = new PatientIdentifier("QWERTY",
				patientService.getPatientIdentifierType(2),
				locationService.getLocation(1));
		patient.addIdentifier(identifier);
		PersonName name = new PersonName("givenName", "middleName",
				"familyName");
	...
	/**
	 * @see PatientService#savePatient(Patient)
	 * @verifies not set the preferred name address and identifier if they
	 *           already exist
	 */
	@Test
	public void savePatient_shouldNotSetThePreferredNameAddressAndIdentifierIfTheyAlreadyExist()
			throws Exception {
		Patient patient = new Patient();
		patient.setGender("M");
		PatientIdentifier identifier = new PatientIdentifier("QWERTY",
				patientService.getPatientIdentifierType(5),
				locationService.getLocation(1));
		PatientIdentifier preferredIdentifier = new PatientIdentifier(
				"QWERTY2", patientService.getPatientIdentifierType(2),
				locationService.getLocation(1));
		...
		PersonName name = new PersonName("givenName", "middleName",
				"familyName");
		PersonName preferredName = new PersonName("givenName", "middleName",
				"familyName");
	 ...
	 */
	@Test
	public void savePatient_shouldNotSetAVoidedNameOrAddressOrIdentifierAsPreferred()
			throws Exception {
		Patient patient = new Patient();
		patient.setGender("M");
		PatientIdentifier identifier = new PatientIdentifier("QWERTY",
	...
	public void savePatient_shouldNotThrowNonUniqueObjectExceptionWhenCalledWithPersonPromotedToPatient()
			throws Exception {
		Person person = personService.getPerson(1);
		Patient patient = patientService.getPatientOrPromotePerson(person
				.getPersonId());
		PatientIdentifier patientIdentifier = new PatientIdentifier(
				"some identifier", new PatientIdentifierType(2),
				new Location(1));
	...
	public void getPatients_shouldFindAPatientsWithAMatchingIdentifierWithNoDigits()
			throws Exception {
		final String identifier = "XYZ";
		Patient patient = patientService.getPatient(2);
		Assert.assertEquals(
				0,
				patientService.getPatients(identifier, (Integer) null,
						(Integer) null).size());
		PatientIdentifier pId = new PatientIdentifier(identifier,
				patientService.getPatientIdentifierType(5),
				locationService.getLocation(1));
	...
	public void getCountOfPatients_shouldReturnTheRightCountOfPatientsWithAMatchingIdentifierWithNoDigits()
		...
		final String identifier = "XYZ";
		Patient patient = patientService.getPatient(2);
		Assert.assertEquals(0, patientService.getCountOfPatients(identifier)
				.intValue());
		PatientIdentifier pId = new PatientIdentifier(identifier,
				patientService.getPatientIdentifierType(5),
				locationService.getLocation(1));
		...
		Assert.assertEquals(1, patientService.getCountOfPatients(identifier)
				.intValue());
	}

	/**
	 * @verifies {@link PatientService#savePatientIdentifier(PatientIdentifier)}
	 *           test = should throw an APIException when a null argument is
	 *           passed
	 */
	@Test(expected = APIException.class)
	...
	public void savePatientIdentifier_shouldThrowAnAPIExceptionWhenANullArgumentIsPassed()
			throws Exception {
		patientService.savePatientIdentifier(null);
	}

	/**
	 * Creates a new Global Property to lock patient identifier types by setting
	 * its value
	 * 
	 * @param propertyValue
	 *            value for patient identifier types locked GP
	 */
	public void createPatientIdentifierTypeLockedGPAndSetValue(
			String propertyValue) {
		GlobalProperty gp = new GlobalProperty(
				OpenmrsConstants.GLOBAL_PROPERTY_PATIENT_IDENTIFIER_TYPES_LOCKED);
	...
	/**
	 * @see PatientService#retirePatientIdentifierType(PatientIdentifierType,
	 *      String)
	 * @throws PatientIdentifierTypeLockedException
	 */
	@Test(expected = PatientIdentifierTypeLockedException.class)
	 ...
	 * @see PatientService#mergePatients(org.openmrs.Patient,
	 *      org.openmrs.Patient)
	 */
	@Test
	public void mergePatients_shouldFailIfNotPreferredPatientHasUnvoidedOrders()
			throws Exception {
		expectedException.expect(APIException.class);
		expectedException.expectMessage(Matchers.is(Context
				.getMessageSourceService().getMessage("Patient.cannot.merge")));
