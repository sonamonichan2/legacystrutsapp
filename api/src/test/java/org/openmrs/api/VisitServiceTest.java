...
public class VisitServiceTest extends BaseContextSensitiveTest {
	...
	public void before() {
		...
		// Allow overlapping visits. Ticket 3963 has introduced optional
		// validation of start and stop dates
		// based on concurrent visits of the same patient. Turning this
		// validation on (i.e. not allowing
		...
		globalPropertiesTestHelper.setGlobalProperty(
				OpenmrsConstants.GLOBAL_PROPERTY_ALLOW_OVERLAPPING_VISITS,
				"true");
	}
	...
	public void getVisitTypeByUuid_shouldGetCorrentVisitType() throws Exception {
		VisitType visitType = Context.getVisitService().getVisitTypeByUuid(
				"c0c579b0-8e59-401d-8a4a-976a0b183519");
		assertNotNull(visitType);
		assertEquals("Initial HIV Clinic Visit", visitType.getName());

		visitType = Context.getVisitService().getVisitTypeByUuid(
				"759799ab-c9a5-435e-b671-77773ada74e4");
		assertNotNull(visitType);
		assertEquals("Return TB Clinic Visit", visitType.getName());

		visitType = Context.getVisitService().getVisitTypeByUuid(
				"759799ab-c9a5-435e-b671-77773ada74e6");
		assertNotNull(visitType);
		assertEquals("Hospitalization", visitType.getName());

		visitType = Context.getVisitService().getVisitTypeByUuid(
				"759799ab-c9a5-435e-b671-77773ada74e1");
		Assert.assertNull(visitType);
	}
	...
	public void getVisitByUuid_shouldReturnAVisitMatchingTheSpecifiedUuid()
		...
		Visit visit = Context.getVisitService().getVisitByUuid(
				"1e5d5d48-6b78-11e0-93c3-18a905e044dc");
		assertNotNull(visit);
	...
	private VisitAttribute createVisitAttributeWithoutCreatorAndDateCreated() {
		...
		VisitAttributeType attributeType = Context.getVisitService()
				.getVisitAttributeType(1);
	...
	public void saveVisit_shouldVoidAnAttributeIfMaxOccursIs1AndSameAttributeTypeAlreadyExists()
		...
		visit.setAttribute(createVisitAttribute(new Date(System
		...
		VisitAttribute firstAttribute = (VisitAttribute) visit.getAttributes()
				.toArray()[0];
	...
	public void saveVisit_shouldUpdateAnExistingVisitInTheDatabase()
		...
		Assert.assertNull(visit.getLocation());// this is the field we are
												// editing
	...
	public void voidVisit_shouldVoidEncountersWithVisit() throws Exception {
		...
		List<Encounter> encountersByVisit = Context.getEncounterService()
				.getEncountersByVisit(visit, false);
		...
		encountersByVisit = Context.getEncounterService().getEncountersByVisit(
				visit, false);
	...
	public void unvoidVisit_shouldUnvoidEncountersVoidedWithVisit()
		...
		List<Encounter> encountersByVisit = Context.getEncounterService()
				.getEncountersByVisit(visit, true);
	...
	public void purgeVisit_shouldEraseTheVisitFromTheDatabase()
			throws Exception {
		VisitService vs = Context.getVisitService();
		Integer originalSize = vs.getVisits(null, null, null, null, null, null,
				null, null, null, true, true).size();
		Visit visit = Context.getVisitService().getVisit(1);
		vs.purgeVisit(visit);
		assertEquals(
				originalSize - 1,
				vs.getVisits(null, null, null, null, null, null, null, null,
						null, true, true).size());
	}
	...
	public void getVisitsByPatient_shouldReturnAllUnvoidedVisitsForTheSpecifiedPatient()
			throws Exception {
		assertEquals(3,
				Context.getVisitService().getVisitsByPatient(new Patient(2))
						.size());
	}
	...
	public void getActiveVisitsByPatient_shouldReturnAllUnvoidedActiveVisitsForTheSpecifiedPatient()
		...
		assertEquals(
				...
				Context.getVisitService()
						.getActiveVisitsByPatient(new Patient(2)).size());
	}
	...
	public void getActiveVisitsByPatient_shouldReturnAllActiveVisitsForTheSpecifiedPatient()
		...
		assertEquals(
				5,
				Context.getVisitService()
						.getVisitsByPatient(new Patient(2), false, true).size());
	}
	...
	public void getActiveVisitsByPatient_shouldReturnAllUnvoidedVisitsForTheSpecifiedPatient()
		...
		assertEquals(
				8,
				Context.getVisitService()
						.getVisitsByPatient(new Patient(2), true, false).size());
	}

	/**
	 * @see VisitService#getVisits(java.util.Collection, java.util.Collection,
	 *      java.util.Collection, java.util.Collection, Date, Date, Date, Date,
	 *      boolean)
	 */
	@Test
	...
	public void getVisits_shouldGetVisitsByIndications() throws Exception {
		assertEquals(
				1,
				Context.getVisitService()
						.getVisits(null, null, null,
								Collections.singletonList(new Concept(5497)),
	...
	public void getVisits_shouldGetVisitsByLocations() throws Exception {
		...
		assertEquals(
				1,
				Context.getVisitService()
						.getVisits(null, null, locations, null, null, null,
	...
	public void getVisits_shouldGetVisitsByVisitType() throws Exception {
		...
		assertEquals(
				4,
				Context.getVisitService()
						.getVisits(visitTypes, null, null, null, null, null,
								null, null, null, true, false).size());
	}
	...
	public void getVisits_shouldGetVisitsEndedBetweenTheGivenEndDates()
		...
		assertEquals(
				2,
				Context.getVisitService()
						.getVisits(null, null, null, null, null, null,
								minEndDate, maxEndDate, null, true, false)
						.size());
	}
	 ...
	 * @verifies get visits that are still open even if minStartDatetime is
	 *           specified
	 */
	@Test
	public void getVisits_shouldGetVisitsThatAreStillOpenEvenIfMinStartDatetimeIsSpecified()
			throws Exception {
		Date minEndDatetime = new SimpleDateFormat("yyyy-MM-dd")
				.parse("2061-01-01");
		// this should get all open non-voided visits (which are ids 1, 2, 3, 4,
		// 5 in standardTestDataset)
		List<Visit> visits = Context.getVisitService()
				.getVisits(null, null, null, null, null, null, minEndDatetime,
	...
	public void getVisits_shouldGetVisitsStartedBetweenTheGivenStartDates()
		...
		assertEquals(
				2,
				Context.getVisitService()
						.getVisits(null, null, null, null, minStartDate,
								maxStartDate, null, null, null, true, false)
						.size());
	}
	...
	public void getVisits_shouldReturnAllVisitsIfIncludeVoidedIsSetToTrue()
		...
		assertEquals(
				6,
				Context.getVisitService()
						.getVisits(null, null, null, null, null, null, null,
								null, null, true, true).size());
	}
	...
	public void getVisitAttributeType_shouldReturnTheVisitAttributeTypeWithTheGivenId()
			throws Exception {
		executeDataSet(VISITS_ATTRIBUTES_XML);
		assertEquals("Audit Date", visitService.getVisitAttributeType(1)
				.getName());
	}
	...
	public void getVisitAttributeTypeByUuid_shouldReturnTheVisitAttributeTypeWithTheGivenUuid()
		...
		assertEquals(
				"Audit Date",
				visitService.getVisitAttributeTypeByUuid(
						"9516cc50-6f9f-11e0-8414-001e378eb67e").getName());
	}

	/**
	 * @see VisitService#getVisitAttributeTypeByUuid(String)
	 * @verifies return null if no visit attribute type exists with the given
	 *           uuid
	 */
	@Test
	public void getVisitAttributeTypeByUuid_shouldReturnNullIfNoVisitAttributeTypeExistsWithTheGivenUuid()
			throws Exception {
		executeDataSet(VISITS_ATTRIBUTES_XML);
		Assert.assertNull(visitService
				.getVisitAttributeTypeByUuid("not-a-uuid"));
	}
	...
	public void purgeVisitAttributeType_shouldCompletelyRemoveAVisitAttributeType()
		...
		executeDataSet(VISITS_ATTRIBUTES_XML);
		assertEquals(3, visitService.getAllVisitAttributeTypes().size());
		visitService.purgeVisitAttributeType(visitService
				.getVisitAttributeType(2));
	...
	public void saveVisitAttributeType_shouldEditAnExistingVisitAttributeType()
		...
		visitService.saveVisitAttributeType(vat);
		assertEquals(3, visitService.getAllVisitAttributeTypes().size());
		assertEquals("A new name", visitService.getVisitAttributeType(1)
				.getName());
	}
	...
	public void getVisitAttributeByUuid_shouldGetTheVisitAttributeWithTheGivenUuid()
		...
		assertEquals(
				"2011-04-25",
				visitService.getVisitAttributeByUuid(
	 ...
	 */
	@Test
	public void getVisits_shouldGetAllVisitsWithGivenAttributeValues()
			throws Exception {
		executeDataSet(VISITS_ATTRIBUTES_XML);
		Map<VisitAttributeType, Object> attrs = new HashMap<VisitAttributeType, Object>();
		attrs.put(visitService.getVisitAttributeType(1), new SimpleDateFormat(
				"yyyy-MM-dd").parse("2011-04-25"));
		List<Visit> visits = visitService.getVisits(null, null, null, null,
				null, null, null, null, attrs, true, false);
	 ...
	 */
	@Test
	public void getVisits_shouldNotFindAnyVisitsIfNoneHaveGivenAttributeValues()
			throws Exception {
		executeDataSet(VISITS_ATTRIBUTES_XML);
		Map<VisitAttributeType, Object> attrs = new HashMap<VisitAttributeType, Object>();
		attrs.put(visitService.getVisitAttributeType(1), new SimpleDateFormat(
				"yyyy-MM-dd").parse("1411-04-25"));
		List<Visit> visits = visitService.getVisits(null, null, null, null,
				null, null, null, null, attrs, true, false);
	...
	public void purgeVisit_shouldFailIfTheVisitHasEncountersAssociatedToIt()
		...
		// sanity check
		assertTrue(Context.getEncounterService()
				.getEncountersByVisit(visit, false).size() > 0);
	...
	public void saveVisit_shouldBeAbleToAddAnAttributeToAVisit()
		...
		assertEquals(new SimpleDateFormat("yyyy-MM-dd").format(now),
				attr.getValueReference());
	}
	...
	public void stopVisits_shouldCloseAllUnvoidedActiveVisitMatchingTheSpecifiedVisitTypes()
		...
		int activeVisitCount = Context.getAdministrationService()
				.executeSQL(openVisitsQuery, true).size();
		// sanity check
		assertTrue(
				"There should be some active visits for this test to be valid",
				activeVisitCount > 0);

		// close any unvoided open visits
		visitService.stopVisits(null);

		activeVisitCount = Context.getAdministrationService()
				.executeSQL(openVisitsQuery, true).size();

		// all active unvoided visits should have been closed
		assertTrue("Not all active unvoided vists were closed",
				activeVisitCount == 0);
	}
	...
	public void saveVisit_shouldSaveNewVisitWithEncountersSuccessfully()
		...
		assertEquals(Integer.valueOf(4), ((Encounter) visit.getEncounters()
				.toArray()[0]).getEncounterId());
	}
	...
	public void saveVisit_shouldAssociateEncounterWithVisitOnSaveEncounter()
		...
		assertEquals(1, visit.getEncounters().size());
		assertEquals(encounterId,
				((Encounter) visit.getEncounters().toArray()[0])
						.getEncounterId());
	}
	...
	public void getAllVisitTypes_shouldGetAllVisitTypesBasedOnIncludeRetiredFlag()
		...
		assertEquals("get all visit types including retired", 3,
				visitTypes.size());
		visitTypes = visitService.getAllVisitTypes(false);
		assertEquals("get all visit types excluding retired", 2,
				visitTypes.size());
	}
