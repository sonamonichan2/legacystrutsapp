...
/**
 * Helps to mock or spy on services. It can be used with {@link InjectMocks}.
 * See {@link org.openmrs.module.ModuleUtilTest} for example. In general you
 * should always try to refactor code first so that this class is not needed. In
 * practice it is mostly enough to replace calls to Context.get...Service with
 * fields, which are injected through a constructor.
 * <p>
 * ContextMockHelper is available in tests extending
 * {@link org.openmrs.test.BaseContextMockTest} and
 * {@link org.openmrs.test.BaseContextSensitiveTest}.
 * 
 * @deprecated Avoid using this by not calling Context.get...Service() in your
 *             code.
...
public class ContextMockHelper {
	...
	/**
	 * Mockito does not call setters if there are no fields so you must add both
	 * a setter and a field.
	 */
	...
	public void revertMocks() {
		for (Map.Entry<Class<?>, Object> realService : realServices.entrySet()) {
			Context.getServiceContext().setService(realService.getKey(),
					realService.getValue());
		}
		realServices.clear();
		...
		if (applicationContextMocked) {
			if (realApplicationContext != null) {
				Context.getServiceContext().setApplicationContext(
						realApplicationContext);
	...
	public void setApplicationContext(ApplicationContext context) {
		if (!applicationContextMocked) {
			realApplicationContext = Context.getServiceContext()
