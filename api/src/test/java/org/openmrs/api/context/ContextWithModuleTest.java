 ...
 * {@link Context#loadClass(String)} method. This method needs to have a module
 * loaded for it to test correctly, so it is put into a separate class The
 * module is stolen/copied from the {@link ModuleInteroperabilityTest}
 * 
 * @see ContextTest
 */
public class ContextWithModuleTest extends BaseContextSensitiveTest {
	...
	public Properties getRuntimeProperties() {
		...
		// the "/metadata/sqldiff.xml" file has been deleted in order to load
		// the modules into hsql.
		// (the sql tables are built from hibernate mapping files automatically
		// in unit tests)
		props.setProperty(
	...
	public void loadClass_shouldLoadClassWithOpenmrsClassLoader()
			throws Exception {
		Class<?> c = Context
				.loadClass("org.openmrs.module.test1.api.Test1Service");
		Assert.assertTrue("Should be loaded by OpenmrsClassLoader",
				c.getClassLoader() instanceof ModuleClassLoader);
	}
