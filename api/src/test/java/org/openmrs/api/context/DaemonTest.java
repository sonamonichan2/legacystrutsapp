...
public class DaemonTest extends BaseContextSensitiveTest {
	 ...
	 */
	@Test
	public void executeScheduledTask_shouldNotBeCalledFromOtherMethodsOtherThanTimerSchedulerTask()
			throws Throwable {
		try {
			...
			Assert.assertTrue(e.getMessage().startsWith(
					Context.getMessageSourceService().getMessage(
							"Scheduler.timer.task.only",
							new Object[]{this.getClass().getName()}, null)));
		}
	}

	/**
	 * This uses a task that just marks itself as run when its "execute" method
	 * is called. This verifies that the Daemon class is getting past the class
	 * check and on to the task running step
	 * 
	 * @see Daemon#executeScheduledTask(Task)
	 * @verifies not throw error if called from a TimerSchedulerTask class
	 */
	@Test
	public void executeScheduledTask_shouldNotThrowErrorIfCalledFromATimerSchedulerTaskClass()
			throws Throwable {
	...
	public void runInNewDaemonThread_shouldThrowErrorIfCalledFromANonDaemonThread() {
		try {
			...
			Assert.assertTrue(
					"Should not hit this line, since the previous needed to throw an exception",
					false);
	 ...
	 */
	@Test
	public void runInNewDaemonThread_shouldNotThrowErrorIfCalledFromADaemonThread()
			throws Throwable {
		Task taskThatStartsAnotherThread = new TaskThatStartsAnotherThread();
		Assert.assertTrue(new PrivateSchedulerTask(taskThatStartsAnotherThread)
				.runTheTest());
	}
	...
	private class PrivateSchedulerTask extends TimerSchedulerTask {
		...
		/**
		 * Returns true/false whether the task was successfully run by the
		 * Daemon user
		 * 
		 * @return
		 */
	...
	/**
	 * A task that starts another Daemon thread that marks *this* thread when it
	 * gets run.
	 */
	...
	/**
	 * A task for testing to ensure that a daemon user always has an associated
	 * person.
	 */
