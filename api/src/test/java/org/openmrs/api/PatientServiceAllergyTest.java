...
public class PatientServiceAllergyTest extends BaseContextSensitiveTest {
	...
	public void getAllergyByUuid_shouldGetAllergyByUuid() throws Exception {
		Allergy allergy = allergyService
				.getAllergyByUuid("21543629-7d8c-11e1-909d-c80aa9edcf4e");
	...
	public void setAllergies_shouldSaveTheAllergyListAndStatus()
		...
		Allergen allergen = new Allergen(AllergenType.DRUG, new Concept(3),
				null);
		Concept severity = new Concept(4);
		Allergy allergy = new Allergy(patient, allergen, severity,
				"some comment", new ArrayList<AllergyReaction>());
		AllergyReaction reaction = new AllergyReaction(allergy,
				new Concept(21), null);
	...
	/**
	 * @see PatientService#setAllergies(Patient,Allergies)
	 * @verifies void removed allergies and maintain status as see list if some
	...
	public void setAllergies_shouldVoidRemovedAllergiesAndMaintainStatusAsSeeListIfSomeAllergiesAreRemoved()
		...
		AllergyReaction reaction = new AllergyReaction(null, new Concept(22),
				null);
		allergies.get(2).addReaction(reaction);

		allergyService.setAllergies(patient, allergies);

		// should remain with three un voided allergies and status maintained as
		// see list
		allergies = allergyService.getAllergies(patient);
		Assert.assertEquals(Allergies.SEE_LIST, allergies.getAllergyStatus());
	...
	/**
	 * @see PatientService#setAllergies(Patient,Allergies)
	 * @verifies void all allergies and set status to unknown if all allergies
	 *           are removed
	 */
	@Test
	...
	/**
	 * @see PatientService#setAllergies(Patient,Allergies)
	 * @verifies void all allergies and set status to no known allergies if all
	...
	public void setAllergies_shouldVoidAllAllergiesAndSetStatusToNoKnownAllergiesIfAllAllergiesAreRemovedAndStatusSetAsSuch()
		...
		allergies = allergyService.getAllergies(patient);
		Assert.assertEquals(Allergies.NO_KNOWN_ALLERGIES,
				allergies.getAllergyStatus());
		Assert.assertEquals(0, allergies.size());
	}
	...
	public void setAllergies_shouldVoidAllergiesWithEditedComment()
		...
		// clear any cache for this object such that the next calls fetch it
		// from the database
		...
		// should remain with four unvoided allergies and status maintained as
		// see list
		allergies = allergyService.getAllergies(patient);
		Assert.assertEquals(Allergies.SEE_LIST, allergies.getAllergyStatus());
		Assert.assertEquals(4, allergies.size());
	...
	public void setAllergies_shouldVoidAllergiesWithEditedNonCodedAllergen()
		...
		editedAllergy.getAllergen().setNonCodedAllergen(
				"some non coded allergen");
	...
	public void setAllergies_shouldSetTheNonCodedConceptForNonCodedAllergenIfNotSpecified()
			throws Exception {
		Patient patient = allergyService.getPatient(2);
		Allergen allergen = new Allergen(AllergenType.DRUG, null,
				"Some allergy name");
