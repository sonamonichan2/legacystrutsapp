...
/**
 * Tests methods of the module activator that do not require refreshing of the
 * spring application context. For those that require refreshing, see
 * WebModuleActivatorTest
 */
...
public class ModuleActivatorTest extends BaseModuleActivatorTest {
	...
	public void shouldStartModulesInOrder() throws Exception {
		...
		assertTrue(moduleTestData.getWillStartCallTime(MODULE1_ID) <= moduleTestData
				.getWillStartCallTime(MODULE2_ID));
		assertTrue(moduleTestData.getWillStartCallTime(MODULE2_ID) <= moduleTestData
				.getWillStartCallTime(MODULE3_ID));

		assertTrue(moduleTestData.getStartedCallTime(MODULE1_ID) <= moduleTestData
				.getStartedCallTime(MODULE2_ID));
		assertTrue(moduleTestData.getStartedCallTime(MODULE2_ID) <= moduleTestData
	...
	public void shouldStopDependantModulesOnStopModule() throws Exception {
		...
		// which is the reverse of the startup. that is module3, module2,
		// module1
		assertThat(moduleTestData.getWillStopCallTime(MODULE3_ID),
				lessThanOrEqualTo(moduleTestData
						.getWillStopCallTime(MODULE2_ID)));
		assertThat(moduleTestData.getWillStopCallTime(MODULE2_ID),
				lessThanOrEqualTo(moduleTestData
						.getWillStopCallTime(MODULE1_ID)));

		assertThat(
				moduleTestData.getStoppedCallTime(MODULE3_ID),
				lessThanOrEqualTo(moduleTestData.getStoppedCallTime(MODULE2_ID)));
		assertThat(
				moduleTestData.getStoppedCallTime(MODULE2_ID),
				lessThanOrEqualTo(moduleTestData.getStoppedCallTime(MODULE1_ID)));
	}
	...
	public void shouldCallWillStopAndStoppedOnShutdown() throws Exception {
		...
		// willStop() should have been called before stopped() for module1,
		// module2, and module3
		assertTrue(moduleTestData.getWillStopCallTime(MODULE1_ID) <= moduleTestData
				.getStoppedCallTime(MODULE1_ID));
		assertTrue(moduleTestData.getWillStopCallTime(MODULE2_ID) <= moduleTestData
				.getStoppedCallTime(MODULE2_ID));
		assertTrue(moduleTestData.getWillStopCallTime(MODULE3_ID) <= moduleTestData
				.getStoppedCallTime(MODULE3_ID));
	}

	@Test
	@Ignore("Unignore after investigating and fixing - TRUNK-4678")
	public void shouldExcludePreviouslyStoppedModulesOnShutdown() {
		// At OpenMRS shutdown, willStop() and stopped() methods get called for
		// all
		// started module's activator EXCLUDING any module(s) that were
		// previously stopped.
	...
	public void shouldStartBeforeAnotherModule() {
		...
		assertTrue(moduleTestData.getWillStartCallTime(MODULE5_ID) <= moduleTestData
				.getWillStartCallTime(MODULE4_ID));
		assertTrue(moduleTestData.getWillStartCallTime(MODULE1_ID) <= moduleTestData
				.getWillStartCallTime(MODULE5_ID));

		assertTrue(moduleTestData.getStartedCallTime(MODULE5_ID) <= moduleTestData
				.getStartedCallTime(MODULE4_ID));
		assertTrue(moduleTestData.getStartedCallTime(MODULE1_ID) <= moduleTestData
				.getStartedCallTime(MODULE5_ID));
	}
}
