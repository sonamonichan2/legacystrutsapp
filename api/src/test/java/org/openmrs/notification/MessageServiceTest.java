...
public class MessageServiceTest extends BaseContextSensitiveTest {
	...
	public void createMessage_shouldCreateMessage() throws Exception {
		...
		Message msg4 = ms.createMessage(recipients, sender, subject, message,
	...
	public void sendMessage_shouldSendMessage() throws Exception {
		Message tryToSend1 = ms.createMessage("recipient@example.com",
				"sender@example.com", "subject", "content");
		try {
			ms.sendMessage(tryToSend1);
		} catch (MessageException e) {
			// So that this test doesn't fail just because the user isn't
		...
		Message tryToSend2 = ms.createMessage(
				"recipient@example.com,recipient2@example.com",
				"openmrs.emailer@gmail.com", "subject", "content", "moo",
				"text/plain", "moo.txt");
		try {
