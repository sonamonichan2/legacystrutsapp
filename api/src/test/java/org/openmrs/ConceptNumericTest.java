...
public class ConceptNumericTest extends BaseContextSensitiveTest {
	...
	/**
	 * Tests if {@link org.openmrs.api.ConceptService#saveConcept(Concept)}
	 * saves a ConceptNumeric with allowDecimal value
	 */
	@Test
	...
	public void shouldSaveAConceptNumericWithAllowDecimalValue()
		...
		Assert.assertFalse(Context.getConceptService().getConceptNumeric(22)
				.getAllowDecimal());

		cn.setAllowDecimal(true);
		Context.getConceptService().saveConcept(cn);
		Assert.assertTrue(Context.getConceptService().getConceptNumeric(22)
				.getAllowDecimal());
	}
}
