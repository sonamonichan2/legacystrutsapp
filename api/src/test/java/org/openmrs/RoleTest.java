...
/**
 * This class tests all methods that are not getter or setters in the Role java
 * object TODO: finish this test class for Role
 * 
 * @see Role
 */
public class RoleTest {
	...
	public void shouldAddRemovePrivilege() throws Exception {
		...
		assertEquals("Incorrect number of privileges", 1, role.getPrivileges()
				.size());
		...
		assertEquals("Incorrect number of privileges", 0, role.getPrivileges()
				.size());
	}
	...
	public void hasPrivilege_shouldReturnTrueIfFound() throws Exception {
		...
		assertTrue("This roles should have the privilege",
				role.hasPrivilege("priv1"));
	}
	...
	public void hasPrivilege_shouldReturnFalseIfNotFound() throws Exception {
		Role role = new Role();
		assertFalse("This roles should not have the privilege",
				role.hasPrivilege("some other privilege name"));
	}
	...
	public void hasPrivilege_shouldReturnTrueForAnyPrivilegeNameIfSuperUser()
		...
		assertTrue(
				"Super users are super special and should have all privileges",
				role.hasPrivilege("Some weird privilege name that shouldn't be there"));
