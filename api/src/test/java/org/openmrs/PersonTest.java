...
public class PersonTest extends BaseContextSensitiveTest {
	...
	public void shouldAddRemoveAddress() throws Exception {
		...
		assertTrue(
				"There should be 1 address in the person object but there is actually : "
						+ p.getAddresses().size(), p.getAddresses().size() == 1);
		...
		// make sure the address is added
		assertTrue(
				"There should be 2 addresses in the person object but there is actually : "
						+ p.getAddresses().size(), p.getAddresses().size() == 2);
		...
		// make sure the address IS added
		assertTrue(
				"There should be 3 addresses in the person object but there is actually : "
						+ p.getAddresses().size(), p.getAddresses().size() == 3);

		p.removeAddress(pa3);
		assertTrue("There should be only 2 address in the person object now", p
				.getAddresses().size() == 2);
		...
		assertTrue("There should be only 2 address in the person object now", p
				.getAddresses().size() == 2);
		p.removeAddress(pa2);
		assertTrue("There should be only 1 address in the person object now", p
				.getAddresses().size() == 1);
		p.removeAddress(pa2);
		assertTrue(
				"There should still be only 1 address in the person object now",
				p.getAddresses().size() == 1);
		p.removeAddress(pa1);
		assertTrue("There shouldn't be any addresses in the person object now",
				p.getAddresses().size() == 0);
	}
	...
	public void shouldAddRemoveName() throws Exception {
		...
		assertTrue(
				"There should be 1 name in the person object but there is actually : "
						+ p.getNames().size(), p.getNames().size() == 1);
		...
		// make sure the name is added
		assertTrue(
				"There should be 2 names in the person object but there is actually : "
						+ p.getNames().size(), p.getNames().size() == 2);
		...
		pa4.setGivenName(pa1.getGivenName()
				+ "string to change the .equals method");
		...
		assertTrue(
				"There should be 3 names in the person object but there is actually : "
						+ p.getNames().size(), p.getNames().size() == 3);

		p.removeName(pa3);
		assertTrue("There should be only 2 name in the person object now", p
				.getNames().size() == 2);
		...
		p.removeName(pa4);
		assertTrue("There should be only 2 names in the person object now", p
				.getNames().size() == 2);
		p.removeName(pa3); // pa3 was never added, but is the same as pa1
		assertTrue("There should be only 1 names in the person object now", p
				.getNames().size() == 1);
		p.removeName(pa2);
		assertTrue("There should be only no names in the person object now", p
				.getNames().size() == 0);
		p.removeName(pa2);
		assertTrue(
				"There should still be only no names in the person object now",
				p.getNames().size() == 0);
		p.removeName(pa1);
		assertTrue("There shouldn't be any names in the person object now", p
				.getNames().size() == 0);
	}
	...
	public void shouldAddRemoveAttribute() throws Exception {
		...
		assertTrue(
				"There should be 1 attribute in the person object but there is actually : "
						+ p.getAttributes().size(),
				p.getAttributes().size() == 1);
		...
		// make sure the attribute is added
		assertTrue(
				"There should be 2 attributes in the person object but there is actually : "
						+ p.getAttributes().size(),
		...
		// status is different)
		pa3.setValue(pa1.getValue()
				+ "addition to make sure the value is different");
		pa3.setVoided(true);
		p.addAttribute(pa3);
		// make sure the attribute IS added
		assertTrue(
				"There should be 3 attributes in the person object but there is actually : "
						+ p.getAttributes().size(),
				p.getAttributes().size() == 3);

		p.removeAttribute(pa3);
		assertTrue("There should be only 2 attribute in the person object now",
				p.getAttributes().size() == 2);
		...
		assertTrue("There should be only 1 attribute in the person object now",
				p.getAttributes().size() == 1);
		p.removeAttribute(pa2);
		assertTrue(
				"There should still be only 1 attribute in the person object now",
				p.getAttributes().size() == 1);
		p.removeAttribute(pa1);
		assertTrue(
				"There shouldn't be any attributes in the person object now", p
	...
	public void getPersonAddress_shouldGetNotvoidedPersonAddressIfPreferredAddressDoesNotExist()
		...
		// addresses
		PersonAddress voidedAddress = PersonAddressBuilder.newBuilder()
				.withPreferred(false).withVoided(true).build();

		// addresses
		PersonAddress notVoidedAddress = PersonAddressBuilder.newBuilder()
				.withPreferred(false).withVoided(false).build();

		PersonAddress expectedPersonAddress = notVoidedAddress;
		Set<PersonAddress> personAddresses = new HashSet<PersonAddress>(
				Arrays.asList(voidedAddress, notVoidedAddress));

		checkGetPersonAddressResultForVoidedPerson(expectedPersonAddress,
				personAddresses);
	}
	...
	public void getPersonAddress_shouldGetPreferredAndNotvoidedPersonAddressIfExist()
		...
		PersonAddress preferredNotVoidedAddress = PersonAddressBuilder
				.newBuilder().withPreferred(true).withVoided(false).build();
		...
		HashSet<PersonAddress> personAddresses = new HashSet<PersonAddress>(
				Arrays.asList(voidedAddress, preferredNotVoidedAddress));
	...
	/**
	 * @see Person#getPersonAddress()
	 * @verifies get voided person address if person is voided and not-voided
	...
	public void getPersonAddress_shouldGetVoidedPersonAddressIfPersonIsVoidedAndNotvoidedAddressDoesNotExist()
		...
		// addresses
		PersonAddress voidedAddress1 = PersonAddressBuilder.newBuilder()
				.withVoided(true).build();
		PersonAddress voidedAddress2 = PersonAddressBuilder.newBuilder()
				.withVoided(true).build();

		Set<PersonAddress> personAddresses = new HashSet<PersonAddress>(
				Arrays.asList(voidedAddress1, voidedAddress2));
	...
	public void getPersonName_shouldGetNotvoidedPersonNameIfPreferredAddressDoesNotExist()
		...
		PersonName notVoidedName = PersonNameBuilder.newBuilder()
				.withVoided(false).build();
		PersonName voidedName = PersonNameBuilder.newBuilder().withVoided(true)
				.build();

		PersonName expectedPersonName = notVoidedName;

		checkGetPersonNameResultForVoidedPerson(
				expectedPersonName,
				new HashSet<PersonName>(Arrays
						.asList(notVoidedName, voidedName)));
	}
	...
	public void getPersonName_shouldGetPreferredAndNotvoidedPersonNameIfExist()
		...
		PersonName preferredNotVoidedName = PersonNameBuilder.newBuilder()
				.withPreferred(true).withVoided(false).build();
		PersonName notVoidedName = PersonNameBuilder.newBuilder()
				.withVoided(false).build();
		PersonName voidedName = PersonNameBuilder.newBuilder().withVoided(true)
				.build();
		...
		checkGetPersonNameResultForVoidedPerson(
				expectedPersonName,
				new HashSet<PersonName>(Arrays.asList(preferredNotVoidedName,
						notVoidedName, voidedName)));
	}
	...
	public void getPersonName_shouldGetVoidedPersonAddressIfPersonIsVoidedAndNotvoidedAddressDoesNotExist()
		...
		checkGetPersonNameResultForVoidedPerson(expectedPersonName,
				new HashSet<PersonName>(Arrays.asList(voidedName)));
	...
	public void getPersonAddress_shouldReturnNullIfPersonIsNotvoidedAndHaveVoidedAddress()
		...
		PersonAddress firstPersonAddress = PersonAddressBuilder.newBuilder()
				.withVoided(true).build();
		PersonAddress secondPersonAddress = PersonAddressBuilder.newBuilder()
				.withVoided(true).build();
	...
	public void getPersonName_shouldReturnNullIfPersonIsNotvoidedAndHaveVoidedNames()
		...
		PersonName firstVoidedName = PersonNameBuilder.newBuilder()
				.withVoided(true).build();
		PersonName secondVoidedName = PersonNameBuilder.newBuilder()
				.withVoided(true).build();
	 ...
	 */
	@Test
	public void getBirthDateTime_shouldReturnBirthDateTimeAlongWithBirthdate()
			throws Exception {
		Person person = new Person();
		...
		Assert.assertEquals(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
				.parse("2012-01-01 11:11:11"), person.getBirthDateTime());
	}
	 ...
	 */
	@Test
	public void getBirthDateTime_shouldReturnNullIfBirthtimeIsNull()
			throws Exception {
		Person person = new Person();
	...
	private void checkGetPersonAddressResultForVoidedPerson(
			PersonAddress expectedPersonAddress,
	...
	private void checkGetPersonNameResultForVoidedPerson(
			PersonName expectedPersonAddress, Set<PersonName> personAddresses) {
