...
/**
 * This class tests all methods that are not getter or setters in the Order java
 * object TODO: finish this test class for Order
 * 
 * @see Order
 */
public class OrderTest extends BaseContextSensitiveTest {
	...
	protected static void assertThatAllFieldsAreCopied(Order original,
			String methodName, String... otherfieldsToExclude) throws Exception {
		if (methodName == null) {
			methodName = "copy";
		}
		List<String> fieldsToExclude = new ArrayList<String>();
		fieldsToExclude.addAll(Arrays.asList("log", "serialVersionUID",
				"orderId", "uuid"));
		...
		Order copy = (Order) MethodUtils.invokeExactMethod(original,
				methodName, null);
		for (Field field : fields) {
			...
			assertNotNull(
					"Order." + methodName + " should set " + field.getName()
							+ " on the new Order", copyValue);
			assertEquals(
					"Order." + methodName + " should set " + field.getName()
							+ " on the new Order", field.get(original),
					copyValue);
		}
	}

	/**
	 * Tests the {@link Order#isDiscontinuedRightNow()} method TODO this should
	 * be split into many different tests
	 * 
	 * @throws Exception
	 */
	@Test
	public void shouldIsDiscontinued() throws Exception {
		...
		Order o = new Order();
		assertFalse("order without dates shouldn't be discontinued",
				o.isDiscontinued(ymd.parse("2007-10-26")));

		o.setDateActivated(ymd.parse("2007-01-01"));
		assertFalse("shouldn't be discontinued before date activated",
				o.isDiscontinued(ymd.parse("2006-10-26")));
		...
		assertFalse("shouldn't be discontinued before date activated",
				o.isDiscontinued(ymd.parse("2006-10-26")));
		assertFalse("shouldn't be discontinued before autoExpireDate",
				o.isDiscontinued(ymd.parse("2007-10-26")));
		assertFalse("shouldn't be discontinued after autoExpireDate",
				o.isDiscontinued(ymd.parse("2008-10-26")));

		OrderUtilTest.setDateStopped(o, ymd.parse("2007-11-01"));
		assertFalse("shouldn't be discontinued before date activated",
				o.isDiscontinued(ymd.parse("2006-10-26")));
		assertFalse("shouldn't be discontinued before dateStopped",
				o.isDiscontinued(ymd.parse("2007-10-26")));
		assertTrue("should be discontinued after dateStopped",
				o.isDiscontinued(ymd.parse("2007-11-26")));

	}

	/**
	 * Tests the {@link Order#isActive()} method TODO this should be split into
	...
	public void shouldCheckIfOrderIsActive() throws Exception {
		...
		o.setDateActivated(ymd.parse("2007-01-01"));
		assertFalse("shouldn't be current before dateActivated",
				o.isActive(ymd.parse("2006-10-26")));
		assertTrue("should be current after dateActivated",
				o.isActive(ymd.parse("2007-10-26")));
		...
		assertFalse("shouldn't be current before dateActivated",
				o.isActive(ymd.parse("2006-10-26")));
		assertTrue(
				"should be current between dateActivated and autoExpireDate",
				o.isActive(ymd.parse("2007-10-26")));
		assertFalse("shouldn't be current after autoExpireDate",
				o.isActive(ymd.parse("2008-10-26")));

		OrderUtilTest.setDateStopped(o, ymd.parse("2007-11-01"));
		assertFalse("shouldn't be current before dateActivated",
				o.isActive(ymd.parse("2006-10-26")));
		assertTrue("should be current between dateActivated and dateStopped",
				o.isActive(ymd.parse("2007-10-26")));
		assertFalse("shouldn't be current after dateStopped",
				o.isActive(ymd.parse("2007-11-26")));
	...
	public void cloneForDiscontinuing_shouldSetAllTheRelevantFields()
		...
		assertNull("Discontinued order should not have orderGroup",
				orderThatCanDiscontinueTheOrder.getOrderGroup());
	...
	public void cloneForRevision_shouldSetAllTheRelevantFields()
		...
		assertThatAllFieldsAreCopied(revisedOrder, "cloneForRevision",
				"creator", "dateCreated", "action", "changedBy", "dateChanged",
				"voided", "dateVoided", "voidedBy", "voidReason", "encounter",
				"orderNumber", "orderer", "previousOrder", "dateActivated",
				"dateStopped", "accessionNumber");
	}
	...
	public void isFuture_shouldReturnFalseForAVoidedOrder() throws Exception {
		...
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		assertFalse(order.isStarted(DateUtils.parseDate("2014-11-01 11:11:09",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isFuture_shouldReturnFalseIfDateActivatedIsNull()
			throws Exception {
		Order order = new Order();
	 ...
	 */
	@Test
	public void isDiscontinued_shouldReturnFalseForAVoidedOrder()
			throws Exception {
		Order order = new Order();
		order.setVoided(true);
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:11", DATE_FORMAT));
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isDiscontinued(DateUtils.parseDate(
				"2014-11-01 11:11:12", DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isDiscontinued_shouldReturnFalseIfDateStoppedAndAutoExpireDateAreBothNull()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		...
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isDiscontinued(DateUtils.parseDate(
				"2014-11-01 11:11:11", DATE_FORMAT)));
	}

	/**
	 * @verifies return false if auto expire date is null and date stopped is
	 *           equal to check date
	 * @see Order#isDiscontinued(java.util.Date)
	 */
	@Test
	...
	public void isDiscontinued_shouldReturnFalseIfAutoExpireDateIsNullAndDateStoppedIsAfterCheckDate()
		...
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:12", DATE_FORMAT));
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isDiscontinued(DateUtils.parseDate(
				"2014-11-01 11:11:11", DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isDiscontinued_shouldReturnFalseIfDateActivatedIsAfterCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT));
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:13", DATE_FORMAT));
		assertFalse(order.isDiscontinued(DateUtils.parseDate(
				"2014-11-01 11:11:11", DATE_FORMAT)));
	}

	/**
	 * @verifies return true if auto expire date is null and date stopped is
	 *           before check date
	 * @see Order#isDiscontinued(java.util.Date)
	 */
	@Test
	public void isDiscontinued_shouldReturnTrueIfAutoExpireDateIsNullAndDateStoppedIsBeforeCheckDate()
		...
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:11", DATE_FORMAT));
		assertNull(order.getAutoExpireDate());
		assertTrue(order.isDiscontinued(DateUtils.parseDate(
				"2014-11-01 11:11:12", DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isDiscontinued_shouldFailIfDateStoppedIsAfterAutoExpireDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		order.setAutoExpireDate(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT));
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:12", DATE_FORMAT));
		expectedException.expect(APIException.class);
		expectedException.expectMessage(Context.getMessageSourceService()
				.getMessage("Order.error.invalidDateStoppedAndAutoExpireDate"));
		order.isDiscontinued(DateUtils.parseDate("2014-11-01 11:11:13",
				DATE_FORMAT));
	}

	/**
	 * @verifies return true if check date is after date stopped but before auto
	 *           expire date
	 * @see Order#isDiscontinued(java.util.Date)
	 */
	@Test
	public void isDiscontinued_shouldReturnTrueIfCheckDateIsAfterDateStoppedButBeforeAutoExpireDate()
		...
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:11", DATE_FORMAT));
		order.setAutoExpireDate(DateUtils.parseDate("2014-11-01 11:11:13",
				DATE_FORMAT));
		assertTrue(order.isDiscontinued(DateUtils.parseDate(
				"2014-11-01 11:11:12", DATE_FORMAT)));
	}
	...
	/**
	 * @verifies return true if check date is after both date stopped auto
	 *           expire date
	 * @see Order#isDiscontinued(java.util.Date)
	 */
	@Test
	public void isDiscontinued_shouldReturnTrueIfCheckDateIsAfterBothDateStoppedAutoExpireDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:11", DATE_FORMAT));
		order.setAutoExpireDate(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT));
		assertTrue(order.isDiscontinued(DateUtils.parseDate(
				"2014-11-01 11:11:13", DATE_FORMAT)));
	}

	/**
	 * @see Order#isDiscontinued(Date)
	 * @verifies return true if the order is scheduled for the future and
	 *           activated on check date but the check date is after date
	 *           stopped
	 */
	@Test
	public void isDiscontinued_shouldReturnTrueIfTheOrderIsScheduledForTheFutureAndActivatedOnCheckDateButTheCheckDateIsAfterDateStopped()
		...
		// the stopped date of the original order is set to a moment before the
		// activated date of the revised order.
	...
	public void isExpired_shouldReturnFalseForAVoidedOrder() throws Exception {
		...
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		order.setAutoExpireDate(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		assertNull(order.getDateStopped());
		assertFalse(order.isExpired(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isExpired_shouldReturnFalseIfDateStoppedAndAutoExpireDateAreBothNull()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		assertNull(order.getDateStopped());
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isExpired(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT)));
	}

	/**
	 * @verifies return false if date stopped is null and auto expire date is
	 *           equal to check date
	 * @see Order#isExpired(java.util.Date)
	 */
	@Test
	 ...
	 */
	@Test
	public void isExpired_shouldReturnFalseIfDateStoppedIsNullAndAutoExpireDateIsAfterCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
	...
	/**
	 * @verifies return false if check date is after both date stopped auto
	 *           expire date
	 * @see Order#isExpired(java.util.Date)
	 */
	@Test
	public void isExpired_shouldReturnFalseIfCheckDateIsAfterBothDateStoppedAutoExpireDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		...
		order.setAutoExpireDate(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT));
		assertFalse(order.isExpired(DateUtils.parseDate("2014-11-01 11:11:13",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isExpired_shouldReturnFalseIfDateActivatedIsAfterCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT));
	...
	/**
	 * @verifies return false if check date is after date stopped but before
	 *           auto expire date
	 * @see Order#isExpired(java.util.Date)
	 */
	@Test
	public void isExpired_shouldReturnFalseIfCheckDateIsAfterDateStoppedButBeforeAutoExpireDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
	 ...
	 */
	@Test
	public void isExpired_shouldFailIfDateStoppedIsAfterAutoExpireDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
	...
	/**
	 * @verifies return true if date stopped is null and auto expire date is
	 *           before check date
	 * @see Order#isExpired(java.util.Date)
	 */
	@Test
	public void isExpired_shouldReturnTrueIfDateStoppedIsNullAndAutoExpireDateIsBeforeCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		order.setAutoExpireDate(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT));
		assertNull(order.getDateStopped());
		assertTrue(order.isExpired(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isActivated_shouldReturnTrueIfAnOrderWasActivatedOnTheCheckDate()
			throws Exception {
		Order order = new Order();
		Date activationDate = DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT);
		order.setDateActivated(activationDate);
		assertNull(order.getDateStopped());
		assertNull(order.getAutoExpireDate());
	 ...
	 */
	@Test
	public void isActivated_shouldReturnTrueIfAnOrderWasActivatedBeforeTheCheckDate()
			throws Exception {
		Order order = new Order();
		Date activationDate = DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT);
	 ...
	 */
	@Test
	public void isActivated_shouldReturnFalseIfDateActivatedIsNull()
			throws Exception {
		Order order = new Order();
		assertNull(order.getDateActivated());
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isActivated(DateUtils.parseDate(
				"2014-11-01 11:11:10", DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isActivated_shouldReturnFalseForAnOrderActivatedAfterTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		assertNull(order.getDateStopped());
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isActivated(DateUtils.addMonths(
				order.getDateActivated(), -2)));
	}
	 ...
	 */
	@Test
	public void isActive_shouldReturnTrueIfAnOrderExpiredOnTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
	 ...
	 */
	@Test
	public void isActive_shouldReturnTrueIfAnOrderWasActivatedOnTheCheckDate()
			throws Exception {
		Order order = new Order();
		Date activationDate = DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT);
	 ...
	 * @verifies return true if an order was activated on the check date but
	 *           scheduled for the future
	 */
	@Test
	public void isActive_shouldReturnTrueIfAnOrderWasActivatedOnTheCheckDateButScheduledForTheFuture()
			throws Exception {
		Order order = new Order();
		order.setUrgency(Urgency.ON_SCHEDULED_DATE);
	 ...
	 */
	@Test
	public void isActive_shouldReturnTrueIfAnOrderWasDiscontinuedOnTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		Date dateStopped = DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT);
		OrderUtilTest.setDateStopped(order, dateStopped);
	 ...
	 */
	@Test
	public void isActive_shouldReturnFalseForADiscontinuedOrder()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		OrderUtilTest.setDateStopped(order,
				DateUtils.parseDate("2014-11-01 11:11:11", DATE_FORMAT));
		assertFalse(order.isActive(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isActive_shouldReturnFalseForAnOrderActivatedAfterTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		assertNull(order.getDateStopped());
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isActive(DateUtils.parseDate("2014-11-01 11:11:09",
				DATE_FORMAT)));
	}
	...
	public void isActive_shouldReturnFalseForAVoidedOrder() throws Exception {
		...
		assertNull(order.getDateStopped());
		assertNull(order.getAutoExpireDate());
		assertFalse(order.isActive(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isActive_shouldReturnFalseForADiscontinuationOrder()
			throws Exception {
		Order order = new Order();
		Date activationDate = DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT);
	...
	public void isStarted_shouldReturnFalseForAVoidedOrder() throws Exception {
		...
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		assertFalse(order.isStarted(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isStarted_shouldReturnFalseIfDateActivatedIsNull()
			throws Exception {
		Order order = new Order();
		assertNull(order.getDateActivated());
		assertFalse(order.isStarted(DateUtils.parseDate("2014-11-01 11:11:11",
	...
	/**
	 * @verifies return false if the order is not yet activated as of the check
	 *           date
	 * @see Order#isStarted(java.util.Date)
	 */
	@Test
	public void isStarted_shouldReturnFalseIfTheOrderIsNotYetActivatedAsOfTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
	...
	/**
	 * @verifies return false if the order was scheduled to start after the
	 *           check date
	 * @see Order#isStarted(java.util.Date)
	 */
	@Test
	public void isStarted_shouldReturnFalseIfTheOrderWasScheduledToStartAfterTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		order.setUrgency(Order.Urgency.ON_SCHEDULED_DATE);
		order.setScheduledDate(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT));
		assertFalse(order.isStarted(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT)));
	}
	...
	/**
	 * @verifies return true if the order was scheduled to start on the check
	 *           date
	 * @see Order#isStarted(java.util.Date)
	 */
	@Test
	public void isStarted_shouldReturnTrueIfTheOrderWasScheduledToStartOnTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		order.setUrgency(Order.Urgency.ON_SCHEDULED_DATE);
		order.setScheduledDate(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT));
		assertTrue(order.isStarted(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT)));
	}

	/**
	 * @verifies return true if the order was scheduled to start before the
	 *           check date
	 * @see Order#isStarted(java.util.Date)
	 */
	@Test
	public void isStarted_shouldReturnTrueIfTheOrderWasScheduledToStartBeforeTheCheckDate()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
		...
		order.setScheduledDate(DateUtils.parseDate("2014-11-01 11:11:11",
				DATE_FORMAT));
		assertTrue(order.isStarted(DateUtils.parseDate("2014-11-01 11:11:12",
				DATE_FORMAT)));
	}
	 ...
	 */
	@Test
	public void isStarted_shouldReturnTrueIfTheOrderIsStartedAndNotScheduled()
			throws Exception {
		Order order = new Order();
		order.setDateActivated(DateUtils.parseDate("2014-11-01 11:11:10",
				DATE_FORMAT));
