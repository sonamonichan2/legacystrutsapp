...
public class TestUtil {
	...
	/**
	 * Additional assert method for testing that will test that two Collections
	 * have equal contents The Collections must be of equal size, and each
	 * object from one Collection must equal an object in the other Collection
	...
	public static void assertCollectionContentsEquals(Collection expected,
			Collection actual) throws AssertionError {
		try {
	 ...
	 * 
	 * @param webappName
	 *            name to use when looking up the runtime properties env var or
	 *            filename
	...
	/**
	 * Convert the given xml output to a string that is assignable to a single
	 * String variable
	 * 
	 * @param output
	 *            multi line string to convert
	 */
	...
	/**
	 * Convert the given multi-line output to lines of StringBuilder.append
	 * lines <br>
	 * <br>
	...
	public static void printStringBuilderOutput(String output) {
		...
		System.out
				.println("StringBuilder correctOutput = new StringBuilder();");
	 ...
	 * @param tableNames
	 *            the name(s) of the table(s) to print out
	 * @throws Exception
	 */
	public static void printOutTableContents(Connection sqlConnection,
	...
	/**
	 * Utility method that allows tests to easily configure and save a global
	 * property
	 * 
	 * @param string
	...
	/**
	 * Utility method to check if a list contains a BaseOpenmrsObject using the
	 * id
	 * 
	 * @param list
	 * @param id
	...
	public static boolean containsId(
			Collection<? extends BaseOpenmrsObject> list, Integer id) {
	 ...
	 * Since the OpenMRS database is only precise to the second, if you want to
	 * test something "later" you need to wait this long. (Also, useful because
	 * the granularity of the clock on some systems is low, so doing a
	...
	/**
	 * Test utility method to create date time using standard 'yyyy-MM-dd
	 * hh:mm:ss' format
	 * 
	 * @param dateTimeString
	 *            in 'yyyy-MM-dd hh:mm:ss' format
	 */
	public static Date createDateTime(String dateTimeString)
			throws ParseException {
		return DateUtils.parseDate(dateTimeString, "yyyy-MM-dd hh:mm:ss.SSS",
