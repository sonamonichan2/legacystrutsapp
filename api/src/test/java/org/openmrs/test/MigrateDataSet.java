...
/**
 * This is a runnable java class that migrates/converts the current dbunit xml
 * files to a new schema structure. After running this class, you are able to
 * pick either a folder to convert. Xmls files are recursively found and
 * converted. The {@link BaseContextSensitiveTest#INITIAL_DATA_SET_XML_FILENAME}
 * file is overwritten by values in the database defined by the runtime
 * properties
 */
public class MigrateDataSet {
	...
	public static void main(String args[]) throws Exception {
		...
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			for (File directory : dirs) {
				System.out.println("Doing migration on directory: "
	...
	private static void doMigration(File fileOrDirectory) throws Exception {
		...
		if (filename.startsWith(".svn")
				|| filename.endsWith("TestingApplicationContext.xml")) {
			// skip .svn files
			...
			InputStream fileOrDirectoryStream = new FileInputStream(
					fileOrDirectory);

			System.out
					.println("Migrating " + fileOrDirectory.getAbsolutePath());

			System.out.println(execMysqlCmd("DROP DATABASE IF EXISTS "
					+ tempDatabaseName, null, false));
			System.out.println(execMysqlCmd("CREATE DATABASE "
					+ tempDatabaseName + " DEFAULT CHARACTER SET utf8", null,
					false));
			...
			Connection con = DriverManager.getConnection(url, credentials[0],
					credentials[1]);
			...
			try {
				...
				System.out.println(execMysqlCmd("SET FOREIGN_KEY_CHECKS=0",
						NEW_UPDATE_FILE, true));
				...
				System.err.println("Unable to convert: " + filename
						+ " Error: " + e.getMessage());
			} finally {
	...
	/**
	 * Execute the given sourceFile and the given command against the temporary
	 * mysql database.
	 * 
	 * @param sourceFile
	 *            (nullable)
	 ...
	 * @throws Exception
	 */
	private static String execMysqlCmd(String cmd, String sourceFile,
			boolean includeDB) throws Exception {
		...
		if (sourceFile != null) {
			shellCommand = shellCommand + "cat " + (cmd != null ? "-" : "")
					+ " " + sourceFile + " | ";
		}

		shellCommand = shellCommand + "mysql -u" + credentials[0] + " -p"
				+ credentials[1];
		...
		try {
			...
			Process p = (wd != null) ? Runtime.getRuntime()
					.exec(cmds, null, wd) : Runtime.getRuntime().exec(cmds);
