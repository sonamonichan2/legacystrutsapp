...
public class StartModuleExecutionListener extends AbstractTestExecutionListener {
	...
	public void prepareTestInstance(TestContext testContext) throws Exception {
		StartModule startModuleAnnotation = testContext.getTestClass()
				.getAnnotation(StartModule.class);

		// if the developer listed some modules with the @StartModule annotation
		// on the class
		if (startModuleAnnotation != null) {

			if (!lastClassRun
					.equals(testContext.getTestClass().getSimpleName())) {
				// mark this with our class so that the services are only
				// restarted once
				...
				String modulesToLoad = StringUtils.join(
						startModuleAnnotation.value(), " ");

				Properties props = BaseContextSensitiveTest.runtimeProperties;
				props.setProperty(
						ModuleConstants.RUNTIMEPROPERTY_MODULE_LIST_TO_LOAD,
						modulesToLoad);
				try {
				...
				Assert.assertTrue(
						...
						startModuleAnnotation.value().length <= ModuleFactory
								.getStartedModules().size());

				/*
				 * Refresh spring so the Services are recreated (aka serializer
				 * gets put into the SerializationService) To do this, wrap the
				 * applicationContext from the testContext into a
				 * GenericApplicationContext, allowing loading beans from
				 * moduleApplicationContext into it and then calling
				 * ctx.refresh() This approach ensures that the application
				...
				while (list.hasMoreElements()) {
					xmlReader.loadBeanDefinitions(new UrlResource(list
							.nextElement()));
				}

				// ensure that when refreshing, we use the openmrs class loader
				// for the started modules.
