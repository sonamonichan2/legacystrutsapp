 ...
 * won't run it because it does not have "Test" in its class name. The
 * {@link BaseContextSensitiveTest#INITIAL_DATA_SET_XML_FILENAME} file is
...
public class CreateInitialDataSet extends BaseContextSensitiveTest {
	...
	/**
	 * This test creates an xml dbunit file from the current database connection
	 * information found in the runtime properties. This method has to
	 * "skip over the base setup" because it tries to do things (like initialize
	 * the database) that shouldn't be done to a standard mysql database.
	 * 
	 * @throws Exception
	 */
	@Test
	...
	public void shouldCreateInitialTestDataSetXmlFile() throws Exception {
		...
		initialDataSet.addTable("encounter_type",
		...
		initialDataSet.addTable("patient_identifier",
				"SELECT * FROM patient_identifier");
		initialDataSet.addTable("patient_identifier_type",
				"SELECT * FROM patient_identifier_type");
		initialDataSet.addTable("patient_program",
		...
		initialDataSet.addTable("person_address",
				"SELECT * FROM person_address");
		initialDataSet.addTable("person_attribute",
				"SELECT * FROM person_attribute");
		initialDataSet.addTable("person_attribute_type",
		...
		initialDataSet.addTable("program_workflow",
				"SELECT * FROM program_workflow");
		initialDataSet.addTable("program_workflow_state",
				"SELECT * FROM program_workflow_state");
		initialDataSet.addTable("relationship", "SELECT * FROM relationship");
		initialDataSet.addTable("relationship_type",
				"SELECT * FROM relationship_type");
		initialDataSet.addTable("role", "SELECT * FROM role");
		initialDataSet.addTable("role_privilege",
		 ...
		 * initialDataSet.addTable("field_answer",
		 * "SELECT * FROM field_answer"); initialDataSet.addTable("field_type",
		 * "SELECT * FROM field_type"); initialDataSet.addTable("form",
		 * "SELECT * FROM form"); initialDataSet.addTable("form_field",
		 * "SELECT * FROM form_field"); initialDataSet.addTable("hl7_source",
		 * "SELECT * FROM hl7_source");
		 */

		FlatXmlDataSet.write(initialDataSet, new FileOutputStream(
				"test/api/org/openmrs/logic/include/LogicBasicTest.xml"));
	...
	/**
	 * Make sure we use the database defined by the runtime properties and not
	 * the hsql in-memory database
	 * 
