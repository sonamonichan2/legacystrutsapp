...
public class ConceptValidatorTest extends BaseContextSensitiveTest {
	...
	/**
	 * @verifies {@link ConceptValidator#validate(Object,Errors)} test = should
	 *           fail if there is a duplicate unretired concept name in the
	 *           locale
	 */
	...
	public void validate_shouldFailIfThereIsADuplicateUnretiredConceptNameInTheLocale()
		...
		String duplicateName = concept.getFullySpecifiedName(
				Context.getLocale()).getName();

		ConceptName newName = new ConceptName(duplicateName,
				Context.getLocale());
	...
	/**
	 * Tests for duplicate names for the same concept and not with names for
	 * other concepts
	 * 
	 * @see ConceptValidator#validate(Object,Errors)
	 */
	...
	public void validate_shouldFailIfThereIsADuplicateUnretiredFullySpecifiedNameInTheSameLocale()
		...
		Assert.assertEquals(true,
				concept.getFullySpecifiedName(Context.getLocale())
						.isFullySpecifiedName());
		String duplicateName = concept.getFullySpecifiedName(
				Context.getLocale()).getName();

		Concept anotherConcept = Context.getConceptService().getConcept(5089);
		anotherConcept.getFullySpecifiedName(Context.getLocale()).setName(
				duplicateName);
	...
	public void validate_shouldFailIfThereIsADuplicateUnretiredPreferredNameInTheSameLocale()
		...
		Context.setLocale(new Locale("en", "GB"));
		Concept concept = Context.getConceptService().getConcept(5497);
		ConceptName preferredName = new ConceptName("preferred name",
				Context.getLocale());
		concept.setPreferredName(preferredName);
		Context.getConceptService().saveConcept(concept);
		Assert.assertEquals("preferred name",
	...
	public void validate_shouldPassIfTheDuplicateConceptNameIsNeitherPreferredNorFullySpecified()
		...
		ConceptName duplicateName = concept.getSynonyms(Context.getLocale())
				.iterator().next();
	...
	/**
	 * @see ConceptValidator#validate(Object,Errors)
	 * @verifies pass if the concept being validated is retired and has a
	 *           duplicate name
	 */
	@Test
	...
	public void validate_shouldPassIfTheConceptHasASynonymThatIsAlsoAShortName()
		...
		concept.setConceptClass(new ConceptClass());
		concept.setDatatype(new ConceptDatatype());
		// Add the short name. Because the short name is not counted as a
		// Synonym.
	...
	public void validate_shouldFailIfATermIsMappedMultipleTimesToTheSameConcept()
		...
		concept.addDescription(new ConceptDescription("some description", null));
		ConceptMap map1 = new ConceptMap(cs.getConceptReferenceTerm(1),
				cs.getConceptMapType(1));
		concept.addConceptMapping(map1);
		ConceptMap map2 = new ConceptMap(cs.getConceptReferenceTerm(1),
				cs.getConceptMapType(1));
	...
	public void validate_shouldPassIfTheDuplicateNameInTheLocaleForTheConceptBeingValidatedIsVoided()
		...
		// sanity check since names should only be unique amongst preferred and
		// fully specified names
		Assert.assertTrue(otherName.isFullySpecifiedName()
				|| otherName.isPreferred());
		...
		Assert.assertEquals(duplicateName,
				concept.getPreferredName(duplicateName.getLocale()));
	...
	/**
	 * @see ConceptValidator#validate(Object,Errors)
	 * @verifies fail if there is a duplicate unretired concept name in the same
	 *           locale different than the system locale
	 */
	@Test(expected = DuplicateConceptNameException.class)
	public void validate_shouldFailIfThereIsADuplicateUnretiredConceptNameInTheSameLocaleDifferentThanTheSystemLocale()
		...
		Concept concept = Context.getConceptService().getConcept(5497);
		Assert.assertEquals(true, concept.getFullySpecifiedName(en)
				.isFullySpecifiedName());
	...
	public void validate_shouldNotFailIfATermHasTwoNewMappingsOnIt()
		...
		ConceptService cs = Context.getConceptService();
		concept.addName(new ConceptName("my name", Context.getLocale()));
		ConceptReferenceTerm newTerm = new ConceptReferenceTerm(
				cs.getConceptSource(1), "1234", "term one two three four");
		ConceptMap map1 = new ConceptMap(newTerm, cs.getConceptMapType(1));
		concept.addConceptMapping(map1);
		ConceptReferenceTerm newTermTwo = new ConceptReferenceTerm(
				cs.getConceptSource(1), "12345", "term one two three four five");
	...
	public void validate_shouldPassIfFullySpecifiedNameIsTheSameAsShortName()
		...
		ConceptName conceptFullySpecifiedName = new ConceptName("YES",
				new Locale("pl"));
	...
	public void validate_shouldPassIfDifferentConceptsHaveTheSameShortNames()
		...
		Assert.assertEquals(true,
				concepts.get(0).getShortNameInLocale(Context.getLocale())
						.getName().equalsIgnoreCase("HSM"));

		Concept concept = new Concept();
		concept.setConceptClass(new ConceptClass());
		concept.setDatatype(new ConceptDatatype());
		ConceptName conceptFullySpecifiedName = new ConceptName(
				"holosystolic murmur", Context.getLocale());
		conceptFullySpecifiedName
				.setConceptNameType(ConceptNameType.FULLY_SPECIFIED);

		ConceptName conceptShortName = new ConceptName("HSM",
				Context.getLocale());
		conceptShortName.setConceptNameType(ConceptNameType.SHORT);
	...
	public void validate_shouldFailIfTheConceptDatatypeIsNull()
		...
		Errors errors = new BindException(concept, "concept");
		new ConceptValidator().validate(concept, errors);
		Assert.assertEquals(errors.getFieldError("datatype").getCode(),
				"Concept.datatype.empty");
	}
	...
	public void validate_shouldFailIfTheConceptClassIsNull() throws Exception {
		...
		Errors errors = new BindException(concept, "concept");
		new ConceptValidator().validate(concept, errors);
		Assert.assertEquals(errors.getFieldError("conceptClass").getCode(),
				"Concept.conceptClass.empty");
	}
}
