...
public class VisitValidatorTest extends BaseContextSensitiveTest {
	...
	public void before() throws Exception {
		...
		// The only reason for adding the four lines below is because without
		// them,
		// some tests fail on my macbook.
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy/MM/dd");
		DATE_TIME_2014_01_04_00_00_00_0 = formatter.parse("2014/01/04")
				.getTime();
		DATE_TIME_2014_02_05_00_00_00_0 = formatter.parse("2014/02/05")
				.getTime();
		DATE_TIME_2014_02_11_00_00_00_0 = formatter.parse("2014/02/11")
				.getTime();

		// Do not allow overlapping visits to test full validation of visit
	 ...
	 * @verifies accept a visit that has the right number of attribute
	 *           occurrences
	 */
	@Test
	 ...
	 */
	@Test
	// This test will throw org.hibernate.PropertyValueException: not-null
	// property references a null or transient value:
	// org.openmrs.VisitAttribute.valueReference
	// This is a general problem, i.e. that validators on Customizable can't
	// really be called unless you set Hibernate's flushMode to MANUAL.
	...
	public void validate_shouldFailIfAnAttributeIsBad() throws Exception {
		...
		Collection<VisitAttribute> activeAttributes = visit
				.getActiveAttributes();
		Errors errors = new BindException(visit, "visit");
		new VisitValidator().validate(visit, errors);
	...
	/**
	 * @verifies reject a visit if startDateTime is equal to startDateTime of
	 *           another visit of the same patient
	 * @see VisitValidator#validate(Object,
	...
	/**
	 * @verifies reject a visit if startDateTime falls into another visit of the
	 *           same patient
	 * @see VisitValidator#validate(Object,
	 *      org.springframework.validation.Errors)
	 */
	...
	/**
	 * @verifies reject a visit if stopDateTime falls into another visit of the
	 *           same patient
	 * @see VisitValidator#validate(Object,
	 *      org.springframework.validation.Errors)
	 */
	...
	/**
	 * @verifies accept a visit if startDateTime is equal to startDateTime of
	...
	public void validate_shouldAcceptAVisitIfStartDateTimeIsEqualToStartDateTimeOfAnotherVoidedVisitOfTheSamePatient()
		...
		assertTrue(patientHasVoidedVisit(visit.getPatient(),
				DATE_TIME_2014_02_05_00_00_00_0,
	...
	/**
	 * @verifies accept a visit if startDateTime falls into another voided visit
	 *           of the same patient
	 * @see VisitValidator#validate(Object,
	 *      org.springframework.validation.Errors)
	 */
	...
	/**
	 * @verifies accept a visit if stopDateTime falls into another voided visit
	 *           of the same patient
	 * @see VisitValidator#validate(Object,
	 *      org.springframework.validation.Errors)
	 */
	...
	/**
	 * @verifies accept a visit if it contains another voided visit of the same
	 *           patient
	 * @see VisitValidator#validate(Object,
	 *      org.springframework.validation.Errors)
	 */
	...
	private boolean patientHasVoidedVisit(Patient patient, long startInMillis,
			long stopInMillis) {

		// To get voided visit from the past, both inactive AND voided visits
		// are queried.
		//
		...
		for (Visit visit : visitList) {
			if (visit.getStartDatetime() != null
					&& visit.getStartDatetime().getTime() == startInMillis
					&& visit.getStopDatetime() != null
					&& visit.getStopDatetime().getTime() == stopInMillis
					&& visit.isVoided()) {
				return true;
			}
		}
