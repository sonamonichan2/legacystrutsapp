...
public class DrugValidatorTest extends BaseContextSensitiveTest {
	 ...
	 * @see DrugValidator#validate(Object,
	...
	public void validate_shouldFailIfTheDrugObjectIsNull() throws Exception {
		...
		expectedException
				.expectMessage("The parameter obj should not be null and must be of type"
						+ Drug.class);
		new DrugValidator().validate(null,
				new BindException(new Drug(), "drug"));
	}
	...
	public void validate_shouldFailIfConceptReferenceTermOnDrugReferenceMapIsNull()
		...
		Assert.assertTrue(errors
				.hasFieldErrors("drugReferenceMaps[0].conceptReferenceTerm"));
	}

	/**
	 * @verifies invoke ConceptReferenceTermValidator if term on
	...
	public void validate_shouldInvokeConceptReferenceTermValidatorIfTermOnDrugReferenceMapIsNew()
		...
		Drug drug = new Drug();
		drug.addDrugReferenceMap(new DrugReferenceMap(
				new ConceptReferenceTerm(), null));
		Errors errors = new BindException(drug, "drug");
		new DrugValidator().validate(drug, errors);
		// reference term validator should have been called which should reject
		// a null code
		Assert.assertTrue(errors
				.hasFieldErrors("drugReferenceMaps[0].conceptReferenceTerm.code"));
	}

	/**
	 * @verifies invoke ConceptMapTypeValidator if conceptMapType on
	 *           drugReferenceMap is new
	 * @see DrugValidator#validate(Object,
	 *      org.springframework.validation.Errors)
	 */
	...
	public void validate_shouldInvokeConceptMapTypeValidatorIfConceptMapTypeOnDrugReferenceMapIsNew()
			throws Exception {
		Drug drug = new Drug();
		drug.addDrugReferenceMap(new DrugReferenceMap(conceptService
				.getConceptReferenceTerm(1), new ConceptMapType()));
		Errors errors = new BindException(drug, "drug");
		new DrugValidator().validate(drug, errors);
		// concept map type validator should have been called which should
		// reject a null name
		Assert.assertTrue(errors
				.hasFieldErrors("drugReferenceMaps[0].conceptMapType.name"));
	}
	...
	public void validate_shouldRejectDrugMultipleMappingsToTheSameTerm()
			throws Exception {
		Drug drug = new Drug();
		DrugReferenceMap term1 = new DrugReferenceMap(
				conceptService.getConceptReferenceTerm(1),
				conceptService.getConceptMapType(1));
		DrugReferenceMap term2 = new DrugReferenceMap(
				conceptService.getConceptReferenceTerm(1),
		...
		new DrugValidator().validate(drug, errors);
		Assert.assertTrue(errors
				.hasFieldErrors("drugReferenceMaps[1].conceptReferenceTerm"));
	}
	...
	public void validate_shouldPassIfAllFieldsAreCorrect() throws Exception {
		Drug drug = new Drug();
		drug.addDrugReferenceMap(new DrugReferenceMap(conceptService
				.getConceptReferenceTerm(1), conceptService
				.getConceptMapType(1)));
		Errors errors = new BindException(drug, "drug");
		new DrugValidator().validate(drug, errors);
