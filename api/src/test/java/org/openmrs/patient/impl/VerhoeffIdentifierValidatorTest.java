...
public class VerhoeffIdentifierValidatorTest {
	...
	private String[] allowedIdentifiers = {"12345678", "87654321", "11111111",
			"64537218", "00000000"};
	...
	private String[] invalidIdentifiers = {"", " ", "-", "adsfalasdf-adfasdf",
			"ABC DEF", "!234*", "++", " ABC", "def ", "ab32kcdak3",
			"chaseisreallycoolyay", "1", "moose", "MOOSE", "MooSE",
			"adD3Eddf429daD999"};
	...
	public void getValidIdentifier_shouldGetValidIdentifier() throws Exception {
		...
		for (int j = 0; j < invalidIdentifiers.length; j++) {
			try {
				...
				fail("Identifier " + invalidIdentifiers[j]
						+ " should have failed.");
			} catch (Exception e) {
			}
		}
	...
	public void shouldIsValid() {
		...
		for (int i = 0; i < allowedIdentifiers.length; i++) {
			try {
				...
				fail("Identifier " + allowedIdentifiers[i]
						+ " should have failed.");
			} catch (Exception e) {
			}
			try {
		...
		for (int j = 0; j < invalidIdentifiers.length; j++) {
			try {
				validator.isValid(allowedIdentifiers[j] + "-"
						+ allowedIdentifiersCheckDigitsInt[j]);
				fail("Identifier " + allowedIdentifiers[j]
						+ " should have failed.");
			} catch (Exception e) {
			}
		}
		...
		for (int i = 0; i < allowedIdentifiers.length; i++) {
			assertFalse(validator.isValid(allowedIdentifiers[i] + "-"
					+ unusedCheckDigit));
		}

		// Now test allowed identifiers that have the right check digit. Test
		// with both
		// chars and ints.
		for (int i = 0; i < allowedIdentifiers.length; i++) {
			assertTrue(validator.isValid(allowedIdentifiers[i] + "-"
					+ allowedIdentifiersCheckDigits[i]));
		}
