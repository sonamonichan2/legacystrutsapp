...
public class MessageSourceServiceTest extends BaseContextSensitiveTest {
	...
	/**
	 * MessageSourceServiceImpl.getMessage()should return last error code if no
	 * localization found
	 * 
	 * @see MessageSourceServiceImpl#getMessage(MessageSourceResolvable
	...
	public void getMessage_shouldReturnTheLastErrorCodeIfnoLocalizationIsFound()
		...
		Assert.assertEquals("myErrorCode.request.myField",
		...
		Assert.assertEquals("myErrorCode", Context.getMessageSourceService()
				.getMessage(fieldError, Context.getLocale()));
	}
}
