...
public class CachedMessageSourceTest {
	...
	/**
	 * The PresentationMessageCollection should be able to contain messages in
	 * different locales.
	 * 
	...
	public void getLocales_shouldShouldBeAbleToContainMultipleLocales()
		...
		testPmc.addPresentation(MockPresentationMessage
				.createMockPresentationMessage("en"));
		testPmc.addPresentation(MockPresentationMessage
				.createMockPresentationMessage("fr"));
		testPmc.addPresentation(MockPresentationMessage
				.createMockPresentationMessage("pt"));

		assertEquals(3, testPmc.getLocales().size());
	}

	/**
	 * The PresentationMessageCollection should return messages that are the
	 * same whether returned as Strings or when as part of full
	 * PresentationMessage object.
	 * 
	...
	public void getPresentation_shouldMatchGetMessageWithPresentationMessage()
		...
		MockPresentationMessage mockPM = MockPresentationMessage
				.createMockPresentationMessage();
		testPmc.addPresentation(mockPM);

		String valueAsString = testPmc.getMessage(mockPM.getCode(), null,
				mockPM.getLocale());
		PresentationMessage valueAsPM = testPmc.getPresentation(
				mockPM.getCode(), mockPM.getLocale());
