...
public class PresentationMessageMapTest {
	...
	/**
	 * PresentationMessageMap should not add PresentationMessages which are not
	 * from the same locale set for the PresentationMessageMap.
	 * 
	...
	public void put_shouldShouldIgnoreNonMatchingLocaleMessages()
			throws Exception {
		PresentationMessageMap testPmm = new PresentationMessageMap(
				Locale.ENGLISH);
		testPmm.put("right_locale",
				MockPresentationMessage.createMockPresentationMessage("en"));
		testPmm.put("wrong_locale", MockPresentationMessage
				.createMockPresentationMessage(Locale.GERMAN));

		assertEquals(1, testPmm.size());
	}

	/**
	 * PresentationMessageMap should only add PresentationMessages which are
	 * from the same locale, even when adding from a batch.
	 * 
	...
	public void putAll_shouldFilterOutNonMatchingLocaleMessagesFromBatchAdd()
		...
		mockMessageMap.put("right_locale",
				MockPresentationMessage.createMockPresentationMessage("en"));
		mockMessageMap.put("wrong_locale", MockPresentationMessage
				.createMockPresentationMessage(Locale.GERMAN));
