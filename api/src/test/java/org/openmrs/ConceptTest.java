...
public class ConceptTest {
	...
	@Before
	public void setup() {
		nameGenerator = GeneratorFactory.getUniqueRegexStringGenerator(
				NAME_PATTERN, 2, 12, Locale.ENGLISH);
	}

	/**
	 * When asked for a collection of compatible names, the returned collection
	 * should not include any incompatible names.
	 * 
	...
	public void getCompatibleNames_shouldExcludeIncompatibleCountryLocales()
		...
		ConceptName name_en_UK = createMockConceptName(
				initialNameCollectionSize + 1, Locale.UK,
				ConceptNameType.FULLY_SPECIFIED, false);
		testConcept.addName(name_en_UK);

		Collection<ConceptName> compatibleNames = testConcept
				.getCompatibleNames(primaryLocale);
	...
	public void getCompatibleNames_shouldExcludeIncompatibleLanguageLocales()
		...
		Assert.assertEquals(0, concept.getCompatibleNames(new Locale("en"))
				.size());
	}

	/**
	 * The Concept should unmark the old conceptName as the locale preferred one
	 * to enforce the rule that a each locale should have only one preferred
	 * name per concept
	 * 
	...
	public void setPreferredName_shouldOnlyAllowOnePreferredName()
		...
		ConceptName initialPreferred = createMockConceptName(3, primaryLocale,
				null, true);
		testConcept.addName(initialPreferred);
		Assert.assertEquals(true, initialPreferred.isLocalePreferred());
		ConceptName newPreferredName = createMockConceptName(4, primaryLocale,
				null, false);
	...
	public void getDescription_shouldNotReturnLanguageOnlyMatchForExactMatches()
		...
		mockConcept.addDescription(new ConceptDescription("en desc",
				new Locale("en")));

		Assert.assertNull(mockConcept.getDescription(new Locale("en", "US"),
				true));
	}
	...
	public void getDescription_shouldNotReturnMatchOnLanguageOnlyIfExactMatchExists()
		...
		mockConcept.addDescription(new ConceptDescription("en desc",
				new Locale("en")));
		mockConcept.addDescription(new ConceptDescription("en_US desc",
				new Locale("en", "US")));

		Concept mockConcept2 = new Concept();
		mockConcept2.addDescription(new ConceptDescription("en_US desc",
				new Locale("en", "US")));
		mockConcept2.addDescription(new ConceptDescription("en desc",
				new Locale("en")));

		Assert.assertEquals("en_US desc",
				mockConcept.getDescription(new Locale("en", "US"), false)
						.getDescription());
		Assert.assertEquals("en_US desc",
				mockConcept2.getDescription(new Locale("en", "US"), false)
						.getDescription());
	}
	...
	public void getName_shouldReturnExactNameLocaleMatchGivenExactEqualsTrue()
		...
		Concept concept = new Concept();
		ConceptName fullySpecifiedName = new ConceptName("some name",
				definedNameLocale);
		...
		Assert.assertEquals("some name", concept.getName(localeToSearch, true)
				.getName());
	}
	...
	public void getName_shouldReturnNameWithinSameLanguageIfExactEqualsFalse()
		...
		Assert.assertEquals("Test Concept",
	...
	public void setPreferredName_shouldAddTheNameToTheListOfNamesIfItNotAmongThemBefore()
		...
		Locale primaryLocale = Locale.US;
		Concept testConcept = createMockConcept(1, primaryLocale);
		ConceptName newPreferredName = createMockConceptName(3, primaryLocale,
				null, false);
		assertEquals(false,
				testConcept.getNames(primaryLocale).contains(newPreferredName));
		testConcept.setPreferredName(newPreferredName);
		assertEquals(true,
				testConcept.getNames(primaryLocale).contains(newPreferredName));
	}
	...
	public void getFullySpecifiedName_shouldReturnTheNameMarkedAsFullySpecifiedForTheGivenLocale()
		...
		Locale primaryLocale = Locale.US;
		Concept testConcept = createMockConcept(1, primaryLocale);
		ConceptName fullySpecifiedName_FR = createMockConceptName(3,
				new Locale("fr"), ConceptNameType.FULLY_SPECIFIED, true);
		testConcept.addName(fullySpecifiedName_FR);
		Assert.assertEquals(primaryLocale,
				testConcept.getFullySpecifiedName(primaryLocale).getLocale());
		Assert.assertEquals(ConceptNameType.FULLY_SPECIFIED, testConcept
				.getFullySpecifiedName(primaryLocale).getConceptNameType());
	}
	...
	public void addSetMember_shouldAppendConceptToTheExistingListOfConceptSet()
		...
		ConceptSet firstConceptSet = (ConceptSet) concept.getConceptSets()
				.toArray()[0];
		ConceptSet secondConceptSet = (ConceptSet) concept.getConceptSets()
				.toArray()[1];
	...
	public void addSetMember_shouldInsertTheConceptBeforeTheFirstWithZeroIndex()
		...
		ConceptSet secondConceptSet = (ConceptSet) concept.getConceptSets()
				.toArray()[1];
		Assert.assertTrue(firstConceptSet.getSortWeight() < secondConceptSet
				.getSortWeight());
	}
	...
	public void addSetMember_shouldInsertTheConceptInTheThirdSlot()
		...
		ConceptSet thirdConceptSet = (ConceptSet) concept.getConceptSets()
				.toArray()[2];
	...
	public void getPreferredName_shouldReturnTheFullySpecifiedNameIfNoNameIsExplicitlyMarkedAsLocalePreferred()
		...
		ConceptName preferredNameEN_US = createMockConceptName(3, Locale.US,
				null, false);
		testConcept.addName(preferredNameEN_US);
		String fullySpecName = testConcept.getFullySpecifiedName(Locale.US)
				.getName();
		// preferred name in en
		ConceptName preferredNameEN = createMockConceptName(4,
				new Locale("en"), null, false);
		testConcept.addName(preferredNameEN);
		Assert.assertEquals(fullySpecName,
				testConcept.getPreferredName(Locale.US).getName());
	}
	...
	public void getPreferredName_shouldReturnTheConceptNameExplicitlyMarkedAsLocalePreferred()
		...
		// preferred name in en
		ConceptName preferredNameEN = createMockConceptName(4,
				new Locale("en"), null, true);
		testConcept.addName(preferredNameEN);
		Assert.assertEquals(preferredNameEN_US,
				testConcept.getPreferredName(Locale.US));
		Assert.assertEquals(preferredNameEN,
				testConcept.getPreferredName(new Locale("en")));
	}
	...
	public void getShortestName_shouldReturnTheShortestNameForTheConceptFromAnyLocaleIfExactIsFalse()
		...
		Assert.assertEquals("shortName",
				concept.getShortestName(Context.getLocale(), false).getName());
	}
	...
	public void getShortestName_shouldReturnTheShortestNameInAGivenLocaleForAConceptIfExactIsTrue()
		...
		concept.addName(new ConceptName("shortName1", Locale.US));
		concept.addName(new ConceptName("shortName", Locale.FRANCE));
		Assert.assertEquals("shortName12",
				concept.getShortestName(Context.getLocale(), true).getName());
	}
	...
	public void setFullySpecifiedName_shouldAddTheNameToTheListOfNamesIfItNotAmongThemBefore()
		...
		concept.setFullySpecifiedName(new ConceptName("some name", Context
				.getLocale()));
	...
	public void setFullySpecifiedName_shouldConvertThePreviousFullySpecifiedNameIfAnyToASynonym()
			throws Exception {
		Concept concept = createMockConcept(1, Context.getLocale());
		ConceptName oldFullySpecifiedName = concept
				.getFullySpecifiedName(Context.getLocale());
		// sanity check
		Assert.assertEquals(ConceptNameType.FULLY_SPECIFIED,
				oldFullySpecifiedName.getConceptNameType());
	...
	public void setShortName_shouldConvertThePreviousShortNameIfAnyToASynonym()
			throws Exception {
		Concept concept = createMockConcept(1, Context.getLocale());
		ConceptName oldShortName = concept.getShortNameInLocale(Context
				.getLocale());
		// sanity check
		Assert.assertEquals(ConceptNameType.SHORT,
				oldShortName.getConceptNameType());
	...
	public void setShortName_shouldSetTheConceptNameTypeOfTheSpecifiedNameToShort()
		...
		Concept concept = new Concept();
		ConceptName cn = new ConceptName("some name", Context.getLocale());
		ConceptName FullySpecName = new ConceptName("fully spec name",
				Context.getLocale());
	...
	public void setBlankShortName_shouldVoidTheOldOne() {
		...
		assertThat(concept.getShortNameInLocale(Context.getLocale()),
				is(nullValue()));
	}
	...
	public void getShortestName_shouldReturnTheNameMarkedAsTheShortNameForTheLocaleIfItIsPresent()
		...
		Assert.assertEquals("shortName1",
				concept.getShortestName(Locale.US, null).getName());
	}
	...
	public void setPreferredName_shouldFailIfThePreferredNameToSetToIsAnIndexTerm()
		...
		ConceptName preferredName = new ConceptName("some pref name",
				Context.getLocale());
	...
	public void addName_shouldMarkTheFirstNameAddedAsFullySpecified()
		...
		Concept concept = new Concept();
		concept.addName(new ConceptName("some name", Context.getLocale()));
		Assert.assertEquals("some name",
				concept.getFullySpecifiedName(Context.getLocale()).getName());
	}
	...
	public void addName_shouldReplaceTheOldFullySpecifiedNameWithACurrentOne()
			throws Exception {
		Concept concept = new Concept();
		ConceptName oldFullySpecName = new ConceptName("some name",
				Context.getLocale());
		concept.addName(oldFullySpecName);
		Assert.assertEquals(true, oldFullySpecName.isFullySpecifiedName());
		ConceptName newFullySpecName = new ConceptName("new name",
				Context.getLocale());
	...
	public void addName_shouldReplaceTheOldPreferredNameWithACurrentOne()
			throws Exception {
		Concept concept = new Concept();
		ConceptName oldPreferredName = new ConceptName("some name",
				Context.getLocale());
		oldPreferredName.setLocalePreferred(true);
		concept.addName(oldPreferredName);
		ConceptName newPreferredName = new ConceptName("new name",
				Context.getLocale());
		...
		Assert.assertEquals("new name",
				concept.getPreferredName(Context.getLocale()).getName());
	}
	...
	public void addName_shouldReplaceTheOldShortNameWithACurrentOne()
			throws Exception {
		Concept concept = new Concept();
		ConceptName oldShortName = new ConceptName("some name",
				Context.getLocale());
		oldShortName.setConceptNameType(ConceptNameType.SHORT);
		concept.addName(oldShortName);
		ConceptName newShortName = new ConceptName("new name",
				Context.getLocale());
		...
		Assert.assertEquals("new name",
				concept.getShortNameInLocale(Context.getLocale()).getName());
	}

	@Test
	public void getSynonyms_shouldSortPreferredFirst() throws Exception {
		Concept concept = new Concept();
		ConceptName conceptNameNotPreferred = new ConceptName("Non Preferred",
				Locale.ENGLISH);
		ConceptName conceptNameNotPreferred2 = new ConceptName(
				"Non Preferred2", Locale.ENGLISH);
		ConceptName conceptNamePreferred = new ConceptName("Preferred",
				Locale.ENGLISH);
		...
		ConceptName conceptNameExpectedPreferred = concept
				.getSynonyms(Locale.ENGLISH).iterator().next();
	...
	public void getShortNameInLocale_shouldReturnTheBestShortNameForAConcept()
		...
		Assert.assertEquals("Gato",
				concept.getShortNameInLocale(new Locale("es", "ES")).getName());
	}
	...
	public void getPreferredName_shouldReturnTheBesLocalePreferred()
		...
		ConceptName preferredNameEN = createMockConceptName(4,
				new Locale("en"), null, true);
		testConcept.addName(preferredNameEN);
		Assert.assertEquals(preferredNameEN.getName(), testConcept
				.getPreferredName(Locale.US).getName());
	}

	/**
	 * Convenient factory method to create a populated Concept with a one fully
	 * specified name and one short name
	 * 
	 * @param conceptId
	 *            the id for the concept to create
	 * @param locale
	 *            the locale of the of the conceptNames for the concept to
	 *            create
	...
	private Concept createMockConcept(int conceptId, Locale locale) {
		...
		ConceptName shortName = createMockConceptName(1, desiredLocale,
				ConceptNameType.SHORT, false);
		ConceptName fullySpecifiedName = createMockConceptName(2,
				desiredLocale, ConceptNameType.FULLY_SPECIFIED, false);
	 ...
	 * 
	 * @param conceptNameId
	 *            id for the conceptName
	 * @param locale
	 *            for the conceptName
	 * @param conceptNameType
	 *            the conceptNameType of the concept
	 * @param isLocalePreferred
	 *            if this name should be marked as preferred in its locale
	 */
	private ConceptName createMockConceptName(int conceptNameId, Locale locale,
			ConceptNameType conceptNameType, Boolean isLocalePreferred) {
	...
	/**
	 * @see Concept#getName()
	 * @verifies return name in broader locale incase none is found in specific
	 *           one
	 */
	@Test
	...
	/**
	 * @see Concept#hasName(String, Locale)
	 * @verifies hasName returns ture if locale parameter Is Null but name is
	 *           found
	 */
	@Test
	...
	/**
	 * @see Concept#hasName(String, Locale)
	 * @verifies hasName returns false if name is not in concept and locale is
	 *           null
	 */
	@Test
