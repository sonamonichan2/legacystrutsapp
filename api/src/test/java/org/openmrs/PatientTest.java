...
/**
 * This class should test all methods on the patient object. It should not worry
 * about the extended Person object -- that testing is done by
 * {@link org.openmrs.PersonTest} This class does not touch the database, so it
 * does not need to extend the normal openmrs BaseTest TODO: Complete this class
...
public class PatientTest {
	...
	public void addIdentifier_shouldAddIdentifierToCurrentList()
		...
		assertTrue(
				"There should be 1 identifier in the patient object but there is actually : "
		...
		// make sure the identifier is added
		assertTrue(
				"There should be 2 identifiers in the patient object but there is actually : "
						+ p.getIdentifiers().size(),
				p.getIdentifiers().size() == 2);
		...
		pa3.setIdentifier(pa3.getIdentifier()
				+ "some new string to make sure it gets added");
		p.addIdentifier(pa3);
		// make sure the identifier IS added
		assertTrue(
				"There should be 3 identifiers in the patient object but there is actually : "
						+ p.getIdentifiers().size(),
				p.getIdentifiers().size() == 3);

		p.removeIdentifier(pa3);
		assertTrue(
				"There should be only 2 identifiers in the patient object now",
				p.getIdentifiers().size() == 2);
		...
		assertTrue(
				"There should be only 1 identifier in the patient object now",
				p.getIdentifiers().size() == 1);
		p.removeIdentifier(pa2);
		assertTrue(
		...
		assertTrue(
				"There shouldn't be any identifiers in the patient object now",
				p.getIdentifiers().size() == 0);
	}
	...
	public void addIdentifier_shouldAddNewIdentifierForExistingVoidedIdentifier() {
		...
		assertTrue(
				"The second identifier has not been added as a new identifier",
				p.getActiveIdentifiers().contains(pa2));
	}
	...
	public void removeIdentifier_shouldTestIdentifierCollectionChanged()
		...
		pis = p.getIdentifiers().toArray(pis); // NOTE: this is correct -- see
												// the order in array in debug
		assertTrue(p.getIdentifiers().contains(pa3)); // this works

		// now change voided on 3rd-date; that should move it to last position:
		// voided IDs are the last in order
		p.removeIdentifier((pa3));
		pa3.setVoided(true);
		p.addIdentifier((pa3));
	...
	public void getActiveIdentifiers_shouldReturnPreferredIdentifiersFirstInTheList()
		...
		assertTrue(
				"With the third identifier voided, there should only be 2 identifiers",
				p.getActiveIdentifiers().size() == 2);
		assertTrue("Preferred identifier should be first in the list", p
				.getActiveIdentifiers().get(0) == pa2);
		assertTrue("Non-preferred identifier should be last in the list", p
				.getActiveIdentifiers().get(1) == pa1);
	}
}
